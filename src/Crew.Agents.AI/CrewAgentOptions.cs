...
/// <summary>
/// Configures <see cref="CrewAgent"/> construction, connection-string binding, and Copilot CLI process settings.
/// </summary>
/// <example>
/// <code>
/// var options = new CrewAgentOptions
/// {
///     CrewFolderPath = @"C:\repo",
/// };
/// options.CliArgs.Add("--yolo");
/// </code>
/// </example>
public sealed class CrewAgentOptions
{
    /// <summary>
    /// Gets or sets the initialized Crew team root.
    /// </summary>
    public string? CrewFolderPath { get; set; }
    ...
    /// <summary>
    /// Gets or sets the CLI process working directory. When unset, <see cref="CrewFolderPath"/> is used.
    /// </summary>
    ...
    /// </summary>
    public string AgentName { get; set; } = "Crew";
    ...
    /// flag. Defaults to <c>"crew"</c>, which selects <c>.github/agents/crew.agent.md</c>
    /// — the Crew coordinator system prompt that drives eager execution, parallel fan-out,
    /// and dispatch through the <c>task</c> tool.
    /// </summary>
    /// <remarks>
    /// <para>
    /// The whole reason <see cref="CrewAgent"/> exists is to wrap a Crew coordinator team,
    /// so the SDK passes <c>--agent crew</c> by default. Without it the CLI uses its built-in
    /// generic agent and the coordinator role-plays responses inline instead of dispatching
    /// real subagents — exactly the inconsistency between <c>copilot --agent crew</c> (CLI)
    /// and <c>CrewAgent.RunAsync</c> (SDK) that this default eliminates.
    /// </para>
    /// <para>
    /// When the named file does not exist at <c>{team-root}/.github/agents/{AgentFileName}.agent.md</c>
    /// (e.g. the team root is not Crew-initialized yet), the SDK silently skips the
    ...
    /// </para>
    /// </remarks>
    public string? AgentFileName { get; set; } = "crew";
    ...
    /// <summary>
    /// Gets or sets a delegate that customizes the underlying <see cref="CopilotClientOptions"/> after Crew
    ...
    /// delegate — Crew enforces routing invariants and will restore its own values if they are changed.
    ...
    /// after Crew has applied its defaults (including an <c>ApproveAll</c>
    ...
    /// <summary>
    /// Gets or sets a callback that receives <see cref="CrewAgentTraceEvent"/> instances for every
    ...
    ///     else if (trace.Kind == CrewAgentTraceEventKind.AssistantMessage && trace.SdkAgentId is not null)
    ///         Console.WriteLine($"[{trace.SdkAgentId}] {trace.Content}");
    /// };
    /// </code>
    /// </example>
    [JsonIgnore]
    public Action<CrewAgentTraceEvent>? OnSubagentTrace { get; set; }
    ...
    /// <see cref="CrewAgentDiagnostics.ActivitySourceName"/> opens one
    /// <see cref="System.Diagnostics.Activity"/> per subagent dispatch (tagged with
    /// <c>crew.subagent.name</c>, <c>crew.subagent.display_name</c>, <c>crew.subagent.sdk_agent_id</c>,
    /// and <c>crew.subagent.reply_preview</c>). Each subagent lifecycle phase
    /// (<c>crew.subagent.start</c>, <c>crew.subagent.message</c>, <c>crew.subagent.completed</c>,
    /// <c>crew.subagent.failed</c>) is also added as a <see cref="System.Diagnostics.ActivityEvent"/>
    ...
    /// </para>
    /// <para>
    /// Hosts that <c>.AddSource(CrewAgentDiagnostics.ActivitySourceName)</c> on their OpenTelemetry
    /// tracer get these spans in their backend automatically — no need to set
    /// <see cref="OnSubagentTrace"/>.
    /// </para>
    /// <para>
    /// Set this to <see langword="false"/> to disable Crew's built-in telemetry (e.g. when you
    ...
    ///
    /// builder.Services.AddCrewAgent(opts => opts.CrewFolderPath = "/team");
    ///
    /// // Opt out (custom telemetry):
    /// builder.Services.AddCrewAgent(opts =>
    /// {
    ///     opts.CrewFolderPath = "/team";
    ...
    public override string ToString()
    {
        ...
        return $"CrewAgentOptions {{ CrewFolderPath = {CrewFolderPath}, AgentName = {AgentName}, " +

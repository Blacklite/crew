...
/// <see cref="CrewAgentTraceEvent"/> envelope and emits matching OpenTelemetry spans on
/// <see cref="CrewAgentDiagnostics.ActivitySource"/>.
/// </summary>
/// <remarks>
/// <para>
...
/// </para>
/// </remarks>
internal sealed class CrewSubagentTraceMapper : IDisposable
{
    private readonly Action<CrewAgentTraceEvent>? _onTrace;
    ...
    public CrewSubagentTraceMapper(Action<CrewAgentTraceEvent>? onTrace, bool emitActivities = true)
    {
        _onTrace = onTrace;
    ...
    public void OnSessionEvent(SessionEvent sessionEvent)
    {
        ...
        CrewAgentTraceEvent? envelope = sessionEvent switch
        {
            SubagentSelectedEvent s => Build(s, CrewAgentTraceEventKind.SubagentSelected, subagentName: s.Data?.AgentName),
            SubagentStartedEvent s => Build(s, CrewAgentTraceEventKind.SubagentStarted, subagentName: s.Data?.AgentName, subagentDisplayName: s.Data?.AgentDisplayName, toolCallId: s.Data?.ToolCallId),
            SubagentCompletedEvent s => Build(s, CrewAgentTraceEventKind.SubagentCompleted, subagentName: s.Data?.AgentName, toolCallId: s.Data?.ToolCallId),
            SubagentFailedEvent s => Build(s, CrewAgentTraceEventKind.SubagentFailed, subagentName: s.Data?.AgentName, toolCallId: s.Data?.ToolCallId, success: false),
            AssistantMessageEvent a => Build(a, CrewAgentTraceEventKind.AssistantMessage, content: a.Data?.Content, requestedToolNames: ExtractToolNames(a.Data?.ToolRequests)),
            ToolExecutionStartEvent t => Build(t, CrewAgentTraceEventKind.ToolStart, toolCallId: t.Data?.ToolCallId),
            ToolExecutionCompleteEvent t => Build(t, CrewAgentTraceEventKind.ToolComplete, toolCallId: t.Data?.ToolCallId, success: t.Data?.Success),
            SessionIdleEvent _ => Build(sessionEvent, CrewAgentTraceEventKind.SessionIdle),
            _ => null,
        };
    ...
    private void ApplyActivityForEnvelope(CrewAgentTraceEvent envelope)
    {
        switch (envelope.Kind)
        {
            case CrewAgentTraceEventKind.SubagentStarted when !string.IsNullOrEmpty(envelope.ToolCallId):
            {
                ...
                {
                    activity = CrewAgentDiagnostics.ActivitySource.StartActivity(
                        $"crew.subagent {envelope.SubagentName ?? envelope.SdkAgentId ?? toolCallId}",
                ...
                if (activity is not null)
                {
                    if (!string.IsNullOrEmpty(envelope.SubagentName))
                        activity.SetTag("crew.subagent.name", envelope.SubagentName);
                    if (!string.IsNullOrEmpty(envelope.SubagentDisplayName))
                        activity.SetTag("crew.subagent.display_name", envelope.SubagentDisplayName);
                    if (!string.IsNullOrEmpty(envelope.SdkAgentId))
                        activity.SetTag("crew.subagent.sdk_agent_id", envelope.SdkAgentId);
                    activity.SetTag("crew.subagent.tool_call_id", toolCallId);
                    activity.AddEvent(new ActivityEvent(
                        "crew.subagent.start",
                        tags: new ActivityTagsCollection
                        {
                            ["crew.subagent.name"] = envelope.SubagentName,
                            ["crew.subagent.display_name"] = envelope.SubagentDisplayName,
                            ["crew.subagent.sdk_agent_id"] = envelope.SdkAgentId,
                            ["crew.subagent.tool_call_id"] = toolCallId,
                        }));
            ...
            case CrewAgentTraceEventKind.AssistantMessage when !string.IsNullOrEmpty(envelope.SdkAgentId):
            {
                    ...
                    _activitiesByToolCallId.TryGetValue(toolCallId, out var liveActivity))
                {
                    ...
                    if (!string.IsNullOrEmpty(envelope.Content))
                    {
                        var preview = envelope.Content!.Length > 512
                            ...
                            : envelope.Content;
                        liveActivity.SetTag("crew.subagent.reply_preview", preview);
                        liveActivity.AddEvent(new ActivityEvent(
                            "crew.subagent.message",
                            tags: new ActivityTagsCollection
                            {
                                ["crew.subagent.name"] = envelope.SubagentName,
                                ["crew.subagent.sdk_agent_id"] = envelope.SdkAgentId,
                                ["crew.subagent.message_preview"] = preview,
                            }));
                    }
                    ...
                    if (envelope.RequestedToolNames.Count > 0)
                    {
                        ...
                        liveActivity.AddEvent(new ActivityEvent(
                            "crew.subagent.tool_requests",
                            tags: new ActivityTagsCollection
                            {
                                ["crew.subagent.name"] = envelope.SubagentName,
                                ["crew.subagent.sdk_agent_id"] = envelope.SdkAgentId,
                                ["crew.subagent.tool_requests"] = toolNamesCsv,
                                ["crew.subagent.tool_requests_count"] = envelope.RequestedToolNames.Count,
                            }));
                    }
                }
                break;
            ...
            case CrewAgentTraceEventKind.SubagentCompleted:
            case CrewAgentTraceEventKind.SubagentFailed:
            {
                    ...
                    _activitiesByToolCallId.TryRemove(toolCallId!, out var completedActivity))
                {
                    ...
                    var isFailure = envelope.Kind == CrewAgentTraceEventKind.SubagentFailed;
                    completedActivity.AddEvent(new ActivityEvent(
                        isFailure ? "crew.subagent.failed" : "crew.subagent.completed",
                        tags: new ActivityTagsCollection
                        {
                            ["crew.subagent.name"] = envelope.SubagentName,
                            ["crew.subagent.sdk_agent_id"] = envelope.SdkAgentId,
    ...
    /// <see cref="CrewAgentTraceEventKind.SubagentDispatched"/> envelope so consumers can react.
    /// </summary>
    private void OnTaskDispatch(ToolExecutionStartEvent evt)
    {
        ...
        if (_emitActivities)
        {
            ...
            var activity = CrewAgentDiagnostics.ActivitySource.StartActivity(
                $"crew.subagent {displayName}",
                ActivityKind.Internal);
            if (activity is not null)
            {
                activity.SetTag("crew.subagent.tool_call_id", toolCallId);
                if (!string.IsNullOrEmpty(personaName))
                    activity.SetTag("crew.subagent.persona.name", personaName);
                if (!string.IsNullOrEmpty(personaDesc))
                    activity.SetTag("crew.subagent.persona.description", personaDesc);
                if (!string.IsNullOrEmpty(agentType))
                    activity.SetTag("crew.subagent.agent_type", agentType);
                if (!string.IsNullOrEmpty(prompt))
                    activity.SetTag("crew.subagent.prompt", Truncate(prompt!, 1024));

                activity.AddEvent(new ActivityEvent(
                    "crew.subagent.dispatched",
                    tags: new ActivityTagsCollection
                    {
                        ["crew.subagent.persona.name"] = personaName,
                        ["crew.subagent.persona.description"] = personaDesc,
                        ["crew.subagent.agent_type"] = agentType,
                        ["crew.subagent.tool_call_id"] = toolCallId,
                    }));
                _activitiesByToolCallId[toolCallId!] = activity;
            }
        ...
        var dispatchEnvelope = new CrewAgentTraceEvent
        {
            Kind = CrewAgentTraceEventKind.SubagentDispatched,
    ...
    private void InvokeConsumer(CrewAgentTraceEvent envelope)
    {
        try
        {
    ...
    private static CrewAgentTraceEvent Build(
        SessionEvent sessionEvent,
        CrewAgentTraceEventKind kind,
    ...
    {
        return new CrewAgentTraceEvent
        {
            Kind = kind,

...
/// <summary>
/// Parses Crew connection strings into <see cref="CrewAgentOptions"/> instances.
/// </summary>
public static class CrewConnectionFactory
{
    /// <summary>
    /// Parses a Crew connection string into agent options.
    /// </summary>
    /// <param name="connectionString">PATH form or `crew://` URI form connection string.</param>
    ...
    /// <remarks>
    /// PATH form maps the entire value to <see cref="CrewAgentOptions.CrewFolderPath"/> and <see cref="CrewAgentOptions.Cwd"/>.
    /// URI form parses `teamRoot`, `cliPath`, `cwd`, `cliArgs`, and `env` query values; the URI host is reserved.
    /// </remarks>
    /// <example>
    /// <code>
    /// var options = CrewConnectionFactory.FromConnectionString(@"C:\repo");
    /// var advanced = CrewConnectionFactory.FromConnectionString(
    ///     "crew://localhost?teamRoot=C%3A%5Crepo&amp;cliArgs=--yolo");
    /// </code>
    /// </example>
    public static CrewAgentOptions FromConnectionString(string connectionString)
    {
        ...
        if (connectionString.StartsWith("crew://", StringComparison.OrdinalIgnoreCase))
        {
            // URI form
            ...
            if (query.TryGetValue("teamRoot", out var teamRoot))
                options.CrewFolderPath = teamRoot;
        ...
        else
        {
            ...
            options.CrewFolderPath = connectionString;

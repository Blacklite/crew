// ============================================================
//  Crew.Agents.AI — Sample application
//
...
//
//  Run: dotnet run --project src/Crew.Agents.AI/samples/Crew.Agents.AI.Sample/
//  Run one flow: dotnet run --project src/Crew.Agents.AI/samples/Crew.Agents.AI.Sample/ -- --flow=1
// ============================================================
...
// Crew team root: read from env var or fall back to current directory.
// Real-world usage: set CREW_TEAM_ROOT to the path of an initialized Crew team.
var teamRoot = System.Environment.GetEnvironmentVariable("CREW_TEAM_ROOT")
               ?? System.IO.Directory.GetCurrentDirectory();
PrintBanner($"Crew.Agents.AI v0.1 — sample run (team root: {teamRoot})");
Console.WriteLine();
...
if (RunFlow(1))
{
    ...
    host1.Services.AddCrewAgent(o =>
    {
        o.CrewFolderPath = @"C:\Users\tamirdresher\source\repos\crew-crew";
        o.AgentName = "SampleCrew";
        ...
        o.OnSubagentTrace = evt =>
        {
            switch (evt.Kind)
            {
                case CrewAgentTraceEventKind.SubagentDispatched:
                    ...
                    Console.ResetColor();
                    break;
                case CrewAgentTraceEventKind.SubagentStarted:
                    Console.WriteLine($"  [started ] {ShortId(evt.SdkAgentId)} catalog_name={evt.SubagentName}");
                    break;
                case CrewAgentTraceEventKind.AssistantMessage when evt.SdkAgentId is not null:
                {
    ...
    var agent1 = app1.Services.GetRequiredService<CrewAgent>();
    ...
    var result1 = await RunWithErrorHandlingAsync(async () =>
    {
        ...
        //return await agent1.RunAsync("check and review the latest Blacklite/crew repo PR", session1);
    });
...
if (RunFlow(2))
{
    ...
    host2.Services.AddKeyedCrewAgent("alpha", o =>
    {
        o.CrewFolderPath = teamRoot;
        o.AgentName = "AlphaCrew";
        o.Instructions = "Be concise. Answer in one sentence.";
    });

    // "beta" agent — verbose mode
    host2.Services.AddKeyedCrewAgent("beta", o =>
    {
        o.CrewFolderPath = teamRoot;
        o.AgentName = "BetaCrew";
        o.Instructions = "Provide a detailed explanation with examples.";
    });

    using var app2 = host2.Build();

    var alphaAgent = app2.Services.GetRequiredKeyedService<CrewAgent>("alpha");
    var betaAgent  = app2.Services.GetRequiredKeyedService<CrewAgent>("beta");
...
// Crew applies its standard values. The routing gate ensures Cwd/CliPath/
...
if (RunFlow(3))
{
    ...
    host3.Services.AddCrewAgent(o =>
    {
        o.CrewFolderPath = teamRoot;
        o.AgentName = "BYOKCrew";

        // ConfigureCopilotClient is the BYOK extension point.
        // It runs after Crew populates its own values; use it to inject a
        ...
        o.ConfigureCopilotClient = clientOpts =>
        {
            ...
            merged["CREW_SAMPLE_VAR"] = "demo-value";
    ...
    var agent3 = app3.Services.GetRequiredService<CrewAgent>();
...
if (RunFlow(4))
{
    ...
    host4.Services.AddCrewAgent(o =>
    {
        o.CrewFolderPath = teamRoot;
        o.AgentName = "StreamCrew";
    });

    using var app4 = host4.Build();

    var agent4 = app4.Services.GetRequiredService<CrewAgent>();
...
// Flow 5: Subagent dispatch + OpenTelemetry observability
//
// Demonstrates the two observability surfaces that fire whenever the crew
// coordinator dispatches a subagent through the LLM's `task` tool:
//
//   1. CrewAgentOptions.OnSubagentTrace — typed envelope callback. Receives:
...
//
//   2. ActivitySource "Microsoft.Agents.AI.Crew" — OpenTelemetry spans.
...
if (RunFlow(5))
{
    ...
    // ── Tap 1: ActivityListener for the crew ActivitySource ────────────────
    ...
    using var spanListener = new ActivityListener
    {
        ...
        ActivityStarted = a =>
        {
            ...
            Console.WriteLine($"  [otel] start  {a.DisplayName}  persona={a.GetTagItem("crew.subagent.persona.name") ?? "(none)"}");
            Console.ResetColor();
        },
        ActivityStopped = a =>
        {
    ...
    var dispatched = new System.Collections.Concurrent.ConcurrentBag<CrewAgentTraceEvent>();

    var host5 = Host.CreateApplicationBuilder(args);
    host5.Logging.SetMinimumLevel(LogLevel.Warning);
    host5.Services.AddCrewAgent(o =>
    {
        o.CrewFolderPath = teamRoot;
        o.AgentName = "ObservedCrew";
    ...
    var agent5 = app5.Services.GetRequiredService<CrewAgent>();
    ...
    if (dispatched.Count == 0)
    {
        ...
        Console.WriteLine("   or the team root does not have a `.github/agents/crew.agent.md`.)");
        Console.ResetColor();
    }
...
static async ValueTask DisposeIfNeeded(CrewAgent agent)
{

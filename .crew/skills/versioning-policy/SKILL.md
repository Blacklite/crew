...
description: "Semver versioning rules for Crew SDK and CLI — prevents prerelease version incidents"
...
Crew is a monorepo with two publishable npm packages (`@blacklite/crew-sdk` and `@blacklite/crew-cli`) managed via npm workspaces. Version mismatches and prerelease leaks have caused production incidents — most notably PR #640, where a `-build.N` prerelease version silently broke workspace dependency resolution.
...
Both `@blacklite/crew-sdk` and `@blacklite/crew-cli` **MUST have the same version** at all times. The root `package.json` version must also match.

`bump-build.mjs` enforces this by updating all three `package.json` files in lockstep (root + `packages/crew-sdk` + `packages/crew-cli`).

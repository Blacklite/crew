...
```powershell
# Fork: personaluser/crew, Upstream: Blacklite/crew
...
gh pr create --repo Blacklite/crew --head personaluser:contrib/fix-docs `
...
- ❌ **Storing personal tokens in `.env`** or `.crew/` files. These get committed by Scribe. Use `gh`'s credential store.

...
1. **Scan** — Read GitHub: list issues with `crew` label, list all PRs
...
3. **Dispatch** — For untriaged items, read `.crew/routing.md` and triage using: module path match → routing rule keywords → role keywords → Lead fallback. Assign `crew:{member}` label and spawn agent if not already assigned
...
- New issue is created with `crew` label
- PR is opened by a crew agent
- Existing issue is reopened
- Manual activation via "Ralph, go" or `crew watch`
...
**CLI-based:**
- `crew watch --interval 10` → Start persistent polling

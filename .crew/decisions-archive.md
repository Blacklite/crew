...
> Old decisions (2026-02-21 through 2026-03-25) preserved append-only. Managed by Scribe. For active decisions, see .crew/decisions.md.

---
### 2026-02-21: User directive — no temp/memory files in repo root
**By:** Brady (via Copilot)
**What:** NEVER write temp files, issue files, or memory files to the repo root. All crew state/scratch files belong in .crew/ and ONLY .crew/. Root tree of a user's repo is sacred.
...
**What:** Crew packages (`@blacklite/crew-sdk` and `@blacklite/crew-cli`) are distributed exclusively via npmjs.com. The GitHub-native `npx github:Blacklite/crew` path has been removed.
...
### `.crew/` Directory Scope — Owner Directive
**By:** Brady (project owner, PR #326 review)  
**Date:** 2026-03-10  

**Directive:** The `.crew/` directory is **reserved for team state only** — roster, routing, decisions, agent histories, casting, and orchestration logs. Non-team data (adoption tracking, community metrics, reports) must NOT live in `.crew/`. Use `.github/` for GitHub platform integration or `docs/` for documentation artifacts.

**Source:** [PR #326 comment](https://github.com/Blacklite/crew/pull/326#issuecomment-4029193833)
...
**Source:** [PR #326 comment](https://github.com/Blacklite/crew/pull/326#issuecomment-4029222967)
...
**Why:** Library consumers importing `@blacklite/crew` were triggering CLI argument parsing and `process.exit()` on import.

### 2026-02-21: Process.exit() refactor — library-safe CLI functions
**By:** Kujan (SDK Expert)
**What:** `fatal()` throws `CrewError` instead of `process.exit(1)`. Only `cli-entry.ts` may call `process.exit()`.
**Pattern:** Library functions throw `CrewError`. CLI entry catches and exits. Library consumers catch for structured error handling.

### 2026-02-21: User directive — docs as you go
**By:** bradygaster (via Copilot)
**What:** Doc and blog as you go during CrewUI integration work. Doesn't have to be perfect — keep docs updated incrementally.
...
### 2026-02-22: Subpath exports in @blacklite/crew-sdk
**By:** Edie (TypeScript Engineer)
...
**By:** Brady (via Copilot)
**What:** Pause docs publication until Brady explicitly gives go-ahead. Tone: lighthearted, welcoming, fun (NOT stuffy). First doc should be "first experience" with crew CLI. All docs: brief, prompt-first, action-oriented, fun. Human tone throughout.
...
**By:** Kovash (REPL Expert)
**What:** Ctrl+C immediately resets `processing` state. Timeout: `CREW_REPL_TIMEOUT` (seconds) > `CREW_SESSION_TIMEOUT_MS` (ms) > 600000ms default. CLI `--timeout` flag sets env var.

### 2026-02-24: Shell Observability Metrics
**By:** Saul (Aspire & Observability)
**What:** Four metrics under `crew.shell.*` namespace, gated behind `CREW_TELEMETRY=1`.
**Convention:** Shell metrics require explicit consent via `CREW_TELEMETRY=1`, separate from OTLP endpoint activation.

### 2026-02-23: Telemetry in both CLI and agent modes
**By:** Brady (via Copilot)
**What:** Crew should pump telemetry during BOTH modes: (1) standalone Crew CLI, and (2) running as an agent inside GitHub Copilot CLI.
...
**By:** Fenster
**What:** Bare semver (e.g., `0.8.5.1`) for version commands. Display contexts use `crew v{VERSION}`.
...
**By:** Brady (via Copilot)
**What:** DO NOT merge PR #547 (Crew Remote Control). Do not touch #547 at all.
...
- Feature documentation lag (#568 `crew run`, #570 consult mode, #572 Ralph smart triage)
...
- #579 (P2): Status shows parent `.crew/` as local (confusing in multi-project workspaces)
...
**What:** Filed 8 missing backlog items from `.crew/identity/now.md` as GitHub issues. These items were identified as "should-fix" polish or "post-M1" improvements but lacked explicit GitHub tracking until now.

**Why:** Brady requested: "Cross-reference the known backlog against filed issues and file anything missing." The team had filed 28 issues this session (#554–#581), but 8 known items from `now.md` remained unfiled. Without GitHub issues, these lack ownership assignment, visibility for Wave E planning, trackability in automated workflows, and routing to crew members.

**Issues Filed:**
- #583 (crew:rabin): Add `homepage` and `bugs` fields to package.json
- #584 (crew:mcmanus): Document alpha→v1.0 breaking change policy in README
- #585 (crew:edie): Add `noUncheckedIndexedAccess` to tsconfig
- #586 (crew:edie): Tighten ~26 `any` types in SDK
- #587 (crew:mcmanus): Add architecture overview doc
- #588 (crew:kujan): Implement CREW_DEBUG env var test
- #589 (crew:kujan): One real Copilot SDK integration test
- #590 (crew:baer): `npm audit fix` for dev-dependency ReDoS warnings
- #591 (crew:hockney, type:bug): Aspire dashboard test fails — docker pull in test suite
- #592 (crew:rabin): Replace workspace:* protocol with version string

**Impact:** Full backlog now visible with explicit issues. No unmapped items. Each issue routed to the crew member domain expert. Issues are independent; can be executed in any order.
...
- Workspace protocol: ❌ VIOLATION — 1 issue filed (#592): `workspace:*` in crew-cli violates npm workspace convention
- Skipped tests: ❌ GAP — 1 issue filed (#588): CREW_DEBUG test is .todo() placeholder
...
2. Soon: Implement CREW_DEBUG test (#588) — fills observable test gap
...
**By:** Fenster (Core Dev)
**Date:** 2026-02-28
**What:** When running from source (`VERSION` contains `-preview`), the CLI checks if `@blacklite/crew-cli` is globally npm-linked. If not, it prompts the developer to link it. Declining creates `~/.crew/.no-auto-link` to suppress future prompts.
...
**Impact:** All crew members: if you modify `loadWelcomeData`, the `firstRunElement` conditional in App.tsx, or the terminal clear sequence in `runShell`, these tests will catch regressions. The warning suppression tests replicate the `cli-entry.ts` pattern — if that pattern changes, update both locations.
...
**Date:** 2026-03-01
**Issue:** #625
**What:** When no roster exists, only the header banner tells the user about `crew init` / `/init`. The `firstRunElement` block returns `null` for the empty-roster case instead of showing a duplicate message. `firstRunElement` is reserved for the "Your crew is assembled" onboarding when a roster already exists.
**Why:** Two competing UI elements both said "run crew init" — visual noise that confuses the information hierarchy. Banner is persistent and visible; it owns the no-roster guidance. `firstRunElement` owns the roster-present first-run experience.
...
**Impact:** Eliminates `ExperimentalWarning` noise in terminal for all Crew CLI users, including when the Copilot SDK spawns subprocesses.
...
**Status:** Implemented
**Issue:** #638
**What:** Created `packages/crew-cli/src/cli/core/cast.ts` as a self-contained casting engine with four exports:
1. `parseCastResponse()` — parses the `INIT_TEAM:` format from coordinator output
2. `createTeam()` — scaffolds all `.crew/agents/` directories, writes charters, updates team.md and routing.md, writes casting state JSON
...
Scribe and Ralph are always injected if missing from the proposal. Casting state is written to `.crew/casting/` (registry.json, history.json, policy.json).
**Why:** Enables coordinator to propose and create teams from within the REPL session after `crew init`.
**Implications:**
...
**Date:** 2026-03-02  
**Context:** Analyzed migration from beta repo (`Blacklite/crew`, v0.5.4) to origin repo (`Blacklite/crew-pr`, v0.8.18-preview). Version gap spans 0.6.x, 0.7.x, 0.8.0–0.8.16 (internal origin development only).
...
**What:** Deprecate `@bradygaster/create-crew` (beta's package name). All future releases use:
- `@blacklite/crew-cli` (user-facing CLI)
- `@blacklite/crew-sdk` (programmatic SDK for integrations)
...
**Action:** When v0.8.17 is ready to publish, release a final version of `@bradygaster/create-crew` with deprecation notice: "This package has been renamed to @blacklite/crew-cli. Install with: npm install -g @blacklite/crew-cli"
...
**Date:** 2026-03-02  
**Context:** Beta repo currently uses GitHub-native distribution (`npx github:Blacklite/crew`). Origin uses npm distribution (`npm install -g @blacklite/crew-cli`). After merge, old path will break.

**Problem:** After migration, `npx github:Blacklite/crew` fails (root `package.json` has no `bin` entry). Users hitting the old path get cryptic npm error.
...
**User Impact:**
- Users running `npx github:Blacklite/crew` see bold error with `npm install -g @blacklite/crew-cli` instruction
- Existing projects running `crew upgrade` work seamlessly (upgrade logic built-in)
...
cd /path/to/project
crew upgrade
crew upgrade --migrate-directory  # Optional: .ai-team/ → .crew/
```
...
**Why:** After `crew init`, team.md exists but is empty. Coordinator received a "route to agents" prompt with no agents listed, causing silent generic AI behavior. Users never got told to cast their team.
...
### 2026-03-02: Connection promise dedup in CrewClient
**By:** Fenster (Core Dev)
**Date:** 2026-03-02
**What:** `CrewClient.connect()` now uses a promise dedup pattern — concurrent callers share the same in-flight `connectPromise` instead of throwing "Connection already in progress".
...
**Impact:** `packages/crew-sdk/src/adapter/client.ts` only. No API surface change.
...
**Why:** Prevents garbage casts from vague or accidental first messages (e.g., "hello", "what can you do?"). Matches the crew.agent.md Init Mode pattern where confirmation is required before creating team files.
...
**Files Changed:**
- packages/crew-cli/src/cli/shell/components/App.tsx — added setProcessing to ShellApi interface + wired in onReady
- packages/crew-cli/src/cli/shell/index.ts — added setProcessing calls in handleInitCast (entry, pendingCastConfirmation return, finally block)
...
**Scope:** Applies to docs pages, README files, blog posts, community-facing content. Internal files (.crew/** memory files, decision docs, agent history) have flexibility.
...
- `docs/community/built-with-crew.md` shows opted-in projects only
...
- ✅ Moved `.crew/adoption/` → `.github/adoption/`
...
- ✅ Removed "Built with Crew" showcase link from README (Tier 2 feature)
...
**What:** Move adoption tracking from `.crew/adoption/` to `.github/adoption/`
...
3. **Clear separation:** `.crew/` = team internal; `.github/` = GitHub platform integration
...
**What:** If a PR diff shows deletions in `.crew/agents/*/history.md` or `.crew/decisions.md`, the PR is blocked until deletions are reverted.
...
## Crew Ecosystem Boundaries & Content Governance

### Crew Docs vs Crew IRL Boundary (consolidated)
**By:** PAO (via Copilot), Flight  
**Date:** 2026-03-10  
**Status:** Active pattern for all documentation PRs

**Litmus test:** If Crew doesn't ship the code or configuration, the documentation belongs in Crew IRL, not the Crew framework docs.

**Categories:**

1. **Crew docs** — Features Crew ships (routing, charters, reviewer protocol, config, behavior)
2. **Crew IRL** — Infrastructure around Crew (webhooks, deployment patterns, logging, external tools, operational patterns)
3. **Gray area:** Platform features (GitHub Issue Templates) → Crew docs if framed as "how to configure X for Crew"

**Examples applied (PR #331):**

| Document | Decision | Reason |
|----------|----------|--------|
| ralph-operations.md | DELETE → IRL | Infrastructure (deployment, logging) around Crew, not Crew itself |
| proactive-communication.md | DELETE → IRL | External tools (Teams, WorkIQ) configured by community, not built into Crew |
| issue-templates.md | KEEP, reframe | GitHub platform feature; clarify scope: "a GitHub feature configured for Crew" |
| reviewer-protocol.md (Trust Levels) | KEEP | Documents user choice spectrum within Crew's existing review system |

**Enforcement:** Code review + reframe pattern ("GitHub provides X. Here's how to configure it for Crew's needs."). Mark suspicious deletions for restore (append-only governance).
...
**By:** Flight  
**Date:** 2026-03-10  
**Status:** Skill created at `.crew/skills/content-triage/SKILL.md`

**Pattern:** External content (blog posts, sample repos, videos, conference talks) that helps Crew adoption must be triaged using the "Crew Ships It" boundary heuristic before incorporation.

**Workflow:**
1. Triggered by `content-triage` label or external content reference in issue
2. Flight performs boundary analysis
3. Sub-issues generated for Crew-ownable content extraction (PAO responsibility)
...
**Why:** Pattern from PR #331 (Tamir Dresher blog) shows parallel extraction of Crew-ownable patterns (scenario guides, reviewer protocol) and infrastructure patterns (Ralph ops, proactive comms). Without clear boundary, teams pollute Crew docs with operational content or miss valuable patterns that should be generalized.

**Impact:** Enables community content to accelerate Crew adoption without polluting core docs. Flight's boundary analysis becomes reusable decision framework. Prevents scope creep as adoption grows.
...
**Decision:** Document emerging patterns in real Crew usage: proactive communication loops and PR review trust spectrum.

**Components:**

1. **Proactive communication patterns** — Outbound notifications (Teams webhooks), inbound scanning (Teams/email for work items), two-way feedback loop connecting external sources to Crew workflow
...
### Remote Crew Access — Phased Rollout (Proposed)
**By:** Flight  
**Date:** 2026-03-10  
**Status:** Proposed — awaits proposal document in `docs/proposals/remote-crew-access.md`

**Context:** Crew currently requires a local clone to answer questions. Users want remote access from mobile, browser, or different machine without checking out repo.

**Phases:**

**Phase 1: GitHub Discussions Bot (Ship First)**
- Surface: GitHub Discussions
- Trigger: `/crew` command or `@crew` mention
- Context: GitHub Actions workflow checks out repo → full `.crew/` state
...
- Trigger: `/crew ask {question}` in any Copilot client
- Context: Extension fetches `.crew/` files via GitHub API (no clone)
...
- Trigger: `@crew` mention in channel
- Context: Webhook fetches `.crew/` via GitHub API
- Response: Bot replies in thread
- Feasibility: 2 weeks
- Why third: Enterprise teams live in chat; high value for companies using Crew

**Constraint:** Crew's intelligence lives in `.crew/` (roster, routing, decisions, histories). Any remote solution must solve context access. GitHub Actions workflows provide checkout for free. Copilot Extension and chat bots use GitHub API to fetch files.

**Implementation:** Before Phase 1 execution, write proposal document. New CLI command: `crew answer --context discussions --question "..."`. New workflow: `.github/workflows/crew-answer.yml`.
...
**What:** The CLI timeout is set too low — Brady tried using Crew CLI in this repo and it didn't work well. Timeout needs to be increased. Not urgent but should be captured as a CLI improvement opportunity.

**Why:** User request — captured for team memory and PRD inclusion

### 2026-03-01: Multi-Crew Storage & Resolution Design
**By:** Keaton (Lead)
**What:** 
- New directory structure: ~/.config/crew/crews/{name}/.crew/ with ~/.config/crew/config.json for registry
- Keep 
esolveGlobalCrewPath() unchanged; add 
esolveNamedCrewPath(name?: string) and listPersonalCrews() on top
- Auto-migration: existing single personal crew moves to crews/default/ on first run
- Resolution priority: explicit (CLI flag) > project config > env var > git remote mapping > path mapping > default
- Global config.json schema: { version, defaultCrew, crews, mappings }

**Why:** 
- crews/ container avoids collisions with existing files at global root
- Backward-compatible: legacy layout detected and auto-migrated; existing code continues to work
- Clean separation: global config lives alongside crews, not inside any one crew
- Resolution chain enables flexible mapping without breaking existing workflows

### 2026-03-01: Multi-Crew SDK Functions
**By:** Kujan (SDK Expert)
**What:**
- New SDK exports: 
esolveNamedCrewPath(), listCrews(), createCrew(), deleteCrew(), switchCrew(), 
esolveCrewForProject()
- New type: CrewEntry { name, path, isDefault, createdAt }
- crews.json registry (separate file, not config.json) with crew metadata and mappings
- CrewDirConfig v2 addition: optional personalCrew?: string field (v1 configs unaffected)
- Consult mode updated: setupConsultMode(options?: { crew?: string }) with explicit selection or auto-resolution

**Why:**
- Lazy migration with fallback chain ensures zero breaking changes to existing users
- Separate crews.json is single source of truth for routing; keeps project config focused
...
### 2026-03-01: Multi-Crew CLI Commands & REPL
**By:** Kovash (REPL)
**What:**
- New commands: crew list, crew create <name>, crew switch <name>, crew delete <name>
- Modified commands: crew consult --crew=<name>, crew extract --crew=<name>, crew init --global --name=<name>
- Interactive picker for crew selection: arrow keys (↑/↓), Enter to confirm, Ctrl+C to cancel
- REPL integration: /crew and /crews slash commands with 	riggerCrewReload signal
- .active file stores current active crew name (plain text)
- Status command enhanced to show active crew and crew list

**Why:**
- Picker only shows when needed (multiple crews) and TTY available; non-TTY gracefully uses active crew
...
- Crew deletion safety: cannot delete active crew; requires confirmation

### 2026-03-01: Multi-Crew UX & Interaction Design
**By:** Marquez (UX Designer)
**What:**
- Visual indicator: current crew marked with ●, others with ○; non-default crews tagged [switched]
- Crew name always visible in REPL header and prompt: ◆ Crew (client-acme)
- Picker interactions: ↑/↓ navigate, Enter select, Esc/Ctrl+C cancel; 5-7 crews displayed, wrap around
- Error states: clear copy with next actions (e.g., "Crew not found. Try @crew:personal." or "Run /crews to list.")
- Copy style: active verbs (Create, Switch, List), human-readable nouns (no jargon), 3-5 words per line
- Onboarding: fresh install defaults to "personal"; existing single-crew users see migration notice

**Why:**
- Persistent context (crew name in header/prompt) prevents "Which crew am I in?" confusion
...
**By:** Fenster  
**Date:** 2026-03-01  
**PR:** #547 "Crew Remote Control - PTY mirror + devtunnel for phone access" by tamirdresher (external)

## Context

External contributor Tamir Dresher submitted a PR adding `crew start --tunnel` command to run Copilot in a PTY and mirror terminal output to phone/browser via WebSocket + Microsoft Dev Tunnels.

## Architectural Question

Is remote terminal access via devtunnel + PTY mirroring in scope for Crew v1 core?
...
**By:** Brady (via Copilot)
**What:** The public migration from crew-pr to crew should target v0.6.0, not v0.8.17. This overrides Kobayashi's Phase 5 Option A recommendation. The public repo (Blacklite/crew) goes from v0.5.4 → v0.6.0 — a clean minor bump.
...
1. **Not integrated with Crew runtime** — doesn't use EventBus, Coordinator, or agent orchestration. Isolated feature.
...
5. **Audit logs** — go to `~/.cli-tunnel/audit/` instead of `.crew/log/` (inconsistent with Crew state location).
...
- Rename command to `crew remote` or `crew tunnel` (avoid `start` collision)
- Move audit logs to `.crew/log/`
...
- **Architectural fit unclear** — not in any Crew v1 PRD. No integration with agent orchestration.
- **Native dependency risk** — `node-pty` adds install friction (C++ compiler required).

**My take:** This belongs in a plugin, not core. External contributor did solid work on the WebSocket bridge, but Crew v1 needs to ship agent orchestration first. Remote access is a nice-to-have, not a v1 must-have.

If you want this in v1, we need a proposal (docs/proposals/) first.

### 2026-03-02: Multi-crew test contract — crews.json schema
**By:** Hockney (Tester)
**Date:** 2026-03-02
**Issue:** #652
...
Tests for multi-crew (PR #690) encode a specific crews.json contract:
...
Crew name validation regex: `^[a-z0-9]([a-z0-9-]{0,38}[a-z0-9])?$` (kebab-case, 1-40 chars).
...
Fenster: Align `multi-crew.ts` types with this schema, or flag if different — Hockney will adjust tests.
...
1. **Extract PRD to proper location:**
   - Move `.crew/identity/prd-consult-mode.md` → `docs/proposals/consult-mode.md`
...
3. **Create implementation issues from PRD phases:**
   - Phase 1: SDK changes (CrewDirConfig, resolution helpers)
...
- Separate extraction command (`crew extract`) ✅
- License risk detection (copyleft) ✅

**What needs rethinking:**
- Reusing `sharing/` module (history split vs learnings extraction — different domains)
- PRD flip-flops between "copy crew" and "remote mode teamRoot pointer"
- No design for how learnings are structured or extracted
- Tests before code (cart before horse)

## Pattern Observed

James Sturtevant is a thoughtful contributor who understands the product vision. The PRD is coherent and well-structured. This connects to his #652 issue (Multiple Personal Crews) — consult mode is a stepping stone to multi-crew workflows.

**Recommendation:** Engage James in architecture discussion before he writes code. This feature has implications for the broader personal crew vision. Get alignment on:
...
External contributors are engaging with Crew's architecture. We need to guide them toward shippable PRs, not just accept aspirational work. Setting clear expectations now builds trust and avoids wasted effort.

## Files Referenced

- `.crew/identity/prd-consult-mode.md` (PRD, should move)
- `test/consult.test.ts` (tests for non-existent code)
- `.crew/agents/fenster/history.md` (claims work done)
- `packages/crew-sdk/src/resolution.ts` (needs `consult` field, unchanged in PR)
...
**By:** Fenster  
**Date:** 2025-07  
**What:** `cli.js` at repo root is a 14-line shim that imports `./packages/crew-cli/dist/cli-entry.js`. It no longer contains bundled CLI code. The deprecation notice only displays when invoked via npm/npx.  
...
**Impact:** `node cli.js` now requires `npm run build` to have been run first (so `packages/crew-cli/dist/cli-entry.js` exists). This was already the case for any development workflow.
...
- **What:** End GitHub-native distribution (`npx github:Blacklite/crew`). Install exclusively via npm registry.
- **How:** Users install via `npm install -g @blacklite/crew-cli` (global) or `npx @blacklite/crew-cli` (per-project).
...
- Merge drivers on `.crew/agents/kobayashi/history.md` ensure this decision appends safely across parallel branches.
...
All distribution now goes through npm. The `npx github:Blacklite/crew` path has been fully removed from:
- Source code (github-dist.ts default template, install-migration.ts, init.ts)
- All 4 copies of crew.agent.md (Ralph Watch Mode commands)
- All 4 copies of crew-insider-release.yml (release notes)
...
- **All team members:** When writing docs or examples, use `npm install -g @blacklite/crew-cli` or `npx @blacklite/crew-cli`. Never reference `npx github:`.
...
### 1. npm Packages: `@blacklite/crew-cli` and `@blacklite/crew-sdk`
...
### 2. Public Repo (Blacklite/crew): GitHub Release Tag `v0.8.17` **[CORRECTED from v0.6.0]**
...
3. **They target different audiences:**
   - npm: Users who install via `npm install -g @blacklite/crew-cli`
   - Public repo: Users who clone `Blacklite/crew` or interact with GitHub releases
...
Brady's directive (2026-03-03T02:16:00Z): "crew-cli and crew-sdk must NOT be bumped down to 0.6.0. They are already shipped to npm at 0.8.17."
...
**What:**
- npm pack → creates tarball of both crew-sdk and crew-cli
...
- Verbal/Keaton: the 4-layer model selection hierarchy documented in crew.agent.md is now supported at the SDK config level.
...
- `packages/crew-sdk/package.json` — v0.8.21-preview.7
- `packages/crew-cli/package.json` — v0.8.21-preview.7
...
- 363a0a8 — feat: Structured model preference & crew-level defaults (#245)
...
- 4 deleted `.crew/decisions/inbox/` files (cleanup, merged to decisions.md)
...
### #247 — Crew Installation Fails 🔴 **CRITICAL BLOCKER**

**Impact:** Users cannot install via `npm install -g @blacklite/crew-cli`.  
**Assignee:** Fenster (actively fixing)  
**Status:** In progress  
**Release Impact:** **SHIP CANNOT PROCEED** until resolved.

**Other Open Issues:**
- #248 — CLI command wiring: `crew triage` does not trigger team assignment loop (minor)
- #242 — Future: Tiered Crew Deployment (deferred, not blocking)
- #241 — New Crew Member for Docs (deferred)
- #240 — ADO configurable work item types (deferred)
- #236 — feat: persistent Ralph (deferred)
- #211 — Crew management paradigms (deferred, release:defer label)
...
- Builder functions (defineTeam, defineAgent, defineRouting, defineCeremony, defineHooks, defineCasting, defineTelemetry, defineCrew)
- `crew build` command with --check, --dry-run, --watch flags
- SDK Mode Detection in Coordinator prompt
- Documentation (SDK-First Mode guide, updated SDK Reference, README quick reference)

### Added — Remote Crew Mode (ported from @spboyer PR #131)
- `resolveCrewPaths()` dual-root resolver
- `crew doctor` command (9-check setup validation)
- `crew link <path>` command
- `crew init --mode remote`
- `ensureCrewPathDual()` / `ensureCrewPathResolved()`

### Changed — Distribution & Versioning
- npm-only distribution (no more GitHub-native `npx github:Blacklite/crew`)
...
- Removed .crew branch protection guard
...
## Decision
We have wrapped `@opentelemetry/api` in a resilient shim (`packages/crew-sdk/src/runtime/otel-api.ts`) and moved the package to `optionalDependencies`.

### Mechanics
- **Runtime detection:** The wrapper attempts to load `@opentelemetry/api`.
- **Graceful fallback:** If loading fails, it exports no-op implementations of `trace`, `metrics`, and `diag` that match the API surface used by Crew.
...
1. **SDK-First Mode** — TypeScript-first authoring, type safety, `crew build` command
...
2. **`025-v0821-comprehensive-release.md`** (new) — Full release wave summary
   ...
   - Scope: All 7 feature areas (SDK-First + Remote Crew + 5 critical fixes), metrics, community credits
...
**Issues filed:**
- #249: crew init --sdk flag for SDK-First mode opt-in
- #250: standalone crew migrate command (subsumes #231)
...
**Details:** 2 closeable, 1 P0 override, 9 for v0.8.22, 5 for v0.8.23+, 1 for v0.9+, 4 backlog. See .crew/orchestration-log/2026-03-07T16-19-00Z-keaton.md for full triage table.
...
**Details:** See .crew/orchestration-log/2026-03-07T16-19-00Z-hockney.md for detailed code assessment.
...
**Details:** 2 docs issues queued for v0.8.22 (#251 restructure, #210 contributors workflow). See .crew/orchestration-log/2026-03-07T16-19-00Z-mcmanus.md for full triage.
...
**Crew-specific (customer concern):**
1. `sync-crew-labels.yml` — Auto-syncs labels from team.md on push
2. `crew-triage.yml` — Auto-triages issues when labeled "crew"
3. `crew-issue-assign.yml` — Auto-assigns issues when crew:{member} labeled
4. `crew-heartbeat.yml` — Ralph heartbeat/auto-triage (cron currently disabled)
5. `crew-label-enforce.yml` — Label mutual exclusivity on label events

**Standard CI/Release (expected):**
6. `crew-ci.yml` — Standard PR/push CI
7. `crew-release.yml` — Tag + release on push to main
8. `crew-promote.yml` — Branch promotion (workflow_dispatch)
9. `crew-main-guard.yml` — Forbidden file guard
10. `crew-preview.yml` — Preview validation
11. `crew-docs.yml` — Docs build/deploy
12-15. Publish/insider workflows

**Directive:** Move crew-specific automation (1-5) into CLI commands. Keep standard CI/release workflows.
...
**Key insight:** CLI-first makes Crew **portable**. If the work lives in CLI commands instead of Actions, Crew can run anywhere — Codespaces, devcontainers, local terminals, persistent ACA containers. Actions lock the control loop to GitHub's event system. CLI-first means the user (or their infrastructure) owns the execution loop, not GitHub Actions.


# CLI Feasibility Assessment — GitHub Actions → CLI Commands
**Author:** Fenster (Core Dev)  
**Date:** 2026-03-07  
**Context:** Brady's request to migrate crew-specific workflows to CLI commands
...
**Key insight:** We already have `crew watch` — it's the local equivalent of `crew-heartbeat.yml`. The workflow runs in GitHub Actions with PAT; watch runs locally with `gh` CLI. They share the same triage logic (`@blacklite/crew-sdk/ralph/triage`).
...
**Existing commands** (`packages/crew-cli/src/cli/commands/`):

| Command | Function | Overlap with Workflows |
|---------|----------|------------------------|
| **watch** | Ralph's local polling — triages issues, monitors PRs, assigns labels. Uses `gh` CLI. | ✅ 80% overlap with `crew-heartbeat.yml` + `crew-triage.yml` |
| plugin | Marketplace add/remove. Uses `gh` CLI for repo access. | ❌ No workflow overlap |
| export | Export crew state to JSON. | ❌ No workflow overlap |
| import | Import crew state from JSON. | ❌ No workflow overlap |
...
| link/init-remote | Link to remote crew repo. | ❌ No workflow overlap |
...
- **`@blacklite/crew-sdk/ralph/triage`** — Shared triage logic (routing rules, module ownership, keyword matching)
...
### 2.1. sync-crew-labels.yml → `crew labels sync`

**Current workflow:** 170 lines. Parses `team.md`, syncs `crew`, `crew:{member}`, `go:*`, `release:*`, `type:*`, `priority:*`, `bug`, `feedback` labels. Uses Octokit.

**Proposed CLI command:**
```bash
crew labels sync [--crew-dir .crew] [--dry-run]
```
...
- **Dependencies:** 
  ...
  - ✅ `parseRoster()` from `@blacklite/crew-sdk/parsers` (already exists)
...
### 2.2. crew-triage.yml → `crew triage` (or extend `crew watch`)

**Current workflow:** 260 lines. On `crew` label, parses `team.md` + `routing.md`, keyword-matches, applies `crew:{member}` label, posts comment.

**Proposed CLI command:**
```bash
crew triage [--issue <number>] [--crew-dir .crew]
```
Or: enhance `crew watch` to post comments (currently it only adds labels).

**Implementation:**
- **Size:** M (4-6 hours)
- **Dependencies:** 
  - ✅ `gh` CLI (already used)
  - ✅ `triageIssue()` from `@blacklite/crew-sdk/ralph/triage` (already used in watch.ts)
...
### 2.3. crew-issue-assign.yml → ???

**Current workflow:** 160 lines. On `crew:{member}` label, posts assignment comment, calls **copilot-swe-agent[bot] assignment API with PAT** (lines 116-161).
...
- **Alternative:** Document `crew watch` as the local equivalent (it can label + post comments, but not trigger bot assignment).
...
### 2.4. crew-heartbeat.yml → Already exists as `crew watch`

**Current workflow:** 170 lines. Runs on cron (disabled), issues closed/labeled, PRs closed. Triages untriaged issues, assigns @copilot to `crew:copilot` issues.

**CLI equivalent:** **Already shipped as `crew watch`** (`packages/crew-cli/src/cli/commands/watch.ts`, 356 lines).

**What `crew watch` does:**
- Polls open issues with `crew` label
- Triages untriaged issues (adds `crew:{member}` label)
...
- Uses shared `@blacklite/crew-sdk/ralph/triage` logic

**What `crew watch` does NOT do (that heartbeat.yml does):**
...
**Migration plan:**
- ✅ **Already done.** `crew watch` is the local heartbeat.
...
**Recommendation:** Enhance `crew watch` with comment posting. Keep copilot auto-assign in workflow.


---

### 2.5. crew-label-enforce.yml → `crew labels enforce`
...
**Proposed CLI command:**
```bash
crew labels enforce [--issue <number>] [--crew-dir .crew]
```
...
## 3. The `crew watch` Connection

**`crew watch` is the local heartbeat.** It already does 80% of what `crew-heartbeat.yml` does:
- ✅ Triage untriaged issues (adds `crew:{member}` label)
...
**Can `crew watch` subsume heartbeat.yml entirely?**
...
**Recommendation:** Keep heartbeat.yml for copilot auto-assign (PAT-only feature). Enhance `crew watch` with comment posting for parity on triage behavior.
...
3. **Triage logic** — Already implemented in `@blacklite/crew-sdk/ralph/triage` and used by both `watch.ts` and `ralph-triage.js`. **Reuse at 100%.**
...
1. **`crew labels sync`** — S (2-3 hours)
   - Reuse `parseRoster()`, add label create/edit loop with `gh`
   - Supports `--dry-run`, `--crew-dir`
   - Zero new deps

2. **`crew labels enforce`** — S (2-4 hours)
...
3. **Enhance `crew watch` with comment posting** — M (4-6 hours)
   ...
   - **Result:** `crew watch` now has full parity with triage + heartbeat workflows (minus copilot auto-assign)

### Heavy Lift (v0.9+ or never)

**Total: 8-12 hours**

4. **`crew copilot assign` (copilot-swe-agent[bot] API)** — XL (8-12 hours)
...
1. **`crew labels sync`** — 2-3 hours. Quick win. Zero deps.
2. **`crew labels enforce`** — 2-4 hours. Quick win. Reuses existing wrappers.

### Ship Next (v0.8.23)

3. **Enhance `crew watch` with comment posting** — 4-6 hours. Medium effort. Full parity with triage workflow (minus copilot auto-assign).
...
5. **`crew watch`** — Already shipped (v0.8.16+). Local equivalent of heartbeat.yml. Triages issues, monitors PRs. Missing comment posting (4-6 hour gap).
...
|----------|-------------|------------|--------------|----------|
| sync-crew-labels.yml | `crew labels sync` | S | ✅ Yes | 2-3 hrs (v0.8.22) |
| crew-label-enforce.yml | `crew labels enforce` | S | ✅ Yes | 2-4 hrs (v0.8.22) |
| crew-triage.yml | Enhance `crew watch` | M | ✅ Partial | 4-6 hrs (v0.8.23) |
| crew-heartbeat.yml | Already `crew watch` | M | ✅ Done | 0 hrs (shipped) |
| crew-issue-assign.yml | N/A | XL | ❌ No | Keep workflow (PAT-only) |
...
2. **If yes:** Fenster implements `crew labels sync` + `crew labels enforce` (4-7 hours total).
3. **If comment posting desired:** Add `gh issue comment` wrapper to `gh-cli.ts`, call it from watch.ts (4-6 hours).
4. **Document:** Copilot auto-assign requires GitHub Actions + PAT. `crew watch` is local equivalent for triage + PR monitoring.
...
Brady's concern is valid: **Crew is surprising users with automated GitHub Actions that consume API quota and execute without explicit user intent.** The current model treats Crew as an automated bot service rather than a user-controlled tool.

**Core principle:** Crew should be a CLI-first tool that users invoke when they want it, not an always-on automation layer that reacts to every label change.

**Recommendation:** Migrate 5 crew-specific workflows to CLI commands. Keep 10 standard CI/CD workflows (expected by any project). Target v0.8.22 for deprecation warnings, v0.9.0 for removal.
...
|----------|---------|----------|
| **crew-ci.yml** | PR/push to dev/insider | Standard CI — every repo needs this |
| **crew-release.yml** | Push to main | Standard release automation — tag + GitHub Release |
| **crew-promote.yml** | workflow_dispatch only | Manual branch promotion — user-triggered |
| **crew-main-guard.yml** | PR/push to main/preview/insider | Prevents forbidden files on release branches — safety net |
| **crew-preview.yml** | Push to preview | Pre-release validation — standard quality gate |
| **crew-docs.yml** | Push to main (docs/**) | Docs build/deploy to GH Pages — standard pattern |
| **publish.yml** | Tag push (v*) | npm publish on tag — standard release flow |
| **crew-publish.yml** | Tag push (v*) | npm publish (monorepo variant) — standard release flow |
| **crew-insider-release.yml** | Push to insider | Insider build tagging — standard preview channel |
| **crew-insider-publish.yml** | Push to insider | Insider npm publish — standard preview channel |
...
### 🟡 MIGRATE TO CLI — Crew-Specific Automation (5 workflows)

These workflows execute Crew logic on GitHub events. They surprise users because they:
...
|----------|---------|-----------------|-----------------|
| **sync-crew-labels.yml** | Push to team.md | 🟡 Moderate — creates ~30+ labels automatically | `crew labels sync` |
| **crew-triage.yml** | issues:[labeled] when "crew" label added | 🔴 HIGH — AI routing + label assignment + comment | `crew triage` or `crew triage <issue>` |
| **crew-issue-assign.yml** | issues:[labeled] when crew:{member} label added | 🟡 Moderate — posts comment, assigns @copilot | `crew assign <issue> <member>` |
| **crew-heartbeat.yml** | issues:[closed/labeled], PR:[closed], cron (disabled) | 🔴 HIGH — Ralph auto-triage every 30min (if enabled) | `crew watch` (user keeps terminal open) |
| **crew-label-enforce.yml** | issues:[labeled] | 🟡 Moderate — removes conflicting labels, posts comments | `crew labels check <issue>` |
...
### 1. **sync-crew-labels.yml** → `crew labels sync`

**Current behavior:** On push to `.crew/team.md`, automatically syncs ~30+ labels (crew:*, go:*, release:*, type:*, priority:*).

**CLI replacement:**
```bash
crew labels sync
# Reads .crew/team.md, creates/updates labels via GitHub API
# Output: "✓ Created 12 labels, updated 18 labels"
```

**When users run it:**
- After editing `.crew/team.md` (new member added)
- During initial Crew setup (`crew init` could offer to run it)
- Manually when they want to refresh label definitions

**Tradeoff:** Labels won't auto-sync. Users must remember to run this.  
**Mitigation:** `crew init` runs it automatically. `crew doctor` warns if team.md changed but labels haven't been synced.


---

### 2. **crew-triage.yml** → `crew triage`

**Current behavior:** On "crew" label added, reads team.md + routing.md, does keyword-based routing, assigns crew:{member} label, posts triage comment.

**CLI replacement:**
```bash
# Triage all issues with "crew" label and no crew:{member} label
crew triage
...
**When users run it:**
- After new issues are labeled with "crew"
- During daily standup / triage sessions
- As part of a larger workflow (`crew watch` could include this)

**Tradeoff:** Triage doesn't happen automatically when label is added.  
**Mitigation:** `crew watch` can poll for untriaged issues and notify the user. User still invokes triage explicitly.


---

### 3. **crew-issue-assign.yml** → `crew assign <issue> <member>`

**Current behavior:** On crew:{member} label added, posts assignment comment. If crew:copilot, assigns copilot-swe-agent[bot] via PAT.

**CLI replacement:**
```bash
# Assign issue to a crew member (adds label, posts comment)
crew assign 42 ripley

# Assign to @copilot (adds label, posts comment, assigns bot)
crew assign 42 copilot
...
- As part of `crew triage` output (suggests assignments, user confirms)

**Tradeoff:** Assignment doesn't happen automatically when label is added.  
**Mitigation:** `crew triage` can assign in one step (triage + assign). User still has control.


---

### 4. **crew-heartbeat.yml** → `crew watch`
...
**Tradeoff:** No background automation. User must keep `crew watch` running.  
**Mitigation:** Users who want automation can keep `crew watch` in a tmux pane or run it in CI. Users who DON'T want automation aren't surprised.


---

### 5. **crew-label-enforce.yml** → `crew labels check`
...
crew labels check 42

# Output:
# ⚠️ Issue #42: Multiple go: labels detected (go:yes, go:no)
#    Run `crew labels fix 42` to resolve
```

**When users run it:**
- Before triage sessions
- As part of `crew doctor` (health check)
- Manually when they notice conflicting labels

**Tradeoff:** Conflicting labels won't be auto-removed.  
**Mitigation:** `crew labels check` is fast. `crew doctor` includes it. Users can run it proactively.
...
|----------------|--------|------------|
| **Auto-sync labels on team.md push** | Labels may be out of sync with team roster | `crew doctor` warns. `crew init` syncs automatically. |
| **Auto-triage on "crew" label** | Issues sit in triage inbox longer | `crew watch` notifies. `crew triage` is one command. |
| **Auto-assign on crew:{member} label** | Manual step to assign after labeling | `crew triage` does both in one step. |
| **Ralph heartbeat (cron auto-triage)** | No background automation | `crew watch` in tmux/screen. Or: users run `crew triage` daily. |
| **Auto-enforce label rules** | Conflicting labels may exist temporarily | `crew labels check` is fast. `crew doctor` includes it. |
...
- Add deprecation warnings to all 5 workflows (at the top of each file):
  ...
  # Use `crew labels sync` instead (see docs/migration/actions-to-cli.md)
  ```
- Implement CLI commands:
  - `crew labels sync`
  - `crew triage [<issue>]`
  - `crew assign <issue> <member>`
  - `crew watch` (basic polling loop)
  - `crew labels check [<issue>]`
...
- Remove from template bundles (`.crew/templates/workflows/`)
- Update `crew init` to NOT install these workflows
- Add `crew upgrade` to remove deprecated workflows from existing repos
...
- Add opt-in GitHub Actions workflow for users who want automation:
  ```yaml
  name: Crew CLI Runner (opt-in)
  on:
    issues: [labeled]
  jobs:
    run-cli:
      - run: npx @blacklite/crew-cli triage ${{ github.event.issue.number }}
  ```
...
**Can Crew work with ZERO custom Actions (just standard CI)?**

**YES.** Here's what it looks like:

### Minimal GitHub Actions Setup
- **crew-ci.yml** — Test on PR (standard)
- **crew-release.yml** — Tag + release on main push (standard)
- **crew-docs.yml** — Build docs on main push (standard)

**That's it.** 3 workflows. Zero Crew-specific logic in GitHub Actions.
...
**Benefits:**
- **Zero API usage surprises** — users invoke Crew when they want it
- **Zero hidden costs** — no cron jobs running every 30min
- **Full transparency** — users see Crew's decisions as they happen
- **User control** — users can override triage decisions before they're applied

**This is the right model.** Crew is a tool users invoke, not a bot that watches them.
...
**Migrate all 5 crew-specific workflows to CLI commands.**
...
**Core belief:** Crew should be a CLI-first tool that users control, not an automation layer that surprises them. This migration aligns with that vision.
...
```
crew labels sync          # Sync labels from team.md
crew labels check [issue] # Check for conflicting labels
crew labels fix <issue>   # Fix conflicting labels

crew triage [issue]       # Triage issue(s) using routing rules
crew assign <issue> <member> # Assign issue to crew member

crew watch               # Watch for new issues (polling loop)
crew doctor              # Health check (labels, triage queue, etc.)
```

### UX Principles
- **Explicit is better than implicit** — users invoke Crew when they want it
- **One command does one thing** — no hidden side effects
- **Fast feedback** — commands complete in <1s for single issues
- **Batch operations** — `crew triage` without args processes all untriaged

### Technical Approach
- All CLI commands use GitHub API (via Octokit)
- `crew watch` uses polling (every 30s) with efficient API usage (If-None-Match headers)
- `crew triage` uses same routing logic as current `crew-triage.yml` (reuse ralph-triage.js)
- `crew doctor` aggregates multiple checks (labels, triage, etc.)
...
|----------|---------|-----------------|
| sync-crew-labels.yml | Push to team.md | ~30 (create/update labels) |
| crew-triage.yml | issues:[labeled] "crew" | ~5-10 (read files, add labels, post comment) |
| crew-issue-assign.yml | issues:[labeled] "crew:*" | ~3-5 (post comment, assign) |
| crew-heartbeat.yml | Cron every 30min (disabled) | ~10-50 (depends on open issues) |
| crew-label-enforce.yml | issues:[labeled] any label | ~2-5 (remove conflicting labels, post comment) |

**Total:** If heartbeat were enabled, Crew would make 50+ API calls every 30 minutes, even if no real work happened. This is the core problem Brady identified.
...
Brady seeks to reduce GitHub Actions usage by migrating automation to Crew CLI. This assessment identifies which workflows are **load-bearing infrastructure** (must stay as Actions) vs. **migration candidates** that can move to CLI-side automation.

**Bottom Line:** ~90 actions-minutes/month can be eliminated by migrating 5 crew-specific workflows (label sync, triage, assignments, label enforcement). However, **9 workflows must remain as Actions** because they provide event-driven guardrails that cannot be replicated CLI-side.
...
|----------|----------|---------|----------------|-------|
| **crew-ci.yml** | CI | PR changes + dev push | ~120 | Runs per PR update, most frequent trigger |
| **crew-release.yml** | Release | Push to main (once/release) | ~15 | Tag creation + GitHub Release |
| **crew-promote.yml** | Release | Manual dispatch | ~20 | dev→preview→main pipeline |
| **crew-main-guard.yml** | CI | PR to main + push | ~10 | File pattern guards (fast) |
| **crew-preview.yml** | CI | Push to preview | ~15 | Full test suite validation |
| **crew-publish.yml** | Publish | Tag push | ~30 | Build + npm publish (2x jobs) |
| **crew-insider-release.yml** | Release | Push to insider | ~15 | Tag creation only |
| **crew-insider-publish.yml** | Publish | Push to insider | ~30 | Build + npm publish |
| **sync-crew-labels.yml** | Crew | team.md changes | ~1 | Lightweight label sync |
| **crew-triage.yml** | Crew | Issue labeled | ~2 | Script runs, ~50-100 issues/month |
| **crew-issue-assign.yml** | Crew | Issue labeled | ~2 | Script runs, ~50-100 issues/month |
| **crew-heartbeat.yml** | Crew | Issue/PR closed, manual | ~5 | Ralph triage script (when enabled) |
| **crew-label-enforce.yml** | Crew | Issue labeled | ~2 | Label mutual exclusivity enforcement |
| **crew-docs.yml** | Docs | Manual + docs push | ~5 | Rarely triggered (on demand mostly) |

### Cost Breakdown

- **CI/Release (MUST STAY):** ~215 minutes/month — essential event-driven guardrails
- **Crew-Specific (MIGRATE):** ~12 minutes/month — low cost but high synchronization burden
...
```
dev branch (crew-ci.yml) 
    ↓
main branch (crew-ci.yml + crew-main-guard.yml)
    ↓
crew-release.yml (validates version, creates tag v*)
    ↓
crew-publish.yml (triggered by tag, publishes to npm)
    ↓
...
|----------|---------|-----------|--------|-----------|
| **crew-ci.yml** | PR open/sync, dev push | — | All downstream | ✅ YES |
| **crew-main-guard.yml** | PR to main/preview | — | Release process | ✅ YES |
| **crew-release.yml** | Push to main | crew-main-guard + crew-ci | crew-publish | ✅ YES |
| **crew-promote.yml** | Manual workflow_dispatch | — | Follows main merge | ⚠️ MANUAL |
| **crew-publish.yml** | Tag push (v*) | All CI/tests upstream | npm distribution | ✅ YES |
| **sync-crew-labels.yml** | team.md changes | — | crew-triage | ⚠️ AUTOMATION |
| **crew-triage.yml** | Issue labeled "crew" | sync-crew-labels output | crew-issue-assign | ⚠️ AUTOMATION |
| **crew-issue-assign.yml** | Issue labeled "crew:*" | crew-triage | @copilot work start | ⚠️ AUTOMATION |
| **crew-heartbeat.yml** | Issue/PR closed, manual | — | Auto-triage | ⚠️ AUTOMATION |
| **crew-label-enforce.yml** | Issue labeled | — | Triage feedback | ⚠️ AUTOMATION |

### Cross-Workflow Triggers (Implicit Dependencies)

1. **crew-triage → crew-issue-assign**: Triage adds `crew:{member}` label → triggers assignment workflow
2. **crew-label-enforce → feedback loop**: Enforces mutual exclusivity → posts triage updates
3. **crew-release → crew-publish**: Successful main push creates tag → triggers publish

**Finding:** crew-release + crew-publish form an **implicit pipeline** — removing either breaks the release chain.
...
#### 1. **crew-ci.yml** — PR/Push Event Guard
...
#### 2. **crew-main-guard.yml** — Protected Branch Enforcement
- **Trigger:** PR to main/preview/insider, push to main/preview/insider
- **Function:** Prevents `.crew/`, `.ai-team/`, internal-only files from reaching production
- **Why it must be Actions:**
...
#### 3. **crew-release.yml** — Tag + Release Creation
...
- **Why it must be Actions:**
  ...
  - Creates artifacts that trigger downstream crew-publish.yml
  - If moved to CLI, requires manual invocation (breaks release automation)
  - **Dependency:** crew-publish.yml is triggered **only** by tag push

#### 4. **crew-publish.yml** — npm Distribution Gate
- **Trigger:** Tag push (v*)
- **Function:** Build monorepo, publish crew-sdk + crew-cli to npm
- **Why it must be Actions:**
...
#### 5. **crew-promote.yml** — Branch Promotion Pipeline
...
#### 6. **crew-preview.yml** — Pre-Release Validation
...
#### 7. **crew-docs.yml** — Documentation Build & Deploy
...
#### 8. **crew-insider-release.yml** — Pre-Release Channel
...
#### 9. **crew-insider-publish.yml** — Insider npm Distribution
- **Trigger:** Push to insider
- **Function:** Publish crew-sdk + crew-cli to npm with `insider` tag
- **Why it must be Actions:**
  - Final distribution step for pre-release channel
  - Mirrors crew-publish.yml for insider builds
...
## Part 4: Migration Candidates (Crew-Specific Workflows)

### Workflows That Should Migrate to CLI

#### 1. **sync-crew-labels.yml** → `crew sync-labels`
...
- **Migration Path:** Run as part of `crew upgrade`, available via `crew sync-labels` command

#### 2. **crew-triage.yml** → `crew triage`
- **Current:** Triggered by "crew" label on issue
- **Proposal:** Move to CLI command that runs on-demand or via Ralph (monitor) agent
- **CLI Implementation:** Detect issues with "crew" label, run routing logic, add member labels + comments
...
#### 3. **crew-issue-assign.yml** → `crew assign`
- **Current:** Triggered by "crew:{member}" label on issue
- **Proposal:** Move to CLI command, combines with triage workflow
- **CLI Implementation:** Detect issues with crew:* labels, post assignment comments, optionally assign @copilot via PAT
...
#### 4. **crew-heartbeat.yml** → `crew heartbeat` / Ralph monitor
...
#### 5. **crew-label-enforce.yml** → `crew validate-labels`
...
- **Migration Path:** Can be called as part of crew-triage → removes conflicting labels before applying member assignment

### Migration Risk Matrix

| Workflow | Complexity | State Risk | Race Conditions | Human Review | Recommendation |
|----------|-----------|-----------|-----------------|---------------|-----------------|
| **sync-crew-labels.yml** | Low | None | None | No | ✅ MIGRATE |
| **crew-triage.yml** | Medium | Low | Possible (concurrent issues) | Yes (lead review) | ✅ MIGRATE |
| **crew-issue-assign.yml** | Medium | Low | Possible (label race) | Yes (PAT required) | ✅ MIGRATE |
| **crew-heartbeat.yml** | Medium | Low | None (async monitor) | Yes (Ralph logic) | ✅ MIGRATE (to Ralph) |
| **crew-label-enforce.yml** | Low | None | None | No | ✅ MIGRATE |
...
## Part 5: The `crew init` Impact

### Current Flow: crew init → Install Workflows

```
crew init [repo]
  ├─ Detect project type (Node.js, Python, Go, etc.)
  ├─ Copy .crew/ template files
  │  ├─ team.md
  │  ├─ routing.md
  │  ├─ charter.md
  │  └─ other YAML configs
  ├─ Copy .github/workflows/ from templates/workflows/
  │  ├─ crew-ci.yml (project-type sensitive stub)
  │  ├─ crew-release.yml (project-type sensitive)
  │  ├─ crew-promote.yml
  │  ├─ crew-main-guard.yml
  │  ├─ crew-preview.yml
  │  ├─ crew-docs.yml
...
**Option A: Remove All Crew-Specific Workflows from init**

```diff
  crew init [repo]
    ├─ Install CI/Release workflows (9 workflows)
    ├─ Skip crew-specific workflows (5 workflows)
    └─ Post message: "To enable smart triage, run: crew init-automation"
```

**Implications:**
- Simpler `crew init` — no automation magic, team must opt-in
- Users who want triage must run second command: `crew init-automation`
- Clearer separation: **core** (CI/Release) vs. **optional** (team automation)

**Option B: Keep All, Make Workflows Optional in Init**

```
crew init [repo] --with-automation
crew init [repo] --automation=none  # skip crew-specific
```
...
**Option C: Hybrid — Install Crew Workflows, Disable Some by Default**

```
crew init [repo]
  ├─ Install ALL workflows
  ├─ Disable (comment out triggers on):
  │  ├─ crew-heartbeat.yml (cron already commented)
  │  ├─ crew-triage.yml (comments say "disabled pre-migration")
  └─ Enable on demand via: crew enable-heartbeat, crew enable-triage
```
...
- crew upgrade can toggle these flags
...
**Problem:** User has all 15 workflows. If we remove crew-specific ones from init, their repo still has old workflows running.

**Solution: `crew upgrade` with workflow management**

```bash
# Update Crew CLI to latest
npm install -g @blacklite/crew-cli@latest

# Then upgrade repo workflows
crew upgrade --workflows

# Shows what changed:
# ✅ Updated crew-ci.yml (v1 schema)
# ⏭️ Deprecated: crew-triage.yml (moving to CLI)
# ⏭️ Deprecated: crew-heartbeat.yml (moving to Ralph)
# Run: crew migrate-automation --help
```
...
|-------|--------|----------|
| **Phase 1** | Document: "Migration path for crew automation to CLI" | v0.9.0 |
| **Phase 2** | Implement: `crew triage`, `crew assign`, `crew sync-labels` as CLI commands | v1.0.0 |
| **Phase 3** | Add deprecation warnings to crew-specific workflows | v1.0.0 |
| **Phase 4** | `crew upgrade --remove-deprecated-workflows` flag | v1.1.0 |
| **Phase 5** | Remove deprecated workflows from init (new repos only) | v1.1.0 |

### Migration Checklist for Users

**If you have crew-triage.yml running:**
1. Wait for `crew triage` CLI command (v1.0.0+)
2. Test: `crew triage --dry-run` on your repo
3. Remove crew-triage.yml from .github/workflows/
4. Add `crew triage` to your automation schedule (manual or cron)

**If you have crew-heartbeat.yml running:**
1. Ralph agent will handle smart triage (v1.0.0+)
2. Remove crew-heartbeat.yml when ready
3. Enable Ralph monitor: `crew enable-ralph`
...
|----------|----------------|-----------|-----------|
| **crew-ci.yml** | None (read-only) | Low | Test failures are visible |
| **crew-release.yml** | Git tags, GitHub Releases | Critical | Version verification, dry-run |
| **crew-promote.yml** | Git branches | Critical | Dry-run mode, human approval |
| **crew-main-guard.yml** | None (blocks merges) | None | Enforcement only |
| **sync-crew-labels.yml** | GitHub labels | Low | Idempotent, can re-sync |
| **crew-triage.yml** | Issue labels, comments | Low | Can be corrected manually |
| **crew-issue-assign.yml** | Issue assignees, comments | Low | Can be corrected manually |
| **crew-heartbeat.yml** | Issue labels, comments | Low | Async, low severity |
| **crew-label-enforce.yml** | Issue labels | Low | Idempotent |

### Critical Workflows (State Corruption Risk)

1. **crew-release.yml**: Creates git tags that trigger downstream pipeline
   - Risk: Duplicate tags, malformed versions
   - Mitigation: Version validation (must exist in CHANGELOG.md) before tagging

2. **crew-promote.yml**: Merges between branches, strips forbidden paths
   - Risk: Lost commits, wrong paths stripped
   - Mitigation: Dry-run preview, manual approval, git log verification

3. **crew-main-guard.yml**: Prevents merges with forbidden paths
...
**Problem:** Developer deletes crew-triage.yml from their branch, but it still runs because .github/workflows/ is read from main.
...
- Only `crew upgrade --remove-deprecated-workflows` removes repo-wide
...
|--------------|-----------------|-----------------|-----------------|
| **New Users** | `crew init` installs 15 workflows | init installs 9 core workflows | None (automatic) |
| **Existing Teams** | 15 workflows in .github/workflows/ | Workflows persist; deprecated ones marked | Crew upgrade notices |
| **Triage Users** | crew-triage.yml runs on issues | CLI: manual `crew triage` or Ralph monitor | Opt-in to CLI command |
| **Heartbeat Users** | crew-heartbeat.yml runs on schedule | Ralph monitor (when enabled) | Enable Ralph |
...
- Migration tools (crew upgrade) handle transition
...
3. **Implement lazy automation** in crew init
   - Add `automation` config flag to .crew/config.json
   - Default: CI + Release enabled, Crew-specific disabled
   - Reduce onboarding cognitive load

### For Integration Teams

1. **CLI commands to implement** (v1.0.0):
   - `crew triage` — Run routing logic on open issues
   - `crew assign` — Assign issues to team members
   - `crew sync-labels` — Sync labels from team.md
   - `crew validate-labels` — Enforce label mutual exclusivity

2. **Ralph integration** (v1.0.0):
   - Ralph monitor loop runs smart triage
   - Replaces crew-heartbeat.yml event triggers
   - Still manual-invokable via CLI

3. **Deprecation strategy** (v0.9.0):
   - Document in CLI README: "crew-triage.yml will move to CLI in v1.0"
   - Add warnings to deprecated workflows in init output
   - Provide `crew migrate-automation` helper command

### For Release Management

1. **Workflows that MUST stay on main**:
   - crew-ci.yml (branch protection)
   - crew-main-guard.yml (forbidden file guard)
   - crew-release.yml (tag creation)
   - crew-publish.yml (npm distribution)

2. **Version gates to enforce**:
   - CHANGELOG.md entry must exist before tag
   - .crew/ files must be stripped from preview branch
   - No tag created without version validation

3. **Disaster recovery**:
   - If crew-release.yml tags wrong version, use `git tag -d` + `git push origin --delete` to recover
   - If crew-promote.yml merges wrong commits, use `git revert` to undo merge commit
...
- ✅ 5 crew-specific workflows (12 minutes/month) can move to CLI
...
**Bottom line:** Migrate crew-specific automation to CLI for maintainability; keep critical CI/Release workflows as Actions for correctness.
...
- `.crew/agents/kobayashi/history.md` — Release coordination history
- `.crew/decisions.md` — Team decisions on workflows, versioning
- `.crew/team.md` — Team roster and capabilities
- `.crew/routing.md` — Work routing rules
- `packages/crew-cli/src/cli/core/workflows.ts` — Workflow generation logic
- `packages/crew-cli/src/cli/core/init.ts` — Init command implementation
...
**Date:** 2026-03-11  
**Context:** Brady raised concern that Crew's automatic GitHub Actions installation during `crew init` creates surprise friction for customers. This analysis evaluates whether moving to CLI-first (with opt-in Actions) is the right call.
...
A developer runs `crew init` in their repo. The CLI installs 5 Crew-specific workflows:
1. **sync-crew-labels.yml** — triggers on every `.crew/team.md` push
2. **crew-triage.yml** — fires on every issue label event (looking for `crew` label)
3. **crew-issue-assign.yml** — fires on every `crew:*` label
4. **crew-label-enforce.yml** — enforces mutual exclusivity on EVERY label event
5. **crew-heartbeat.yml** — Ralph's triage engine (cron disabled, but fires on issue/PR close events)

**The "Oh No" Moment:**
- User runs `crew init` ✅ 
- User looks at their Actions tab for the first time after a day of active labeling
- They see **10–20 workflow runs** in the Actions history from Crew operations they didn't explicitly ask for
- **Mental model breaks:** "I didn't start these. Why is my Actions tab full? Is Crew spamming my quota? Am I going to get billed?"
...
- **Each workflow run on ubuntu-latest:** ~30–60 seconds (measured from recent Crew runs)
...
- `sync-crew-labels`: 4 runs (team.md updated ~1/week) = 4 × 0.5min = 2 min
- `crew-triage`: 20 runs (label crew) + 50 runs (crew:* labels + enforce) = 70 runs × 0.5min = 35 min
- `crew-label-enforce`: ~80 runs (cascading from all labeling) × 0.5min = 40 min
- `crew-heartbeat`: ~15 runs (issue close/PR close events) × 1min = 15 min
...
**Message:** "Crew puts *you* in control. No surprise automations. You decide when and how Crew runs."

This reframes the value prop:
- ✅ Transparency — you see every command you run
- ✅ Control — you decide your team's workflow, not Crew
...
```
$ crew init
→ Installs .crew/ structure (NO workflows)
→ Shows: "Crew is ready. Use 'crew triage' to label issues manually."
→ User runs: $ crew triage
→ Crew triages open issues via CLI
→ User happy: "I have full control."

$ crew init --with-actions (for power users)
→ Installs automation workflows
→ User knows exactly what they're opting into
```

### Messaging for Existing Users
**Blog post: "Introducing CLI-First Crew"**
...
2. **What happens to existing installs:**
   ...
   - `crew upgrade` downloads latest, no forced removal
   - Users can manually delete workflows if they want

3. **Upgrade path:**
   - **Do nothing:** Current workflows stay. You're not on the new path yet.
   - **Adopt CLI-first:** Run `crew init --clean-actions` to remove workflows, use CLI commands
   - **Stay hybrid:** Keep workflows and use CLI as you prefer


---

## 4. Competitive Positioning — Crew vs. Cursor, Aider, etc.
...
### Crew's Differentiation
- **Unique:** Multi-agent orchestration + GitHub native (Actions + SDK)
- **Risk:** If perceived as "Crew spams my repo with automation," it becomes a *negative* differentiator
...
```bash
crew init                           # No workflows installed
crew triage                         # User explicitly runs triage
crew rc                             # Connect remote crew mode
```

**Tier 2: Semi-Automated (Opt-In)**
```bash
crew init --with-automation         # Installs key workflows only
  - sync-crew-labels (on team.md push)
  - crew-triage (on label event)
  - crew-heartbeat (Ralph's triage, manual + event-driven)
```

**Tier 3: Full Automation (Enterprise)**
```bash
crew init --with-full-automation    # All 5+ workflows, cron enabled
  - Everything in Tier 2
  - crew-label-enforce (auto-fix labels)
  - crew-issue-assign (auto-route assignments)
...
```bash
# Post-init opt-in
crew actions install              # Install tier 2 (semi-auto)
crew actions install --full       # Install tier 3 (full auto)
crew actions uninstall            # Remove all workflows
crew actions status               # Show which workflows are active + usage stats

# Power user config
crew init --with-actions=heartbeat,triage  # Cherry-pick workflows
```
...
- Current: Mentions Crew installs and runs automatically
- New: Lead with CLI-first story
- Add: "Crew gives you full control. No background automation by default."

#### 2. **docs/getting-started.md** (New)
- Step 1: `crew init` + quick wins with CLI
- Step 2 (optional): Explore automation with `crew actions install`
...
- Add new commands: `crew triage`, `crew actions *`
- Update `crew init` docs with `--with-actions` and `--with-full-automation` flags

#### 5. **CHANGELOG.md** (Next Release Notes)
- Breaking change: `crew init` no longer installs workflows
...
- Title: "Crew is Now CLI-First — Workflows Are Optional"
- Sections:
  - Why we changed
  - How to upgrade
...
- Install NO workflows by default during `crew init`
...
- `crew init --with-automation` is the "easy mode"
...
- `crew upgrade` doesn't force removal
...
| **Perception** | "Crew does things to my repo" | "Crew does what I ask" |
...
4. **Design UX** for `crew init --with-actions` flag
...
Filed [#252](https://github.com/Blacklite/crew/issues/252) as the public RFC for migrating Crew's 5 crew-specific GitHub Actions workflows to CLI commands. This is the community-facing version of the internal strategy decided earlier today.

## Key Points

- **Tiered model is the default path.** `crew init` installs zero workflows (Tier 1). Automation is opt-in via `--with-automation` (Tier 2) or `--with-full-automation` (Tier 3).
- **9 CI/release workflows stay as Actions.** Only the 5 crew-specific workflows migrate.
...
- All crew members should review #252 and be prepared to address community feedback.
...
**By:** Brady (via Copilot)
**What:** Delete `.github/workflows/crew-main-guard.yml` entirely in v0.8.22. Crew state in repos is fine — no longer need to block `.crew/` from protected branches.
**Why:** User directive — "i want that guard GONE in the next release. completely and totally gone." The original policy of keeping `.crew/` off main/preview is obsolete. Crew files in repos are now welcome and expected.
...
**Why:** User request — captured for team memory. This aligns with SDK-First roadmap and addresses the growing complexity of crew.agent.md.



---

# Decision: `crew init` Default is Markdown-Only, `--sdk` for Typed Config
...
Crew init previously hardcoded `configFormat: 'typescript'` and always generated a `crew.config.ts` file using the OLD `CrewConfig` type format. Brady wanted:
1. **Default behavior**: Markdown-only (old, boring, no config file)
2. **Opt-in SDK**: New builder syntax with `defineCrew()` / `defineTeam()` / `defineAgent()`

## Decision

`crew init` now supports a `--sdk` flag:

- **`crew init`** (no flag): `configFormat: 'markdown'` → NO config file generated, only `.crew/` directory structure
- **`crew init --sdk`**: `configFormat: 'sdk'` → generates `crew.config.ts` with SDK builder syntax
...
- **Generator function**: `packages/crew-sdk/src/config/init.ts` line ~337: `generateSDKBuilderConfig()`
- **Config file skip**: When `configFormat === 'markdown'`, config file generation is skipped entirely

## Files Modified

- `packages/crew-cli/src/cli-entry.ts` — flag parsing + help text
- `packages/crew-cli/src/cli/core/init.ts` — option passthrough
- `packages/crew-sdk/src/config/init.ts` — new format support + generator
...
```bash
crew init
# Creates: .crew/, .github/agents/, workflows
# Does NOT create: crew.config.ts
```

### SDK builder format
```bash
crew init --sdk
# Creates: .crew/, crew.config.ts (with defineCrew() syntax)
```
...
- **Edie**: This is NOT the same as migrate.ts — this is for NEW crew creation, not converting existing crews
- **Users**: Default experience unchanged — markdown-only is the default

## Future Considerations

- `crew build` command should work with SDK configs to generate markdown from TypeScript
- Teams may want `crew migrate --to-sdk` to convert markdown → SDK config (that's Edie's migrate.ts, not this)



---

# Decision: `crew migrate` Command Implementation
...
Users with existing markdown-only crews (`.crew/` directory with team.md, routing.md, agent charters) need a way to convert to SDK-First mode. Conversely, SDK-First users should be able to revert to markdown-only if desired.

## Decision

Implemented `crew migrate` command with three migration paths:

### 1. `crew migrate --to sdk` (markdown → SDK-First)

**Input:** `.crew/` directory with markdown files  
**Output:** `crew.config.ts` with builder syntax
...
**Code generation:**
- Uses builder functions: `defineCrew()`, `defineTeam()`, `defineAgent()`, `defineRouting()`, `defineCasting()`
...
### 2. `crew migrate --to markdown` (SDK-First → markdown)

**Input:** `crew.config.ts`  
**Output:** Updated `.crew/` directory, config moved to `.bak`

**Process:**
1. Run `crew build` to regenerate `.crew/` from config
2. Move `crew.config.ts` → `crew.config.ts.bak`
3. `.crew/` directory becomes source of truth

### 3. `crew migrate --from ai-team` (legacy upgrade)

**Input:** `.ai-team/` directory  
**Output:** `.crew/` directory

**Process:**
- Subsumes existing `upgrade --migrate-directory` flag
- Delegates to `migrateDirectory()` function (already implemented)
- Suggests running `crew migrate --to sdk` afterward
...
Running `crew migrate --to sdk && crew build` should produce identical `.crew/` output. The migrate command preserves all metadata during conversion.

## Implementation

- File: `packages/crew-cli/src/cli/commands/migrate.ts`
- Wired into: `packages/crew-cli/src/cli-entry.ts` (after upgrade block, line ~240)
...
- Add ceremony parsing when `.crew/ceremonies.md` format stabilizes
...
- Issue #249: `crew init` builder mode (Fenster)
...
**Date:** 2026-03-07
**Context:** Issue #255 — Decompose crew.agent.md into pluggable skills
**Decision made by:** Verbal (Prompt Engineer)

## Decision

Crew coordinator capabilities are now **skill-based** — self-contained modules loaded on demand rather than always-inline in crew.agent.md.

## What Changed

### 1. SDK Builder Added

Added `defineSkill()` builder function to the SDK (`packages/crew-sdk/src/builders/`):
...
- **Why:** SDK-First mode needed a typed way to define skills in `crew.config.ts`
...
- `source: "extracted"` — marks decomposition from crew.agent.md

### 3. crew.agent.md Compacted
...
**Skill:** Read `.crew/skills/init-mode/SKILL.md` when entering Init Mode.
...
`crew build` now generates `.crew/skills/{name}/SKILL.md` when `config.skills` is defined in `crew.config.ts`:
...
if (config.skills && config.skills.length > 0) {
  for (const skill of config.skills) {
    files.push({
      relPath: `.crew/skills/${skill.name}/SKILL.md`,
...
### For Coordinators
- **Smaller context window:** crew.agent.md drops from 840 → 711 lines. Further decomposition can continue.
...
### For SDK Users
- **Typed skill definitions:** Define skills in `crew.config.ts` using `defineSkill()`, get validation and type safety.
- **Programmatic skill authoring:** Skills can be composed, shared, and versioned like code.
- **Build-time generation:** `crew build` generates SKILL.md from config — single source of truth.

### For the Team
- **Parallel with ceremony extraction:** Follows the same pattern as ceremony skill files (#193).
- **Reduces merge conflicts:** Smaller crew.agent.md = fewer line-based conflicts when multiple PRs touch governance.
...
2. **crew.agent.md must still work:** Core rules remain inline. Coordinator knows WHEN to load each skill without needing the skill file first.
3. **Type collision avoided:** BuilderSkillDefinition vs runtime SkillDefinition — import from `@blacklite/crew-sdk/builders` subpath in CLI to avoid ambiguity.

## Future Work

- Extract 3+ more skills from crew.agent.md (target: <500 lines for core orchestration)
- Add skill discovery/loading to runtime (currently manual references)
- Skill marketplace: share skills via npm, discover in `crew marketplace`
...
- Issue: #255
- Files changed:
  - `packages/crew-sdk/src/builders/types.ts`
  - `packages/crew-sdk/src/builders/index.ts`
  - `packages/crew-sdk/src/index.ts`
  - `packages/crew-cli/src/cli/commands/build.ts`
  - `.github/agents/crew.agent.md`
  - `.crew/skills/init-mode/SKILL.md` (new)
  - `.crew/skills/model-selection/SKILL.md` (new)
  - `.crew/skills/client-compatibility/SKILL.md` (new)
  - `.crew/skills/reviewer-protocol/SKILL.md` (new)
...
**Event Chain:**
1. Code merged to `main` (via crew-promote or direct merge)
2. `crew-release.yml` creates tag + GitHub Release (if version bumped)
3. `publish.yml` triggers on `release.published` event
4. Publishes @blacklite/crew-sdk → @blacklite/crew-cli (correct order)
...
- Deprecated `.github/workflows/crew-publish.yml` (redundant)
...
- Manual `npm unpublish @blacklite/crew-{pkg}@{version}` if issues detected

## Related Files

- `.github/workflows/publish.yml` — npm publish workflow
- `.github/workflows/crew-release.yml` — GitHub Release creation
- `.crew/agents/kobayashi/history.md` — Implementation details
...
**By:** bradygaster (via Copilot)  
**What:** Always run `gh auth switch --user bradygaster` before any GitHub operation. Brady has two sessions: personal (bradygaster) and EMU (bradyg_microsoft). EMU account gets "Unauthorized" on the Blacklite/crew repo.  
...
**By:** Brady (via Copilot)  
**What:** All MCP tools/servers available in the Copilot CLI session MUST be available to crew agents at runtime. All skills loaded into the session — whether from .copilot/skills/ or via plugins (Azure skills, etc.) — must also be available. The three-layer model must surface everything the parent session has.  
...
- The "institutional knowledge" is already encoded in `.crew/skills/` and the charter
...
- The merge driver setup for `.crew/` state integrity is clever
...
From charter.md:
> "Zero tolerance for state corruption — if .crew/ state gets corrupted, everything breaks"
...
3. **Rollback Runbook**
   ...
   - Keep runbook in `.crew/skills/release-rollback/`
...
- Comprehensive knowledge of Crew's branching model and CI/CD infrastructure
...
- The release process skill exists now (`.crew/skills/release-process/SKILL.md`) — but Kobayashi should have created this after failure 2, not after failure 3
...
**Brady is right to be furious.** Six hours of `latest` pointing to a phantom npm version is a production incident. External users saw broken state. This damages Crew's credibility.
...
**Recommendation:**
1. **Archive Kobayashi's charter** to `.crew/agents/kobayashi-archived/` with full history preserved
...
3. **Initialize new agent with:**
   ...
   - `.crew/skills/release-process/SKILL.md` as foundational knowledge
...
- `.crew/agents/trejo/charter.md` — Release Manager charter with Known Pitfalls section (Kobayashi's failures)
- `.crew/agents/trejo/history.md` — Seeded with project context and v0.8.22 disaster lessons
- `.crew/agents/drucker/charter.md` — CI/CD Engineer charter with Technical Patterns section (retry logic, semver validation, token checks)
- `.crew/agents/drucker/history.md` — Seeded with CI/CD context and npm propagation delay lessons

## Kobayashi Status

Moved to `.crew/agents/_alumni/kobayashi/` (already done). Charter preserved as learning artifact.
...
- Release process skill (`.crew/skills/release-process/SKILL.md`) remains the definitive runbook

## Next Steps

1. ✅ Charters created for Trejo and Drucker
2. ⏳ Update `.crew/team.md` to reflect roster change (Scribe's task)
3. ⏳ Update `.crew/routing.md` to route release issues to Trejo, CI/CD issues to Drucker (Scribe's task)
...
8. ❌❌❌ **npm MANGLED 0.8.21.4 into 0.8.2-1.4** (major.minor.patch-prerelease). This went to the npm registry. The `latest` dist-tag pointed to a phantom version that was never intended. Anyone running `npm install @blacklite/crew-sdk` got version `0.8.2-1.4` — a version that doesn't exist in our repo.
...
- [ ] Write release process skill document (`.crew/skills/release-process/SKILL.md`) with step-by-step checklist — **IN THIS RETROSPECTIVE**
...
- [ ] Document NPM_TOKEN requirements in `.crew/decisions.md` (Automation token, no 2FA)
...
- [ ] Read `.crew/skills/release-process/SKILL.md` before ANY release work
...
Created `.crew/skills/release-process/SKILL.md` with the definitive step-by-step release checklist. This is now the ONLY way to release Crew.
...
**Key Patterns:**
- Load-bearing: crew-ci, crew-npm-publish, crew-insider-publish, crew-release, crew-preview, crew-promote, crew-insider-release
- Administrative: crew-triage, crew-issue-assign, crew-label-enforce, sync-crew-labels, crew-heartbeat, crew-docs, crew-docs-links
- Identified potential weakness: `crew-release` and `crew-npm-publish` both trigger on `release: published` with no explicit job dependency — works but fragile

---

### Pre-Publish Preflight Job (Booster)
**Date:** 2026-03-23  
**Status:** Implemented in crew-npm-publish.yml  
...
**Impact:** All crew members — publish pipeline will reject any PR that accidentally leaves `file:` references. No team changes needed; this is passive safety.

---

### `crew version` Subcommand Handler (EECOM)
**Date:** 2026-07-15  
**What:** `crew version` returned "Unknown command" while `crew --version` worked. Fixed by handling `version` inline in `cli-entry.ts` alongside `--version`/`-v` flag, rather than creating a separate command file.
...
**What:** "I always want the crew to facilitate this for me" — all publishing, deployments, and release processes must be driven by crew agents (primarily Surgeon), not by Coordinator or user manually.
...
- Release process skill created at `.crew/skills/release-process/SKILL.md`
...
**Implementation:** 
- Removed all `npx @blacklite/crew-cli` alternatives from user-facing docs
- Replaced with `npm install -g @blacklite/crew-cli` for install; `crew <command>` for usage
- Insider builds: `npm install -g @blacklite/crew-cli@insider` + `crew upgrade`
...
- `npx` for dev tools (changeset, vitest, astro, pagefind) — not Crew CLI
...
**Rationale:** README had grown to 512 lines — ~212 were SDK internal docs duplicating `docs/src/content/docs/reference/`. New users got overwhelmed before running `crew init`. Brady confirmed: "QUITE long."
...
**What:** Comprehensive blog post documenting Crew's biggest release to date. 10 features with storytelling format:
...
**Features covered:**
- Personal Crew (ambient discovery + Ghost Protocol)
...
**No npx:** All install references use `npm install -g @blacklite/crew-cli`. Firm per Brady's distribution directive.

**Breaking changes:** None — all opt-in. Existing Crews work as-is.
...
- Personal Crew Governance Layer
...
2. **GitHub Actions Workflow Cache Race (Infrastructure)** — After deleting `crew-publish.yml`, GitHub workflow index didn't refresh for 10+ minutes. 422 error persisted even after file deletion. Infrastructure bug, not your code. FIX: Documented in runbook; escalation protocol (if workflow_dispatch fails twice, switch to local publish).
...
**Date:** 2026-03-23  
**Issue:** #577  
**What:** Agent name extraction logic extracted from inline regex in `shell/index.ts` into dedicated pure function `parseAgentFromDescription(description, knownAgentNames)` in `packages/crew-cli/src/cli/shell/agent-name-parser.ts`.  
...
**Date:** 2026-03-23  
**Issue:** #577  
**What:** All spawn templates in `.crew-templates/crew.agent.md` MUST include `name: "{name}"` parameter set to agent's lowercase cast name (e.g., `name: "eecom"`, `name: "fido"`).  
...
**Impact:** Any new spawn template or template update must include `name` parameter. `.crew-templates/crew.agent.md` is canonical; all derived copies in agent charters are secondary.

3. **npm Workspace Publish Broken (Tool Gap)** — `npm -w packages/crew-sdk publish` hangs indefinitely when npm 2FA set to `auth-and-writes` (needs OTP from authenticator app). Local machine without authenticator becomes soft hang. FIX: Policy — 2FA must be `auth-only`; always `cd` into package directory for publish.
...
- **Features Shipped, Discussions Pending Close:**
  ...
  - #299 (crew CLI vs copilot) — answered with docs link; safe to close
...
4. **Token types matter.** User tokens ≠ Automation tokens. This should have been in `.crew/decisions.md` from day one.
...
2. **Follow the skill document.** `.crew/skills/release-process/SKILL.md` is now the source of truth. Read it. Follow it. Don't skip steps.
...
**Create `.crew/skills/release-process/SKILL.md`:**
...
5. **Wait for publish:** Monitor workflow at https://github.com/Blacklite/crew/actions
6. **Verify npm:** Check packages at npmjs.com/@blacklite/crew-cli and crew-sdk
...
1. Within 72 hours: `npm unpublish @blacklite/crew-cli@{VERSION}`
2. After 72 hours: Publish corrected version with patch bump

## Known Failure Modes

See `.crew/agents/kobayashi/charter.md` Failure 3 for complete incident report.
```
...
**Completed:**
- GitHub Release published: https://github.com/Blacklite/crew/releases/tag/v0.8.21
...
- History: `.crew/agents/kobayashi/history.md` (Release v0.8.21 section)
...
3. **Published to public registry:** `@blacklite/crew-sdk@0.8.2-1.4` is LIVE on npm (verified 2026-03-07)
4. **Made `latest` for ~5 minutes** — any user running `npm install @blacklite/crew-sdk` during that window got garbage
...
- Users explicitly installing `@blacklite/crew-sdk@0.8.2-1.4` will get the broken version forever
...
Kobayashi proposed guardrails in `.crew/decisions/inbox/kobayashi-release-guardrails.md`:
...
But Kobayashi corrupted **npm distribution state** — which is WORSE than .crew/ state corruption. npm state is:
...
Users deserve a distribution pipeline they can trust. Right now, `@blacklite/crew-sdk@0.8.2-1.4` is on npm forever. 
...
**Gaps (non-blocking):**
- `streams` alias routed but not smoke-tested (same code path as tested `subcrews` — low risk)
...
**By:** Brady (via Copilot)
**What:** When the user says "team take a nap", the coordinator should run the `crew nap` CLI command rather than treating it as a casual sign-off.
**Why:** User request — captured for team memory
...
2. **Extract & deduplicate findings** — Both identified same critical issues (crew-release.yml broken, semver validation missing, bump-build.mjs footgun, dev branch unprotected). Merged into single list.
...
- crew-release.yml is completely broken (test failures) — **P0 blocker**
...
2. **CI validation:** crew-ci.yml validates versions, tests pass before merge
...
- v0.8.22 disaster as motivation (worst release in Crew history)
...
- **P0 (5 items):** Fix crew-release.yml tests, add semver validation, fix bump-build.mjs, enforce SKIP_BUILD_BUMP, protect dev branch
- **P1 (10 items):** NPM_TOKEN checks, dry-run, fix crew-ci.yml tests, resolve insider/insiders naming, preview branch decision, apply validation to insider publish, consolidate workflows, pre-publish checklist, dist-tag hygiene, automated rollback
...
- **Decision 1:** Consolidate publish.yml and crew-publish.yml? → **Recommendation:** Delete crew-publish.yml (use publish.yml as canonical)
- **Decision 2:** Delete or fix crew-release.yml? → **Recommendation:** Fix (automation is valuable, tests are fixable)
...
- **Phase 4:** Hardening (5-7 days) — fix crew-ci.yml, harden insider publish, pre-publish checklist, rollback automation, tag validation
...
- crew-release.yml success rate ≥ 95% (no more than 1 failure per 20 runs)
...
crew-release.yml: 9+ consecutive failures due to ES module syntax errors (`require()` instead of `import` with `"type": "module"`). This is blocking ALL releases from main. **Fix this first.**
...
15 workflows, 3 are unclear/redundant (crew-publish.yml, preview workflows, heartbeat). Consolidation needed.
...
2. **Drucker takes P0 items #1-4** — Fix crew-release.yml tests, add semver validation, fix bump-build.mjs, enforce SKIP_BUILD_BUMP.
...
- **Unblocks releases:** Fixing crew-release.yml tests enables releases from main.
...
**By:** Flight  
**What:** Integration map for Andi's distributed-mesh extension into Crew core.  
**Why:** The distributed mesh pattern is proven (3-model consensus), solves multi-machine coordination, and fits Crew's zero-dependency architecture. This guidance ensures clean integration without architectural drift.
...
- `templates/skills/distributed-mesh/SKILL.md` — Shipped with Crew npm package, copied into new projects via init/upgrade
- `packages/crew-sdk/templates/skills/distributed-mesh/SKILL.md` — SDK template for programmatic access
- `packages/crew-cli/templates/skills/distributed-mesh/SKILL.md` — CLI template for scaffolding

**Runtime location:**
- `.crew/skills/distributed-mesh/SKILL.md` — User-owned, never overwritten by upgrades (follows existing skill convention)
...
- Front matter: `{ title: "Distributed Mesh", description: "Coordinate crews across machines using git as transport" }`
- Content sections:
...
**Cross-references:**
- Link from `scenarios/multiple-crews.md` → "For crews on different machines, see [Distributed Mesh](../features/distributed-mesh.md)"
- Link from `features/streams.md` → "SubCrews partition work within a repo. Distributed Mesh connects crews across machines."
...
**What it does:** One-time snapshot export → zip → import into another crew. Cherry-pick skills, merge histories, handle version conflicts.

**Relationship:** **Complementary, not overlapping.**
- **Export/import** = One-time knowledge transfer when creating a new crew or merging teams.
- **Distributed mesh** = Continuous coordination between running crews on different machines.

**Example:** Export frontend-crew's skills, import them into backend-crew during onboarding (export/import). After onboarding, both crews coordinate via mesh for daily work (mesh).

**Code changes needed:** NONE. Export/import stays as-is. Mesh is additive.

### 2. `src/multi-crew.ts` — Local Crew Resolution
**What it does:** Resolves multiple personal crews on the same machine via `crews.json` in global config directory (`~/.config/crew/crews.json`). Each crew has its own `.crew/` state directory. Supports active crew switching.

**Relationship:** **Orthogonal.**
- **multi-crew.ts** = "Which .crew/ directory am I using on this machine?"
- **Distributed mesh** = "How do I coordinate with crews whose .crew/ directories are on other machines?"

**Example:** Developer has three local crews (auth-crew, api-crew, infra-crew) in their `~/.config/crew/crews.json`. Each crew's mesh.json can point to remote crews on CI runners or other developers' machines.

**Code changes needed:** NONE. Multi-crew and mesh solve different problems.

### 3. `src/streams/` — SubCrews (Label-Based Partitioning)
**What it does:** Partitions work within a single repo across multiple Codespaces. Each SubCrew filters by GitHub label (`team:ui`, `team:backend`) and restricts to specific directories. Enables parallel work without agent context overload.

**Relationship:** **Scoping axis is different.**
- **SubCrews** = Scope work within a repo (same .crew/ state, different label filters).
- **Distributed mesh** = Connect crews across repos/machines (different .crew/ state directories).

**Example:** Frontend SubCrew and Backend SubCrew both run in the same repo, each in their own Codespace, filtering by label. Both SubCrews might use distributed mesh to coordinate with a CI-crew running on a remote server.

**Code changes needed:** NONE. SubCrews and mesh are composable.

### 4. `src/remote/` — RemoteBridge (WebSocket PWA Control)
**What it does:** WebSocket server that bridges Crew's EventBus to a PWA client. Enables remote control of a running Crew instance from a browser — send prompts, see streaming output, approve permissions. Synchronous RPC-style interaction. Requires a running server.

**Relationship:** **Mesh is the replacement for remote-to-remote agent-to-agent use cases.**

**Decision:** The `src/remote/` module stays for **human-to-agent** remote control (PWA → Crew). Distributed mesh handles **agent-to-agent** coordination across machines.
...
✅ **Update `docs/src/content/docs/scenarios/multiple-crews.md`** — Add paragraph + link: "For crews on different machines, see Distributed Mesh"  
✅ **Update `docs/src/content/docs/features/streams.md`** — Add note: "SubCrews partition work within a repo. Distributed Mesh connects crews across machines."  
...
2. **Community PR quality gate:** FIDO identified systemic issues in lower-tier PRs:
   - **Changeset package naming:** 4 PRs used unscoped `crew-cli` instead of `@blacklite/crew-cli`
...
❌ **crew.config.ts** — Does NOT need a `mesh` section. The mesh config lives in `mesh.json` as a separate concern. crew.config.ts is for agent behavior, not transport.

❌ **crew.agent.md** — Does NOT need mesh awareness. Agents learn mesh patterns from the skill file, not the coordinator prompt.
...
❌ **CLI commands** — NO `crew mesh sync` command. The sync scripts are reference implementations, not core CLI features. Users run them directly (`./scripts/mesh/sync-mesh.sh`) or integrate them into CI workflows (`github-actions`, `cron`). Rationale: Crew is an agent framework, not a sync orchestrator. Mesh is convention + optional scripts.

### Optional Future Enhancements (Not Blocking v1)
🔮 **Init flow enhancement:** During `crew init`, ask "Will this crew coordinate with remote crews?" If yes, copy `mesh.json.example` → `mesh.json` and prompt for first remote entry. Implementation: ~20 lines in init flow.

🔮 **Auto-sync hooks:** Git pre-commit hook that runs `sync-mesh.sh` before push. Implementation: Add to `.crew/templates/hooks/pre-commit.sample`. User enables manually (`chmod +x`).

🔮 **Mesh health check:** `crew doctor` command extension that validates mesh.json schema, tests git auth for Zone 2 remotes, validates HTTP endpoints for Zone 3. Implementation: ~50 lines, non-critical.
...
- [ ] Copy extension files into Crew repo as documented above
- [ ] Write `docs/features/distributed-mesh.md`
- [ ] Update cross-reference docs (multiple-crews.md, streams.md)
- [ ] Add mesh.json.example to templates
- [ ] Add SKILL.md to all three template directories
- [ ] Verify skill appears in new crew init

**Phase 1 — Validation (Test in Practice):**
- [ ] Use mesh in Crew's own development (coordinate across developer machines)
- [ ] Validate mesh works with existing multi-crew setup
- [ ] Confirm no conflicts with SubCrews feature
...
3. **Respects write partitioning:** Each crew owns its directory. Structurally conflict-free.
...
- Crew is an agent framework, not a sync scheduler.
...
- RemoteBridge is for human-to-agent control (PWA → Crew).
- Mesh is for agent-to-agent coordination (Crew → Crew).
- Mixing the two leads to MCP federation, the subsystem we intentionally killed.

**Why mesh.json is NOT in crew.config.ts:**
- crew.config.ts is TypeScript, requires compilation, stores agent behavior config.
...
**Dependencies:**
- Git must be installed (already required by Crew)
...
- `packages/crew-sdk/templates/skills/distributed-mesh/` — SDK templates
- `packages/crew-cli/templates/skills/distributed-mesh/` — CLI templates
- `.crew/skills/distributed-mesh/` — this crew's runtime skills
...
The sync scripts (~40 lines each, bash and PowerShell) materialize remote crew state locally using git/curl. No daemons, no running processes — Phase 1 distributed coordination.
...
The distributed mesh enables crews on different machines to coordinate via git (same org) and HTTP (cross-org). Source material existed in `C:\dev\crew-architecture\distributed-mesh\README.md`.
...
- `features/` — ✅ correct home; features are "what Crew can do and how to enable it"
...
- Relation to SubCrews (within-repo partitioning vs cross-machine coordination)
...
Added pointer in `scenarios/multiple-crews.md`:
> Want continuous coordination instead? See [Distributed Mesh](../features/distributed-mesh.md) — it syncs remote crew state via git and HTTP.

This guides readers from snapshot-based export/import to continuous mesh coordination.

## Why This Matters

Users asking "how do I coordinate multiple crews?" now have two paths clearly documented:
1. **Snapshot-based:** Export/import (one-time copy, scenarios/multiple-crews.md)
...
### Implementation Plan: Ambient Personal Crew (#329 + #344)

**Author:** Flight (Lead)  
**Date:** 2026-03-22  
**Issues:** #329, #344  

Personal crew implementation across 4 PRs with clear phase dependencies:

- **PR #1 — SDK Foundation** (EECOM): ResolvedCrewPaths.personalDir, resolvePersonalCrewDir(), PersonalAgentMeta type, resolvePersonalAgents(), mergeSessionCast(), ensureCrewPathTriple()
- **PR #2 — CLI Surface** (EECOM): crew personal {init,list,add,remove}, crew cast, --team-root flag, wiring + exports
- **PR #3 — Governance** (Procedures, concurrent): crew.agent.md updates for personal crew, Ghost Protocol in templates, personal-charter.md template
...
- CREW_NO_PERSONAL env var gates ambient discovery at Phase 1, earliest point
- Dual-root path guard (ensureCrewPathDual) extended to triple-root (ensureCrewPathTriple) for personal crew dirs
- Personal crew paths resolved via platform detection (never hard-coded)
...
**Activation:**
- Persistent: `"economyMode": true` in `.crew/config.json`
- Session: `--economy` CLI flag (CREW_ECONOMY_MODE=1 env var)
- Toggle: `crew economy on|off` command

**Implementation:**
- ECONOMY_MODEL_MAP + applyEconomyMode() in packages/crew-sdk/src/config/models.ts
- readEconomyMode() + writeEconomyMode() in config/models.ts
- resolveModel() accepts economyMode option
- crew economy {on|off} command
...
Governance additions needed in crew.agent.md:
...
**Status:** DRAFT — awaiting Flight review before merging to crew.agent.md.

### Personal Crew Governance — Consult Mode Awareness (#344)

**Author:** Procedures (Prompt Engineer)  
**Date:** 2026-03-22  

Governance additions for crew.agent.md to close consult-mode coordinator awareness gap:

1. **Consult Mode Detection** — check config.json for `"consult": true` after resolving team root
2. **Personal Crew Path Reference** — platform-specific paths (Linux, macOS, Windows) resolved via resolveGlobalCrewPath(), never hard-coded
...
**Proposed Skill** (post-approval): `.crew/skills/consult-mode/SKILL.md` — coordinator behavior for consult mode (detection, spawn guidance, extraction workflow).
...
Model selection uses 5-layer hierarchy with Layer 0 (Persistent Config) stored in .crew/config.json:
...
When Crew detects a rate limit, offer actionable recovery: (1) switch to equivalent/alternative models, (2) offer economy mode (#500) as fallback. Rate limits should be a pivot point, not a dead end. *Status: Directive captured; soft dependency on #500.*
...
Template files have 5 duplicate locations. Canonical source is `.crew-templates/`. All copies in `templates/`, `packages/crew-cli/templates/`, `packages/crew-sdk/templates/`, `.github/agents/` must match canonical.

**Enforcement:** `test/template-sync.test.ts` enforces byte-for-byte parity for casting-policy.json, universe count parity for crew.agent.md, cross-file count validation.
...
`crew doctor` now detects both Layer 1 and Layer 2 issues. Matches vscode-jsonrpc v9.x forward-compatibility.

**Impact:** If users report ESM errors on Node 22/24, direct them to `crew doctor`.
...
1. **Persistent:** `"economyMode": true` in `.crew/config.json` (survives sessions)
2. **Session:** `--economy` CLI flag (sets `CREW_ECONOMY_MODE=1` env var, current session only)
3. **Toggle command:** `crew economy on|off` writes to config.json
...
- `ECONOMY_MODEL_MAP` + `applyEconomyMode()` in `packages/crew-sdk/src/config/models.ts`
- `readEconomyMode()` + `writeEconomyMode()` in `packages/crew-sdk/src/config/models.ts`
...
- `crew economy [on|off]` command in `packages/crew-cli/src/cli/commands/economy.ts`
- `--economy` global flag in `cli-entry.ts`
...
Workshop participants reported `ERR_UNKNOWN_BUILTIN_MODULE` when using Crew on Node <22.5.0. The `node:sqlite` built-in (used by the Copilot SDK for session storage) requires Node 22.5.0+.
...
**Hard-fail at startup with a clear, actionable message.** If Node <22.5.0 is detected, Crew exits immediately (`process.exit(1)`) with:

```
✗ Crew requires Node.js ≥22.5.0 (you have v20.18.0).
...
- `crew doctor` now includes a Node version check so users can proactively diagnose
...
When Crew catches a rate limit error (HTTP 429, "rate limit", "quota exceeded"), surface it explicitly rather than hiding it under "Something went wrong processing your message."

## Rationale

Generic error messages fail the user in two ways:
1. They don't explain *why* the error happened (rate limit vs. network vs. bug)
2. They give no recovery path — the user is stuck with "run crew doctor" which previously showed nothing useful
...
1. **`error-messages.ts`** — added `rateLimitGuidance()` and `extractRetryAfter()`. Rate limit guidance shows:
   ...
   - Recovery: time until reset (if parseable), `crew economy on`, and config.json model override

2. **`shell/index.ts` catch block** — detects rate limits via `instanceof RateLimitError` OR regex on the error message (`/rate.?limit|quota.*exceed|429/i`). Writes `.crew/rate-limit-status.json` on detection for doctor to read.

3. **`doctor.ts`** — added `checkRateLimitStatus()`. Reads `.crew/rate-limit-status.json` and reports:
...
- **Making a live API call in `crew doctor`** — rejected, adds latency and may itself be rate-limited
...
**Labels:** `crew:eecom`, `crew:procedures`, `crew:flight`
...
| issue-lifecycle.md referenced but missing | ✅ Confirmed — two broken refs in crew.agent.md |
| crew.agent.md has Worktree Awareness section | ✅ Confirmed — lines 569–607 |
| git-workflow skill defaults to checkout -b | ✅ Confirmed — worktree is documented but separate path |
| resolveCrew() detects .git as file (worktree pointer) | ✅ Confirmed — resolution.ts line 66–93 |
...
**Why it's P2 not P1:** Most current Crew users run single-agent sequential workflows. Parallel multi-agent execution is the advanced case. The SubCrews/Workstreams design (#509–#511) will eventually need this, but those aren't in Wave 1.
...
1. **Doc fix: Create issue-lifecycle.md** (quick win, 1 hour)  
   Owner: `crew:procedures`  
   Fix the broken reference in crew.agent.md. Standalone — no code changes.
...
3. **Coordinator: Pre-spawn worktree creation**  
   Owner: `crew:procedures`, `crew:eecom`  
...
**Recommendation:** Sub-issue #1 (doc fix) is a quick win — ship immediately. Sub-issues #2–#5 go into the post-Wave-1 queue, likely alongside SubCrews work where parallel execution becomes a hard requirement.

## Top 5 Priority Recommendations for v1 Progress

1. **#508 — Ambient Personal Crew** — Wave 1 in progress, highest user-facing value
2. **#498 — Remove .crew/ from version control** — Critical for v1 GA; repos shouldn't ship team state
...
5. **#347 — Shore up crew init --sdk** — Onboarding gate; first impression for SDK users
...
# Proposal: Economy Mode Integration in crew.agent.md

**By:** Procedures (Prompt Engineer)  
**Date:** 2026-03-22  
**Issues:** #500  
**Status:** DRAFT — for Flight review before merging to crew.agent.md

---

## Summary

Economy mode is a new session/persistent modifier that shifts Layer 3 (Task-Aware Auto-Selection) to cost-optimized alternatives. This proposal documents the governance additions needed in `crew.agent.md`.

**Note to Flight:** Procedures owns the skill design. Crew.agent.md is governance — Flight reviews before commit.
...
- **Activation (persistent):** User says "always use economy mode" OR `"economyMode": true` in `.crew/config.json` → persists across sessions.
- **Deactivation:** "turn off economy mode" or remove `economyMode` from `config.json`.
- **On session start:** Read `.crew/config.json`. If `economyMode: true`, activate economy mode before any spawns.
...
- `claude-sonnet-4.6` appears in the model-selection SKILL.md but is absent from the valid models list in crew.agent.md. Add it under Standard.
...
(Added `claude-sonnet-4.6`, `gpt-5.4`, `gpt-5.3-codex` which appear in the model-selection SKILL.md fallback chains but are missing from crew.agent.md's catalog.)

---

## 5. Config Schema Addition

Add `economyMode` to the config schema reference in crew.agent.md (wherever `defaultModel` is documented):
...
The skill (`economy-mode/SKILL.md`) covers the coordinator behavior in detail. This proposal is the governance side — ensuring crew.agent.md is the authoritative source for the feature.

---

## References

- Skill: `.crew/skills/economy-mode/SKILL.md`
- Issue: #500
- Model selection skill: `.crew/skills/model-selection/SKILL.md`

---

# Proposal: Personal Crew Governance Awareness in crew.agent.md
...
Crew has a consult mode (implemented, per `prd-consult-mode.md`) and personal crew semantics (via `resolveGlobalCrewPath()`), but `crew.agent.md` doesn't tell the coordinator how to reason about either. This proposal documents the gaps and the governance additions needed.

---

## Gap Analysis

### Gap 1: Init Mode references `--global` without explaining personal crew resolution

Current Init Mode says "run `crew init --global` for a personal crew" (implied by CLI docs) but crew.agent.md doesn't explain what a personal crew IS or how the coordinator should detect it.

**What agents need to know:**
- Personal crew = a crew at the global path (resolved via `resolveGlobalCrewPath()`)
  - Linux/macOS: `~/.config/crew/.crew`
  - macOS (alt): `~/Library/Application Support/crew/.crew`
  - Windows: `%APPDATA%\crew\.crew`
- If `.crew/config.json` contains `"consult": true`, the coordinator is working inside a consult session
- `sourceCrew` in `config.json` points to the original personal crew (for Scribe extraction context)

### Gap 2: No coordinator guidance for consult mode

`crew.agent.md` mentions nothing about consult mode. The coordinator doesn't know:
- How to recognize it's in a consult session
- That writes go to the project `.crew/` (isolated copy) — NOT the personal crew
- That Scribe's charter is patched with extraction instructions
- That `.crew/extract/` is a staging area for generic learnings

### Gap 3: TEAM_ROOT works, but personal crew semantics are absent

The coordinator resolves `TEAM_ROOT` correctly (Worktree Awareness section), but:
- No distinction between "project crew" vs "personal crew copy in consult mode"
- No guidance on what to tell agents about their crew context when in consult mode

### Gap 4: Charter templates have no personal-crew-aware patterns

Agent charters have no concept of:
- Consult mode restrictions (agents shouldn't commit to project, shouldn't pollute personal crew)
...
## Proposed crew.agent.md Additions
...
**Consult Mode Detection:** After resolving team root, check `.crew/config.json` for `"consult": true`.

- If `consult: true` → **Consult mode is active.** This is a personal crew consulting on a project.
  - The `.crew/` directory is an isolated copy of the user's personal crew.
  - `sourceCrew` in `config.json` contains the path to the original personal crew.
  - Do NOT read or write to `sourceCrew` — it's out of scope. Only operate within TEAM_ROOT.
  - Scribe's charter is already patched with extraction instructions — no coordinator action needed.
  - Include `🧳 consult` in your session acknowledgment: `Crew v{version} (🧳 consult — {projectName})`
...
### Addition 2: Personal Crew Path Reference

Add a new subsection under "Worktree Awareness":

---

**Personal Crew Paths:** The global crew path is resolved by `resolveGlobalCrewPath()`:

| Platform | Path |
|----------|------|
| Linux | `~/.config/crew/.crew` |
| macOS | `~/Library/Application Support/crew/.crew` |
| Windows | `%APPDATA%\crew\.crew` |

The coordinator should NEVER hard-code these paths. Use `crew --global` or `resolveGlobalCrewPath()` to resolve. Only relevant in consult mode (to understand the `sourceCrew` field) — the coordinator does NOT read the personal crew directly during a session.
...
2. They should NOT reference personal crew paths or personal crew agent names
...
🧳 consult mode active — Fenster (claude-sonnet-4.5) — refactoring auth module
     ↳ decisions staged in .crew/extract/ for review before extraction
```
...
**Skill needed:** `.crew/skills/consult-mode/SKILL.md`
...
- STOP: extraction is always user-driven via `crew extract` — coordinator never auto-extracts
...
```markdown
**Consult Mode Awareness:** If `CONSULT_MODE: true` is in my spawn prompt, I'm working on a project outside my home crew. My decisions here are project-isolated. Write extractable rationale so Scribe can classify them for `crew extract` review.
```

This should be added to `.crew/templates/charter.md` (if it exists) and `.crew/agents/scribe/charter.md` (Scribe already has extraction logic, but clarifying the classification responsibility is valuable).

---

## Rationale

Consult mode is fully implemented at the SDK level (`prd-consult-mode.md`, `crew consult` command) but the coordinator has no awareness of it. The result: agents running in a consult session have no context that they're in a temporary, isolated copy of a personal crew. They might make decisions as if they're permanent, or reference the project in ways that pollute the personal crew on extraction.
...
- Consult mode PRD: `.crew/identity/prd-consult-mode.md`
- Issue: #344
- Flight ambient personal crew note: `.crew/decisions/inbox/flight-ambient-personal-crew.md`
...
- `ensureGitignore` skips entries already covered by a parent path in the existing file (avoids redundant entries like `.crew/log/` when `.crew/` is present).
...
**Note:** `docs/public/robots.txt` references `https://crew.dev/sitemap-index.xml` but `astro.config.mjs` still has `site: 'https://bradygaster.github.io'`. If the crew.dev domain is live, this is fine. If not, the sitemap URL needs updating before merge.
...
**Required:**
1. Move from `.crew/skills/` (team-state) to `templates/skills/` (library content) so it ships as a Crew template, not as hardcoded team state.
...
- `.crew/` is for team state; reusable skill templates belong in `templates/skills/`.
...
3. **#518 (Rate Limiting SDK)** — Approve after fixing test import paths to use `@blacklite/crew-sdk/ralph/rate-limiting` instead of relative `../packages/` paths.
...
**Success Criteria:**
- Phase 1: All members (user-added, Ralph, @copilot) in `crew.config.ts` without manual edits
...
### crew.config.ts is the source of truth for SDK-mode projects

**By:** CAPCOM  
**Date:** 2026-03-11  

When `crew.config.ts` exists, it is the canonical team roster. Markdown files (.crew/team.md, routing.md) are **generated output** from `crew build`.
...
**Impact:**
- `crew build` regenerates markdown from config
- REPL init flow writes crew.config.ts after casting
- Manual team.md edits in SDK mode trigger a warning (suggest `crew migrate --to sdk`)
...
**Why:** Ralph is a core team member, not an optional add-on. Should be present in every Crew project.
...
Prioritize crew.config.ts sync fixes over new commands. Implement in this order:

1. **Fix 1 — crew.config.ts sync utility** (regex-based, upgrade to AST if edge cases arise)
...
**Why:** crew.config.ts sync is load-bearing for the rest. Ralph fixes are quick wins completing a half-implemented feature. CastingEngine is high-value but medium-risk. Hire/remove/flags are polish.
...
**Reference:** Full roadmap at `.crew/identity/sdk-init-implementation-roadmap.md`
...
**Why:** User request — captured for team memory. GitHub Actions cron burns minutes and money. Crew uses event-based triggers and local watch mode instead.
...
**What:**
1. Add `edited` to CI pull_request trigger types in `.github/workflows/crew-ci.yml` (types: [opened, synchronize, reopened, edited]) to catch PR retargeting.
...
**By:** Brady (via Copilot)
**What:** Crew featured on the GitHub Blog ("How Crew runs coordinated AI agents inside your repository") and .NET Rocks! Episode 1994, both published March 19, 2026. First major press coverage.
...
3. Observability: Add a `crew doctor` check that detects whether `vscode-jsonrpc` has proper exports.
...
- Two publish channels: stable (crew-npm-publish.yml) and insider (crew-insider-publish.yml)
...
- [ ] SDK dependency in crew-cli is a version range, not `file:../crew-sdk`
- [ ] `npm run build` succeeds clean (no TypeScript errors)
- [ ] `npm -w packages/crew-sdk pack --dry-run` and `npm -w packages/crew-cli pack --dry-run` both succeed
- [ ] Git tag matches package.json versions
- [ ] CHANGELOG.md updated for this version
- [ ] GitHub Release draft created (triggers crew-npm-publish.yml on publish)

## Publish via CI (Recommended Path)
- Create GitHub Release → triggers `crew-npm-publish.yml`
...
- Monitor: Actions tab → "Crew npm Publish" workflow

## Publish via workflow_dispatch (Manual Trigger)
- Go to Actions → "Crew npm Publish" → Run workflow
...
- Steps:
  ...
  3. `cd packages/crew-sdk && npm publish --access public --otp=<CODE>`
  4. Verify: `npm view @blacklite/crew-sdk@<VERSION> version`
  5. `cd ../crew-cli && npm publish --access public --otp=<CODE>`
  6. Verify: `npm view @blacklite/crew-cli@<VERSION> version`
...
- **If you get 422 "Version already exists":**
  1. Check if the package IS actually published: `npm view @blacklite/crew-<pkg>@<VERSION>`
...
- `npm view @blacklite/crew-sdk@<VERSION> version`
- `npm view @blacklite/crew-cli@<VERSION> version`
- `npx @blacklite/crew-cli@<VERSION> --version` (cold install test)
...
- Files to update: root `package.json`, `packages/crew-sdk/package.json`, `packages/crew-cli/package.json`
...
**Reviewer:** Flight  
**Where it runs:** New job in `crew-ci.yml`, runs on every PR and push to dev/insider  
**What it checks:** Scans `.github/workflows/*.yml` for `npm publish` invocations that are NOT workspace-scoped.

### Exact Implementation

Add a new job to `.github/workflows/crew-ci.yml`:
      ...
      - name: Reject non-workspace npm publish in workflows
        run: |
          ...
          if [ "$VIOLATIONS" -gt 0 ]; then
            ...
            echo "Policy: Always use 'npm -w packages/crew-sdk publish' or 'npm -w packages/crew-cli publish'."
...
1. **List all runs for the ghost workflow:**
   ```bash
   gh api repos/Blacklite/crew/actions/workflows/250121956/runs \
...
2. **Delete every run:**
   ...
   while read run_id; do
     ...
     gh api -X DELETE repos/Blacklite/crew/actions/runs/$run_id
   done
   ```

3. **Verify the workflow is gone:**
   ```bash
   gh api repos/Blacklite/crew/actions/workflows/250121956
   ```
...
| 2 | #557 | Add `publish-policy` job to crew-ci.yml | FIDO or Procedures | Nothing |
...
- No changes to `crew-npm-publish.yml` or `crew-insider-publish.yml` (they're working correctly)
...
# Decision: `init --global` bootstraps personal-crew/ directory
...
`init --global` now also creates the `personal-crew/` directory (via `ensurePersonalCrewDir()`) alongside the full `.crew/` structure. Repo-level `init` detects and acknowledges existing personal crews.

## Why

`resolveGlobalCrewPath()` returns `~/.config/crew/` — the container. But `resolvePersonalCrewDir()` looks for `~/.config/crew/personal-crew/`. Without the bridge, `init --global` never created the subdirectory that the rest of the personal crew system depends on.

## Impact

- `ensurePersonalCrewDir()` is a new SDK export — any code that needs to guarantee the personal crew directory exists should use it.
...
- **#604 vs #603:** Functionally identical. #603 uses `.crew/` paths matching project conventions; #604 uses `templates/` (non-standard for agents).
...
- **#610** (docs broken link) → crew:pao, P1 — 5-minute fix blocking diberry's PR #611 CI
- **#590** (getPersonalCrewRoot bug) → crew:eecom, P0 — personal crew init broken for all users since v0.9.1
- **#591** (hiring wiring docs) → crew:procedures, P1 — matches PR #592 (joniba), docs-only, high clarity
...
- **#597** (upgrade CLI docs) → crew:pao + crew:network, P2 — user confusion, docs fix + UX improvement
- **#588** (model list update) → crew:procedures, P2 — hardcoded model list in crew.agent.md + templates
- **#554** (broken external links) → crew:pao, P2 — automated link checker output, investigate failures

### Questions (No Crew Assignment)
- **#589** (skills placement) → community reply — clarify `.copilot/skills` vs `.github/skills` vs `.claude/skills`
- **#494** (model vs crew model) → community reply — clarify Copilot CLI `/models` vs crew.agent.md model preference

### Long-Horizon Feature Work (P2-P3)
- **#581** (ADO Support PRD) → crew:flight, P2 — comprehensive PRD, but blocked until SDK-first parity (#341) ships

## Work Session Priority (Top 5)

1. **#610** → PAO — fix broken link (5 min), unblocks #611
2. **#590** → EECOM — fix getPersonalCrewRoot(), critical user-facing bug
...
### 2026-03-25: Personal Crew Path Canonicalization

**By:** EECOM (Core Developer)  
**Date:** 2026-03-25  
**Issue:** #590  

**Decision:** The canonical subdirectory for personal crew (inside global config dir) is personal-crew/. All path resolution functions must use this name. The .crew name is reserved for project-local crew directories only.

**Context:** getPersonalCrewRoot() in consult.ts used .crew as subdirectory, while esolvePersonalCrewDir() and nsurePersonalCrewDir() in esolution.ts both use personal-crew. This path mismatch broke all personal crew discovery.

**Implications:**
- Any new function resolving personal crew path must use personal-crew, not .crew
- Consider extracting subdirectory name as shared constant (PERSONAL_CREW_DIR = 'personal-crew') if more resolution helpers are added
...
**Implementation:** All 5 crew.agent.md template copies synchronized via scripts/sync-templates.mjs
...
**Date:** 2026-03-25  
**Summary:** Copilot CLI shipped 8 releases (1.0.4→1.0.11) between March 11-23. Three contain high-impact changes directly affecting Crew routing.

**Critical CLI Changes Identified:**

| Version | Date | Change | Crew Impact |
|---------|------|--------|--------------|
| **1.0.11** | Mar 23 | Monorepo instruction discovery — finds *.agent.md and copilot-instructions.md at EVERY directory level (cwd → git root) | 🔴 HIGH: Crew has 6+ copies in templates → duplicate/conflicting instructions merged → dilutes routing |
...
**Key Insight:** SDK version pinning (^0.1.32) only controls Crew's SDK dependency. The Copilot CLI runtime (system prompt assembly, agent lifecycle, task tool behavior) auto-updates independently. **Crew needs platform-resilient prompt injection, not just "works with current CLI."**
...
**Full Report:** .crew/decisions/inbox/capcom-cli-research.md

---

### 2026-03-25: Crew v0.9.0/v0.9.1 Routing Regression Analysis

**By:** GNC (Node.js Runtime)  
**Date:** 2026-03-25  
**Summary:** Regression introduced in Crew v0.9.0 (March 22). v0.9.0→v0.9.1 was CI/publish fixes only — identical coordinator prompts.
...
- New content (personal crew governance, worktree lifecycle, gap analysis) adds inline coordinator responsibilities
- Dilutes core "route, don't do domain work" constraint (buried at line 1016 of 946-line prompt)

**Symptom 2: Routing feels broken**
- **Primary:** Workstream→Personal Crew replacement broke existing routing configs
- **Contributing:** Skills path changed (.copilot/skills/ → .crew/skills/)
...
**Template Status:** scripts/sync-templates.mjs correctly propagates from .crew-templates/ to all 4 targets. All copies in sync — no template drift.
...
**P1 — Next Release:**
2. Audit coordinator prompt weight (consider moving worktree lifecycle, personal crew governance, gap analysis to skills)
3. Reinforce routing constraint near top of template
4. Create routing regression tests

**P2 — Track/Validate:**
5. Document workstream→personal crew migration for users
6. Implement "prompt budget" metric (alert if growth >5% per release)

**Full Report:** .crew/decisions/inbox/gnc-internal-research.md
...
**By:** Procedures (Prompt Engineer)  
**Date:** 2026-03-25  
**Issue:** #613 — VSCode Autopilot breaks Crew agent execution  

...
| T1.3 | Marketing hype | "Crew is the best-in-class solution for this" |
...
|---------|---------------|-----------------|----------------------|
| T2.1 | Welcome | Author acknowledgment | "Hey @user! Welcome to Crew 👋" |
| T2.2 | Troubleshooting | Action-oriented close | "Let us know if that helps!" |
| T2.3 | Feature guidance | Context on current state | "Currently, Crew handles this by..." |
...
| T4.3 | Thread where crew member already responded | Skip (no draft) |
...
> Hey @newuser! Welcome to Crew 👋 Thanks for opening this — great first issue!
>
> To answer your question: Crew uses `.crew/team.md` to define your team roster. You can add members by editing that file directly or running `crew init`.
>
...
> 1. Check that `.crew/casting/registry.json` exists and is valid JSON
> 2. Run `crew build` to regenerate from source
> 3. If that doesn't work, try deleting `.crew/casting/` and re-running `crew init`
>
> Let us know if that helps, or if you're seeing something different.

### GS-3: Feature Guidance (Medium confidence)
> Great question! Right now, Crew handles cross-repo orchestration through the hub-and-spoke model — you'd have a central `.crew/` in one repo and reference it from others.
>
...
>
> For now, the workaround is to use `crew export` to share config across repos. Hope that helps!
...
> - It's not a config issue (your crew.config.ts parses correctly)
>
> Could you share the output of `crew build --verbose`? That would help us narrow it down. We'll dig deeper and update this thread.

### GS-6: Redirect (High confidence)
> Thanks for reaching out, @asker! This one is actually better suited for the Crew-IRL repo, which covers deployment patterns and infrastructure around Crew.
>
> You can open it here: https://github.com/bradygaster/Crew-IRL — they'll be able to help with the Terraform setup specifically.
>

...
SDK initialization (`crew init --sdk`) produces incomplete and inconsistent crew state. Users who consume `@blacklite/crew-sdk` to build their own crews encounter three critical gaps:

1. **Config ↔ team.md sync broken** — When members are added/removed through SDK workflows, `crew.config.ts` doesn't update. The coordinator flow updates `team.md`, `routing.md`, and `registry.json`, but has no step for `crew.config.ts`. Result: SDK consumers can't reference new members in code.

2. **Built-in members missing from SDK surface** — Ralph (work monitor) and @copilot (coding agent) exist at the coordinator level but aren't included in generated `crew.config.ts`. Ralph works in CLI crews but SDK consumers have no way to reference it. Similarly, `routing.md` has `@copilot` rules pre-configured, but `team.md` roster and `crew.config.ts` are missing the entry.
...
**Impact:**
- SDK crews can't access full feature set (Ralph, @copilot unavailable)
- Adding members requires manual `crew.config.ts` edits (breaks "code-gen from team.md" promise)
...
- ❌ `crew.config.ts` member exports

**Why it matters:** SDK consumers import from `crew.config.ts`. Without this update, newly added members are invisible to user code.

### 2. Built-in Member Gaps
**Issues:** #338, #339  
Ralph and @copilot are first-class members in CLI crews but second-class citizens in SDK crews:
- Ralph: Missing from `crew.config.ts` generation (exists only in coordinator internals)
- @copilot: Pre-configured in `routing.md` but missing from `team.md` roster and `crew.config.ts`

**Why it matters:** SDK users can't leverage work monitoring (Ralph) or coding agent routing (@copilot) — features that work perfectly in CLI crews.

### 3. CastingEngine Bypass
**Issue:** #342  
The `CastingEngine` class is Crew's universe template system — it knows Apollo 13, The Usual Suspects, and curated role definitions. But during `crew init`, the CLI path calls the LLM directly, bypassing `CastingEngine` entirely.
...
**Work:**
1. **Config sync** (#337)
   - Add `crew.config.ts` update step to coordinator's `add-member` and `remove-member` flows
   - Ensure `crew.config.ts` regenerates when `team.md` roster changes
   - Test: Add member via coordinator → verify `crew.config.ts` export appears

2. **Ralph inclusion** (#338)
   - Add Ralph to `crew.config.ts` template generation
   - Include charter path placeholder (Ralph has no charter file — document this as known gap)
   - Test: `crew init --sdk` → verify Ralph in config exports

3. **@copilot roster entry** (#339)
   - Add @copilot to `team.md` roster template (status: 🤖 Coding Agent)
   - Add @copilot to `crew.config.ts` generation
   - Align with existing `routing.md` rules (already configured)
   - Test: `crew init --sdk` → verify @copilot in roster and config

**Success criteria:**
- All three members (user-added, Ralph, @copilot) appear in `crew.config.ts`
- `team.md` ↔ `crew.config.ts` sync verified through add/remove cycles
...
1. **Route CLI init through CastingEngine**
   - Refactor `packages/crew-cli/src/commands/init.ts` to use `CastingEngine.cast()`
...
2. **Universe selection UX**
   - Prompt user to choose universe during `crew init`:
...
3. **Template validation**
   ...
   - Ensure templates produce valid `crew.config.ts` exports
...
1. **Active exercise testing** (#340)
   - Systematically exercise all 29 untested features in SDK crews
...
**Success criteria:**
- All 50 features verified in SDK crews
...
1. **Config completeness:** `crew.config.ts` includes all members (user-added, Ralph, @copilot) without manual intervention
2. **Universe quality:** 90%+ of `crew init` runs use curated templates (Apollo 13/Usual Suspects) vs. custom LLM generation
3. **Feature parity:** 50/50 features verified (100%) in SDK crews
4. **Zero gaps:** No undocumented differences between CLI and SDK crew capabilities
...
2. **@copilot auto-routing:** Should SDK crews enable `copilot-auto-assign: true` by default, or leave it opt-in?
...
- #316 — Cross-Crew Orchestration (may need SDK init patterns)
- #321 — crew upgrade/init does not stamp version (version stamping gap)
- #329 — Personal Crew updates (may intersect with config sync patterns)

...
`crew init --sdk` is **partially implemented but incomplete**. The CLI generates `crew.config.ts` with SDK builder syntax (`defineCrew`, `defineAgent`), but:

1. **CastingEngine is bypassed** — CLI uses a hardcoded personality generator instead of the themed universe templates
2. **Config/team sync is one-way** — Adding/removing members updates markdown files but never touches `crew.config.ts`
...
## Finding 1: crew.config.ts Generation
...
When `crew init --sdk` is run (or `configFormat: 'sdk'` is passed to `initCrew()`):
...
✅ Uses proper imports from `@blacklite/crew-sdk`
...
**Critical gap:** The CLI init command only passes `scribe`. The interactive shell (REPL) init flow creates a full team via `createTeam()` in `cast.ts`, which generates charter.md/history.md files but **never updates crew.config.ts**.
...
3. Pass all cast members (Lead, Developer, Tester, etc.) to `initCrew()` in the `agents` array
...
- Call `CastingEngine.castTeam()` before `initCrew()`
...
5. `createTeam(teamRoot, proposal)` generates:
   - `.crew/agents/{name}/charter.md` (using `generateCharter()`)
   - `.crew/agents/{name}/history.md`
   - `.crew/team.md` (Members table)
   - `.crew/routing.md` (Work Type → Agent table)
   - `.crew/casting/registry.json`
   - `.crew/casting/history.json`
   - `.crew/casting/policy.json`
...
❌ **SDK init does NOT add Ralph** — `initCrew()` only gets `[{ name: 'scribe', ... }]`

### @copilot Handling

**File:** `packages/crew-cli/src/cli/core/team-md.ts`  
**Lines:** 28-88
...
- In `.crew/agents/copilot/charter.md`
- In `crew.config.ts` agents array
- In `registry.json`
...
3. **REPL init should update crew.config.ts** — After `createTeam()` generates markdown files, call `generateSDKBuilderConfig()` and write it
...
```bash
crew init --sdk "Build a React app"
```
...
3. Calls `sdkInitCrew(initOptions)` (line 138)
4. SDK writes:
   - `crew.config.ts` (if `--sdk` flag set)
   - `.crew/agents/scribe/charter.md`
   - `.crew/agents/scribe/history.md`
   - `.crew/team.md` (minimal, no roster)
   - `.crew/.init-prompt` (stores the prompt for REPL auto-cast)

**Critical:** The CLI init does NOT cast a full team. It writes the prompt to `.crew/.init-prompt` and expects the user to run `crew` (REPL) to trigger `handleInitCast()`.
...
1. Check for `.crew/.init-prompt` (line 839-847)
...
- CLI `crew init --sdk` writes config but no team
- REPL auto-cast generates team but doesn't update config
- Result: **crew.config.ts has only Scribe, team.md has full roster**

### What Should Happen

**Option A: CLI does full casting**
1. `crew init --sdk "prompt"` calls CastingEngine immediately
2. Generates full `crew.config.ts` with all agents
...
3. **FIX:** REPL writes `crew.config.ts` after casting
...
**File:** `packages/crew-cli/src/cli/shell/coordinator.ts`  
**Lines:** 42-86

```typescript
export function buildInitModePrompt(config: CoordinatorConfig): string {
  return `You are the Crew Coordinator in Init Mode.
...
When you add or remove agents via REPL or `crew migrate`, the system updates:
- `.crew/team.md` (Members table)
- `.crew/routing.md` (Work Type table)
- `.crew/casting/registry.json` (Agent metadata)

But **never updates `crew.config.ts`**.
...
- ❌ `crew.config.ts` — **No code path**

**File:** `packages/crew-cli/src/cli/commands/migrate.ts`  
**Lines:** 438-483

`migrate --to sdk` generates `crew.config.ts` from existing team.md, but this is a **one-time migration**, not an ongoing sync.

### What Should Happen

1. **Add `updateCrewConfig()` function** in `cli/core/cast.ts`
   - Read current `crew.config.ts`
...
3. **Add to `build` command** — `crew build` already reads `crew.config.ts` and generates markdown; make it bidirectional
...
- **Don't sync** — Make `crew.config.ts` the canonical source
...
```
❌ Ralph is missing from crew.config.ts
...
3. **Decide config sync strategy** (design decision → medium implementation)
   - Option A: Config-as-source-of-truth (crew.config.ts canonical, markdown is output)
...
5. **REPL init writes crew.config.ts** (small)
   - After `createTeam()` in `finalizeCast()`, call `generateSDKBuilderConfig()`
   - Write to `crew.config.ts` if `configFormat === 'sdk'`
...
7. **Make @copilot a real agent** (design decision → small implementation)
   ...
   - Include in crew.config.ts agents array
...
### Finding 1: crew.config.ts Generation
- `packages/crew-cli/src/cli/core/init.ts` (lines 109-125) — Add Ralph to agents array
- `packages/crew-sdk/src/config/init.ts` (lines 339-376) — Extend `generateSDKBuilderConfig()` to accept personality/backstory

### Finding 2: CastingEngine Integration
- `packages/crew-cli/src/cli/core/cast.ts` (lines 218-300) — Replace `personalityForRole()` with CastingEngine
- `packages/crew-cli/src/cli/shell/index.ts` (lines 892-935) — Call `CastingEngine.castTeam()` before `createTeam()`

### Finding 3: CLI Casting Flow
- `packages/crew-cli/src/cli/core/cast.ts` (lines 367-526) — Add `updateCrewConfig()` call to `createTeam()`

### Finding 4: Init Flow Entry
- `packages/crew-cli/src/cli/shell/index.ts` (lines 959-962) — Add config write to `finalizeCast()`

### Finding 5: Coordinator Init Prompt
- `packages/crew-cli/src/cli/shell/coordinator.ts` (lines 52) — Update prompt to specify exact universes

### Finding 6: Config/Team Sync
- **New file:** `packages/crew-cli/src/cli/core/config-sync.ts` — Config AST editing utilities
- `packages/crew-cli/src/cli/commands/build.ts` — Make config-as-source-of-truth canonical

### Finding 7: Ralph and @copilot
- `packages/crew-cli/src/cli/core/init.ts` (line 114) — Add Ralph agent
- `packages/crew-cli/src/cli/core/team-md.ts` (lines 28-88) — Document @copilot as pseudo-agent OR convert to real agent
...
3. **Documentation:**
   - Update README with `crew init --sdk` behavior

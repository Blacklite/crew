...
**Framework:** 4 sequential waves, all work through PRs with crew member review before merge
...
- [ ] #341: First-run wow moment architecture
  ...
  - First-launch detection via `.crew/.first-run` marker
...
- [ ] #431, #429: Bare semver canonical standard
  ...
  - Display contexts (help, banner) use `crew v{VERSION}` for branding
...
**All work flows through PRs with crew member review before merge.**

1. Assign issues to crew members
2. Create PR for each issue (or group related issues)
3. Require crew member review (assign reviewer in PR template)

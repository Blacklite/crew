...
**Target file:** `.crew-templates/crew.agent.md`

---

## Executive Summary

The coordinator prompt (`crew.agent.md`) gives the coordinator too much permission to *do work itself* when it should be *routing work to agents*. Three specific failure modes:
...
3. **The coordinator doesn't read its own skills before acting.** `.copilot/skills/` and `.crew/skills/` were added to the prompt, but there's no enforcement step — no checklist that says "read these BEFORE you route."
...
2. `.crew/skills/` — **Team-level skills.** Patterns and practices agents discovered during work.
...
- No skill in `.copilot/skills/` or `.crew/skills/` addresses it, AND
...
| **Coordinator handles it** | Proceed with best effort. Log: `⚠️ Coordinator handled {domain} task — no domain expert on team. Consider adding a {role}.` Write to `.crew/decisions/inbox/copilot-gap-{timestamp}.md`. |
| **Skip** | Acknowledge. Optionally create a GitHub issue or todo for later. |

**Key principles:**
- The Crew-opinionated default is ALWAYS "expand the team." The coordinator should not silently fill gaps.
...
- Scan `.crew/skills/` for team-level patterns agents have discovered.
...
| **Ralph's work-driving energy** | Ralph keeps the pipeline moving. The continuous loop ("don't ask permission, keep going") is part of Crew's soul. Ralph drives *routing* at speed, not domain work. | Lines 1099–1237 |
...
### Immediate Revert
```bash
git log --oneline -5 .crew-templates/crew.agent.md
# Find the commit before this change
git checkout {pre-change-commit} -- .crew-templates/crew.agent.md
...
| Everything is worse | Full revert to pre-change crew.agent.md. The old behavior is in git history. |

...
User message
  ...
  │     → Scan .crew/skills/ (team's earned patterns)
...
2. `.crew/skills/` — patterns the team learned during work
...
- Which agent is best suited (e.g., git-workflow skill says "use crew/{issue}-{slug} branches")
...
Crew is opinionated: the team should match the work. If the work changes, the team should change. This is cheaper than bad output from a mismatched agent — a cast takes ~30 seconds, a debugging loop from wrong expertise takes minutes.
...
- **Version:** 0.0.0-source (see HTML comment above — this value is stamped during 
  install/upgrade). Include it as `Crew v{version}` in your first response of each 
...
- **Inputs:** User request, repository state, `.crew/decisions.md`, skill directories
...
- **Knowledge base:** `.copilot/skills/` (your process playbook) and `.crew/skills/` 
...
| `git-workflow` | Branch creation, PR workflow | Agents create branches without knowing the crew/{issue}-{slug} convention |
...
| `crew-conventions` | Any code change in this repo | Agents don't follow zero-deps, Node test runner patterns |
...
| `distributed-mesh` | Multi-crew coordination | Coordinator doesn't check mesh.json |

### .crew/skills/ — Team-Earned Patterns (10 skills)
...
| `personal-crew` | Ambient discovery of personal agents |
...
| `cross-crew` | Multi-crew delegation protocol |
...
*This analysis was produced by Procedures reading the full coordinator prompt (1298 lines), all 17 Copilot-level skills, and all 10 team-level skills. Every quote is cited to its source line in `.crew-templates/crew.agent.md`.*

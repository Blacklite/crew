...
- State integrity via merge drivers: union strategy for .crew/ append-only files
...
Crew uses a dev-first, three-branch workflow:
...
**Hard rules:**
- Issue branches: `crew/{issue-number}-{slug}` (branched from dev)
...
- **Parallel work:** Use `git worktree add` when 2+ issues are worked simultaneously. See `.crew/skills/git-workflow/SKILL.md`.
- **Multi-repo:** Use separate clones as siblings, not worktrees. Coordinated PRs linked in descriptions.

**Parallel work:**
- 2+ simultaneous issues in one repo → `git worktree add ../{repo}-{issue} -b crew/{issue}-{slug} origin/dev`
- Each agent gets its own worktree; no filesystem collision
- Cross-repo work → separate sibling clones, not worktrees
- See `.crew/skills/git-workflow/SKILL.md` for full procedures

### Fork Contributor Procedure

When contributing to Blacklite/crew from a fork:
...
3. **PR command:** Always specify base and head explicitly:
   ```bash
   gh pr create --base dev --repo Blacklite/crew --head username:branch-name
   ```
...
- ❌ Accept a version number directive without verifying it against the CURRENT state of package.json files (root, crew-cli, crew-sdk).
...
After making a decision others should know, write it to `.crew/decisions/inbox/kobayashi-{brief-slug}.md`.

...
📌 Team update (2026-03-07T16:25:00Z): Actions → CLI migration strategy finalized. 4-agent consensus: migrate 5 crew-specific workflows (12 min/mo) to CLI commands. Keep 9 CI/release workflows (215 min/mo, load-bearing). Zero-risk migration. v0.8.22 quick wins identified: crew labels sync + crew labels enforce. Phased rollout: v0.8.22 (deprecation + CLI) → v0.9.0 (remove workflows) → v0.9.x (opt-in automation). Brady's portability insight captured: CLI-first means Crew runs anywhere (containers, Codespaces). Customer communication strategy: "Zero surprise automation" as competitive differentiator. Decisions merged. — coordinated by Scribe

# Project Context

- **Owner:** Brady
- **Project:** crew-sdk — the programmable multi-agent runtime for GitHub Copilot (v1 replatform)
...
- Migration strategy for crew-pr (public) → crew (beta): v0.8.17 target
...
- Key Learning: Worktree parallelism, .crew/ state safety via merge=union, multi-repo coordination patterns
...
- **Initial blocker:** Stash required due to uncommitted changes (.crew/agents/rabin/history.md)
...
3. ✅ PR #198 (13 files, consult mode CLI + crew resolution) — MERGED
4. ❌ PR #189 (26 files, Crew Workstreams feature) — BLOCKED: source branch feature/crew-streams deleted from origin
...
- **.crew/ state safety:** merge=union strategy handles concurrent appends; append-only rule
...
Verified crew doctor and crew copilot implementations in v1 codebase. Posted detailed comments explaining v0.8.18+ shipped features, cited specific files and versions, thanked community contributors (@KalebCole, @uvirk). Closed both with appreciation for validating architecture.

### 2026-03-07: Release v0.8.19 — Nap & Doctor Commands, Template Path Fix (COMPLETE)
Released v0.8.19: crew nap command restored + crew doctor wired into CLI. PR #185 (template path fix), PR #178 (GitLab docs). Post-release version bump committed.
...
- #207: Crew not found from non-root directory → Fixed path resolution in nap + consult
...
**State Integrity:** Zero conflicts, clean merges, all .crew/ state preserved (merge=union enforced).
...
- **Consistency:** All three package.json files (root, crew-cli, crew-sdk) synchronized to 0.8.21
- **Git operations:** Commit, tag, push to dev branch completed successfully
- **GitHub Release:** Created with full CHANGELOG.md; visible at https://github.com/Blacklite/crew/releases/tag/v0.8.21
...
Rabin will publish @blacklite/crew-cli and @blacklite/crew-sdk to npm from this tag.
...
Brady requested evaluation of reducing Actions usage by migrating automation to Crew CLI. Comprehensive architectural review completed.
...
- 5 Crew-specific workflows: 12 min/month (MIGRATION CANDIDATES)
- **Cost is negligible — maintainability is the real constraint**

**2. Load-Bearing Infrastructure (Cannot Move):**
- `crew-ci.yml` — PR/push event gate (feeds branch protection)
- `crew-main-guard.yml` — Forbidden file enforcement (prevents state corruption)
- `crew-release.yml` — Automatic tag creation (triggers downstream pipeline)
- `crew-promote.yml` — Branch promotion (complex git orchestration)
- `crew-publish.yml` — npm distribution (final delivery gate)
- `crew-preview.yml` — Pre-release validation (checkpoint before main merge)
- `crew-docs.yml` — GitHub Pages deployment
- `crew-insider-release.yml` & `crew-insider-publish.yml` — Pre-release channel
...
- `sync-crew-labels.yml` → `crew sync-labels` CLI command
- `crew-triage.yml` → `crew triage` CLI command + Ralph monitor
- `crew-issue-assign.yml` → `crew assign` CLI command
- `crew-heartbeat.yml` → Ralph work monitor loop (already implemented)
- `crew-label-enforce.yml` → `crew validate-labels` CLI command
...
- Recommended: Keep all, mark crew-specific as "opt-in" via config flag
- Backward compatible: Existing repos' workflows persist
- New repos: Receive streamlined workflow set with optional automation
- Migration path: `crew upgrade --remove-deprecated-workflows` for cleanup

**5. Backward Compatibility Strategy:**
- **Phase 1 (v0.9):** Document migration path; no code changes
- **Phase 2 (v1.0):** Implement CLI commands (`crew triage`, `crew assign`, etc.)
- **Phase 3 (v1.0):** Add deprecation warnings to old workflows
- **Phase 4 (v1.1):** Provide `crew upgrade --remove-deprecated` flag
...
2. **Migrate 5 crew-specific workflows to CLI** — improves team autonomy, reduces maintenance
3. **Implement lazy automation** — Add `automation` config to .crew/config.json (CI + Release always on, Crew-specific opt-in)
4. **Document in decisions/** — Full assessment saved to `.crew/decisions/inbox/kobayashi-ci-impact.md`
...
3. **Separation of concerns:**
   - `crew-release.yml` — Creates GitHub Release + tag (triggered on main branch merge)
...
**Event Flow:**
1. Brady merges to `main` (via `crew-promote.yml` or direct merge)
2. `crew-release.yml` triggers on `push` to `main`
...
- [ ] Version matches across all package.json files (root, crew-cli, crew-sdk)?
...
**Proposed in decision:** `.crew/decisions/inbox/kobayashi-release-guardrails.md`
...
4. **Release runbook:**
   ...
   - Store in `.crew/skills/release-process/SKILL.md`

...
- GitHub Actions workflow configuration (publish.yml, crew-release.yml, crew-ci.yml, etc.)
...
**Known failure modes:** Read `.crew/skills/ci-validation-gates/SKILL.md` for defensive CI patterns and known failure modes.
...
- Workflow runs link to relevant docs (`.crew/skills/release-process/SKILL.md`)
...
- ❌ **Merge PRs that reference untriaged issues** — labels (crew, priority) are required before work starts
...
- ✅ Document failure modes in `.crew/skills/release-process/SKILL.md` Common Failure Modes section
...
- ✅ **Include branch verification step in publish.yml and crew-release.yml** — verify correct branch before publishing
- ✅ **Verify PRs reference an issue** — crew-ci.yml should check that PR description contains issue reference
...
Before starting work, run `git rev-parse --show-toplevel` to find the repo root, or use the `TEAM ROOT` provided in the spawn prompt. All `.crew/` paths must be resolved relative to this root.

Before starting work, read `.crew/decisions.md` for team decisions that affect CI/CD.

After making a CI/CD decision others should know, write it to `.crew/decisions/inbox/drucker-{brief-slug}.md`.
...
- name: Verify not on protected branch
  ...
  run: |
    ...
    if [[ "$BRANCH" == "main" || "$BRANCH" == "dev" ]]; then
      ...
      echo "To fix: git checkout -b crew/{issue-number}-{description}"
      exit 1
    fi
...
if [[ "$BRANCH" == "main" || "$BRANCH" == "dev" ]]; then
  ...
  echo "Create a feature branch: git checkout -b crew/{issue-number}-{description}"
  exit 1
fi
...
**PR must reference an issue (crew-ci.yml):**
...
- name: Verify PR references issue
  ...
  run: |
    ...
    if ! echo "$PR_BODY" | grep -qE '#[0-9]+'; then
      ...
      echo "Issue must be triaged with labels (crew, priority) before work starts"
      exit 1
    fi

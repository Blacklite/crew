...
Procedures assigned:
- **#485 (Agent Specification PRD)** → crew:flight + crew:procedures (architecture decision + formal spec structure)
...
📌 **Team update (2026-03-22T09-35Z — Wave 1):** Economy mode governance proposal and personal crew consult-mode governance proposal authored for crew.agent.md — both DRAFT, awaiting Flight review before merging. Economy mode adds Layer 3 table + spawn convention (`💰 economy`) + model catalog audit. Personal crew adds consult mode detection, path reference table, spawn guidance. Persistent model preference (Layer 0) documented. Proposed new skill: `.crew/skills/consult-mode/SKILL.md` (post-approval). Deterministic skill pattern proven effective. PR #503 open with skills module. Next: Flight review → merge governance to crew.agent.md. No blocking issues.
...
1. **Bundle resources IN the skill directory:** Copy `sync-mesh.sh`, `sync-mesh.ps1`, and `mesh.json.example` into `.crew/skills/distributed-mesh/`
...
4. **Replicate to templates:** Copy entire skill directory to `templates/skills/`, `packages/crew-cli/templates/skills/`, `packages/crew-sdk/templates/skills/`
...
1. **Agent Error Lockout** — Added to crew.agent.md. After 2 cumulative errors (build/test failures or reviewer rejection) on the same task, agent is locked out for that task only. Different agent takes over. Coordinator tracks and enforces; Scribe logs lockout events.

2. **Product Isolation Rule** — Added to every charter and crew.agent.md Constraints. Tests, CI, and product code must NEVER depend on specific agent names from any crew. "Our crew" must not impact "the crew." Use generic/parameterized values (e.g., "test-agent-1") instead of real agent names (Flight, EECOM, FIDO).
...
**Implementation:** Updated `.github/agents/crew.agent.md` (new section + constraint) and all 19 active agent charters in `.crew/agents/*/charter.md`.

**Pattern:** Policies added as subsections under "How I Work" in charters to ensure they're loaded with agent context. Coordinator-level policies live in crew.agent.md with explicit enforcement instructions.
...
### 2026-03-22: Economy mode skill and personal crew governance (#500, #344)

**Task:** Two governance tasks — economy mode skill design and personal crew coordinator awareness.

**Economy mode (SKILL.md):**
- Created `.crew/skills/economy-mode/SKILL.md` as a Layer 3 modifier, not a new resolution layer
...
**Personal crew governance (proposals):**
- Gap analysis: coordinator has no consult mode awareness despite full SDK implementation
- Five gaps identified: Init Mode missing personal crew resolution, no consult mode detection, TEAM_ROOT has no personal-crew semantics, charter templates lack consult-mode patterns, no consult-mode skill
...
**Governance workflow pattern:** When proposals touch crew.agent.md (governance territory), write to `decisions/inbox/` for Flight review. Don't directly edit crew.agent.md — Flight reviews governance changes.

**Catalog audit finding:** `claude-sonnet-4.6`, `gpt-5.4`, `gpt-5.3-codex` appear in model-selection SKILL.md fallback chains but are absent from crew.agent.md's "Valid models" catalog. Documented in economy-mode governance proposal for Flight to address.

### Session 2 Summary (2026-03-22)

Wave 1 governance work on #500 and #344: authored economy-mode skill (`SKILL.md`), economy-mode governance proposal, and personal-crew governance proposal. Caught `claude-sonnet-4.6` missing from valid models catalog. PR #503 (`crew/500-344-governance`) merged to dev.

### 2025-07: Spawn template `name` parameter fix (#577)

**Problem:** Agent cast names weren't displayed during work — the tasks panel showed generic slugs like "general-purpose-task" instead of the cast name. Root cause: spawn templates in `crew.agent.md` specified `description` but NOT the `name` parameter for the `task` tool. The `name` parameter generates the human-readable agent ID shown in the tasks panel.

**Fix:** Added `name: "{name}"` (lowercase cast name) to all spawn templates in `.crew-templates/crew.agent.md`:
...
📌 **Team update (2026-03-23T23:15Z):** Orchestration complete. Agent name display refactor shipped: spawn templates updated with mandatory `name` parameter across all 4 template variants. VOX and FIDO coordinated on parser extraction and cascading pattern strategies. All decisions merged to decisions.md. Canonical source: `.crew-templates/crew.agent.md` (all derived copies secondary).

📌 **Team update (2026-03-25T18:11Z):** Model catalog updated to current platform offerings — removed 2 stale models (claude-opus-4.6-fast, gpt-5), added 5 new models (claude-sonnet-4.6, claude-opus-4.6-1m, gpt-5.4, gpt-5.3-codex, gpt-5.4-mini), bumped defaults (code: claude-sonnet-4.6, specialist: gpt-5.3-codex), restructured fallbacks. All 5 crew.agent.md template copies synchronized. Merged in #588.

### 2025-07: Model catalog refresh (#588)

**Problem:** The valid models catalog, fallback chains, role-to-model mappings, and default model references in `crew.agent.md` were stale — missing `claude-sonnet-4.6`, `gpt-5.4`, `gpt-5.3-codex`, `gpt-5.4-mini`, `claude-opus-4.6-1m` and still referencing removed models `claude-opus-4.6-fast` and standalone `gpt-5`.
...
2. Template duplication: CLI 1.0.11 discovers all `*.agent.md` from cwd to git root. Crew has 5 copies — only `.github/agents/crew.agent.md` should be discoverable.
...
**Branch:** `crew/613-vscode-routing-enforcement` — canonical source edited, synced to all 5 copies via `scripts/sync-templates.mjs`, build verified clean.
...
- Removed duplication with `.crew/copilot-instructions.md` (5 sections were verbatim copies; kept compact versions since both files load)

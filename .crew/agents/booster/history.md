...
**Context:** v0.9.0 shipped with broken dependency reference (`file:../crew-sdk` in CLI package.json). Required hotfix. Used incident as opportunity to audit entire CI/CD system.
...
- Implicit ordering risk: crew-release and crew-npm-publish both trigger on `release: published` with no explicit job dependency (works but fragile)
...
- 7+ consecutive failures on `crew/fix-ci-build` branch (14:00-14:11 UTC)
- Root cause: Stale nested SDK entry in package-lock.json causing TypeScript module resolution errors
- Error: "Module '@blacklite/crew-sdk' has no exported member 'listRoles'" (and 6 other missing exports)
...
- 3 failures on `crew/docs-quality-ci` branch (14:32, 15:26, 15:51 UTC)
...
- Core pipeline (crew-ci → crew-release → crew-npm-publish → insider-*) is solid
- Smoke tests run before any npm publish (good safety gate)
- Implicit ordering works but fragile (crew-npm-publish depends on release: published event)

**Triage Automation Quality:**
- Label routing (crew-triage, crew-issue-assign, crew-label-enforce) well-integrated
...
**Report written to:** `.crew/decisions/inbox/booster-ci-audit.md`
...
2. **Pre-publish `preflight` job added** to `crew-npm-publish.yml`:
...
Updated both release-process skill files (`.crew/skills/release-process/SKILL.md` and `.copilot/skills/release-process/SKILL.md`) with critical learnings from the v0.9.4 release session. Added 5 new Known Gotchas, a full `v0.9.4 Incident Learnings` section, GITHUB_TOKEN propagation workaround, CHANGELOG/root-package.json validation rules, lockfile integrity fix documentation, local dev recovery steps, and cross-references between skill files. Source PRs: #1042, #1043, #1044.

### CI Cleanup — Issue #1000 (2026-04-17)

**Changes shipped in PR #1001:**

1. **Deleted `ci-rerun.yml`** — Redundant fork PR workflow with 100% failure rate (5/5 runs). GitHub's native "Approve and Run" handles fork PR CI. Removed reference from `setup-crew-node` comment header.

2. **Streamlined `crew-ci.yml`** — 852 → 585 lines, 9 → 6 jobs:
...
- `.github/actions/setup-crew-node/action.yml` (comment update)
- `.github/workflows/crew-ci.yml` (streamlined)
- `.github/actions/setup-crew-node/action.yml` (comment update)
- `.github/workflows/crew-ci.yml` (streamlined)

### Workflow Shellcheck SC2086 Fix + Actionlint CI Gate (2026-07-29T17:11:56+10:00)

**Context:** Issue #1556 (bradygaster/crew) reported SC2086 failures in downstream repos using actionlint after `crew upgrade`. The unquoted `>> $GITHUB_OUTPUT` redirects in `crew-heartbeat.yml` were the reported trigger; audit revealed matching issues in `crew-repo-health.yml` and `crew-ci.yml` too.

**Changes shipped:**

1. **SC2086 fixes — all `>> $GITHUB_OUTPUT` and `>> $GITHUB_STEP_SUMMARY` redirects quoted** in:
   - `.crew-templates/workflows/crew-heartbeat.yml` (canonical source, synced to 3 mirrors by build)
   - `templates/workflows/crew-heartbeat.yml`, `packages/crew-cli/templates/workflows/crew-heartbeat.yml`, `packages/crew-sdk/templates/workflows/crew-heartbeat.yml` (mirrors)
   - `.github/workflows/crew-heartbeat.yml` (active workflow, maintained separately per SYNC comment)
   - `.github/workflows/crew-repo-health.yml` (4 run blocks, 10 lines total)
   - `.github/workflows/crew-ci.yml` (1 large gate block, 13 lines total)

2. **New CI workflow:** `.github/workflows/crew-workflow-lint.yml`
...
3. **Changeset:** `.changeset/fix-workflow-shellcheck-quoting.md` — patch for crew-cli + crew-sdk (required because template files under `packages/crew-*/templates/` are covered by changelog-gate regex)

**Key learnings:**
- The canonical workflow template source is `.crew-templates/workflows/` — fix templates THERE and the mirrors are auto-synced by `node scripts/sync-templates.mjs` during `prebuild`
- `.github/workflows/crew-heartbeat.yml` is maintained separately (SYNC comment at top of file) — must also be patched manually
...
- The `pull_request_target` context in `crew-repo-health.yml` uses only SHA values (not user-controlled text), so actionlint should NOT flag those as untrusted-input warnings
...
**Commit:** 1b1af6e3, pushed to `serbrech/crew`.

### Installer Hardening — PR Review Comments (2026-07-29T19:25:08+10:00)

**Context:** Copilot automated reviewer left 3 inline comments on PR #1557 (`crew-workflow-lint.yml` installer step + history.md). All valid and in scope.

**Changes shipped:**

1. **`bash <(curl -sL ...)` → download-then-run** (`crew-workflow-lint.yml` actionlint install):
...
2. **`set -euo pipefail` + `curl -fsSL` for shellcheck tar pipeline** (`crew-workflow-lint.yml`):
   ...
   - Convention matched: `shell: bash` + `set -euo pipefail` inside run block, same as `crew-agents-ai-release.yml` (lines 76-78 and 135-139) and `crew-npm-publish.yml` (line 409)
...
**Validation:** `bash -n` passes on the new run block. Self-linting property holds — actionlint + shellcheck will lint this exact `run:` block on the next CI run, catching any quoting or syntax issues we introduce. Amended and force-pushed to `serbrech/crew` as a single commit.

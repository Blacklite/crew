...
- **ISSUE TRIAGE BEFORE WORK (MANDATORY):** Add crew/priority/category labels + triage comment before any work begins on an issue.
- Releases follow a strict checklist — no improvisation. Read `.crew/skills/release-process/SKILL.md` before any release work.
...
- **POST-PUBLISH SMOKE TEST (MANDATORY):** `npm install -g @blacklite/crew-cli@latest && crew --version && crew doctor` in a clean shell after every publish. If it fails, rollback.

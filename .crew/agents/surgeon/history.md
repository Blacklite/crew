...
**Release process skill created:** `.crew/skills/release-process/SKILL.md` documents all patterns and procedures.
...
- Personal Crew, Worktree, Machine Capability Discovery, Rate Limiting, Economy Mode, Telemetry, Templates, Skills, Docs, ESLint patterns
...
### Release History
v0.8.24 released successfully. npm packages: @blacklite/crew-sdk@0.8.24, @blacklite/crew-cli@0.8.24. publish.yml triggers on `release: published` (NOT draft). Test baseline at release: 3,931 tests, 149 files.
...
**Feature Categories:**
- Personal Crew (governance layer + ambient discovery)
...
2. **GitHub Actions Workflow Cache Race Condition** — After deleting `crew-publish.yml`, GitHub workflow index didn't refresh for 10+ minutes. `workflow_dispatch` returned 422 error that persisted despite file deletion.
3. **npm Workspace Publish Broken** — `npm -w packages/crew-sdk publish` hung indefinitely (2FA auth issue on machine without authenticator app).
...
**Documentation:** Full retrospective in `.crew/decisions/inbox/surgeon-v091-retrospective.md` and `.crew/log/2026-03-23-v091-retrospective.md`.
...
2. **CI Improvement Plan** — 7 GitHub issues + 4 CI cleanup actions:
   ...
   - Issue D: crew-promote workflow (dev → preview → main automation)
   - Issue E: crew-preview validation (forbidden files, versions, dependencies)
   - Issue F: crew version command fix (show correct CLI version)
...
3. **Session Log Cleanup** — Deleted 17 files from today's session logs:
   - 15 files from `.crew/orchestration-log/`
   - 2 files from `.crew/log/`
   - Preserved retro findings in `.crew/decisions/inbox/` (surgeon-v091-retrospective.md)
...
📌 **Team update (2026-07-29T17:11:56+10:00 — Issue #1556 / PR #1557):** A changeset `.changeset/fix-workflow-shellcheck-quoting.md` (patch, crew-cli + crew-sdk) landed in PR #1557. Separately, a new `crew-workflow-lint` job (`.github/workflows/crew-workflow-lint.yml`) runs actionlint 1.7.12 + shellcheck 0.10.0 on workflow templates — this is a distinct check, entirely independent of `changelog-gate`, which only enforces changeset/CHANGELOG requirements and has no role in linting. For future releases, be aware that PRs touching `.github/workflows/`, `.crew-templates/workflows/`, `templates/workflows/`, or either package's `templates/workflows/` will trigger this lint check. Whether `crew-workflow-lint` is configured as a required branch-protection gate has not been verified — confirm before treating it as a hard release blocker.

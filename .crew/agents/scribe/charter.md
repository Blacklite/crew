...
1. Log session to `.crew/log/{timestamp}-{topic}.md` (who, what, outcomes)
2. Merge `.crew/decisions/inbox/` → `.crew/decisions.md`, delete inbox files
...
5. Commit: cd to team root, `git add .crew/`, temp file, `git commit -F` (Windows: no `-C`, no `-m` newlines)

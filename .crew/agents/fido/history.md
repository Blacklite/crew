...
Tests reading live .crew/ files must assert structure/behavior, not specific agent names. Names change during team rebirths. Two test classes: live-file tests (survive rebirths, property checks) and inline-fixture tests (self-contained, can hardcode).
...
CastingEngine augments LLM casting with curated names for recognized universes. Unrecognized universes preserve LLM names. Import from `@blacklite/crew-sdk/casting`, use casting-engine.ts AgentRole type (9 roles). Partial mapping: unmapped roles skip engine casting.

### PR #331 Quality Gate Review — NO-GO (Blocking Issues Found) (2026-03-10T14:13:00Z)

**CRITICAL VIOLATIONS DETECTED:**

1. **Stale Test Assertions (Hard Rule Violation)** — EXPECTED_SCENARIOS array in test/docs-build.test.ts contains only 7 values ['issue-driven-dev', 'existing-repo', 'ci-cd-integration', 'solo-dev', 'monorepo', 'team-of-humans', 'cross-org-auth'], but 25 scenario files exist on disk (aspire-dashboard, client-compatibility, disaster-recovery, keep-my-crew, large-codebase, mid-project, multi-codespace, multiple-crews, new-project, open-source, private-repos, release-process, scaling-workstreams, switching-models, team-portability, team-state-storage, troubleshooting, upgrading, + 7 in array). My charter: "When I add test count assertions, I MUST keep them in sync with the actual files on disk. Stale assertions that block CI are MY responsibility to prevent." This is MY responsibility to catch.
...
**Commit:** 6599db6 on branch crew/289-crew-dir-explainer
...
1. **Casting directory scaffolding** — After `initCrew()` and `runInit()`, verifies `.crew/casting/` directory and all three JSON files (registry.json, policy.json, history.json) exist and parse as valid JSON. Also confirms re-init does not overwrite existing casting files.
...
Pattern: Tests follow existing `test/cli/init.test.ts` and `test/cli/doctor.test.ts` conventions — vitest, `randomBytes` temp dirs in cwd, imports from compiled dist via package exports (`@blacklite/crew-cli/core/init`, `@blacklite/crew-cli/commands/doctor`, `@blacklite/crew-sdk`).

Commit: 7660a27 on branch crew/579-init-scaffolding-hardening.

### Personal Crew Init Discovery Tests (#576)

**Task:** Write tests for personal crew discovery and init flows (Issue #576 — npx init --global not discovering personal crew).

**Test file:** `test/personal-crew-init.test.ts` — 35 tests, 10 describe blocks, all passing.

**Coverage areas:**
1. `resolveGlobalCrewPath()` — platform-specific path resolution (Windows APPDATA, Linux XDG_CONFIG_HOME, consistency)
2. `resolvePersonalCrewDir()` — kill-switch (CREW_NO_PERSONAL), directory existence, npx-agnostic discovery
3. `personalInit` contract — directory structure creation, config.json shape, idempotency
4. `resolveCrewPaths()` — personalDir field inclusion, null when disabled
5. Edge: empty personal-crew dir (exists but no agents/)
...
- **PRs #604 / #603** are complete duplicates — both add Challenger agent template + fact-checking skill. #604 has `templates/challenger.md` (153 lines), #603 has `.crew/templates/agents/challenger.md` + `.crew/skills/fact-checking/SKILL.md` (133 lines). File locations differ but content is nearly identical. **Verdict: Close one as duplicate** — recommend #603 (file locations match project conventions).
...
1. **Changeset package name pattern:** 4 of 8 Tamir PRs (#623, #622, #621, #614) use unscoped `"crew-cli"` / `"crew-sdk"` instead of `"@blacklite/crew-cli"` / `"@blacklite/crew-sdk"`. Only #625 got this right. This is a recurring community contributor mistake — consider adding guidance to CONTRIBUTING.md or PR template.

2. **File path pattern:** PRs #607 and #606 place files at root `ceremonies/`, `skills/`, `docs/`, `templates/` directories that don't exist. Skills belong in `packages/crew-cli/templates/skills/` and SDK equivalent. Community contributors don't know the monorepo layout.

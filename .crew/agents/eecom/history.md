...
**Context:** Template files (crew.agent.md, init-mode/SKILL.md) contained hardcoded "Brady" examples in greetings, routing examples, and comments. LLMs treated these as patterns, greeting every user as "Brady" regardless of their actual `git config user.name`.

**Fix:** Replaced all hardcoded "Brady" in template examples with generic `{user}` / `{name}` placeholders. Canonical sources: `.crew-templates/crew.agent.md` and `.copilot/skills/init-mode/SKILL.md`. Template sync (`node scripts/sync-templates.mjs`) propagated crew.agent.md to `.github/agents/` but did NOT sync init-mode SKILL.md to package templates — those required manual edits in both `packages/crew-cli/templates/skills/init-mode/SKILL.md` and `packages/crew-sdk/templates/skills/init-mode/SKILL.md`.
...
**Context:** Three Copilot review comments on PR #767: (1) `teamRoot` was set to `workTreeRoot` but `.crew/` may live in the main checkout when running inside a git worktree — should derive from `detectCrewDir().path`, (2) `generateLoopFile()` hardcoded the full loop.md scaffold inline, duplicating `templates/loop.md`, (3) docs said `gh` was optional but code hard-requires `gh copilot` unless `--agent-cmd` is passed.

**Fixes:**
1. **teamRoot:** Changed `const teamRoot = workTreeRoot` to `path.dirname(crewDirInfo.path)` — `.crew/`-relative operations now always use the directory where `.crew/` was actually found.
2. **Template dedup:** Replaced 48-line hardcoded template with `readFileSync` reading from `templates/loop.md`. Used `import.meta.url` + `fileURLToPath` to resolve the path from the compiled file (3 levels up to package root). Created `packages/crew-cli/templates/loop.md` since it was missing from the CLI package's templates dir.
...
**Context:** Copilot code review on PR #767 flagged three issues in the loop command: (1) `execFile` buffered stdout/stderr but never printed it — users saw no Copilot output during loop rounds, (2) `loop.md` was resolved relative to `dest` but execution used `teamRoot` (derived from `.crew/` parent), creating a CWD mismatch in worktree scenarios, (3) docs said `description` defaults to `""` but code uses `'Crew Loop'`.
...
3. **Docs:** Updated `docs/src/content/docs/features/loop.md` description default from `""` to `"Crew Loop"`.

**Key file:** `packages/crew-cli/src/cli/commands/loop.ts` — `runLoop()` entry point (~line 302), `executeRound()` inner function (~line 427).
...
**Context:** `archiveDecisions()` in `packages/crew-cli/src/cli/core/nap.ts` silently returned `null` when all `###` entries were <30 days old (`old.length === 0`), even if the file was well over 20KB. Active projects generating many decisions per session could hit 145KB+ — 35K tokens burned per agent spawn.
...
**Context:** `crew init` in a fresh `git init` repo (no remote) printed `error: No such remote 'origin'` to stderr and `crew doctor` reported `casting/registry.json` missing. Two independent bugs in `packages/crew-sdk/src/config/init.ts`.

**Fix 1 — Stderr leak:** Three `execFileSync('git', ['remote', 'get-url', 'origin'])` calls in `initCrew()` were missing `stdio: ['pipe','pipe','pipe']`. The try/catch caught the error but git's stderr still leaked to the console. Added stdio piping to all three call sites (lines ~713, ~732, ~1039).

**Fix 2 — Missing casting files:** The init flow created the `.crew/casting/` directory but never populated it. Added a scaffolding block after directory creation that copies `casting-policy.json`, `casting-registry.json`, and `casting-history.json` from SDK templates (with inline fallbacks). Respects `skipExisting` — never overwrites user files.
...
**Context:** `crew version` returned "Unknown command: version" even though `crew --version` and `crew -v` worked fine. Classic "unwired command" bug but for a flag-to-subcommand gap rather than a missing import.
...
### `crew version` subcommand (2026-07-15)

**Context:** Running `crew version` returned "Unknown command: version" because the subcommand was never routed in `cli-entry.ts`, even though `--version` and `-v` flags worked fine. Classic "unwired command" bug class, but for a flag-to-subcommand gap rather than a missing import.
...
📌 **Team update (2026-03-22T09-35Z — Wave 1):** Economy mode fully implemented: ECONOMY_MODEL_MAP + resolveModel() integration in SDK, `crew economy on|off` CLI command, `--economy` flag, 34 tests passing. PR #504 open for review. Soft dependency: #464 rate limit UX should offer economy mode as recovery. Next: Phase 1 of ambient personal crew (T1–T5, T19) — ready to start immediately after merging current work. Procedures wrote governance proposals for crew.agent.md — awaiting Flight review.
### Rate Limit UX (#464) (2026-03-20)

**Context:** Users hitting Copilot rate limits saw generic "Something went wrong processing your message." Crew hid the actual error. `crew doctor` reported nothing — useless to diagnose.

**Root cause:** The catch block in `shell/index.ts` line ~1119 always emitted `genericGuidance()` unless `CREW_DEBUG=1`. Rate limit errors never got special treatment despite `RateLimitError` existing in `adapter/errors.ts`.

**Fix:**
1. `error-messages.ts` — Added `rateLimitGuidance({ retryAfter?, model? })` and `extractRetryAfter(message)` utilities. Rate limit guidance shows clear message + recovery options (retry time, `crew economy on`, config.json model override).
2. `shell/index.ts` — Catch block now detects rate limits via `instanceof RateLimitError` OR regex on the raw message. Writes `.crew/rate-limit-status.json` on detection.
...
**Pattern:** Rate limit status written to `.crew/rate-limit-status.json` as `{ timestamp, retryAfter, model, message }`. Doctor reads it on next run. File is never deleted automatically — doctor marks it `pass` when > 4h stale.

**Import path for `RateLimitError`:** `@blacklite/crew-sdk/adapter/errors` (subpath export, not in main barrel).

**PR:** #464 fix — crew/464-rate-limit-ux
...
**Context:** Two cross-platform bugs broke Crew on Windows: (1) log filenames contained colons in ISO 8601 timestamps (illegal on Windows), (2) `.crew/config.json` contained absolute machine-specific `teamRoot` path.
...
3. Updated live `.crew/config.json` in repo to remove machine-specific path
...
**Context:** CastingEngine class (Issue #138, M3-2) existed in SDK with curated universe templates (The Usual Suspects, Ocean's Eleven) but was completely bypassed during `crew init`. LLM picked arbitrary names, and charter generation used regex-based `personalityForRole()` instead of template backstories.
...
**Type Import Pattern:**
- Import CastingEngine from `@blacklite/crew-sdk/casting` (not main barrel export)
...
**Context:** PR #427 (PAO external communications Phase 1) conflicted with upstream/dev after team recast (#423 Usual Suspects → Apollo 13) and model updates. Cross-repo PR (diberry/crew → Blacklite/crew). Initial rebase attempts failed due to git worktree confusion — main worktree was checked out to a different branch, causing git checkout commands to silently switch to wrong branches.

**Problem:** Git commands (checkout, rebase) kept switching to unrelated branches (crew/agent-on-disk-concept, crew/320-fix-migration-guide-version-local) mid-rebase. Root cause: main worktree at C:\Users\diberry\repos\project-crew\crew was checked out to crew/agent-on-disk-concept. Git was treating checkout commands as worktree operations and switching the main worktree's HEAD, aborting the rebase.
...
1. **.crew/agents/_alumni/mcmanus/charter.md** - Merged both rule sets: DOCS-TEST SYNC (from upstream reskill) and EXTERNAL COMMS, HUMANIZER, AUDIT TRAIL (from PR #427). Used PowerShell regex to extract and combine both sides.
2. **.crew/routing.md** - Accepted Apollo 13 team names (EECOM, PAO, FIDO) from upstream via `git checkout --ours` (in rebase context, "ours" = upstream, "theirs" = our branch). PAO external comms infrastructure is team-agnostic.
3. **.crew/agents/keaton/history.md** - Accepted deletion via `git rm` (file moved to _alumni in upstream recast).

**Rebase Commits:** 7 commits from crew/426-pao-external-comms rebased onto upstream/dev (f87a7a5), covering #423 team reskill, #424 SDK switch, #425/#428 test parity, #429 model updates.

**Force Push:** `git push origin crew/426-pao-external-comms --force-with-lease` succeeded. PR #427 comment posted via gh CLI.
...
Traced complete `crew init --sdk` flow end-to-end for unified PRD. Key findings: (1) Init flow has two phases: CLI init creates skeleton files, REPL auto-cast creates team members. (2) Critical gap: crew.config.ts is never updated after auto-cast — members exist in .crew/ but not in config. (3) Ralph is inconsistently created (auto-cast yes, CLI init no). (4) No commands exist for adding/removing members post-init. (5) CastingEngine class exists but is never called during init — LLM-based Init Mode prompt is used instead. Roadmap written to .crew/identity/sdk-init-implementation-roadmap.md with 7 fixes prioritized by dependency graph. Critical path: sync utility → Ralph fixes → CastingEngine integration → hire/remove commands. High-risk items: crew.config.ts AST parsing (considered regex alternative). Open questions: AST vs regex for config sync, CastingEngine augment vs replace LLM, Ralph always-on vs opt-in.
...
EECOM assigned:
- **#481 (StorageProvider PRD)** → crew:control + crew:eecom (type system abstraction + runtime integration)
- **#479 (history-shadow race condition)** → crew:eecom + crew:retro (production bug; mitigation through StorageProvider atomicity)
...
3. `resolveModel()` in `config/models.ts` updated with `economyMode?: boolean` option; falls back to reading from `crewDir` if not provided
4. `resolveModel()` in `agents/model-selector.ts` updated with `economyMode?: boolean` — both SDK resolvers are economy-aware
5. `crew economy [on|off]` command in CLI for persistent toggle
6. `--economy` global flag in `cli-entry.ts` sets `CREW_ECONOMY_MODE=1` env var for session scope
...
**PR:** #500 branch `crew/500-economy-mode`
...
2. **doctor.ts:** Added `checkNodeVersion()` to `crew doctor` — exported with optional version param for testability.
...
**PR:** #506 branch `crew/502-node-sqlite-dependency`
...
**Implementation:**
1. `error-messages.ts` — `rateLimitGuidance()` shows actual reason + 3 recovery options (retry time, `crew economy on`, config.json model override)
2. `shell/index.ts` — Detects rate limits via `instanceof RateLimitError` or regex; writes `.crew/rate-limit-status.json`
3. `doctor.ts` — `checkRateLimitStatus()` reads status file and warns if recent
4. 36 new tests — all pass

**PR:** #505 `crew/464-rate-limit-ux` — merged (rebased after #504)
...
### Personal Crew Init via npx (#576) (2026-03-23)

**Context:** `init --global` (used via npx to set up personal crew) created a full `.crew/` structure at `~/.config/crew/` but never created the `personal-crew/` subdirectory. `resolvePersonalCrewDir()` looks for `personal-crew/`, so subsequent repo-level `init` couldn't discover the user's personal agents.

**Root cause:** Two separate concepts - `init --global` scaffolds a full crew, `personal init` creates `personal-crew/`. The `--global` flag never bridged between them.

**Fix:**
1. `resolution.ts` - Added `ensurePersonalCrewDir()` idempotent helper to SDK.
2. `cli-entry.ts` - `init --global` now suppresses workflows and passes `isGlobal` flag.
3. `init.ts` - After global init, calls `ensurePersonalCrewDir()`. After repo init, detects personal crew.
4. `personal.ts` - Refactored to reuse `ensurePersonalCrewDir()`.
5. `resolution.test.ts` - Added 3 tests.

**Pattern:** `resolveGlobalCrewPath()` returns the container; `ensurePersonalCrewDir()` creates the subdirectory the rest of the system looks for.
📌 **Team update (2026-03-25T18:11Z):** Fixed #590 personal crew path regression — getPersonalCrewRoot() now uses canonical personal-crew/ subdirectory like esolvePersonalCrewDir() and nsurePersonalCrewDir(). Committed on crew/590-fix-personal-crew-root. FIDO found same bug in shell/index.ts → work passed to CONTROL for full sweep revision. Awaiting FIDO re-review.

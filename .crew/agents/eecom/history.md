...
Replaced all hardcoded "Brady" in template examples with generic `{user}`/`{name}` placeholders. Canonical sources: `.crew-templates/crew.agent.md` and `.copilot/skills/init-mode/SKILL.md`. Template sync only covers `.crew-templates/`; init-mode SKILL.md package copies required manual edits. Key distinction: only files copied to user repos were changed; Brady references in project docs are legitimate content.
...
Replaced soft-warn with synchronous version check + `process.exit(1)` for Node <22.5.0. Added `checkNodeVersion()` to `crew doctor`. Corrected `engines.node` to `>=22.5.0` in all package.json files. **Pattern:** Always confirm `git branch` before editing files — edits don't follow you to a new branch.

### Personal Crew Init via npx (#576) (2026-03-23)
`init --global` now calls `ensurePersonalCrewDir()` after global init. Added idempotent `ensurePersonalCrewDir()` to SDK resolution.ts. `resolveGlobalCrewPath()` returns the container; `ensurePersonalCrewDir()` creates the `personal-crew/` subdirectory the rest of the system looks for.

## Historical Learnings Summary (condensed)

- **Loop command (#767):** For interactive/long-running child processes, always attach `.on('data')` listeners for real-time output. When using `node:fs` mocked in tests, use `vi.importActual<typeof import('node:fs')>('node:fs')` in `beforeAll` to get real filesystem access. teamRoot must derive from `detectCrewDir().path` not `workTreeRoot` in worktree scenarios.
- **Init scaffolding (no-remote, casting files) (#579):** Three execFileSync calls for git remote needed `stdio: ['pipe','pipe','pipe']` to suppress stderr. Init flow creates .crew/casting/ but never populated it — added scaffolding block. Pattern: execFileSync inside try/catch still leaks stderr without piped stdio.
...
- **Economy Mode (#500):** Layer 3 modifier only — never downgrades Layers 0-2 (user intent). `ECONOMY_MODEL_MAP` + `applyEconomyMode()` + CLI `crew economy on|off` + `--economy` flag. 34 tests.
- **Rate Limit UX (#464):** `rateLimitGuidance()` + `.crew/rate-limit-status.json` + `checkRateLimitStatus()` in doctor. Import `RateLimitError` from `@blacklite/crew-sdk/adapter/errors` (subpath export, not main barrel).
- **CastingEngine CLI Integration (#342):** CastingEngine.castTeam() was never called in CLI flow. `augmentWithCastingEngine()` in cast.ts replaces LLM names with engine characters for recognized universes. Import from `@blacklite/crew-sdk/casting` (not main barrel). 9 AgentRole enum values, not 6.
...
- **SDK Init Flow Deep Dive:** Critical gap: crew.config.ts never updated after auto-cast. CastingEngine exists but is never called. Roadmap: sync utility → Ralph fixes → CastingEngine integration → hire/remove commands.
- **Adoption Tracking Tier 1:** Moved `.crew/adoption/` → `.github/adoption/`. Aggregate metrics only — never publish individual repo lists without opt-in.

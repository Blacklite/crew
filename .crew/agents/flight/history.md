...
- **Adoption tracking architecture:** Three-tier opt-in system: Tier 1 (aggregate-only `.github/adoption/`), Tier 2 (opt-in registry), Tier 3 (public showcase ≥5 projects). `.crew/` is team state only.
- **Content triage skill:** "Crew Ships It" litmus test. Triggered by `content-triage` label. Output: boundary analysis, sub-issues for PAO, IRL reference for Scribe.
...
- **Distributed mesh integration:** Zero code changes. Convention-first additive layer (skills + scripts + docs). 125:1 ratio. mesh.json stays separate from crew.config.ts.
- **Remote crew access:** Three-phase rollout: Phase 1 GitHub Discussions bot (1 day, zero hosting), Phase 2 Copilot Extension via Contents API (1 week), Phase 3 Slack/Teams bot (2 weeks). Must solve `.crew/` context access.
- **PR #331 boundary review:** "Crew Ships It" pattern — remove external infrastructure docs, reframe platform integration docs, keep Crew behavior/config docs.
...
- **Ambient personal crew (#329/#344):** 5 gaps identified: resolvePersonalAgents() function, ghost protocol for orchestration log, --team-root scope, crew personal init command, CREW_NO_PERSONAL env var. ensureCrewPathTriple() required in resolution.ts to enforce ghost protocol.
...
- **Issue triage (2026-03-24 — 14 issues):** P0: #590 (getPersonalCrewRoot broken). P1: #610 (broken docs), #591 (hiring wiring). P2: #597, #588, #554. Questions: #589, #494. Deferred: #581 (ADO PRD, blocked until #341).

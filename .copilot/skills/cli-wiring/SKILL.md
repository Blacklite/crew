...
**Bug class:** Commands implemented in `packages/crew-cli/src/cli/commands/` but never routed in `cli-entry.ts`.

## Checklist — Adding a New CLI Command

1. **Create command file** in `packages/crew-cli/src/cli/commands/<name>.ts`
   - Export a `run<Name>(cwd, options)` async function (or class with static methods for utility modules)

2. **Add routing block** in `packages/crew-cli/src/cli-entry.ts` inside `main()`:
   ```ts
   if (cmd === '<name>') {

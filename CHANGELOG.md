...
### Added
- (cli) Add `crew preset install <source>` for installing shared presets from GitHub URLs or local paths (#1224).
- (cli) Add `crew registry add/list/remove` for discovery-only peer crews, plus the bundled cross-crew communication skill.
...
- (sdk+cli) Expose memory tools through the `crew_state` MCP server and improve Fact Checker auto-scaffolding/plumbing.

### Changed
- (sdk+cli) Slim the bundled `crew.agent.md` by extracting long guidance into satellite skills (#1311).
...
- (sdk+cli) Restore the green test suite by reconciling personal crew paths, stabilizing observer category handling, guarding storage case-insensitivity, and skipping environment-dependent tests when prerequisites are missing (#1416).
...
- (sdk+cli) Fix state-backend handshake, CREW_HOME/CREW_PERSONAL_DIR resolution, preset apply wiring, routing example quote stripping, YAML escaping, and MCP config pollution regressions.
...
### Performance
- (cli) Add /fleet hybrid dispatch mode for crew watch --execute (#775) — enables parallel issue analysis via Copilot CLI /fleet, 2.9x faster than sequential dispatch for read-heavy work
- (sdk) perf(agents): parallelize charter discovery with concurrency limit to reduce multi-agent load time
- (sdk+cli) perf(resolution): memoize crew-dir lookups; deduplicate crews.json reads to reduce filesystem I/O on repeated resolution calls
...
- (sdk+cli) Add routing.md to crew export/import functionality
- (sdk) Add SkillScriptLoader, handler types, and applySkillHandlers for executable skill handler scripts
- (cli) feat: APM integration — crew skill publish/install + apm.yml in init
- (sdk) feat: Cooperative rate limiting with predictive circuit breaker
- (sdk+cli) Cross-crew orchestration — discovery, delegation, and manifest (#316). Adds manifest schema, `crew discover` / `crew delegate` commands, and runtime module for coordinating work across multiple Crew instances.
- (cli) Add deprecation warnings for tunnel, rc, and REPL commands. The interactive shell (no-args), `crew start`, `crew start --tunnel`, `crew rc`, and `crew rc-tunnel` now emit yellow deprecation notices pointing users to the GitHub Copilot CLI. No behavior changes — all commands still work.
...
- (sdk+cli) Add external state storage — move .crew/ out of the working tree (#792)
- (sdk+cli) Add fact-checker as a built-in agent role (#789)
- (cli) Fix crew doctor and upgrade for insiders installs: add .crew/casting/ to ENSURE_DIRECTORIES, scaffold casting defaults from templates with permission-safe fallback, and clarify ESM warnings for global installs.
- (sdk) Sync crew.agent.md roster when agents added to team.md
- (cli) fix(ci): fix multiple CI test failures introduced by PR #1035 coordinator slimming
- (sdk+cli) Warn when crew.agent.md template is missing during upgrade or init instead of silently skipping file creation. Adds `warnings` field to `InitResult` for structured error reporting.
- (sdk+cli) feat(sdk): git-notes + orphan-branch state backends for .crew/
...
- (cli) feat(cli): Add `crew loop` command — prompt-driven continuous work loop
- (sdk+cli) feat: Machine capability discovery and `needs:*` label-based issue routing
- (sdk+cli) Add `crew init --mcp-frontmatter` to write MCP server config into the Crew agent frontmatter instead of `.copilot/mcp-config.json` for compatible agent harnesses.
...
- (sdk+cli) Add Rai as Crew's third built-in agent — a Responsible AI (RAI) reviewer
- (sdk+cli) feat: add reflect skill for in-session learning capture and mistake prevention
- (sdk+cli) Add `--repo` and `--branch` flags to `crew export` and `crew import` commands, enabling users to push/pull Crew configuration directly to/from a GitHub repository via the GitHub Contents API.
- (sdk+cli) Add retro enforcement skill with Test-RetroOverdue and ceremonies template update.
- (sdk) Add `.crew/.scratch/` directory for organized temp file management (#790)
...
- (cli) Add `crew upgrade --self` to upgrade the CLI package itself (#798)
- (cli) feat: session init update check with extensible session-init reference
- (cli) feat: add crew-commands skill for in-chat command discovery
- (sdk+cli) feat: add CREW_HOME env var and preset system
...
- (cli) Add --verbose flag to crew watch for debugging empty boards and silent failures (#781)
...
- (cli) fix(cli): revert detect-crew-dir to zero-dependency bootstrap
...
- (cli) fix(cli): wire --team-root / CREW_TEAM_ROOT into crew resolution for nap, status, and cost commands (#734)
...
- (cli) Fix post-init message to recommend `copilot --agent crew` instead of deprecated `crew` command (closes #1034, closes #1007)
- (sdk+cli) Fix state-backend and upgrade regressions (#1163, #1185, #1190, #1191, #1194)
- (cli) Fix `crew <command> --help` silently running the command instead of showing help (#1201)
- (cli) Extend CREW_TEAM_ROOT to all resolveCrew() call sites for subprocess compatibility
- (sdk+cli) Fix triage label slug derivation: use `slugify()` instead of `toLowerCase()` so multi-word agent names produce valid GitHub labels (e.g. "Steve Rogers" → `crew:steve-rogers` instead of `crew:steve rogers`).
...
- (sdk) Fix crew-commands skill routing and configuration for Copilot integration
...
- (sdk+cli) Fix Windows Azure CLI execution and ADO state mapping in crew watch
- (sdk) fix(sdk): wire `crew_route` tool handler to actually create target sessions
...
- (cli) Ship 8 built-in skills with crew init/upgrade (#788)
- (cli) Allow memory diagnostics log level to be configured from `.crew/config.json`.
- (sdk+cli) Enable persistent Ralph — heartbeat cron + healthCheck timer
- (sdk+cli) Crew coordinator now scans all 5 project skill directories: Copilot CLI's 3
- (sdk+cli) Generic, provider-agnostic scheduler for Crew (#296) — unified schedule.json manifest with LocalPolling and GitHubActions provider adapters, CLI commands (list, run, init, status), schema validation, and cron/interval trigger evaluation.
- (sdk+cli) Harden runtime state tools so write, append, and delete only mutate approved Crew runtime state paths, and refresh prompt templates to require state-tool persistence and literal datetime propagation.
...
### Added — Crew.Agents.AI NuGet preview
- Added `Crew.Agents.AI`, a `net10.0` Microsoft Agent Framework package that exposes a Crew team as an `AIAgent`.
- Public surface: `CrewAgent`, `CrewAgentOptions`, `CrewConnectionFactory`, and `CrewServiceCollectionExtensions`.
...
- Added README documentation for the `Crew.Agents.AI` package in `src/Crew.Agents.AI/README.md`.

## [0.9.0] - 2026-03-23

### Added — Personal Crew Governance Layer
- **Personal Crew concept** (#508) — isolated developer workspace with own team.md, routing.md, and agent roster
- **Ambient discovery** — automatic detection of personal crew at `~/.crew/` via environment variables and fallback paths
- **CLI commands** (#508) — `crew personal init`, `crew personal list`, `crew personal use`, `crew personal remove`
- **Governance integration** (#508) — hooks, ceremonies, telemetry isolated per personal crew
- **Unit & integration tests** (#508) — full test coverage for personal crew flows

### Added — Worktree Spawning & Orchestration
- **Worktree creation** (#529) — coordinator spawns managed worktrees for parallel agent work via `crew worktree spawn`
- **Cross-crew orchestration** (#446) — agents coordinate work across multiple crews and worktrees
...
- **Auto-wire in initCrewTelemetry()** (#281) — telemetry initialization no longer requires manual wiring
- **OpenTelemetry integration** — automatic context propagation across crew sessions
...
- **Integrated into crew init** — available to all new teams via `.crew/templates/issue-lifecycle.md`
...
- **Documentation** (#519) — setup guide for Kubernetes-based crew deployments
...
- **Integration** — available as a core team skill via `.crew/skills/session-recovery/SKILL.md`
...
- **Multi-account GitHub workflows** (#470) — safely switch GitHub identities within a crew session
- **Integration** — available as `.crew/skills/gh-auth-isolation/SKILL.md`
...
- **Autonomous agents guide** (#492) — comprehensive guide for building autonomous agents with Crew SDK
...
- **Gitignore parent coverage** (#549) — `.gitignore` updates for nested crew directories
- **ADO configurable work item types** (#444) — flexible schema for Azure DevOps work items
- **Template directory audit** (#461) — all template references aligned with `.crew/templates/`
...
- **Installation:** `npm install -g @blacklite/crew-cli` (standard npm registry)
...
- **PAO external communications** (#426) — Phase 1 infrastructure for multi-channel crew updates
- **Casting reference** (#814133d) — comprehensive casting configuration guide

### By the Numbers
- **40+ commits** merged since v0.8.25
- **6+ major features** (Personal Crew, Worktree Spawning, Capability Discovery, Cooperative Rate Limiting, Economy Mode, Auto-wire Telemetry)
...
- **Governance layer** — new Personal Crew feature enables distributed developer workflows
...
- **Cross-project ADO config** via `.crew/config.json` — work items can live in a different org/project than the repo
...
### Added — SubCrews (Community-Voted Rename)
- Workstreams → SubCrews across CLI, types, and docs
- CLI: `crew subcrews` (with `workstreams` and `streams` as deprecated aliases)
...
- `.crew/skills/secret-handling/SKILL.md` team reference

### Added — Docs Site Improvements
- Contributor Guide page in docs site Guide section
- Crew Contributors Guide page (36+ contributors honored)
...
- 153 new tests (92 platform + 15 comms + 46 SubCrews)
- 59 security tests
- 8 issues closed
...
- **Node 24+ `crew init` crash fix (#XXX)** — v0.8.23 resolves `ERR_MODULE_NOT_FOUND: Cannot find module 'vscode-jsonrpc/node'` crash that occurs on Node.js 24+ and GitHub Codespaces. Root cause: upstream ESM import issue in `@github/copilot-sdk`. Two-layer defense implemented:
...
### Added — Crew RC Documentation
- Comprehensive guide for `crew rc` (Remote Control) covering:
...
- **Builder functions** — Type-safe team configuration with runtime validation
  ...
  - `defineCrew()` — Top-level composition builder
- **`crew build` command** — Compile TypeScript definitions to `.crew/` markdown
  - Generates `.crew/team.md`, `.crew/routing.md`, agent charters, ceremonies
...
- `crew doctor` command — 9-check setup validation with emoji output
- `crew link <path>` command — link a project to a remote team root
- `crew init --mode remote` — initialize with remote team root config
- `ensureCrewPathDual()` / `ensureCrewPathResolved()` — dual-root write guards

### Fixed — Critical Crash & Stability Issues
- **Installation crash fix (#247)** — `npx @blacklite/crew-cli` was failing on fresh installs due to hard dependency on `@opentelemetry/api` that couldn't resolve in isolated npm environments. Created `otel-api.ts` resilient wrapper with full no-op fallbacks. Moved OTel to optional dependencies. Telemetry now gracefully degrades when absent.
...
- **Distribution:** npm-only distribution channel. No more GitHub-native distribution (`npx github:Blacklite/crew`). Users now install via `npm install -g @blacklite/crew-cli` or `npx @blacklite/crew-cli` from npm registry.
...
### Fixed
- **Template path fix (#190):** Corrected all references from `.crew-templates/` to `.crew/templates/` to align with the project's directory structure. This ensures the CLI correctly resolves team member charters and other template resources.
- **Init test templates:** Updated initialization tests to reference the corrected `.crew/templates/` directory path.

## [0.8.18-preview] - TBD

### Added — Remote Crew Mode (ported from @spboyer's [Blacklite/crew#131](https://github.com/Blacklite/crew/pull/131))
- `resolveCrewPaths()` dual-root resolver — project-local vs team identity directories (#311)
- `crew doctor` command — 9-check setup validation with emoji output (#312)
- `crew link <path>` command — link a project to a remote team root (#313)
- `crew init --mode remote` — initialize with remote team root config (#313)
- `ensureCrewPathDual()` / `ensureCrewPathResolved()` — dual-root write guards (#314)

### Changed — npm Distribution & Monorepo Structure
- **Distribution:** Migrated from GitHub-native (`npx github:Blacklite/crew`) to npm packages (`npm install -g @blacklite/crew-cli` / `npx @blacklite/crew-cli`)
- **Packages:** Independent versioning via @changesets/cli — `@blacklite/crew-sdk` and `@blacklite/crew-cli` evolve on separate cadences
- **Structure:** Monorepo layout with workspace packages (SDK + CLI)
- **Directory:** `.crew/` directory structure (migration from `.ai-team/`)
...
- Removed `.crew` branch protection guard (`crew-main-guard.yml`) — no longer needed with npm workspace `files` field exclusions
...
- Migrated to npm workspace publishing (`@blacklite/crew-sdk`, `@blacklite/crew-cli`)

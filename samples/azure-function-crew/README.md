...
Content review crew deployed as an HTTP-triggered Azure Function. A multi-agent review pipeline analyzes submitted content through three specialist agents: tone reviewer, technical reviewer, and copy editor. Results are aggregated into a structured JSON response with per-agent scores and findings.
...
- How to use `defineCrew()`, `defineTeam()`, `defineAgent()`, and `defineRouting()` to compose a crew configuration
- How to wire an Azure Function HTTP trigger to a Crew SDK application
...
- How to deploy a Crew application to a serverless platform

## How it works

The sample defines a review crew with three specialist agents using the SDK builder API. When an HTTP POST is received at `/api/crew-prompt` with a `{"prompt": "..."}` body, the Azure Function loads the crew config, instantiates the review agents, and distributes the content for analysis. Each agent reviews the content according to its role: the tone reviewer assesses audience fit and engagement, the technical reviewer checks factual accuracy and code validity, and the copy editor reviews grammar and readability. The function aggregates results and returns a JSON response with per-agent scores, findings, and an overall consensus.
...
```json
{
  "prompt": "Building multi-agent systems with the Crew SDK is straightforward...",
...
| File | Purpose |
|---|---|
| `src/functions/crew-prompt.ts` | Azure Function HTTP trigger — entry point |
| `src/crew/config.ts` | Crew configuration using builder API |
| `src/crew/handlers.ts` | Agent review handlers (mock logic for demo) |
...
1. Replace mock handlers with real `CrewClient` calls for live reviews
...
- See [autonomous-pipeline](../autonomous-pipeline/README.md) for a full showcase of Crew SDK features
- Check the [Crew SDK documentation](../../README.md) for builder API details

...
const TEST_ROOT = join(tmpdir(), 'hello-crew-test-' + Date.now());
const CREW_DIR = join(TEST_ROOT, '.crew');
...
describe('resolveCrew', () => {
  it('finds the .crew directory from a subdirectory', () => {
    const sub = join(TEST_ROOT, 'a', 'b');
    mkdirSync(sub, { recursive: true });
    const result = resolveCrew(sub);
    expect(result).toBe(CREW_DIR);
  });

  it('returns null when no .crew directory exists', () => {
    const empty = join(tmpdir(), 'no-crew-' + Date.now());
    mkdirSync(empty, { recursive: true });
    // Place a .git marker so the walk stops
    mkdirSync(join(empty, '.git'));
    const result = resolveCrew(empty);

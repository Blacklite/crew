# Hello Crew — SDK Sample PRD

## Overview
The simplest possible Crew SDK sample. Create a team, cast agents from a thematic universe, initialize their sessions, and display the roster. This is the "hello world" — shows that the casting engine works and teams persist as typed objects.

## Target Audience
Developers new to Crew. No async patterns, no tooling, no governance. Pure SDK mechanics: resolve config, cast a team, print the roster.

## SDK APIs Demonstrated

| API | Module | What It Shows |
|-----|--------|--------------|
| `resolveCrew()` | `resolution` | Locate the `.crew/` directory from the current working directory |
| `loadConfig()` | `config` | Parse `crew.yaml` into a typed configuration object |
...
| Config types | `config` | `CrewConfig`, `AgentDefinition`, `UniverseId` |

## Code Highlights

**Initial setup — resolve and load:**
```typescript
import { resolveCrew, loadConfig, CastingEngine, onboardAgent } from '@blacklite/crew-sdk';
...
2. SDK resolves `.crew/` from cwd
3. SDK loads `crew.yaml` 
...
```bash
# Clone crew and navigate to samples/hello-crew
npm install

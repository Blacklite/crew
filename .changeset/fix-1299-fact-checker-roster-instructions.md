...
When a user runs `crew init` followed by `copilot --agent crew`, the coordinator's first-time casting flow correctly creates `.crew/agents/fact-checker/` on disk (per merged PR #1223) but **omits Fact Checker from the `## Members` table** in `.crew/team.md`. Rai is included, Ralph is included, Scribe is included — Fact Checker is silently dropped.

## Root cause

`.crew-templates/crew.agent.md` (the canonical coordinator instructions, mirrored to all package templates) had two gaps:
...
2. Added a full `## Fact Checker — Verification & Devil's Advocate` section that mirrors the Rai structure:
   ...
   - Explicit **Roster Entry** line: *"Fact Checker always appears in `team.md`: `| Fact Checker | Fact Checker | .crew/agents/fact-checker/charter.md | 🔍 Verifier |`"*
...
- `.crew-templates/crew.agent.md` (canonical source) updated.
- `sync-templates.mjs --sync` propagates to all 4 mirror targets:
  ...
  - `.github/agents/crew.agent.md` (this repo's own coordinator file)

## Test coverage

New `test/crew-agent-roster.test.ts` runs against **all 4 template targets** and asserts:
...
After this PR ships, a fresh `crew init` + `copilot --agent crew` produces a `team.md ## Members` table that includes Fact Checker alongside Scribe, Ralph, and Rai.

...
### Path 1: `crew` — no .crew/ exists
**File:** `cli-entry.ts:654–677`
Shows welcome text: "Get started → crew init". Clean dead end.
**Verdict: ✅ Solid.**

### Path 2: `crew init` — no prompt
**File:** `cli-entry.ts:690–722` → `init.ts:runInit()`
Scaffolds `.crew/` with an empty team.md roster. No `.init-prompt`. Final message tells user to run `crew init "prompt"` or start a Copilot session.
**Verdict: ⚠️ Creates a half-state.** User has `.crew/` but no team. They have to know to do something else. Running `crew` next puts them in a shell with an empty roster and no auto-cast trigger.

### Path 3: `crew init "prompt"` — with prompt
**File:** `cli-entry.ts:706–721` → `init.ts:runInit({ prompt })`
Same scaffold as Path 2, plus writes `.crew/.init-prompt`. Message: "Run crew to cast your team."
**Verdict: ✅ Good — sets up deferred casting.**

### Path 4: `crew` — .crew/ exists, empty roster, .init-prompt exists
...
### Path 7: crew.agent.md Init Mode (Copilot extension)
**File:** `templates/crew.agent.md:23–91`
...
Running `crew init "new prompt"` after a team already exists writes `.init-prompt`, but the auto-cast check only fires when the roster is empty. The file sits there forever, unused.

### 🟡 Issue: No cast confirmation in CLI flow
The CLI path (`handleInitCast`) sends a prompt to the LLM, gets back an `INIT_TEAM:` block, parses it, and creates all team files — no confirmation step. The agent prompt path (crew.agent.md) explicitly stops and waits for user approval. CLI users get no say in who's on their team.

### 🟡 Issue: Two divergent casting flows
`buildInitModePrompt()` (coordinator.ts) is a bare-minimum prompt: "propose 4-5 agents, use INIT_TEAM: format." The crew.agent.md Init Mode is rich: resonance scoring, universe selection algorithm, git user detection, 2-phase confirmation. Same product, two completely different experiences depending on how you got here.

### 🟢 Non-issue: `crew init` then `crew init "prompt"` (idempotency)
...
**The golden path works:** `crew init "Build a React app"` → `crew` → auto-cast fires → team created → message re-dispatched.

**The fragile paths:**
1. `crew init` (no prompt) → `crew` → empty shell, no auto-cast, user confused
...
```
User runs crew          ──→ No .crew/?  ──→ "Run crew init to get started"
                               ↓ yes
...
5. **Make `/init` actually useful.** Two options:
   ...
   - **(b) Better:** `/init "prompt"` triggers casting directly from the REPL, same as `crew init "prompt"` from the CLI but without leaving the shell.

#### P2: Add confirmation before casting
6. **Show the proposed team before creating files.** The `formatCastSummary` function already exists. After parsing the proposal, display it and ask "Look good? (Y/n)" before calling `createTeam`. This matches the crew.agent.md flow and prevents garbage casts from bad prompts.

   *Trade-off:* This adds friction to the auto-cast-on-shell-start path. Option: skip confirmation only when the prompt came from `.init-prompt` (the user explicitly provided it via `crew init "prompt"`), confirm when the prompt is a freeform REPL message.

#### P3: Align the two casting flows (future)
7. **Long-term:** The CLI casting engine (`buildInitModePrompt` + `parseCastResponse`) should converge with the crew.agent.md Init Mode logic. The agent prompt has better casting (resonance scoring, proper universe selection). The CLI prompt is a thin stub. Two options:
...
| File | What changes |
|------|-------------|
| `packages/crew-cli/src/cli/shell/index.ts` | Race condition fix (move auto-cast to onReady), Ctrl+C abort wiring, .init-prompt cleanup |
| `packages/crew-cli/src/cli/shell/commands.ts` | `/init` command behavior |
| `packages/crew-cli/src/cli/shell/components/App.tsx` | Empty-roster banner messaging |
| `packages/crew-cli/src/cli/shell/coordinator.ts` | (P2) Cast confirmation flow |
| `packages/crew-cli/src/cli/core/init.ts` | No changes needed |
| `packages/crew-cli/src/cli/core/cast.ts` | No changes needed |

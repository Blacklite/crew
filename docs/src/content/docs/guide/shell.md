...
> 💡 **Recommended: Use GitHub Copilot CLI** — The best way to interact with Crew today is through the [GitHub Copilot CLI](https://docs.github.com/en/copilot/github-copilot-in-the-cli). Run `copilot --agent crew` to start a session with your team. The Copilot CLI provides the richest agent experience, including tool use, parallel execution, and full MCP integration.

The Crew interactive shell gives you a persistent connection to your team.Instead of spawning short-lived CLI invocations, the shell maintains a real-time session where you can talk to agents, issue commands, and watch work happen.
...
With no arguments, `crew` enters the interactive shell. You'll see a prompt:
...
Display the current state of your crew: active agents, sessions, and recent work.
...
crew > /sessions
crew > /resume a1b2
crew > @Keaton, where were we on the auth work?
```
...
```
crew > Keaton, set up the database schema for user authentication
```

Or without naming an agent — the coordinator routes to whoever is best suited:

```
crew > Write a blog post about our new casting system
```
...
4. **Agents write results** → To `.crew/` (decisions, history, skills, etc.)
...
```
crew > @Keaton, check on Verbal and resume if needed
```
...
```
# Don't:
crew > Build the auth system. Use JWT. Refresh tokens every 1 hour...

# Do:
crew > Build the auth system. See the auth decision in decisions.md.
```
...
```
crew > @Keaton, here's what needs doing:
...
```
crew > @Keaton, this is critical: we need the deployment script fixed
```
...
```
crew > Write the API spec
crew > Build the React components
crew > Set up the database
...
```
crew > @Verbal, what's left on the auth endpoints?
crew > @McManus, show me what you've written so far
crew > @Fenster, are the tests passing?
```
...
```
crew > @Keaton, when Verbal finishes the auth API, have him route testing to Fenster
```
...
```
crew > @Keaton, route this task to @Verbal and report any blocks
```

### Shell Quit Unexpectedly

Run `crew` again to restart. Check `.crew/log/` for error context.
...
- [VS Code](../features/vscode.md) — Crew in VS Code

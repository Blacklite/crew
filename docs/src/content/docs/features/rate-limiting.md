...
Crew monitors API rate limit headroom in real time and pauses work before limits are reached — not after. This prevents cascading failures across concurrent agents.

---

## How It Works

Crew tracks the rate limit headers returned by every API call. Before dispatching the next request, it checks remaining headroom against a configurable threshold. If headroom is below the threshold, it pauses and waits for the window to reset.
...
Crew uses a three-state model for rate limit health:
...
2. Crew polls the rate limit reset timestamp from the API response headers.
3. At reset, Crew resumes from the queue — oldest requests first.
...
In Amber state, Crew reduces the number of agents dispatching simultaneously. This distributes the remaining quota across a longer window rather than exhausting it instantly.

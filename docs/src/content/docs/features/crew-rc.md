...
`crew rc` (remote control) exposes GitHub Copilot CLI over a secure WebSocket tunnel, letting you chat with Copilot from your phone, tablet, or any browser. Unlike `crew start` (which mirrors terminal output), `crew rc` uses **ACP passthrough** — raw JSON-RPC communication directly with Copilot's Agent Communication Protocol. You get full Copilot capabilities, not just terminal visibility.
...
## How It Differs from `crew start`

| Feature                  | `crew rc`                          | `crew start`                    |
|--------------------------|-------------------------------------|----------------------------------|
...
| **Team Roster**          | Loaded from `.crew/team.md`        | Not applicable                   |

**When to use `crew rc`:** You want to control Copilot remotely (ask questions, run commands, access full agent capabilities).

**When to use `crew start`:** You want to mirror a terminal session to your phone (demos, pairing, watching long-running processes).

> 💡 **Looking for terminal mirroring?** See [crew start](./remote-control.md).
...
**Different directory:**
```bash
crew rc --path ~/my-project --tunnel
```
...
crew rc --tunnel --path ~/repos/my-app
```
...
┌─────────────────────────────────────────────────────────────┐
│ Your Repository                                             │
│  • Copilot reads .crew/team.md (optional)                  │
...
If `.crew/team.md` exists, `crew rc` parses the Active members table:
...
`crew rc` implements 7 layers of security:
...
- **Labels:** Tunnel tagged with `crew`, `repo`, `branch`, `machine` labels
...
```
Crew RC | Repo: crew-pr | Branch: main | Agents: 5 | Copilot: passthrough | Connections: 2
```

### `/agents`
Lists all agents from `.crew/team.md`:
```
Team Roster:
• Fenster (Core Dev)
...
```
Routed to the named agent if supported by your crew configuration.
...
### Log Location
```
~/.cli-tunnel/audit/crew-audit-<timestamp>.jsonl
```
...
```bash
# View live logs
tail -f ~/.cli-tunnel/audit/crew-audit-*.jsonl

# Search for prompts
grep '"type":"prompt"' ~/.cli-tunnel/audit/crew-audit-*.jsonl | jq .
```
...
**Fix:**
1. **Refresh the QR code:** Stop `crew rc` (Ctrl+C) and restart
...
**Cause:** Copilot's MCP servers are still loading (first 15-20s after `crew rc` starts).
...
**Fix:**
```bash
crew rc --port 0  # Auto-assign free port
# OR
crew rc --port 8080  # Specific port
```
...
**Fix:** Restart `crew rc --tunnel` to create a new tunnel.

---

## See Also

- [crew start](./remote-control.md) — PTY mirror mode for terminal streaming
- [CLI Reference](../reference/cli.md) — All crew commands
- [Remote Control Protocol](https://github.com/Blacklite/crew/blob/main/packages/crew-sdk/src/remote/protocol.ts) — Wire protocol types
- [RemoteBridge SDK](https://github.com/Blacklite/crew/blob/main/packages/crew-sdk/src/remote/bridge.ts) — Server implementation

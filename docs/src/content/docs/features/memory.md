...
Crew remembers the durable things that help future work — decisions, conventions,
...
Crew's memory is layered. Each layer serves a different purpose, and knowledge grows with every session.

By default, Crew memory is local worktree memory stored in `.crew/` files. Future
...
External semantic memory is opt-in; it is not required for `crew init` or Copilot custom
agents using the prompt-only `.crew/` fallback.

`crew init` and `crew upgrade` scaffold a local-only governance policy at
`.crew/memory/config.json`. The default provider is `local`, Copilot Memory is disabled,
and audit records are written to `.crew/memory/audit.jsonl` without storing memory content.
...
```bash
crew memory classify "Always run tests before merge"
crew memory write --content "Use Vitest for SDK regression tests" --class DECISION --author scribe
crew memory search --query "Vitest"
crew memory audit
crew memory provider
```
...
Precedence is: explicit CLI switch, then `CREW_MEMORY_LOG_LEVEL`, then
`.crew/config.json` `memory.logLevel`, then the default `none`. Diagnostics are written
...
Prompt-only Copilot custom agents still fall back to direct `.crew/` file edits. That
...
exists in the installed SDK/tooling. Crew does not invent endpoints or fake a remote memory
...
```
.crew/decisions/inbox/{agent-name}-{slug}.md
```
...
1. Reads all entries from `.crew/decisions/inbox/`
...
As your project grows, `decisions.md` accumulates hundreds of blocks. Stale sprint artifacts, completed analysis docs, and one-time planning fragments consume context window space without adding value. When this happens, old decisions are archived to `.crew/decisions-archive.md` — preserved for reference but no longer loaded into agent context.
...
- **Commit intentional `.crew/` state** — anyone who clones the repo gets the team with
  its accumulated decisions and skills. Never store secrets, credentials, raw logs, or
  private customer data in Crew memory.

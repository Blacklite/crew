...
You don't always have the repo cloned locally. Sometimes you want to ask your Crew a question from the browser, the GitHub CLI, or a mobile device — without pulling code.

---

## Current options

Crew already supports several remote interaction paths. Each trades off convenience, depth, and setup effort.

### 1. Copilot Chat with crew.agent.md

If the repo has `.github/agents/crew.agent.md`, GitHub Copilot Chat reads it automatically when you ask questions about the repo.
...
Create a GitHub issue and assign it to `@copilot`. If the repo has Crew's issue-assign workflow (`.github/workflows/crew-issue-assign.yml`), the coding agent picks up the issue and works it using your Crew configuration.

**How it works:**
1. Create an issue describing the question or task
2. Assign it to `@copilot`
3. The workflow triggers and Crew processes it
...
### 3. Use `crew:` labels on issues

Add a `crew:{member}` label to any issue, and Crew routes it to the right team member.

**How it works:**
1. Create or label an issue with `crew:fenster` (or any member name)
2. The triage workflow assigns it to the appropriate agent
3. Work proceeds through the normal Crew flow
...
A Discussions-based Q&A channel where Crew monitors and answers questions would make remote interaction feel conversational. This would need a new workflow trigger on `discussion` events.

### Issue comment commands

A `/crew ask "question"` command in issue comments that triggers Crew to respond inline would enable threaded Q&A without creating new issues.

---

## Tips

- **Start with Copilot Chat.** It's the lowest-effort path and works today for repos with `crew.agent.md`.

...
Ralph runs periodically via GitHub Actions to handle housekeeping between Copilot sessions — triage new issues, apply crew labels, check stale branches, archive old decisions.
...
- Triage new issues
- Apply crew labels based on routing rules
- Check for stale branches
- Archive old decisions

The workflow is in `.github/workflows/crew-heartbeat.yml` and runs on issue close, PR merge, and manual dispatch (or via `crew watch` for local polling).

**You don't have to do anything** — it's installed automatically (along with 9 other workflows) when you run `crew`.
...
Crew uses GitHub labels to drive workflows:

**Routing labels** (auto-applied by Ralph):
- `crew:neo` — routed to Neo (Lead)
- `crew:trinity` — routed to Trinity (Frontend Dev)
- `crew:morpheus` — routed to Morpheus (Backend Dev)
- `crew:tank` — routed to Tank (Tester)
...
Ralph applies `crew:*` and `type:*` labels automatically. You apply `go:*` labels manually when you want autonomous processing.
...
1. Ralph labels issue #42 with `crew:morpheus` (backend work)
  ...
  auto-assign:
    ...
    steps:
      ...
      - name: Spawn Copilot session
        run: |
          copilot --agent crew --message "${{ steps.agent.outputs.agent }}, handle issue #${{ github.event.issue.number }}"
```
...
**Crew agents require a live Copilot session.** They can't run in a GitHub Actions runner without Copilot CLI access.

This means:

❌ You **can't** run `Crew, build the feature` inside a GitHub Actions workflow  
...
2. **Ralph (heartbeat)** runs, reads routing rules, applies `crew:morpheus` and `type:feature`
...
## 6. Workflow Templates Ship with Crew

When you run `crew`, these workflow templates are installed:

- `.crew/templates/workflows/crew-heartbeat.yml` → Ralph runs every 6 hours
- `.crew/templates/workflows/copilot-auto-assign.yml` → Triggers Copilot on `go:*` labels
- `.crew/templates/workflows/pr-review-reminder.yml` → Reminds you of open PRs needing review

To activate them:

```bash
cp .crew/templates/workflows/*.yml .github/workflows/
git add .github/workflows/
git commit -m "Enable Crew workflows"
git push
```
...
```bash
crew heartbeat --dry-run
```

**Have agents work on labeled issues:**

```
> Team, review all open issues labeled crew:morpheus and tell me
...
```
> Ralph, triage the 10 newest issues and apply crew labels.
...
- **Workflows are templates.** Customize `.crew/templates/` to match your CI/CD setup, then copy to `.github/workflows/`.
- **Heartbeat frequency is configurable.** Edit `crew-heartbeat.yml` to change from every 6 hours to daily, hourly, etc.

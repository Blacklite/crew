...
Three ways to get Crew running. Pick the one that fits.
...
The CLI is the fastest way to use Crew from any terminal.
...
| **You want to...** | **Use** | **Why** |
|--------------------|---------|---------|
| Try Crew quickly | **CLI** with `npx` | No install needed. Run `npx @blacklite/crew-cli init` and you're testing it. |
| Use Crew across all projects | **CLI** with `--global` | One install. Works everywhere. Run `crew` from any terminal. |
| Work inside VS Code | **VS Code** (just open your project) | Already using Copilot? Crew just works. Same `.crew/` directory as CLI. |
| Build tools on top of Crew | **SDK** | Typed APIs, routing config, agent lifecycle hooks. Programmatic access to everything. |

Can't decide? → Start with **CLI**. You can always add VS Code or the SDK later. Your `.crew/` directory works identically everywhere.

---

## 2. VS Code

Crew works in VS Code through GitHub Copilot. Your `.crew/` directory works identically in both CLI and VS Code — same agents, same decisions, same memory.

> **Tip:** Initialize your team with the CLI (`crew`), then open the project in VS Code to keep working with the same crew.

---

## 3. SDK

Building your own tooling on top of Crew? Install the SDK as a project dependency:
...
This creates your personal crew directory — a personal team root that any project can inherit from. See [Upstream Inheritance](../features/upstream-inheritance.md) for details.
...
After installing, initialize Crew in your project:
...
.crew/                        — team state directory
```

### Configuration (optional)

For typed configuration, create a `crew.config.ts` at your project root:
...
`defineConfig()` gives you full autocomplete and validation. But you don't need it to get started — Crew works out of the box with sensible defaults.
...
```bash
npm list -g @blacklite/crew-cli
```

If it's not listed, re-run `npm install -g @blacklite/crew-cli`.
...
Run `crew init` in your project root, or `crew init --global` for a personal crew.
...
New to Crew? Check out [**Tamir's Crew Skills Workshop**](https://github.com/tamirdresher/crew-skills/tree/main/workshop) for hands-on learning and practical patterns.

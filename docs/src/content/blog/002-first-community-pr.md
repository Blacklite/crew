...
wave: null
tags: [crew, community, contribution, pr-2]
status: published
...
- **GitHub Issues Mode** — full lifecycle from issue to merged PR, including `crew/{issue-number}-{slug}` branch naming, `Closes #N` linking, review comment handling, and merge with auto-close. *Built by [@spboyer](https://github.com/spboyer).*
...
| Branch convention introduced | `crew/{issue-number}-{slug}` |
| Routing table entries added | 3 |

## What We Learned

- **External contributors see gaps the team doesn't.** GitHub Issues Mode, PRD Mode, and Human Team Members are all features that connect Crew to how real teams work. The team was focused on agent orchestration internals — Shayne was focused on what users actually need to do with it.
- **The `crew/{issue-number}-{slug}` branch convention is worth stealing.** It traces every branch back to an issue, and every PR back to a branch. Simple, auditable, and it came from outside the team.
...
Shayne's three features are live on the `wave-2` branch. Issues Mode, PRD Mode, and Human Team Members will ship as part of Crew's next release. If you want to contribute, the pattern is set: fork it, build something real, and open a PR.

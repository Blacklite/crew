...
wave: null
tags: [crew, community, contribution, pr-1]
status: published
...
- **Worktree Awareness** — two strategies for resolving the team root: worktree-local (branch-isolated state, recommended for concurrent work) and main-checkout (shared state, single-session only). Auto-detection checks if `.crew/` exists in the current worktree, falls back to main checkout if not. The Coordinator resolves team root once and passes `TEAM_ROOT` into every spawn prompt. *Built by [@amolchanov](https://github.com/amolchanov).*
- **Scribe Auto-Commit** — Scribe commits `.crew/` changes after every session with detailed `docs(ai-team):` conventional commit messages. Itemizes what was logged, merged, and propagated. *Built by [@amolchanov](https://github.com/amolchanov).*
...
PR #1 came from amolchanov's fork — `worktree-awareness-and-scribe-commit` — and landed on February 7th. It was +365 lines, -5 removed, across 5 files. Four distinct features, each solving a real problem, from the very first person who looked at Crew and decided to build on it.

Let's be specific about what was broken before this PR: Crew didn't work in real multi-branch scenarios. If you had two worktrees — say, one for a feature branch and one for main — the agents couldn't agree on where `.crew/` lived. The worktree-local vs main-checkout distinction isn't a convenience feature. It's the reason Crew can run in parallel across branches at all.

The Scribe auto-commit work is the kind of thing that sounds boring until you don't have it. Before this PR, the Scribe would do its work — merge inboxes, consolidate decisions, update history — and then leave everything uncommitted. You'd end up with dirty state in `.crew/` and no record of what changed or why. amolchanov wired up conventional commits with itemized messages. Now you can `git log` the `.crew/` directory and see exactly what the Scribe did, when, and to which files.
...
amolchanov's follow-up comment told the backstory: they'd been experimenting with Crew to build a Unity game. That's where the worktree insight came from — real usage on a real project. They suggested per-worktree commits so you could see exactly who did what, and floated the idea of "working tree per crew member as it would be in the real life." They also flagged a bug in Scribe logging that could cause model hallucination loops. That's a contributor who's paying attention.
...
- **PR #1 set the architectural foundation.** Not a typo fix. Not a README tweak. The first external contributor built the worktree system that Crew's multi-branch workflow depends on. That's not typical, and it's worth acknowledging.
- **Real usage generates real contributions.** amolchanov found the worktree gap by actually using Crew to build a Unity game. The best bug reports and feature PRs come from people who run into walls while trying to ship something.
...
amolchanov's worktree foundation is live on main. Every Crew session that runs in a worktree — which is most of them — uses the resolution logic from this PR. If you're running Crew across branches and things just work, this is why.

---
title: "What's New in v0.9.0: Personal Crew, Worktrees, Cooperative Rate Limiting, and More"
date: 2026-03-23
author: bradygaster
wave: 8
tags: [crew, release, v0.9.0, features, personal-crew, worktree, rate-limiting, economy-mode]
status: published
hero: "Crew's biggest release yet — personal agents that follow you across repos, isolated worktrees for conflict-free parallel work, cooperative rate limiting, economy mode for cost control, and a refreshed docs site."
---
...
```bash
crew personal init          # Declare your personal agents
crew personal list          # See what's available
```

**Governance layer** — Personal agents declare what they need (memory, tools, platforms) and operate within guardrails. A personal coding agent won't blindly touch your enterprise repo; it reads your project's agent charters and `.crew/config.json` first.

**What this enables:**
- One global Crew definition across all your repos
- Agents that know your coding style, tools, and preferences
- Safe cross-repo work without conflicts
- Team crews + personal crews = complete context

:::tip
Personal Crew is foundational. It's how your favorite coding agent follows you into a new project without needing re-hiring.
:::
...
Ralph's dispatcher reads these labels and routes the work to the machine in your crew pool that has those capabilities. No more "sorry, I can't run Docker here."
...
- Multi-region crews balance load based on capabilities
...
Crew now implements **RAAS** — Rate-Aware Agent Scheduler. When your API calls trigger rate limiting, Crew maps `X-RateLimit-Remaining` headers to traffic light states:
...
All agents in your crew see the same traffic light state. When one agent hits amber, the entire team backs off. No thundering herd, no cascading failures.
...
```bash
crew skill economy-mode enable --budget=50  # USD per agent per day
```
...
**Governance:**
```typescript
const role = await crew.resolveRole('coding-agent', {
...
`initCrewTelemetry()` now auto-creates the EventBus and CostTracker:

```typescript
import { initCrewTelemetry } from '@blacklite/crew-sdk';

const { eventBus, costTracker } = await initCrewTelemetry({
...
**Consistent install path**: All user-facing docs now reference `npm install -g @blacklite/crew-cli`.
...
- ✅ **5 major features** (Personal Crew, Worktrees, Capability Discovery, Cooperative Rate Limiting, Economy Mode)
...
**None.** All changes are additive. Existing Crews work as-is. New features are opt-in via CLI or config.
...
```bash
crew upgrade  # Walks you through any project-level config updates
```

**Docs:** [Upgrade Guide](../scenarios/upgrading.md) | [Troubleshooting](../scenarios/troubleshooting.md)

If you hit issues, [open a GitHub issue](https://github.com/Blacklite/crew/issues). We're here to help.
...
- **Process template introspection** — Auto-detect ADO work item types at crew init
- **Teams webhook CommunicationAdapter** — Full implementation of Teams mobile notifications
- **SubCrew orchestration** — Compose crews from other crews; cross-team work at scale

This is an exciting time. v0.9.0 is the release where Crew scales from solo developers to distributed teams.

---

**Questions?** Drop by [Crew Discussions](https://github.com/Blacklite/crew/discussions) or ping us on [Discord](https://discord.gg/crew-community).

...
date: 2026-03-30
author: "Brady"
tags: [crew, ralph, watch, work-monitor, automation, execution]
status: published
...
`crew watch` used to be a simple triage loop. Every N minutes, it would:

1. Fetch open issues with the `crew` label
2. Route them to team members based on routing rules
3. Add labels like `crew:eecom` or `crew:gnc`
4. Idle when done
...
```bash
crew watch --execute --interval 15
```
...
Each flag is opt-in. Existing `crew watch` behavior is unchanged — triage only, no execution.
...
4. **Execute**: Spawn Copilot sessions for actionable issues
   - Issues are actionable if: assigned to a crew member, not assigned to a human, and not blocked
...
7. **Housekeep**: Governance checks
   ...
   - Decision hygiene: if `.crew/decisions/inbox/` has >5 files, spawn a merge session
...
| **Local watchdog** | You're away but machine is on | `crew watch --execute` |
...
- **SubCrew discovery**: automatically detect `.crew/subcrews/` for routing across multiple codebases
...
**Full feature set:**
```bash
crew watch --execute --board --two-pass --monitor-teams --retro --decision-hygiene --max-concurrent 2
```

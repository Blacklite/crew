...
wave: 6
tags: [crew, release, v0.4.1, patch, quality]
status: published
...
The orchestration logger wasn't persisting across sessions. Scribe wasn't recording work. We fixed the session-state detection so logs flow to `orchestration-log/` consistently. Crewmates building together now have complete history.
...
```
npx @bradygaster/create-crew upgrade
```

...
wave: null
tags: [crew, adapter, copilot-sdk, typescript, type-safety, codespace, bug]
status: published
hero: "A P0 crash in Codespaces led to a 7-issue sprint that eliminated every unsafe cast in Crew's adapter layer. Zero `as any` remaining."
---
...
> _A P0 crash in Codespaces led to a 7-issue sprint that eliminated every unsafe cast in Crew's adapter layer. Zero `as any` remaining._

## The P0

Issue #315 came in hot. Crew running in GitHub Codespaces threw:

```
TypeError: sendMessage is not a function
```

The `@github/copilot-sdk` session object in Codespaces exposes `send()`, not `sendMessage()`. Crew's adapter assumed the method name. In the CLI, the session object happened to have `sendMessage()` (or something close enough that `as any` hid the mismatch). In Codespaces, the mask came off.
...
**#317 — EVENT_MAP.** Built a typed mapping object with 10 entries connecting Crew's internal event names to `@github/copilot-sdk` event names. No more string literals scattered across files.
...
- **CopilotSessionAdapter as the single integration point.** One file mediates between Crew and the SDK. One file to audit. One file to update when the SDK changes.
...
The adapter layer is clean. The type system is honest. Now it's time to bring in a feature from the beta that the community has been asking about: remote crew mode. And it comes with a story about team collaboration.

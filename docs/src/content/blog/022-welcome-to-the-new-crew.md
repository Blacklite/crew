---
title: "Welcome to the New Crew"
date: 2026-03-10
author: "McManus (DevRel)"
wave: null
tags: [crew, v0.8.18, release, launch, sdk, cli, typescript, samples, migration]
status: published
hero: "Crew v0.8.18 is here — a full TypeScript rewrite, npm distribution, 16 CLI commands, 8 SDK samples, 2200+ tests, and a feature set that turns multi-agent development from experiment to production tool."
---
...
Crew started as a scrappy beta experiment. A few scripts, some clever prompting, and a vision: what if you could build an AI team that lived in your repo, learned your codebase, and got better with every session?

Today, Crew is a proper npm-distributed, TypeScript-strict, fully-tested multi-agent runtime. It's not just a rewrite — it's a ground-up rebuild of everything we learned from six months of usage, feedback, and production deployments.
...
- ✅ npm-only distribution (`@blacklite/crew-cli`)
- ✅ Global install: `npm install -g @blacklite/crew-cli`
...
If you were on the beta (v0.5.x), this is your upgrade moment. If you're new to Crew, this is the best time to jump in.
...
| **Distribution** | `npx github:Blacklite/crew` | `npm install -g @blacklite/crew-cli` |
| **Packages** | Monolithic | SDK + CLI (independent versioning) |
| **Config** | JSON only | Markdown + optional `crew.config.ts` |
...
### 1. **Global Install** — `crew` Works Everywhere
...
That's it. Crew is now a first-class terminal command.

### 2. **Interactive Shell** — Live Multi-Agent REPL

Run `crew` with no arguments and you drop into a live terminal:
...
No more typing `crew` before every command. The shell is stateful, context-aware, and built for conversation.

### 3. **`crew doctor`** — 9-Check Health Validation
...
### 4. **`crew start --tunnel`** — Stream Your Terminal to Your Phone

Demoing Crew to a remote audience? Run:
...
### 5. **`crew triage`** — Auto-Scan GitHub Issues

Connect Crew to your GitHub repo and watch it auto-triage:

```bash
crew triage
```

Crew scans open issues, labels them by category (bug/feature/docs/security), and assigns them to the right agent based on their role and skills. Runs continuously — every 10 minutes by default.

```bash
crew triage --interval 5   # Poll every 5 minutes
```

Your backlog manages itself.

### 6. **`crew copilot`** — Add @copilot as a Team Member
...
This adds `@copilot` as an agent in `.crew/team.md` and configures auto-assignment. When Crew triages an issue, @copilot picks it up, creates a branch, writes code, runs tests, and opens a PR.
...
Most teams want one Crew per repo. But some organizations want to share a team identity across projects. Dual-root mode gives you both:

```bash
crew init --mode remote ~/my-org-team
```

This stores agent charters and team config in `~/my-org-team/.crew/`, while project-specific state lives in `./crew/`. Agents are consistent across repos, but each project has its own decisions, skills, and context.

Perfect for agencies, consulting teams, and orgs with standardized agent roles.

### 8. **Plugin Marketplace** — Community Skill Packs

Extend Crew with community-built skill packs:

```bash
crew plugin marketplace list
crew plugin marketplace add https://github.com/user/crew-skills
```
...
### 9. **`crew export`/`import`** — Portable Crew Snapshots

Need to share your team config with a colleague or back up your setup?

```bash
crew export > my-crew.json
crew import my-crew.json
```
...
### 10. **`crew aspire`** — Observability Dashboard

Crew exports OpenTelemetry traces and metrics. Launch the Aspire dashboard to see:
...
### 11. **`crew upgrade`** — Safe Upgrades That Never Touch Your Team State
...
This updates Crew-owned files (templates, CLI scripts, SDK code) but never touches:
- `.crew/agents/**` (your agent charters)
- `.crew/decisions.md` (architectural decisions)
- `.copilot/skills/**` (learned patterns)
- `.crew/history/**` (session logs)
...
- `@blacklite/crew-sdk` — the runtime (casting, streaming, governance, cost tracking)
- `@blacklite/crew-cli` — the interface (commands, REPL, triage loop)
...
**Difficulty:** Beginner  
**SDK APIs:** `resolveCrew()`, `CastingEngine.castTeam()`, `onboardAgent()`
...
**What it does:** Two Copilot sessions trade live knock-knock jokes, streaming token-by-token. Demonstrates `CrewClientWithPool`, casting, and `StreamingPipeline`.
...
**SDK APIs:** `CrewClientWithPool`, `CastingEngine`, `StreamingPipeline`, `SessionPool`, `EventBus`
...
**Difficulty:** Advanced  
**SDK APIs:** `CrewClientWithPool`, `SessionPool`, `StreamingPipeline`, `EventBus`, system prompts
...
**What you'll see:**
```
crew > design a REST API for recipes
...
**Difficulty:** Intermediate  
**SDK APIs:** `CrewClient`, `createSession()`, `StreamingPipeline`, `onDelta()`, `onUsage()`, `EventBus`
...
This is the "wow demo" — everything Crew can do in one running script.

**Difficulty:** Advanced  
**SDK APIs:** `CrewClient`, `CastingEngine`, `HookPipeline`, `CostTracker`, `EventBus`, `StreamingPipeline`, `resolveCrew()`, `createSession()`
...
This redacts all emails in `.crew/` files:
...
Crew exports traces and metrics via OpenTelemetry. Three-layer API:

1. **Low-level:** `otel.ts` — raw OTel SDK wrappers
2. **Bridge:** `otel-bridge.ts` — Crew-specific spans and metrics
...
```bash
# 1. Back up your current crew
cp -r .crew/ .crew-backup/
...
# 4. Reinitialize (this will detect and migrate your .crew/ directory)
crew init
...
4. Was using `@bradygaster/create-crew`
5. Was using `npx github:` distribution
6. `.crew/` directory broke after upgrading
7. Have `.ai-team/` from an older version
8. Using Crew in CI/CD
9. Using Crew SDK programmatically

**Key changes to watch for:**
- Package name: `@bradygaster/create-crew` → `@blacklite/crew-cli`
- Directory: `.ai-team/` → `.crew/` (auto-migrated with `--migrate-directory`)
- Config: JSON → Markdown (`.crew/team.md`, `.crew/decisions.md`)
- Commands: Some were renamed or merged (check `crew help`)

---

## What's Next

Crew v0.8.18 is the foundation. Here's what we're working on:
...
- Multi-repo crew support (one team, many projects)
- Enhanced observability (real-time dashboards, cost alerts)
- CI/CD integrations (GitHub Actions, GitLab CI, CircleCI)
- Team collaboration features (shared crews, sync protocols)

### Long-Term (Next 6 Months)
- Visual crew builder (drag-and-drop agent creation)
- Agent marketplace (community-built agent templates)
- Cloud-hosted crews (no local setup required)
- Enterprise features (SSO, audit logs, compliance hooks)

**Community contributions welcome!** Issues and PRs at [github.com/Blacklite/crew](https://github.com/Blacklite/crew).

---

## Get Started Today

Crew v0.8.18 is live on npm. Install it:
...
- **SDK on npm:** [@blacklite/crew-sdk](https://www.npmjs.com/package/@blacklite/crew-sdk)
- **CLI on npm:** [@blacklite/crew-cli](https://www.npmjs.com/package/@blacklite/crew-cli)
- **Migration Guide:** [docs/get-started/migration.md](../get-started/migration.md)
- **Samples:** [samples/README.md](https://github.com/Blacklite/crew/blob/main/samples/README.md)
...
Welcome to Crew v0.8.18. Let's build better software, together. 🚀

...
1. the Crew coordinator custom agent was selected, but its `crew.agent.md` payload failed to inject entirely; and
2. the running agent is a proven non-Crew agent that should not receive the Crew coordinator payload.
...
The dual canary should be treated as a two-state payload-integrity detector within an independently-known Crew coordinator session:
...
`HEAD absent + EOF absent` is externally unobservable from inside the missing payload. It is not a success state and cannot prove safe non-Crew discrimination.

## Owning runtime surface

Repository inspection shows Crew installs `.github/agents/crew.agent.md` and points users or wrapper processes at `copilot --agent crew`, but this repo does not own the custom-agent resolver or the file-read / prompt-assembly path.

Observed boundary:

- `crew init` installs/advises the custom-agent path and invocation (`packages/crew-cli/src/cli/core/templates.ts:31-36`, `packages/crew-cli/src/cli/core/init.ts:142-155`, `packages/crew-cli/src/cli/core/init.ts:515-518`).
- `crew upgrade` refreshes `.github/agents/crew.agent.md` from the template (`packages/crew-cli/src/cli/core/upgrade.ts:1048`, `packages/crew-cli/src/cli/core/upgrade.ts:1102-1108`, `packages/crew-cli/src/cli/core/upgrade.ts:1124-1137`).
- `packages/crew-cli` passes through `copilotFlags`, `agentCmd`, or `--agent <config.agent>` to the `copilot` process, but does not read/assemble `.github/agents/*.agent.md` into model messages (`packages/crew-cli/src/cli/commands/watch/agent-spawn.ts:95-140`, `packages/crew-cli/src/cli/commands/loop.ts:136-148`, `packages/crew-cli/src/cli/commands/watch/index.ts:608-615`, `packages/crew-cli/src/cli/commands/copilot-bridge.ts:72-84`).
- The SDK wrapper also confirms that the Copilot CLI `--agent` flag is the path that reads on-disk agent definitions (`src/Crew.Agents.AI/CrewAgent.cs:249-282`, `src/Crew.Agents.AI/CrewAgentOptions.cs:87-112`, `src/Crew.Agents.AI/README.md:155-165`).

Therefore the reliable identity beacon must be a GitHub Copilot CLI host capability captured at the **agent-selection step** and persisted in host/session metadata before any `.agent.md` file access. Crew-provided MCP state tools cannot close this gap; if the coordinator payload is wholly absent, no instruction inside that absent payload can tell the model to call any `crew_state_*` identity tool.
...
1. `selected_agent_id`
   ...
   - Carries raw host identity and registry keys, for example:
     - `custom_agent.name = "crew"`
     - `custom_agent.file = ".github/agents/crew.agent.md"`
     - `custom_agent.role = "root-coordinator"`
     - `agent_version = "<resolved Crew agent/template version>"`
...
- `UNREGISTERED`: no manifest entry exists, the version key is unknown, the artifact is user-local, or the Crew artifact is newly updated/unpublished.
...
| `selected_agent_id` absent because a Crew coordinator was independently proven not selected | `SKIP`; safe non-Crew classification is based on host selection state, not canary absence |
...
|---|---|---|---|
| 1. Full payload, registered | Select `--agent crew`; full `crew.agent.md` read and attached; manifest entry exists | `selected_agent_id` set -> `payload_read_status=LOADED`, `source_sha_after_read` -> `prompt_attach_status=ATTACHED_FULL`, `attached_sha` -> `transport_integrity=INTACT` -> `release_integrity=VERIFIED` -> `inventory_drift=NONE` or declared `EXPECTED_VARIANT` | `LOADED` |
| 2. Head-only / oversized-prompt clip | Select `--agent crew`; source may read fully, but assembled prompt loses tail/EOF | ordered events emitted; `prompt_attach_status=ATTACHED_PARTIAL` or missing boundary; `transport_integrity=PARTIAL`; release verdict still recorded if possible | `HALT_CORRUPT` |
| 3. Missing/empty Crew payload | Select `--agent crew`; suppress, empty, or fail reading coordinator payload | `selected_agent_id` set -> `payload_read_status=EMPTY` or `ABSENT` -> `prompt_attach_status=NOT_ATTACHED` -> `transport_integrity=UNKNOWN`; release usually `UNREGISTERED`/unavailable | `UNKNOWN_HALT` |
| 4. Proven non-Crew | Host proves Crew coordinator was not selected | Crew `selected_agent_id` absent; non-Crew/default selection evidence emitted by host | `SKIP` |
...
| 6. LEGITIMATE UNREGISTERED-LOCAL | Healthy user-local custom agent or freshly-updated/unpublished Crew artifact with no manifest entry | `transport_integrity=INTACT`; `release_integrity=UNREGISTERED`; source/attached SHA recorded | `LOADED_WITH_REPORT`; MUST NOT false-halt |
...
## In-Crew vs upstream split

**In Crew's control:** install and upgrade the custom-agent file, pass/advise `--agent crew`, keep canary payload-integrity wording accurate, write/load `.mcp.json`, document Cases 1-4, and consume the future host state machine when exposed.

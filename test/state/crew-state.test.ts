/**
 * CrewState integration tests.
 *
 * Uses InMemoryStorageProvider seeded with sample .crew/ files
 * to verify the full facade: CrewState → Collections → Handles → IO.
 */
...
import { NotFoundError } from '../../packages/crew-sdk/src/state/domain-types.js';
...
## Project Context
Crew SDK — TypeScript monorepo for AI team orchestration.
...
function seedStorage(storage: InMemoryStorageProvider): void {
  storage.writeSync(`${ROOT}/.crew/agents/EECOM/charter.md`, CHARTER_EECOM);
  storage.writeSync(`${ROOT}/.crew/agents/EECOM/history.md`, HISTORY_EECOM);
  storage.writeSync(`${ROOT}/.crew/agents/RETRO/charter.md`, CHARTER_RETRO);
  storage.writeSync(`${ROOT}/.crew/agents/RETRO/history.md`, `# RETRO\n\n## Learnings\n`);
  storage.writeSync(`${ROOT}/.crew/team.md`, TEAM_MD);
  storage.writeSync(`${ROOT}/.crew/decisions.md`, DECISIONS_MD);
  storage.writeSync(`${ROOT}/.crew/routing.md`, ROUTING_MD);
  // Skills
  storage.writeSync(`${ROOT}/.crew/skills/typescript-testing/SKILL.md`, SKILL_TYPESCRIPT_TESTING);
  storage.writeSync(`${ROOT}/.crew/skills/code-review/SKILL.md`, SKILL_CODE_REVIEW);
  // Templates
  storage.writeSync(`${ROOT}/.crew/templates/charter.md`, TEMPLATE_CHARTER);
  storage.writeSync(`${ROOT}/.crew/templates/decision.md`, TEMPLATE_DECISION);
  // Log entries
  storage.writeSync(`${ROOT}/.crew/log/2026-07-24-session.md`, '# Session Log\nSpawned EECOM for feature work.');
  storage.writeSync(`${ROOT}/.crew/log/2026-07-25-review.md`, '# Review Log\nCode review completed.');
}
...
describe('CrewState', () => {
  ...
  beforeEach(async () => {
    ...
    seedStorage(storage);
    state = await CrewState.create(storage, ROOT);
  });

  // ── Factory ────────────────────────────────────────────────────────────

  describe('create()', () => {
    it('succeeds when .crew/ exists', async () => {
      expect(state).toBeInstanceOf(CrewState);
    });

    it('throws NotFoundError when .crew/ is missing', async () => {
      const empty = new InMemoryStorageProvider();
      await expect(CrewState.create(empty, '/empty')).rejects.toThrow(
        NotFoundError,
      );
    });
  });
  ...
  describe('isInitialized()', () => {
    it('returns true when .crew/ exists', async () => {
      expect(await state.isInitialized()).toBe(true);
    });

    it('returns false when .crew/ is missing', async () => {
      const empty = new InMemoryStorageProvider();
      // Bypass create() validation with a direct instantiation trick
      // by testing on a state that was created then had crew/ removed
      await storage.deleteDir(`${ROOT}/.crew`);
  ...
  describe('agents', () => {
    ...
    describe('get().history()', () => {
      ...
      it('returns empty array for agent with no history file', async () => {
        await storage.delete(`${ROOT}/.crew/agents/RETRO/history.md`);
    ...
    describe('get().update()', () => {
      it('updates agent role in team.md', async () => {
        const handle = state.agents.get('EECOM');
        await handle.update({ role: 'Lead Dev' } as Partial<import('../../packages/crew-sdk/src/state/domain-types.js').Agent>);
      ...
      it('throws NotFoundError when agent not in team.md', async () => {
        const handle = state.agents.get('GHOST');
        await expect(
          handle.update({ role: 'Nobody' } as Partial<import('../../packages/crew-sdk/src/state/domain-types.js').Agent>),
  ...
  describe('decisions', () => {
    describe('list()', () => {
      ...
      it('returns empty array when file missing', async () => {
        await storage.delete(`${ROOT}/.crew/decisions.md`);
        const decisions = await state.decisions.list();
    ...
    describe('add()', () => {
      it('appends a new decision', async () => {
        await state.decisions.add({
          ...
          author: 'EECOM',
          body: 'CrewState provides typed access to all collections.',
  ...
  describe('routing', () => {
    describe('get()', () => {
      ...
      it('populates moduleOwnership from Module Ownership section', async () => {
        await storage.write(`${ROOT}/.crew/routing.md`, ROUTING_MD_WITH_OWNERSHIP);
        const fresh = await CrewState.create(storage, ROOT);
      ...
      it('throws NotFoundError when file missing', async () => {
        await storage.delete(`${ROOT}/.crew/routing.md`);
  ...
  describe('team', () => {
    describe('get()', () => {
      ...
      it('populates projectContext from content above Members', async () => {
        await storage.write(`${ROOT}/.crew/team.md`, TEAM_MD_WITH_CONTEXT);
        const fresh = await CrewState.create(storage, ROOT);
        const config = await fresh.team.get();
        expect(config.projectContext).toContain('High-stakes systems under pressure');
        expect(config.projectContext).toContain('Crew SDK');
      });

      it('throws NotFoundError when file missing', async () => {
        await storage.delete(`${ROOT}/.crew/team.md`);
  ...
  describe('skills', () => {
    describe('list()', () => {
      ...
      it('returns empty array when skills directory is missing', async () => {
        const empty = new InMemoryStorageProvider();
        empty.writeSync(`${ROOT}/.crew/team.md`, TEAM_MD);
        const s = await CrewState.create(empty, ROOT);
  ...
  describe('config', () => {
    describe('get()', () => {
      it('returns parsed config when file exists', async () => {
        storage.writeSync(
          `${ROOT}/.crew/config.json`,
          JSON.stringify({ cacheEnabled: true, cacheTtlMs: 60_000 }),
        );
        const s = await CrewState.create(storage, ROOT);

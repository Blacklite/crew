/**
 * CrewState Coverage Gaps — Tests for edge cases and error paths.
 *
...
import {
  ...
  WriteConflictError,
  ProviderError,
} from '../../packages/crew-sdk/src/state/domain-types.js';
import { resolveCollectionPath } from '../../packages/crew-sdk/src/state/schema.js';
import {
  parseDecisions,
  serializeDecision,
  serializeDecisions,
} from '../../packages/crew-sdk/src/state/io/decisions-io.js';
import {
  parseRouting,
  serializeRouting,
} from '../../packages/crew-sdk/src/state/io/routing-io.js';
import {
  parseTeam,
  serializeTeam,
} from '../../packages/crew-sdk/src/state/io/team-io.js';
import { createAgentHandle } from '../../packages/crew-sdk/src/state/handles.js';
import { parseHistory } from '../../packages/crew-sdk/src/state/io/history-io.js';
import { InMemoryStorageProvider } from '../../packages/crew-sdk/src/storage/in-memory-storage-provider.js';
import {
  DecisionsCollection,
  RoutingCollection,
  TeamCollection,
} from '../../packages/crew-sdk/src/state/collections.js';
...
vi.mock('../../packages/crew-sdk/src/state/io/decisions-io.js', async (importOriginal) => {
  const mod = await importOriginal<typeof import('../../packages/crew-sdk/src/state/io/decisions-io.js')>();
  return { ...mod, parseDecisions: vi.fn(mod.parseDecisions) };
});
vi.mock('../../packages/crew-sdk/src/state/io/routing-io.js', async (importOriginal) => {
  const mod = await importOriginal<typeof import('../../packages/crew-sdk/src/state/io/routing-io.js')>();
  return { ...mod, parseRouting: vi.fn(mod.parseRouting) };
});
vi.mock('../../packages/crew-sdk/src/state/io/team-io.js', async (importOriginal) => {
  const mod = await importOriginal<typeof import('../../packages/crew-sdk/src/state/io/team-io.js')>();
  return { ...mod, parseTeam: vi.fn(mod.parseTeam) };
});
vi.mock('../../packages/crew-sdk/src/state/io/history-io.js', async (importOriginal) => {
  const mod = await importOriginal<typeof import('../../packages/crew-sdk/src/state/io/history-io.js')>();
...
describe('StateError Hierarchy', () => {
  describe('StateError base class', () => {
    ...
    it('supports all StateErrorKind values', () => {
      const kinds: Array<import('../../packages/crew-sdk/src/state/domain-types.js').StateErrorKind> = [
        'not-found',
        'parse-error',
        'write-conflict',
        'provider-error',
...
describe('resolveCollectionPath', () => {
  it('resolves static paths without id', () => {
    expect(resolveCollectionPath('decisions')).toBe('.crew/decisions.md');
    expect(resolveCollectionPath('routing')).toBe('.crew/routing.md');
    expect(resolveCollectionPath('team')).toBe('.crew/team.md');
    expect(resolveCollectionPath('log')).toBe('.crew/log');
    expect(resolveCollectionPath('config')).toBe('.crew/config.json');
  });

  it('resolves function paths with id', () => {
    expect(resolveCollectionPath('agents', 'eecom')).toBe('.crew/agents/eecom');
    expect(resolveCollectionPath('skills', 'typescript-testing')).toBe('.crew/skills/typescript-testing');
    expect(resolveCollectionPath('templates', 'charter.md')).toBe('.crew/templates/charter.md');
  });
  ...
  it('handles Unicode ids', () => {
    expect(resolveCollectionPath('agents', '文件')).toBe('.crew/agents/文件');
    expect(resolveCollectionPath('skills', 'résumé-writing')).toBe('.crew/skills/résumé-writing');
  });

  it('handles ids with special characters', () => {
    expect(resolveCollectionPath('agents', 'agent-007')).toBe('.crew/agents/agent-007');
    expect(resolveCollectionPath('templates', 'issue.template.md')).toBe('.crew/templates/issue.template.md');
  });

  it('does not throw when static path called with id', () => {
    // Static paths ignore the id parameter
    expect(resolveCollectionPath('team', 'ignored')).toBe('.crew/team.md');
  });
});
...
describe('IO Round-Trip', () => {
  ...
  describe('team-io', () => {
    ...
    it('handles team metadata', () => {
      ...
      expect(serialized).toContain('# Alpha Crew');
...
describe('createAgentHandle edge cases', () => {
  it('handles agent name with spaces', async () => {
    ...
    const rootDir = '/test';
    storage.writeSync(`${rootDir}/.crew/agents/Agent Smith/charter.md`, '# Agent Smith\nTest');
    storage.writeSync(`${rootDir}/.crew/agents/Agent Smith/history.md`, '# Agent Smith\n');
    storage.writeSync(`${rootDir}/.crew/team.md`, `# Team\n\n## Members\n\n| Name | Role | Charter | Status |\n|------|------|---------|--------|\n| Agent Smith | Dev | \`.crew/agents/Agent Smith/charter.md\` | ✅ Active |\n`);
  ...
  it('handles agent name with Unicode', async () => {
    ...
    const rootDir = '/test';
    const name = '文件管理员';
    storage.writeSync(`${rootDir}/.crew/agents/${name}/charter.md`, `# ${name}\nTest`);
    storage.writeSync(`${rootDir}/.crew/agents/${name}/history.md`, `# ${name}\n`);
    storage.writeSync(`${rootDir}/.crew/team.md`, `# Team\n\n## Members\n\n| Name | Role | Charter | Status |\n|------|------|---------|--------|\n| ${name} | Dev | \`.crew/agents/${name}/charter.md\` | ✅ Active |\n`);
  ...
  it('appendHistory handles empty timestamp', async () => {
    const storage = new InMemoryStorageProvider();
    const rootDir = '/test';
    storage.writeSync(`${rootDir}/.crew/agents/test/charter.md`, '# test');
    storage.writeSync(`${rootDir}/.crew/agents/test/history.md`, '# test\n\n## Learnings\n');
    storage.writeSync(`${rootDir}/.crew/team.md`, `# Team\n\n## Members\n\n| Name | Role | Charter | Status |\n|------|------|---------|--------|\n| test | Dev | \`.crew/agents/test/charter.md\` | ✅ Active |\n`);
...
describe('appendHistory adversarial markdown', () => {
  ...
  const HISTORY = `${ROOT}/.crew/agents/${AGENT}/history.md`;
  const CHARTER = `${ROOT}/.crew/agents/${AGENT}/charter.md`;
  const TEAM = `${ROOT}/.crew/team.md`;
  const TEAM_MD = [
    '# Team', '', '## Members', '',
    '| Name | Role | Charter | Status |',
    '|------|------|---------|--------|',
...
describe('ParseError wrapping at facade boundary', () => {
  ...
  it('DecisionsCollection.list() wraps parser errors with ParseError', async () => {
    const storage = new InMemoryStorageProvider();
    const rootDir = '/test';
    storage.writeSync(`${rootDir}/.crew/decisions.md`, 'malformed content');
  ...
  it('RoutingCollection.get() wraps parser errors with ParseError', async () => {
    const storage = new InMemoryStorageProvider();
    const rootDir = '/test';
    storage.writeSync(`${rootDir}/.crew/routing.md`, 'malformed content');
  ...
  it('TeamCollection.get() wraps parser errors with ParseError', async () => {
    const storage = new InMemoryStorageProvider();
    const rootDir = '/test';
    storage.writeSync(`${rootDir}/.crew/team.md`, 'malformed content');
  ...
  it('AgentHandle.history() wraps parser errors with ParseError', async () => {
    ...
    const rootDir = '/test';
    storage.writeSync(`${rootDir}/.crew/agents/test/charter.md`, '# test');
    storage.writeSync(`${rootDir}/.crew/agents/test/history.md`, 'malformed content');
    const handle = createAgentHandle('test', storage, rootDir);

/**
 * SDK-tier coverage for the documented crew_route contract.
 *
 ...
 *
 *   const registry = new ToolRegistry('./.crew', undefined, undefined, undefined, () => deps);
 ...
 * jagilber/crew#9).
 *
 * @module test/sdk/crew-route
 */

import { describe, it, expect, vi } from 'vitest';
import { ToolRegistry, type RouteRequest } from '@blacklite/crew-sdk/tools';
...
describe('SDK contract: crew_route (documented snippet)', () => {
  const ctx = {
  ...
  it('matches the registry construction example from docs/reference/sdk.md', () => {
    ...
    //   registry.getTool('crew_route');
    const registry = new ToolRegistry('./.crew-sdk-contract');
    const tool = registry.getTool('crew_route');
  ...
  it('sanitizes spawn error messages (no raw crewRoot paths in textResultForLlm)', async () => {
    const crewRoot = './.crew-sdk-contract';
    const deps = buildEmbedderFanOutDeps({
      compileCharter: vi.fn(async () => {
        throw new Error(`ENOENT: ${crewRoot}/agents/ghost/charter.md not found`);
      }),
    });
    const registry = new ToolRegistry(
      crewRoot,
    ...
    expect(result).toMatchObject({ resultType: 'failure' });
    const text = (result as { textResultForLlm: string }).textResultForLlm;
    // sanitizeErrorForLlm replaces crewRoot with [team-root]
    expect(text).not.toContain(crewRoot);
    expect(text).toContain('[team-root]');
  });

  it('lists crew_route in registry.getToolsForAgent (documented filter API)', () => {
    // sdk.md §"ToolRegistry":
    //   registry.getToolsForAgent(['crew_route', 'crew_decide']);
    const registry = new ToolRegistry('./.crew-sdk-contract');
    const filtered = registry.getToolsForAgent(['crew_route', 'crew_decide']);
  ...
  it('does not leak raw errors in structured fields', async () => {
    ...
    const deps = buildEmbedderFanOutDeps({
      compileCharter: vi.fn(async () => {
        throw new Error(`ENOENT: ${crewRoot}/internal/secret.key\nstack trace line 1\nstack trace line 2`);
      }),
    });
    const registry = new ToolRegistry(
      crewRoot,

/**
 * Regression tests for Blacklite/crew#1299 — the coordinator must
 ...
 * casting.
 *
 * After #1308 phase 1 (slim crew.agent.md), the casting "Determine team
 * size" instruction moved from crew.agent.md into the satellite skill
...
const INIT_MODE_SKILL_TARGETS = [
  '.crew/skills/coordinator-init-mode/SKILL.md',
  'packages/crew-cli/templates/skills/coordinator-init-mode/SKILL.md',
  'packages/crew-sdk/templates/skills/coordinator-init-mode/SKILL.md',
];

describe('crew.agent.md.template — always-on roster instructions (#1299)', () => {
  for (const rel of TEMPLATE_TARGETS) {
    describe(rel, () => {
      ...
      it('Init Mode stub points at the coordinator-init-mode satellite skill (#1308)', () => {
        ...
        // satellite skill. crew.agent.md must keep a stub that:
      ...
      it('declares Fact Checker as dual operating mode (Verification + Devil\'s Advocate)', () => {
        ...
        // Blacklite/crew#789 + #1254. The template must reinforce it so
...
describe('coordinator-init-mode/SKILL.md — always-on roster instructions (post-#1308)', () => {
  ...
  // crew.agent.md now lives in this satellite skill. Re-pin the same
...
/**
 * Regression for Blacklite/crew#1454 — the Phase 1 ask_user confirmation

...
public class CrewSubagentTraceTests
{
    ...
    {
        CrewAgentTraceEvent? captured = null;
        var mapper = new CrewSubagentTraceMapper(evt => captured = evt);
        ...
        Assert.Equal(CrewAgentTraceEventKind.SubagentStarted, captured!.Kind);
    ...
    public void Mapper_MapsAssistantMessageEvent_ToTypedEnvelopeWithContent()
    {
        ...
        Assert.NotNull(captured);
        Assert.Equal(CrewAgentTraceEventKind.AssistantMessage, captured!.Kind);
    ...
    {
        var deliveries = new List<CrewAgentTraceEvent>();
        var mapper = new CrewSubagentTraceMapper(deliveries.Add);
    ...
    {
        var mapper = new CrewSubagentTraceMapper(_ => throw new InvalidOperationException("boom"));
    ...
    public void Mapper_EmitsActivityForSubagentStartedAndDisposesOnCompleted()
    {
        ...
        using var mapper = new CrewSubagentTraceMapper(onTrace: null);
        ...
        Assert.Equal("crew.subagent Picard", startedActivities[0].DisplayName);
        Assert.Equal("Picard", startedActivities[0].GetTagItem("crew.subagent.name"));
        Assert.Equal("Captain Picard", startedActivities[0].GetTagItem("crew.subagent.display_name"));
        Assert.Equal("toolu_otel_1", startedActivities[0].GetTagItem("crew.subagent.sdk_agent_id"));
    ...
    public void Mapper_AssistantMessageOnLiveSubagent_TagsActivityWithReplyPreview()
    {
        ...
        Assert.Equal("Captain, the answer is 42.", activity!.GetTagItem("crew.subagent.reply_preview"));
    }
    ...
    public void Mapper_DisposesAllLiveActivities_WhenSessionEndsMidDispatch()
    {
        ...
        var mapper = new CrewSubagentTraceMapper(onTrace: null);
        mapper.OnSessionEvent(new SubagentStartedEvent
        {
    ...
    {
        var events = new List<CrewAgentTraceEvent>();
        var mapper = new CrewSubagentTraceMapper(events.Add);
        ...
        var toolStart = Assert.Single(events, e => e.Kind == CrewAgentTraceEventKind.ToolStart);
        Assert.Equal("toolu_dispatch_1", toolStart.ToolCallId);

        var dispatched = Assert.Single(events, e => e.Kind == CrewAgentTraceEventKind.SubagentDispatched);
    ...
    public void Mapper_TaskDispatch_OpensActivityTaggedWithPersona()
    {
        ...
        Assert.Equal("crew.subagent data", activity.DisplayName);
        Assert.Equal("data", activity.GetTagItem("crew.subagent.persona.name"));
        Assert.Equal("🔧 Data: Review PR #1378 coordinator inline-dispatch gate", activity.GetTagItem("crew.subagent.persona.description"));
        Assert.Equal("general-purpose", activity.GetTagItem("crew.subagent.agent_type"));
        Assert.Equal("toolu_dispatch_2", activity.GetTagItem("crew.subagent.tool_call_id"));
    }
    ...
    public void Mapper_TaskDispatchThenSubagentStarted_AugmentsExistingActivity()
    {
        ...
        Assert.Equal("crew.subagent probe", activity.DisplayName);
        Assert.Equal("probe", activity.GetTagItem("crew.subagent.persona.name"));
        Assert.Equal("general-purpose", activity.GetTagItem("crew.subagent.name"));
        Assert.Equal("General Purpose Agent", activity.GetTagItem("crew.subagent.display_name"));
        Assert.Equal("toolu_sdk_probe_xyz", activity.GetTagItem("crew.subagent.sdk_agent_id"));
        Assert.Equal("toolu_join_1", activity.GetTagItem("crew.subagent.tool_call_id"));
    }
    ...
    public void Mapper_TaskDispatchThenSubagentCompleted_ClosesActivity()
    {
        ...
        Assert.Equal("sentinel", stopped.GetTagItem("crew.subagent.persona.name"));
    }
    ...
    public void Mapper_TaskDispatchThenAssistantMessage_TagsReplyPreviewOnPersonaSpan()
    {
        ...
        Assert.NotNull(activity);
        Assert.Equal("Hardening complete. Three findings filed.", activity!.GetTagItem("crew.subagent.reply_preview"));
    }
    ...
    public void Mapper_NonTaskToolExecutionStart_DoesNotEmitSubagentDispatchedOrActivity()
    {
        ...
        var events = new List<CrewAgentTraceEvent>();
        using var mapper = new CrewSubagentTraceMapper(events.Add);
        ...
        Assert.Single(events, e => e.Kind == CrewAgentTraceEventKind.ToolStart);
        Assert.DoesNotContain(events, e => e.Kind == CrewAgentTraceEventKind.SubagentDispatched);
    ...
    public void Mapper_TaskDispatchWithMissingArguments_StillEmitsEnvelopeAndActivity()
    {
        ...
        var activity = Assert.Single(startedActivities);
        Assert.Equal("crew.subagent toolu_bare_1", activity.DisplayName);
    }
    ...
    {
        // Verify that even when Content is empty, the OTel span gets a `crew.subagent.tool_requests`
        ...
        using var mapper = new CrewSubagentTraceMapper(_ => { });
        ...
        Assert.NotNull(activity);
        var toolEvent = activity!.Events.FirstOrDefault(e => e.Name == "crew.subagent.tool_requests");
        Assert.NotEqual(default, toolEvent);
        var tags = toolEvent.Tags.ToDictionary(t => t.Key, t => t.Value);
        Assert.Equal("view,edit", tags["crew.subagent.tool_requests"]);
        Assert.Equal(2, tags["crew.subagent.tool_requests_count"]);
        // reply_preview should NOT be set (Content was empty).
        Assert.Null(activity.GetTagItem("crew.subagent.reply_preview"));
    }
}

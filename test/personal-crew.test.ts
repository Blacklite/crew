/**
 * Personal Crew Feature Tests
 * 
 * Tests for Ambient Personal Crew implementation (Issue #508):
 * - resolvePersonalCrewDir() with kill switch and directory detection
 * - resolvePersonalAgents() discovery and metadata parsing
 * - mergeSessionCast() deduplication and precedence
 * - ensureCrewPathTriple() path validation for triple-root mode
...
import type { AgentManifest } from '@blacklite/crew-sdk/config';
import { runPersonal } from '@blacklite/crew-cli/commands/personal';

const TEST_ROOT = join(process.cwd(), `.test-personal-${randomBytes(4).toString('hex')}`);

// Helper to create a mock global crew directory
function createMockGlobalDir(): string {
  const mockGlobal = join(TEST_ROOT, 'global-crew');
  mkdirSync(mockGlobal, { recursive: true });
  return mockGlobal;
}

// Helper to create a personal crew directory structure
async function setupPersonalCrew(globalDir: string, withAgents = false): Promise<string> {
...
describe('resolvePersonalCrewDir', () => {
  let originalEnv: string | undefined;
  
  beforeEach(() => {
    if (existsSync(TEST_ROOT)) rmSync(TEST_ROOT, { recursive: true, force: true });
    mkdirSync(TEST_ROOT, { recursive: true });
    originalEnv = process.env['CREW_NO_PERSONAL'];
  ...
  afterEach(() => {
    if (originalEnv !== undefined) {
      process.env['CREW_NO_PERSONAL'] = originalEnv;
    } else {
      delete process.env['CREW_NO_PERSONAL'];
    }
    if (existsSync(TEST_ROOT)) rmSync(TEST_ROOT, { recursive: true, force: true });
  ...
  it('returns null when CREW_NO_PERSONAL is set', () => {
    process.env['CREW_NO_PERSONAL'] = '1';
    expect(resolvePersonalCrewDir()).toBeNull();
  });
  
  it('returns null when CREW_NO_PERSONAL is set to any truthy value', () => {
    process.env['CREW_NO_PERSONAL'] = 'true';
    expect(resolvePersonalCrewDir()).toBeNull();
    
    process.env['CREW_NO_PERSONAL'] = 'yes';
    expect(resolvePersonalCrewDir()).toBeNull();
  });
  
  it('returns null when personal dir does not exist', () => {
    // The actual global dir may not have personal-crew subdir
    const result = resolvePersonalCrewDir();
    // This test assumes the real personal crew dir doesn't exist on the test machine
  ...
  it('returns path when personal dir exists', async () => {
    // Mock resolveGlobalCrewPath to point to our test directory
    const mockGlobal = createMockGlobalDir();
    const personalDir = await setupPersonalCrew(mockGlobal);
    
    // We can't easily mock resolveGlobalCrewPath without module mocking,
    ...
    const expectedPersonalPath = join(globalPath, 'personal-crew');
    
    if (existsSync(expectedPersonalPath)) {
      expect(resolvePersonalCrewDir()).toBe(expectedPersonalPath);
    } else {
      expect(resolvePersonalCrewDir()).toBeNull();
    }
  });
...
describe('resolvePersonalAgents', () => {
  ...
  beforeEach(async () => {
    ...
    // Create a mock personal crew directory in our test root
    mockGlobalDir = createMockGlobalDir();
    personalDir = join(mockGlobalDir, 'personal-crew');
  });
  ...
  it('discovers agents with charter.md files', async () => {
    await setupPersonalCrew(mockGlobalDir, true);
...
describe('ensureCrewPathTriple', () => {
  const projectDir = join(TEST_ROOT, '.crew');
  const teamDir = join(TEST_ROOT, 'team-crew');
  const personalDir = join(TEST_ROOT, 'personal-crew');
  ...
  it('allows paths inside projectDir', () => {
    ...
    expect(ensureCrewPathTriple(p, projectDir, teamDir, personalDir)).toBe(p);
  });
  ...
  it('allows paths inside system temp dir', () => {
    const tmp = tmpdir();
    const p = join(tmp, 'crew-temp-123.txt');
    expect(ensureCrewPathTriple(p, projectDir, teamDir, personalDir)).toBe(p);
  });
  ...
  it('throws for paths outside all allowed directories', () => {
    ...
    expect(() => ensureCrewPathTriple(evil, projectDir, teamDir, personalDir))
  ...
  it('handles null personalDir gracefully', () => {
    ...
    expect(ensureCrewPathTriple(insideProject, projectDir, teamDir, null)).toBe(insideProject);
    expect(ensureCrewPathTriple(insideTeam, projectDir, teamDir, null)).toBe(insideTeam);
    
    // Should reject outside paths
    expect(() => ensureCrewPathTriple(outside, projectDir, teamDir, null))
      .toThrow(/outside all allowed directories/);
  });
  ...
  it('prevents path traversal with ../', () => {
    ...
    expect(() => ensureCrewPathTriple(traversal, projectDir, teamDir, personalDir))
      .toThrow(/outside all allowed directories/);
  });
});
...
describe('Ghost Protocol', () => {
  ...
  it('personal agents are filtered out when CREW_NO_PERSONAL is set', async () => {
    process.env['CREW_NO_PERSONAL'] = '1';
...
describe('CLI personal commands', () => {
  ...
  it('personal init creates directory structure', async () => {
    ...
    const personalDir = join(testGlobalDir, 'personal-crew');
    const agentsDir = join(personalDir, 'agents');

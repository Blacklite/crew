/**
 * Tests for crew init prompt storage.
 * Verifies that `crew init "prompt"` stores the prompt for REPL auto-casting.
 */
...
import { runInit } from '../packages/crew-cli/src/cli/core/init.js';

describe('crew init with prompt', () => {
  let testDir: string;

  beforeEach(() => {
    testDir = join(tmpdir(), `crew-init-prompt-test-${Date.now()}`);
  ...
  it('stores .init-prompt when prompt option is provided', async () => {
    ...
    const promptFile = join(testDir, '.crew', '.init-prompt');

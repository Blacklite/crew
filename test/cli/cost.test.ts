...
const ORCH_DIR = join(TEAM_ROOT, '.crew', 'orchestration-log');
const LOG_DIR = join(TEAM_ROOT, '.crew', 'log');
...
describe('CLI: cost command', () => {
  ...
  it('module exports runCost function', async () => {
    const mod = await import('../../packages/crew-cli/src/cli/commands/cost.ts');
    expect(typeof mod.runCost).toBe('function');
  });

  it('aggregates all orchestration log usage with --all', async () => {
    const { runCost } = await import('../../packages/crew-cli/src/cli/commands/cost.ts');
  ...
  it('prints the no-data message when orchestration log directory is missing', async () => {
    const { runCost } = await import('../../packages/crew-cli/src/cli/commands/cost.ts');
    rmSync(join(TEAM_ROOT, '.crew'), { recursive: true, force: true });

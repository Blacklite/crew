 ...
 *
 * Tests module exports and error handling for missing crew directory.
 */
...
describe('CLI: copilot command', () => {
  ...
  it('module exports runCopilot function', async () => {
    const mod = await import('@blacklite/crew-cli/commands/copilot');
  ...
  it('throws when no crew directory exists', async () => {
    const { runCopilot } = await import('@blacklite/crew-cli/commands/copilot');
    mkdirSync(TEST_ROOT, { recursive: true });

    await expect(runCopilot(TEST_ROOT, {})).rejects.toThrow(/crew/i);
  });

  it('handles --off flag when copilot is not on team', async () => {
    const { runCopilot } = await import('@blacklite/crew-cli/commands/copilot');
    mkdirSync(join(TEST_ROOT, '.crew'), { recursive: true });
    writeFileSync(join(TEST_ROOT, '.crew', 'team.md'), '# Team\n\n## Members\n');
  ...
  it('adds copilot section to team.md', async () => {
    ...
    const content = readFileSync(join(TEST_ROOT, '.crew', 'team.md'), 'utf-8');
...
describe('CLI: copilot with externalized state (#1397)', () => {
  ...
  it('adds copilot to the external team.md, not a local copy', async () => {
    ...
    // The marker-only local .crew/ must not grow a team.md
    expect(existsSync(join(TEST_ROOT, '.crew', 'team.md'))).toBe(false);
  });

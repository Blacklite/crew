...
describe('CLI: extract command', () => {
  ...
  it('module exports runExtract function', async () => {
    const mod = await import('@blacklite/crew-cli/commands/extract');
  ...
  it('throws when no .crew/config.json exists', async () => {
    const { runExtract } = await import('@blacklite/crew-cli/commands/extract');
  ...
  it('throws when config.json is invalid JSON', async () => {
    ...
    writeFileSync(join(TEST_ROOT, '.crew', 'config.json'), '{broken json!!!');

...
describe('CLI: link command', () => {
  beforeEach(() => {
    ...
    mkdirSync(join(TEAM_DIR, '.crew'), { recursive: true });
  });
  ...
  it('module exports runLink function', async () => {
    const mod = await import('@blacklite/crew-cli/commands/link');
    expect(typeof mod.runLink).toBe('function');
  });

  it('creates .crew/config.json with relative teamRoot', async () => {
    const { runLink } = await import('@blacklite/crew-cli/commands/link');
  ...
  it('creates .crew directory if it does not exist', async () => {
    const { runLink } = await import('@blacklite/crew-cli/commands/link');
  ...
  it('throws when target has no .crew/ or .ai-team/ directory', async () => {
    const { runLink } = await import('@blacklite/crew-cli/commands/link');

...
describe('OTel Metrics — Token Usage (#261)', () => {
  it('recordTokenUsage calls all four counters with correct attributes', () => {
    ...
    const input = getInstrument('crew.tokens.input');
    const output = getInstrument('crew.tokens.output');
    const cost = getInstrument('crew.tokens.cost');
    const total = getInstrument('crew.tokens.total');
  ...
  it('creates metric instruments with correct names and descriptions', () => {
    ...
    expect(spyMeter.createCounter).toHaveBeenCalledWith('crew.tokens.input', expect.objectContaining({ unit: 'tokens' }));
    expect(spyMeter.createCounter).toHaveBeenCalledWith('crew.tokens.output', expect.objectContaining({ unit: 'tokens' }));
    expect(spyMeter.createCounter).toHaveBeenCalledWith('crew.tokens.cost', expect.objectContaining({ unit: 'USD' }));
    expect(spyMeter.createUpDownCounter).toHaveBeenCalledWith('crew.tokens.total', expect.objectContaining({ unit: 'tokens' }));
  });
});
...
describe('OTel Metrics — Agent Performance (#262)', () => {
  it('recordAgentSpawn increments spawn counter and active gauge', () => {
    ...
    const spawns = getInstrument('crew.agent.spawns');
    const active = getInstrument('crew.agent.active');
  ...
  it('recordAgentDuration records histogram on success', () => {
    ...
    const duration = getInstrument('crew.agent.duration');
  ...
  it('recordAgentDuration increments error counter on error status', () => {
    ...
    const duration = getInstrument('crew.agent.duration');
    const errors = getInstrument('crew.agent.errors');
  ...
  it('creates agent instruments with correct metric names', () => {
    ...
    expect(spyMeter.createCounter).toHaveBeenCalledWith('crew.agent.spawns', expect.any(Object));
    expect(spyMeter.createHistogram).toHaveBeenCalledWith('crew.agent.duration', expect.objectContaining({ unit: 'ms' }));
    expect(spyMeter.createCounter).toHaveBeenCalledWith('crew.agent.errors', expect.any(Object));
    expect(spyMeter.createUpDownCounter).toHaveBeenCalledWith('crew.agent.active', expect.any(Object));
  });
});
...
describe('OTel Metrics — Session Pool (#263)', () => {
  it('recordSessionCreated increments created counter and active gauge', () => {
    ...
    const created = getInstrument('crew.sessions.created');
    const active = getInstrument('crew.sessions.active');
  ...
  it('recordSessionIdle decrements active and increments idle', () => {
    ...
    const active = getInstrument('crew.sessions.active');
    const idle = getInstrument('crew.sessions.idle');
  ...
  it('recordSessionClosed decrements active and increments closed', () => {
    ...
    const active = getInstrument('crew.sessions.active');
    const closed = getInstrument('crew.sessions.closed');
  ...
  it('recordSessionError increments error counter', () => {
    ...
    const errors = getInstrument('crew.sessions.errors');
  ...
  it('creates session instruments with correct metric names', () => {
    ...
    expect(spyMeter.createUpDownCounter).toHaveBeenCalledWith('crew.sessions.active', expect.any(Object));
    expect(spyMeter.createUpDownCounter).toHaveBeenCalledWith('crew.sessions.idle', expect.any(Object));
    expect(spyMeter.createCounter).toHaveBeenCalledWith('crew.sessions.created', expect.any(Object));
    expect(spyMeter.createCounter).toHaveBeenCalledWith('crew.sessions.closed', expect.any(Object));
    expect(spyMeter.createCounter).toHaveBeenCalledWith('crew.sessions.errors', expect.any(Object));
  });
});
...
describe('OTel Metrics — Response Latency (#264)', () => {
  it('recordTimeToFirstToken records histogram value', () => {
    ...
    const ttft = getInstrument('crew.response.ttft');
  ...
  it('recordResponseDuration records histogram value', () => {
    ...
    const duration = getInstrument('crew.response.duration');
  ...
  it('recordTokensPerSecond records gauge value', () => {
    ...
    const tps = getInstrument('crew.response.tokens_per_second');
  ...
  it('creates latency instruments with correct names and units', () => {
    ...
    expect(spyMeter.createHistogram).toHaveBeenCalledWith('crew.response.ttft', expect.objectContaining({ unit: 'ms' }));
    expect(spyMeter.createHistogram).toHaveBeenCalledWith('crew.response.duration', expect.objectContaining({ unit: 'ms' }));
    expect(spyMeter.createGauge).toHaveBeenCalledWith('crew.response.tokens_per_second', expect.objectContaining({ unit: 'tokens/s' }));
  });
});
...
describe('OTel Metrics — _resetMetrics()', () => {
  it('clears all cached metric instances', () => {
    ...
    expect(spyMeter._instruments.has('crew.tokens.input')).toBe(true);
  });
  ...
  it('each category is independently reset', () => {
    ...
    expect(spyMeter._instruments.has('crew.agent.spawns')).toBe(true);
    expect(spyMeter._instruments.has('crew.tokens.input')).toBe(false);
  });
});

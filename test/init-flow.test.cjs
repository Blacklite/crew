...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'crew-init-flow-test-'));
}
...
// Read the generated crew.agent.md from an initialized temp dir
function readGeneratedAgent(dir) {
  const agentPath = path.join(dir, '.github', 'agents', 'crew.agent.md');
  assert.ok(fs.existsSync(agentPath), 'crew.agent.md should exist after init');
...
// to the source repo template at packages/crew-sdk/templates/skills/<name>/
// SKILL.md so the test passes regardless of which init entry the test runner
// uses (legacy index.cjs scaffold or modern packages/crew-cli/dist/cli-entry.js).
...
function readSatelliteSkill(dir, skillName) {
  ...
  const sourcePath = path.join(
    __dirname, '..',
    'packages', 'crew-sdk', 'templates', 'skills',
    skillName, 'SKILL.md'
  );
...
// Extract ALL Init Mode sections from crew.agent.md (handles two-phase split)
//
// After #1308/#1311 the Init Mode prose was moved to the satellite skill
// `coordinator-init-mode/SKILL.md` and crew.agent.md only retains a short
...
function extractInitMode(content, dir) {
  ...
  assert.ok(phase1Match, 'crew.agent.md should contain an Init Mode heading');
  ...
  // prompt — not just the pointer in crew.agent.md.
...
describe('Init Mode prompt structure (#66)', () => {
  ...
  describe('init creates crew.agent.md with Init Mode section', () => {
    it('crew.agent.md is generated by init', () => {
      initCrew(tmpDir);
    ...
    it('generated file contains Init Mode section', () => {
      ...
      assert.ok(
        ...
        'crew.agent.md should contain an "## Init Mode" section'
      );
    });
  });
  ...
  describe('Init Mode step 5 exists and contains confirmation question', () => {
    ...
    it('confirmation step is numbered before file creation step', () => {
      ...
      // Find the step number that contains creating .crew/
      // Match lines like "6. Create the `.crew/` directory structure"
      const createMatch = initMode.match(/^(\d+)\.\s.*(?:create|Create).*\.crew/m);
  ...
  describe('no tool calls between propose and confirm', () => {
    it('prompt does not instruct creating files in the same step as proposing', () => {
      ...
      const hasFileCreation =
        /create.*\.crew/i.test(betweenProposeAndConfirm) ||
  ...
  describe('ask_user referenced in Init Mode for confirmation', () => {
    it('Init Mode instructs coordinator to use ask_user for confirmation', () => {
      ...
      assert.ok(
        ...
        'explicitly waits for user input before creating .crew/ files'
      );
    });
  });
});
...
describe('Identity layer files (now.md, wisdom.md) — #107', () => {
  ...
  it('init creates .crew/identity/now.md', () => {
    initCrew(tmpDir);
    const nowPath = path.join(tmpDir, '.crew', 'identity', 'now.md');
    assert.ok(fs.existsSync(nowPath), '.crew/identity/now.md should exist after init');
  ...
  it('init creates .crew/identity/wisdom.md', () => {
    initCrew(tmpDir);
    const wisdomPath = path.join(tmpDir, '.crew', 'identity', 'wisdom.md');
    assert.ok(fs.existsSync(wisdomPath), '.crew/identity/wisdom.md should exist after init');
  ...
  it('upgrade creates missing identity files', () => {
    ...
    // Modify crew.agent.md to simulate an old version
    const agentPath = path.join(tmpDir, '.github', 'agents', 'crew.agent.md');
    ...
    const identityDir = path.join(tmpDir, '.crew', 'identity');

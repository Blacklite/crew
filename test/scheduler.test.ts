...
import {
  ...
  validateTaskRef,
} from '../packages/crew-sdk/src/runtime/scheduler.js';
import type {
  ScheduleManifest,
  ScheduleEntry,
  ScheduleState,
...
describe('Scheduler: GitHubActionsProvider', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'crew-test-'));
  });
  ...
  it('should generate workflow YAML for cron triggers', async () => {
    ...
    expect(files[0]).toContain('crew-schedule-daily-check.yml');

    const content = fs.readFileSync(files[0]!, 'utf8');
    expect(content).toContain("cron: '0 9 * * *'");
    expect(content).toContain('Crew: Test Task');
    expect(content).toContain('Auto-generated by Crew Scheduler');
  });
...
describe('Scheduler: State Persistence', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'crew-state-'));
...
describe('Scheduler: parseSchedule', () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'crew-parse-'));
  });

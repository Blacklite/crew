 ...
 * error count, and session count. Verifies opt-in gating via CREW_TELEMETRY=1.
 *
...
import {
  ...
  _resetShellMetrics,
} from '@blacklite/crew-cli/shell/shell-metrics';
...
beforeEach(() => {
  ...
  _resetShellMetrics();
  originalEnv = process.env['CREW_TELEMETRY'];
});

afterEach(() => {
  if (originalEnv === undefined) {
    delete process.env['CREW_TELEMETRY'];
  } else {
    process.env['CREW_TELEMETRY'] = originalEnv;
  }
});

// =============================================================================
// Opt-in gating — CREW_TELEMETRY=1
// =============================================================================

describe('Shell Metrics — Opt-in Gate', () => {
  it('isShellTelemetryEnabled returns false when CREW_TELEMETRY is not set', () => {
    delete process.env['CREW_TELEMETRY'];
    expect(isShellTelemetryEnabled()).toBe(false);
  });

  it('isShellTelemetryEnabled returns true when CREW_TELEMETRY=1', () => {
    process.env['CREW_TELEMETRY'] = '1';
    expect(isShellTelemetryEnabled()).toBe(true);
  });

  it('isShellTelemetryEnabled returns false for other values', () => {
    process.env['CREW_TELEMETRY'] = 'true';
    expect(isShellTelemetryEnabled()).toBe(false);
    process.env['CREW_TELEMETRY'] = '0';
    expect(isShellTelemetryEnabled()).toBe(false);
  });
  ...
  it('enableShellMetrics returns true when CREW_TELEMETRY=1', () => {
    process.env['CREW_TELEMETRY'] = '1';
    const result = enableShellMetrics();
...
describe('Shell Metrics — Session Lifetime (#508, #520)', () => {
  ...
  it('enableShellMetrics increments session_count', () => {
    const counter = getInstrument('crew.shell.session_count');
  ...
  it('recordShellSessionDuration records to the histogram', () => {
    ...
    const hist = getInstrument('crew.shell.session_duration_ms');
  ...
  it('session_duration_ms histogram has correct config', () => {
    expect(spyMeter.createHistogram).toHaveBeenCalledWith(
      'crew.shell.session_duration_ms',
...
describe('Shell Metrics — Agent Response Latency (#508, #526)', () => {
  ...
  it('recordAgentResponseLatency records with agent name and dispatch type', () => {
    ...
    const hist = getInstrument('crew.shell.agent_response_latency_ms');
...
describe('Shell Metrics — Error Rate (#530)', () => {
  ...
  it('recordShellError increments error_count with source', () => {
    ...
    const counter = getInstrument('crew.shell.error_count');
    expect(counter.add).toHaveBeenCalledWith(1, {

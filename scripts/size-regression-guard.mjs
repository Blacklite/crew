...
const ARTIFACT_PATH = '.github/agents/crew.agent.md';
const TEMPLATE_PATH = '.crew-templates/crew.agent.md';
const BUDGET_PATH = '.github/size-budget.json';
const REPORT_PATH = 'size-guard-report.json';
const HEAD_CANARY = 'CREW_COORDINATOR_CANARY_HEAD_b7d2';
const EOF_CANARY = 'CREW_COORDINATOR_CANARY_a8f3';
...
function measureAlwaysLoadedSkillFrontmatter() {
  const skillFiles = [...findSkillFiles('.copilot/skills'), ...findSkillFiles('.crew/skills')];
  const alwaysOn = [];
...
function measureCrewStateToolSchema() {
  const candidates = [
    'packages/crew-cli/src/cli/commands/state-mcp.ts',
    'packages/crew-cli/dist/cli/commands/state-mcp.js',
  ];
  ...
  return {
    ...
    note: located
      ? 'crew_state MCP schemas are assembled from the runtime ToolRegistry; static schema JSON was not locally extractable without importing project packages.'
      : 'No crew_state MCP source/schema file was located.',
  };
}

function computeD2(artifactBytes) {
  const skillFrontmatter = measureAlwaysLoadedSkillFrontmatter();
  const crewStateToolSchema = measureCrewStateToolSchema();
  const measuredAdditions = [skillFrontmatter, crewStateToolSchema]
  ...
  return {
    ...
    components: {
      ...
      crew_state_tool_schema: crewStateToolSchema,
    },
  };
}
...
function findSlimAnchor() {
  try {
    ...
    const match = log.split('\n').find(line => /1308|slim crew\.agent/i.test(line));
    if (match) {
  ...
  for (const sha of allCommits) {
    if (gitShowText(sha, [ARTIFACT_PATH, TEMPLATE_PATH])) {
      return { ref: sha, label: 'slim-era anchor fallback', note: 'Could not locate #1308; earliest reachable crew.agent.md used.' };
    }
  }
  return undefined;
}
...
function backtestReport() {
  ...
  else refs.push({ ref: null, label: '#1308 slim commit', note: 'unavailable: no reachable crew.agent.md history found' });
...
function renderCurrent(report) {
  ...
  return [
    ...
    `D2 local estimate only: ${d2.total_bytes} bytes; skill frontmatter=${d2.components.always_loaded_skill_frontmatter.status}; crew_state schema=${d2.components.crew_state_tool_schema.status}`,

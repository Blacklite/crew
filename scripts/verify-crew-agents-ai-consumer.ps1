...
<#
.SYNOPSIS
  Hermetic consumer-level packaging check for Crew.Agents.AI.

.DESCRIPTION
  Restores a scratch consumer project against ONLY the locally-produced Crew.Agents.AI
  ...
  package itself was built/floor-pinned against) instead of the version Crew.Agents.AI
  actually references. See Directory.Build.props and the GenerateCopilotSdkVersionBridge
  target in src/Crew.Agents.AI/Crew.Agents.AI.csproj for the fix; this script is the
  ...
  folder produced by `dotnet pack` in this same repo checkout). The Crew.Agents.AI
...
.PARAMETER NupkgsDir
  Directory containing exactly one packed Crew.Agents.AI .nupkg (e.g. the CI job's
...
.PARAMETER ExpectedCopilotCliVersion
  ...
  the GitHub.Copilot.SDK version Crew.Agents.AI references), e.g. "1.0.71". Must match
  the same narrow version grammar validated in crew-agents-ai-ci.yml; this script
...
# Same grammar as the version-validation guard in crew-agents-ai-ci.yml (kept in sync by
...
$candidateNupkgs = @(Get-ChildItem -Path $nupkgsFull -Filter "crew.agents.ai.*.nupkg")
if ($candidateNupkgs.Count -eq 0) {
    throw "No 'crew.agents.ai.*.nupkg' found in '$nupkgsFull'. Did the Pack step run before this check?"
}
if ($candidateNupkgs.Count -gt 1) {
    throw "Expected exactly one packed Crew.Agents.AI nupkg in '$nupkgsFull', found $($candidateNupkgs.Count): $($candidateNupkgs.Name -join ', ')"
}

$packedNupkgPath = $candidateNupkgs[0].FullName
$nupkgMatch = [System.Text.RegularExpressions.Regex]::Match($candidateNupkgs[0].Name, '^crew\.agents\.ai\.(?<version>.+)\.nupkg$', [System.Text.RegularExpressions.RegexOptions]::IgnoreCase)
...
Write-Host "Detected packed Crew.Agents.AI version: $PackageVersion (from $($candidateNupkgs[0].Name))"

$packedNupkgHash = Get-Sha512Base64 -FilePath $packedNupkgPath
Write-Host "Packed nupkg SHA512 (base64): $packedNupkgHash"

$scratchDir = Join-Path ([System.IO.Path]::GetTempPath()) "crew-agents-ai-consumer-$([guid]::NewGuid())"
...
try {
    ...
    $csprojRoot = [System.Xml.Linq.XElement]::new(
        ...
        [System.Xml.Linq.XElement]::new(
            ...
            [System.Xml.Linq.XElement]::new(
                ...
                [System.Xml.Linq.XAttribute]::new($xns::op_Implicit('Include'), 'Crew.Agents.AI'),
    ...
    $programContent = 'System.Console.WriteLine("Crew.Agents.AI consumer packaging check placeholder.");'
    ...
    # Mapping pins Crew.Agents.AI to ONLY the local source (an identically named/versioned
    # package could otherwise exist, now or in future, on nuget.org and be silently
    # substituted); everything else (Crew.Agents.AI's transitive dependencies, whatever
    ...
    $packageSources = [System.Xml.Linq.XElement]::new(
        ...
        [System.Xml.Linq.XElement]::new(
            $xns::op_Implicit('add'),
            [System.Xml.Linq.XAttribute]::new($xns::op_Implicit('key'), 'local-crew-agents-ai'),
    ...
    $packageSourceMapping = [System.Xml.Linq.XElement]::new(
        ...
        [System.Xml.Linq.XElement]::new(
            ...
            [System.Xml.Linq.XElement]::new(
                ...
                [System.Xml.Linq.XAttribute]::new($xns::op_Implicit('pattern'), 'Crew.Agents.AI')
            )
        ),
        [System.Xml.Linq.XElement]::new(
            $xns::op_Implicit('packageSource'),
    ...
    Write-Section "Proving Crew.Agents.AI was restored from the freshly packed local nupkg"
    # Package Source Mapping above already restricts restore of Crew.Agents.AI to the
    ...
    $restoredHashPath = Join-Path $nugetPackagesRoot "crew.agents.ai/$($PackageVersion.ToLowerInvariant())/crew.agents.ai.$($PackageVersion.ToLowerInvariant()).nupkg.sha512"
    if (-not (Test-Path $restoredHashPath)) {
        throw "Restored Crew.Agents.AI package hash file not found at '$restoredHashPath'. Cannot verify restore came from the local nupkg."
    }
    $restoredHash = (Get-Content -Raw -Path $restoredHashPath).Trim()
    if ($restoredHash -ne $packedNupkgHash) {
        throw "Restored Crew.Agents.AI package SHA512 ('$restoredHash') does not match the freshly packed local nupkg ('$packedNupkgHash'). This would mean the consumer resolved a DIFFERENT Crew.Agents.AI package than the one this job just built, defeating the purpose of this check."
    }
    Write-Host "Restored Crew.Agents.AI SHA512 matches the freshly packed local nupkg exactly."
    ...
    if (-not $nativeRoot) {
        throw "No 'runtimes' directory found in consumer build output '$outDir'. The Copilot CLI native payload did not propagate to a real PackageReference consumer of Crew.Agents.AI."
    }
    ...
    foreach ($forbidden in $ForbiddenCopilotCliVersions) {
        if ($downloadedVersions -contains $forbidden) {
            throw "Forbidden stale Copilot CLI version '$forbidden' was downloaded for the consumer. This is the exact regression (Crew.Agents.AI PR #1519 / Reviewer finding #1) this check exists to catch: the Microsoft.Agents.AI.GitHub.Copilot buildTransitive bridge resolved an older GitHub.Copilot.SDK version instead of the one Crew.Agents.AI references."
        }
    }

    Write-Host ""
    Write-Host "PASS: consumer resolved Copilot CLI $ExpectedCopilotCliVersion (not $($ForbiddenCopilotCliVersions -join ', ')); native executable and runtime library both present under runtimes/*/native/; Crew.Agents.AI restored from the freshly packed local nupkg (SHA512 verified)." -ForegroundColor Green
}
finally {

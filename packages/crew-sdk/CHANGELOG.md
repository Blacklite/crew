...
- 1446050: ADO configurable work item types, area paths, and iteration support (#240)
  ...
  - Enhance `crew init` to introspect available work item types and store `_availableTypes` hint in `.crew/config.json`
...
- dafc495: Add dual-mode deployment support for capabilities routing.
  ...
  - `CREW_POD_ID` env var for pod-specific capability manifests
  - `CREW_DEPLOYMENT_MODE` env var (`agent-per-node` | `crew-per-pod`)
  - Pod-specific manifest loading: `.crew/machine-capabilities-{podId}.json`
...
- 5996db4: Add `.crew/.scratch/` directory for organized temp file management (#790)

  - `scratchDir()` — resolve and optionally create the scratch directory
  - `scratchFile()` — create named temp files inside `.scratch/`
  - Init scaffolds `.crew/.scratch/` and adds it to `.gitignore`
...
- e0898ad: fix(sdk): wire `crew_route` tool handler to actually create target sessions

  The `crew_route` tool is documented in `packages/crew-sdk/README.md`,
  ...
  (`toolRegistry.getTool('crew_route').handler({ ... })`) received a
  ...
  - When `fanOutDepsGetter` returns dependencies, `crew_route` calls
  ...
  (b) intercept `crew_route` via `CrewSessionHooks.onPreToolUse`.
...
- d8c1382: Sync crew.agent.md roster when agents added to team.md
...
- eb17e15: Fix crew-commands skill routing and configuration for Copilot integration

  - **templates.ts**: Correct destination path for crew-commands SKILL.md from `skills/crew-commands/SKILL.md` to `../.copilot/skills/crew-commands/SKILL.md` to match other built-in skills
  - **init.ts**: Add `'crew-commands'` to MANIFEST_SKILL_NAMES array to include the skill in project initialization
  - **crew-commands SKILL.md**: Convert triggers from YAML list to inline array format and remove generic triggers ("help", "how do I") that conflict with other skills; retain: "crew commands", "what can crew do", "show me crew options", "slash commands"
  - **Template sync**: Regenerate all mirrors via sync-templates.mjs to propagate routing updates to .copilot and .crew templates

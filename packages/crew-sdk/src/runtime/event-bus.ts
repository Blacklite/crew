...
/**
 * Core lifecycle events emitted by Crew sessions.
 * All lifecycle events are persistent and logged for debugging.
 */
export type CrewLifecycleEvent =
  | 'session:created'
  | 'session:idle'
  | 'session:error'
  | 'session:destroyed';
 ...
 */
export type CrewOperationalEvent =
  | 'session:message'
  | 'session:tool_call'
  | 'agent:milestone'
  | 'coordinator:routing'
 ...
 */
export type CrewEventType = CrewLifecycleEvent | CrewOperationalEvent;
 ...
 */
export type EventHandler = (event: CrewEvent) => void | Promise<void>;
...
export class EventBus {
  private handlers: Map<CrewEventType, Set<EventHandler>> = new Map();
  private allHandlers: Set<EventHandler> = new Set();
  private errorHandlers: Set<(error: Error, event: CrewEvent) => void> = new Set();
   ...
   * ```
   */
  subscribe(type: CrewEventType, handler: EventHandler): UnsubscribeFn {
   ...
   */
  unsubscribe(type: CrewEventType, handler: EventHandler): void {
   ...
   */
  onError(handler: (error: Error, event: CrewEvent) => void): UnsubscribeFn {
   ...
   * ```
   */
  async emit(event: CrewEvent): Promise<void> {
   ...
   */
  private async executeHandler(handler: EventHandler, event: CrewEvent): Promise<void> {
    try {
   ...
   */
  getHandlerCount(type: CrewEventType): number {

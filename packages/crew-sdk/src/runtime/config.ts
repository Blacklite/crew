 ...
 * 
 * Loads and validates crew.config.ts or crew.config.json configuration files.
...
/**
 * Root Crew configuration object.
 */
...
/**
 * Default Crew configuration with typed defaults.
 */
...
/**
 * Discovers Crew configuration file by walking up directory tree.
 * 
...
/**
 * Loads Crew configuration from crew.config.ts, crew.config.js, crew.config.json, or .crew/config.json.
 * 
 * Search order:
 * 1. crew.config.ts (ESM import)
 * 2. crew.config.js (ESM import)
 * 3. crew.config.json (JSON parse)
 * 4. .crew/config.json (JSON parse)
...
export async function loadConfig(cwd: string = process.cwd()): Promise<ConfigLoadResult> {
  ...
  const localConfigs = [
    { path: join(resolvedCwd, 'crew.config.ts'), type: 'ts' as const },
    { path: join(resolvedCwd, 'crew.config.js'), type: 'js' as const },
    { path: join(resolvedCwd, 'crew.config.json'), type: 'json' as const },
    { path: join(resolvedCwd, '.crew', 'config.json'), type: 'json' as const }
  ];
    ...
    if (storage.existsSync(configPath)) {
      try {
        if (type === 'ts' || type === 'js') {
          ...
          const config = module.default || module.crewConfig;
          
          if (!config) {
            throw new Error(`${configPath} must export default or named export "crewConfig"`);
          }
  ...
  if (discoveredPath) {
    try {
      if (discoveredPath.endsWith('.ts') || discoveredPath.endsWith('.js')) {
        ...
        if (!config) {
          throw new Error(`${discoveredPath} must export default or named export "crewConfig"`);
        }
...
/**
 * Validates Crew configuration schema and returns detailed result.
 * 
...
export function validateConfigDetailed(config: unknown): ValidationResult {
  ...
  const cfg = config as Partial<CrewConfig>;
...
/**
 * Validates Crew configuration schema.
 * 
 * @param config - Configuration object to validate
...
/**
 * Synchronously loads Crew configuration.
 * Only supports crew.config.json (TypeScript config requires async import).
 * 
 * @param cwd - Working directory to search for config files
 * @returns Configuration load result with validation
 * @throws {ConfigValidationError} If config validation fails
...
export function loadConfigSync(cwd: string = process.cwd()): ConfigLoadResult {
  ...
  // Only check crew.config.json (sync loading of .ts not supported)
  const jsonConfigPath = join(resolvedCwd, 'crew.config.json');
  if (storage.existsSync(jsonConfigPath)) {
    try {
      ...
      throw new Error(`Failed to load crew.config.json: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

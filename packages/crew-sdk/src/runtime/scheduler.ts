/**
 * Scheduler — Generic, provider-agnostic scheduler for Crew (#296)
 *
 ...
 * `.crew/schedule.json` configuration file.
 *
 * Provider model:
...
export class GitHubActionsProvider implements ScheduleProvider {
  ...
  async generate(manifest: ScheduleManifest, outDir: string): Promise<string[]> {
    ...
    for (const entry of manifest.schedules) {
      ...
      const workflowName = `crew-schedule-${entry.id}.yml`;
      const workflowPath = path.join(outDir, workflowName);

      const yaml = [
        `# Auto-generated by Crew Scheduler from schedule.json`,
...
/**
 * Default schedule.json template for `crew schedule init`.
 */
    ...
    schedules: [
      {
        ...
        task: { type: 'workflow', ref: '.github/workflows/crew-heartbeat.yml' },

...
function ensureTokenMetrics(): TokenMetrics {
  if (!_tokenMetrics) {
    const meter = getMeter('crew-sdk');
    _tokenMetrics = {
      inputCounter: meter.createCounter('crew.tokens.input', {
        description: 'Total input tokens consumed',
        unit: 'tokens',
      }),
      outputCounter: meter.createCounter('crew.tokens.output', {
        description: 'Total output tokens produced',
        unit: 'tokens',
      }),
      costCounter: meter.createCounter('crew.tokens.cost', {
        description: 'Estimated cost in USD',
        unit: 'USD',
      }),
      totalCounter: meter.createUpDownCounter('crew.tokens.total', {
...
function ensureAgentMetrics(): AgentMetrics {
  if (!_agentMetrics) {
    const meter = getMeter('crew-sdk');
    _agentMetrics = {
      spawnsCounter: meter.createCounter('crew.agent.spawns', {
        description: 'Total agent spawns',
      }),
      durationHistogram: meter.createHistogram('crew.agent.duration', {
        description: 'Agent task duration in milliseconds',
        unit: 'ms',
      }),
      errorsCounter: meter.createCounter('crew.agent.errors', {
        description: 'Total agent errors',
      }),
      activeGauge: meter.createUpDownCounter('crew.agent.active', {
...
function ensureSessionPoolMetrics(): SessionPoolMetrics {
  if (!_sessionPoolMetrics) {
    const meter = getMeter('crew-sdk');
    _sessionPoolMetrics = {
      activeCounter: meter.createUpDownCounter('crew.sessions.active', {
        description: 'Currently active sessions',
      }),
      idleCounter: meter.createUpDownCounter('crew.sessions.idle', {
        description: 'Currently idle pooled sessions',
      }),
      createdCounter: meter.createCounter('crew.sessions.created', {
        description: 'Total sessions created',
      }),
      closedCounter: meter.createCounter('crew.sessions.closed', {
        description: 'Total sessions closed',
      }),
      errorsCounter: meter.createCounter('crew.sessions.errors', {
...
function ensureLatencyMetrics(): LatencyMetrics {
  if (!_latencyMetrics) {
    const meter = getMeter('crew-sdk');
    _latencyMetrics = {
      ttftHistogram: meter.createHistogram('crew.response.ttft', {
        description: 'Time to first token in milliseconds',
        unit: 'ms',
      }),
      durationHistogram: meter.createHistogram('crew.response.duration', {
        description: 'Total response duration in milliseconds',
        unit: 'ms',
      }),
      tokensPerSecGauge: meter.createGauge('crew.response.tokens_per_second', {
...
function ensureReworkMetrics(): ReworkMetrics {
  if (!_reworkMetrics) {
    const meter = getMeter('crew-sdk');
    _reworkMetrics = {
      rateGauge: meter.createGauge('crew.rework.rate', {
        description: 'Current rework rate percentage',
        unit: '%',
      }),
      cyclesHistogram: meter.createHistogram('crew.rework.cycles', {
        description: 'Review cycles per PR',
      }),
      rejectionRateGauge: meter.createGauge('crew.rework.rejection_rate', {
        description: 'Percentage of PRs with changes requested',
        unit: '%',
      }),
      timeHistogram: meter.createHistogram('crew.rework.time_ms', {

...
import type { EventBus, UnsubscribeFn, CrewEvent } from './event-bus.js';
...
function recordSpan(event: TelemetryEvent): void {
  const tracer = getTracer('crew-sdk');
  ...
  switch (event.name) {
    case 'crew.init': {
      const span = tracer.startSpan('crew.init', { attributes: attrs });
      span.end();
      break;
    }

    case 'crew.agent.spawn': {
      const span = tracer.startSpan('crew.agent.spawn', { attributes: attrs });
      span.end();
      break;
    }

    case 'crew.error': {
      const span = tracer.startSpan('crew.error', { attributes: attrs });
    ...
    case 'crew.run': {
      ...
      const span = tracer.startSpan('crew.run', { attributes: attrs });
      span.end();
      break;
    }

    case 'crew.upgrade': {
      const span = tracer.startSpan('crew.upgrade', { attributes: attrs });
      span.end();
      break;
    }
 ...
 *
 * This is the **mid-level** OTel integration point — it wires the Crew
 ...
 * @example
 * ```ts
 * import { EventBus, initializeOTel, bridgeEventBusToOTel } from 'crew-sdk';
 *
...
export function bridgeEventBusToOTel(bus: EventBus): UnsubscribeFn {
  return bus.subscribeAll((event: CrewEvent) => {
    const tracer = getTracer('crew-sdk');
    const attrs: Record<string, string | number | boolean> = {

...
export interface CrewEventPayloadMap {
...
/**
 * A strongly-typed CrewEvent where the payload type is inferred from the
 * event type discriminant. Use this for type-safe event construction.
 */
export interface TypedCrewEvent<T extends CrewEventType = CrewEventType> {
  type: T;
  sessionId?: string;
  agentName?: string;
  payload: T extends keyof CrewEventPayloadMap ? CrewEventPayloadMap[T] : unknown;
  timestamp: Date;
}

/**
 * Type guard: narrows a CrewEvent to a specific typed event.
 */
export function isCrewEventOfType<T extends CrewEventType>(
  event: CrewEvent,
  type: T,
): event is TypedCrewEvent<T> {
  return event.type === type;
}

/**
 * Helper to construct a typed CrewEvent with payload validation at compile time.
 */
export function createCrewEvent<T extends CrewEventType>(
  type: T,
  payload: T extends keyof CrewEventPayloadMap ? CrewEventPayloadMap[T] : unknown,

 ...
 * consumers (e.g., CrewOffice visualization) can subscribe to
...
import type { EventBus, CrewEvent, UnsubscribeFn } from './event-bus.js';
 ...
 *
 * Message format matches the CrewOffice WSMessage envelope:
 * ```json
 * { "kind": "event", "payload": { ...CrewEvent... } }
 * ```
 *
...
export function startWSBridge(
  ...
  const unsubscribe: UnsubscribeFn = bus.subscribeAll((event: CrewEvent) => {
...
/** Serialize a CrewEvent for JSON transport (Date to ISO string). */
function serializeEvent(event: CrewEvent): Record<string, unknown> {
  return {
    type: event.type,

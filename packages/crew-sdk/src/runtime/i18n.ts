...
/** Result of an accessibility audit against a Crew config. */
...
export const defaultCatalog: MessageCatalog = {
  // CLI
  'cli.version': 'crew {version}',
  'cli.help.header': 'crew {version} — Programmable multi-agent runtime for GitHub Copilot',
  'cli.help.usage': 'Usage: crew [command] [options]',
  'cli.unknown_command': 'Unknown command: {command}',
  'cli.init.success': 'Initialized Crew project in {path}',
  'cli.init.already_exists': 'Crew config already exists at {path}',
  ...
  'config.not_found': 'No configuration file found. Run `crew init` to create one.',
  ...
  // Migration
  'migration.detected': 'Legacy setup detected at {path}. Consider migrating to crew.config.ts.',
...
/**
 * Audit a Crew configuration for accessibility concerns.
 * Returns findings with severity levels; `passed` is true when no errors exist.
 */
export function auditAccessibility(config: CrewConfig): AccessibilityReport {

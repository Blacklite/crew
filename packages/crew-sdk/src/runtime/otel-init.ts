 ...
 * just call `initCrewTelemetry()` and everything lights up.
 *
...
/** Options for the high-level {@link initCrewTelemetry} helper. */
export interface CrewTelemetryOptions extends OTelConfig {
  /**
...
/** Handle returned by {@link initCrewTelemetry} for lifecycle control. */
export interface CrewTelemetryHandle {
...
/**
 * One-call OTel setup for Crew SDK consumers.
 *
 ...
 * 4. Bridges the EventBus so every Crew event becomes an OTel span.
 ...
 * @example
 * ```ts
 * import { initCrewTelemetry } from 'crew-sdk';
 *
 ...
 * ```
 */
export function initCrewTelemetry(options: CrewTelemetryOptions = {}): CrewTelemetryHandle {
 ...
 *
 * Pre-configures `serviceName` to `'crew-copilot-agent'` and
 ...
 */
export function initAgentModeTelemetry(options: Omit<CrewTelemetryOptions, 'serviceName' | 'mode'> = {}): CrewTelemetryHandle {
  return initCrewTelemetry({
    ...options,
    serviceName: 'crew-copilot-agent',
    mode: 'copilot-agent',
  });
}

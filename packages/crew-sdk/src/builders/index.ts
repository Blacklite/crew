...
export function defineBudget(config: BudgetDefinition): BudgetDefinition {
 ...
 *   charter: '.crew/agents/edie/charter.md',
...
/**
 * Define crew-level defaults applied to all agents unless overridden.
 *
 * ```ts
...
// ---------------------------------------------------------------------------
// defineCrew — top-level composition
// ---------------------------------------------------------------------------
 ...
 * ```
 */
export function defineCrew(config: CrewSDKConfig): CrewSDKConfig {
  assertObject(config, 'defineCrew');
  assertOptionalString(config.version, 'version', 'defineCrew');

  // Validate nested sections via their respective builders
  defineTeam(config.team);
  assertArray(config.agents, 'agents', 'defineCrew');
  ...
  if (config.ceremonies !== undefined) {
    assertArray(config.ceremonies, 'ceremonies', 'defineCrew');
  ...
  if (config.skills !== undefined) {
    assertArray(config.skills, 'skills', 'defineCrew');

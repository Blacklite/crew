 ...
 *
 * @see https://github.com/Blacklite/crew/issues/515
 */
...
export async function loadRatePool(teamRoot?: string): Promise<RatePool | null> {
  ...
  if (teamRoot) {
    candidates.push(path.join(teamRoot, '.crew', 'rate-pool.json'));
  }
  candidates.push(path.join(os.homedir(), '.crew', 'rate-pool.json'));

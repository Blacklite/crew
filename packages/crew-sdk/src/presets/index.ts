 ...
 *
 * Presets are curated agent collections stored in `<crew-home>/presets/<name>/`.
...
import { resolvePresetsDir, ensureCrewHome } from '../resolution.js';
import type { PresetManifest, PresetApplyResult, PresetAgent } from './types.js';
import { scaffoldPresetIntoCrew } from './scaffold.js';
...
/**
 * List all available presets from the crew home presets directory.
 *
...
/**
 * Apply a preset — copy its agents into a target crew directory.
 *
 * By default, existing agents are skipped (not overwritten).
 * Pass `force: true` to overwrite existing agents.
 *
 * @param presetName - Name of the preset to apply.
 * @param targetDir  - Target directory to install agents into (e.g. `.crew/agents/`).
...
export function applyPreset(
  ...
  const presetsDir = resolvePresetsDir();
  if (!presetsDir) {
    return [{ agent: presetName, status: 'error', reason: 'No presets directory found. Run `crew preset init --remote` to set up, or `crew preset init` for local-only.' }];
  }
  ...
  if (storage.existsSync(savedRoutingPath)) {
    const crewDir = path.dirname(targetDir);
    const destRoutingPath = path.join(crewDir, 'routing.md');
  ...
  // treats every session as Init Mode — see Blacklite/crew#1288. We only
  ...
  if (wireableAgents.length > 0) {
    const crewDir = path.dirname(targetDir);
    try {
      scaffoldPresetIntoCrew(crewDir, wireableAgents, presetName);
    } catch (err) {
      ...
      // happens to ship an agent that shares the preset's name (`crew
...
/**
 * Install a preset into crew home from an external source directory.
 * Copies the preset directory into `<crew-home>/presets/<name>/`.
 *
...
/**
 * Save the current project's crew agents as a reusable preset.
 *
 * Reads agents from the given crew directory (e.g. `.crew/agents/`),
 * generates a `preset.json` manifest, and copies everything into
 * `<crew-home>/presets/<name>/`.
 *
 * @param name      - Name for the new preset.
 * @param crewDir  - Path to the project's .crew/ directory containing agents/.
...
export function savePreset(
  ...
  if (!storage.existsSync(agentsDir) || !isDirSync(agentsDir)) {
    throw new Error(`No agents/ directory found in ${crewDir}`);
  }
...
export interface InstallPresetResult {
  ...
  /** Absolute path of the installed preset dir under CREW_HOME. */
  installedDir: string;
...
/**
 * Install a preset from a remote git source or local path into `$CREW_HOME/presets/<name>/`.
 *
 ...
 *
 * Fragment handling (post-review on Blacklite/crew#1225):
...
function resolveInstallSource(source: string): { workDir: string; subPath: string | null; nameHint: string | null; cleanup: () => void } {
  ...
  const tmpBase = path.join(os.tmpdir(), `crew-preset-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
  ...
  try {
    ...
    // `|`, backticks, `$()`, etc. Per the review on Blacklite/crew#1225,
...
/**
 * Seed crew home with built-in presets if they don't already exist.

 ...
 *
 * `applyPreset` copies agent charters into `.crew/agents/`, but to make the
 * resulting crew usable by the coordinator (and to satisfy the post-init
 ...
 *
 * The cast command (`packages/crew-cli/src/cli/core/cast.ts`) has its own
 ...
 * focused on making preset apply produce a Coordinator-ready crew.
 *
 ...
 *
 * Mirrors the role-to-status mapping in `packages/crew-cli/src/cli/core/cast.ts:652-655`
...
function memberRow(agent: PresetAgent): string {
  ...
  return `| ${agent.name} | ${agent.role} | \`.crew/agents/${nameLower}/charter.md\` | ${statusForRole(agent.role)} |`;
}
...
/**
 * Write or update `.crew/team.md` so its `## Members` table contains the
 ...
 */
function writeOrMergeTeamMembers(crewDir: string, agents: PresetAgent[], presetName: string): void {
  const teamPath = path.join(crewDir, 'team.md');
  const existing = storage.existsSync(teamPath) ? (storage.readSync(teamPath) ?? '') : '';

  if (!existing) {
    // Create from scratch (preset apply onto a bare .crew/ directory)
    const fresh = [
      '# Crew Team',
      '',
      `> Created by \`crew preset apply ${presetName}\``,
      '',
      '## Coordinator',
      '',
      '| Name | Role | Notes |',
...
/**
 * Write or update `.crew/routing.md` so it includes a `## Work Type → Agent`
 ...
 * created.
 */
function writeOrMergeRouting(crewDir: string, agents: PresetAgent[]): void {
...
/**
 * Write or update `.crew/casting/registry.json`, `history.json`, and
...
function writeOrMergeCastingState(
  ...
  const castingDir = path.join(crewDir, 'casting');
 ...
 */
export function scaffoldPresetIntoCrew(
  crewDir: string,
  agents: PresetAgent[],
  presetName: string,
): void {
  ...
  writeOrMergeTeamMembers(crewDir, agents, presetName);
  writeOrMergeRouting(crewDir, agents);
  writeOrMergeCastingState(crewDir, agents, { universe });
}

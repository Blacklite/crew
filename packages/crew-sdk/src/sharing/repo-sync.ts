/**
 * Repo Sync — export/import Crew configuration to/from a GitHub repository.
 *
 * Uses the GitHub Contents API via `gh api` for authentication and transport.
 * The bundle is stored at a configurable path (default: `.crew/crew-export.json`)
...
const DEFAULT_BUNDLE_PATH = '.crew/crew-export.json';
...
/**
 * Export a local Crew bundle JSON string to a GitHub repository.
 */
export async function exportToRepo(
  ...
  const message = options?.message ?? 'chore: update Crew configuration export';
...
/**
 * Import a Crew bundle JSON string from a GitHub repository.
...
export async function importFromRepo(
  ...
  if (!result) {
    throw new Error(
      `No Crew export found at ${repoSpec.owner}/${repoSpec.repo}/${filePath}${branch ? ` (branch: ${branch})` : ''}`,
    );
  }

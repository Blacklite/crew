...
function getGitHubRalphCommands(): RalphCommands {
  return {
    listUntriaged:
      'gh issue list --label "crew:untriaged" --json number,title,labels,assignees --limit 20',
    listAssigned:
      'gh issue list --label "crew:{member}" --state open --json number,title,labels,assignees --limit 20',
    listOpenPRs:
...
function getAzureDevOpsRalphCommands(): RalphCommands {
  return {
    listUntriaged:
      `az boards query --wiql "SELECT [System.Id],[System.Title],[System.State],[System.Tags] FROM WorkItems WHERE [System.Tags] Contains 'crew:untriaged' ORDER BY [System.CreatedDate] DESC" --output table`,
    listAssigned:
      `az boards query --wiql "SELECT [System.Id],[System.Title],[System.State],[System.Tags] FROM WorkItems WHERE [System.Tags] Contains 'crew:{member}' AND [System.State] <> 'Closed' ORDER BY [System.CreatedDate] DESC" --output table`,
    listOpenPRs:

 ...
 * Tokens stored per-identity in <crew-home>/teams-tokens-{hash(tenantId:clientId)}.json
 * (crew-home defaults to ~/.crew/ but respects the CREW_HOME env var).
 *
...
function getCrewDir(): string {
  return resolveCrewHome(true) ?? join(homedir(), '.crew');
}

/** Legacy single-file path (pre-tenant-scoped) — migrated away on first use */
const LEGACY_TOKEN_PATH = join(homedir(), '.crew', 'teams-tokens.json');
...
function getTokenPath(tenantId: string, clientId: string): string {
  ...
  return join(getCrewDir(), `teams-tokens-${hash}.json`);
}
...
function saveTokens(tenantId: string, clientId: string, tokens: StoredTokens): void {
  const tokenPath = getTokenPath(tenantId, clientId);
  const crewDir = getCrewDir();
  if (!existsSync(crewDir)) {
    mkdirSync(crewDir, { recursive: true, mode: 0o700 });
  }
...
<html><head><meta charset="utf-8"><title>Crew — Authenticated</title>

 ...
 * Planner buckets map to crew assignments (crew:untriaged, crew:riker, etc.)
 *
...
export class PlannerAdapter {
  ...
  async createWorkItem(options: { title: string; description?: string; tags?: string[] }): Promise<WorkItem> {
    // Resolve target bucket from tags (first crew: tag), default to untriaged
    let bucketId: string | undefined;
    let bucketName = 'crew:untriaged';
    if (options.tags?.length) {
      for (const tag of options.tags) {
    ...
    if (!bucketId) {
      bucketId = await this.getBucketId('crew:untriaged');
    }

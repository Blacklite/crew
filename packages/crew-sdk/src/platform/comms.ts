 ...
 *
 * Reads `.crew/config.json` for the `communications` section.
...
/**
 * Read communication config from `.crew/config.json`.
 */
function readCommsConfig(repoRoot: string): CommunicationConfig | undefined {
  const configPath = join(repoRoot, '.crew', 'config.json');
 ...
 *
 * Priority:
 * 1. Explicit config in `.crew/config.json` → `communications.channel`
...
/**
 * Read Teams-specific config from `.crew/config.json`.

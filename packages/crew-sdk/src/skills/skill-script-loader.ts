...
import type { CrewTool, CrewToolHandler, CrewToolResult } from '../adapter/types.js';
...
): CrewToolHandler<TArgs> {
  return async (args, _invocation) => {
    const span = tracer.startSpan('crew.skill.call', {
      attributes: { 'tool.name': name, 'tool.skill_dispatched': true },
    });
    const startTime = Date.now();
    try {
      const result = await skillHandler(args, backendConfig) as CrewToolResult;
      const durationMs = Date.now() - startTime;
      span.addEvent('crew.skill.result', typeof result === 'string'
      ...
      span.addEvent('crew.skill.error', {
 ...
 * 2. With teamRoot: resolve `.copilot/` paths from projectRoot and strip legacy `.crew/` paths relative to teamRoot
...
export function resolveSkillPath(
  ...
  // 2. With teamRoot: resolve .copilot/ paths from projectRoot, legacy .crew/ paths from teamRoot
  if (teamRoot) {
    ...
    const stripped = skillPath.startsWith('.crew/') ? skillPath.slice('.crew/'.length) : skillPath;
...
export class SkillScriptLoader {
   ...
   * 3. For each file, derive tool name: prepend 'crew_' to the filename stem
   ...
   *    e. Produce a CrewTool entry with wrapSkillHandler()
  ...
  async load(
    ...
    const tools: CrewTool<any>[] = [];

    // 3. Load each discovered handler script
    for (const scriptName of scriptFiles) {
      // Derive tool name from filename: create_issue.js → crew_create_issue
      const toolName = 'crew_' + scriptName.slice(0, -3);
      ...
      try {
        ...
        // e. Create CrewTool entry — wrap with OTel span instrumentation at load time
        // so skill-dispatched calls appear in traces with 'tool.skill_dispatched: true'.
        const tool: CrewTool<any> = {
          name: toolName,

/**
 * Git-native state backends for `.crew/` state storage.
 *
 ...
 * above any realistic `.crew/` state payload while still capping memory.
 */
...
export class WorktreeBackend implements StateBackend {
  ...
  constructor(crewDir: string) {
    if (crewDir.includes('..')) throw new Error('Path traversal not allowed');
    this.root = crewDir;
  }
  read(relativePath: string): string | undefined {
...
export class GitNotesBackend implements StateBackend {
  ...
  private atomicSaveBlob(blob: Record<string, string>, expectedOldRefSha: string | null): { ok: boolean; stderr: string } {
    ...
    try {
      ...
      commitSha = gitExecWithRetry(['commit-tree', treeSha, ...parentArgs, '-m', 'Update crew state'], this.cwd);
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
...
export class OrphanBranchBackend implements StateBackend {
  ...
  private readonly breaker = new CircuitBreaker();
  constructor(repoRoot: string, branch = 'crew-state') {
  ...
  private ensureBranch(): void {
    ...
    try {
      commit = gitExecWithRetry(
        ['commit-tree', tree, '-m', 'Initialize crew-state branch'],
        this.cwd,
      );
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
...
export class StateBackendStorageAdapter implements StorageProvider {
  constructor(private backend: StateBackend, private crewDir: string) {}
  ...
  private toRelative(filePath: string): string {
    ...
    const resolvedCrew = path.resolve(this.crewDir);

    const isWindows = process.platform === 'win32';
    const fileCmp = isWindows ? resolvedFile.toLowerCase() : resolvedFile;
    const crewCmp = isWindows ? resolvedCrew.toLowerCase() : resolvedCrew;

    const prefix = crewCmp.endsWith(path.sep) ? crewCmp : crewCmp + path.sep;
    if (fileCmp.startsWith(prefix)) {
      return resolvedFile.slice(resolvedCrew.length + 1).replace(/\\/g, '/');
    }
    if (fileCmp === crewCmp) {
      return '.';
    }
    ...
    // Absolute path that doesn't live under crewDir — this would produce a
    // corrupt git-notes key (absolute path leaking into the ref namespace).
    throw new Error(
      `[crew] toRelative: path is outside crewDir and cannot be used as a state key.\n` +
      `  path:     ${resolvedFile}\n` +
      `  crewDir: ${resolvedCrew}`
    );
  }
}
...
export class TwoLayerBackend implements StateBackend {
  ...
  /** True for refs that look like `crew/<name>` — alphanumerics, dash, underscore, slash. */
...
export function resolveStateBackend(crewDir: string, repoRoot: string, cliOverride?: StateBackendType): StateBackend {
  ...
  try {
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.warn(`⚠️  Failed to read state backend config from ${path.join(crewDir, 'config.json')}: ${msg}`);
  }
...
function normalizeBackendType(type: string): StateBackendType {
  ...
  if (type === 'external') {
    if (!_warnedExternalStubMigration) {
      ...
      console.warn(
        "[deprecation] stateBackend: 'external' is renamed to 'external-stub'; please update .crew/config.json. " +
        "Note: 'external-stub' is an unimplemented placeholder that falls back to 'local'. " +
        "To store state outside the working tree, use `crew externalize` instead."
      );
    }
  ...
  if (type === 'git-notes') {
    if (!_warnedGitNotesMigration) {
      ...
      console.warn(
        "[crew] State backend 'git-notes' is deprecated and has been removed. " +
        "Your config is being silently migrated to 'two-layer', which creates a " +
        "'crew-state' orphan branch in your repository. " +
        "To suppress this warning, update .crew/config.json: " +
        "set \"stateBackend\": \"two-layer\". " +
        "See https://github.com/Blacklite/crew/blob/dev/docs/state-backends.md for upgrade instructions."
      );
    }
    return 'two-layer';
  }
...
function createBackend(type: StateBackendType, crewDir: string, repoRoot: string): StateBackend {
  switch (type) {
    case 'local':     return new WorktreeBackend(crewDir);
    case 'orphan':

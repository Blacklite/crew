...
// --- Generate manifest from CrewConfig ---

export function generateManifest(config: CrewConfig): MarketplaceManifest {
  return {
    name: slugify(config.team.name),
    version: config.version,
    description: config.team.description ?? `${config.team.name} — Crew-powered Copilot extension`,
    author: '',
    repository: '',

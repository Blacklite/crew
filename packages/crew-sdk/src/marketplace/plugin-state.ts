...
export async function buildActivePluginContext(
  ...
  const sections: string[] = [
    '## Active Crew Plugins',
    [
      'The following enabled Crew plugins contributed declarative static artifacts.',
      'Crew has not installed upstream packages, started MCP servers, or run external plugin commands.',
  ...
  for (const pluginId of activePluginIds) {
    ...
    for (const file of [...plugin.files].sort((a, b) => a.target.localeCompare(b.target))) {
      if (!isSafeInstalledPluginTarget(file.target)) {
        sections.push(`#### Installed artifact: .crew/${file.target}\nSkipped unsafe plugin state target.`);
        continue;
      }
...
function formatProviderContracts(providers: PluginProviderContract[]): string {
  const lines = [
    '#### Provider contracts',
    'Provider contracts are declarative metadata only. Crew does not start MCP servers, call provider tools, or query live provider backends during agent spawn.',
  ];
  ...
  for (const provider of [...providers].sort((a, b) => a.id.localeCompare(b.id))) {
    const details = [
      ...
      provider.artifact ? `artifact=.crew/${provider.artifact}` : undefined,

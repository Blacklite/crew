...
export interface CrewPluginManifest {
  id: string;
  name: string;
  version: string;
  description?: string;
...
export function parsePluginManifestContent(content: string, fileName = 'crew-plugin.json'): CrewPluginManifest {
...
export function validatePluginManifest(manifest: CrewPluginManifest): PluginValidationResult {
  const errors: string[] = [];
  ...
  if (manifest.crew !== undefined && typeof manifest.crew !== 'string') {
    errors.push('crew compatibility must be a string when provided');
  }
...
export function derivePluginRoles(manifest: CrewPluginManifest): PluginComponentKind[] {
...
function normalizePluginManifest(raw: unknown): CrewPluginManifest {
  if (!isRecord(raw)) {
  ...
  if (!Array.isArray(filesRaw)) {
    return {
      ...
      crew: readOptionalString(raw, 'crew'),
...
function validateRelativePath(field: string, value: string, errors: string[]): void {
  ...
  if (isAbsolute(value) || normalized.startsWith('../') || normalized === '..' || segments.includes('..')) {
    errors.push(`${field} must be a relative path that does not escape the plugin or .crew directory`);
  }
...
function validateUpstreamMetadata(
  ...
  if (upstream.installCommand) {
    warnings.push('upstream.installCommand is metadata only; Crew will not execute it');
  }
}
...
function validateMcpMetadata(
  ...
  if (mcp.installCommand) {
    warnings.push('mcp.installCommand is metadata only; Crew will not execute it');
  }
}
  ...
  for (const [index, provider] of providers.entries()) {
    ...
    if (provider.mcp !== undefined) {
      if (!isRecord(provider.mcp)) {
        ...
        warnings.push(`${prefix}.mcp is provider metadata only; Crew will not start MCP servers or call provider tools`);
      }
    }
  }
}
...
function validateRuntimeManifest(
  ...
  for (const [index, capability] of runtime.capabilities.entries()) {
    ...
    warnings.push(`${prefix} uses built-in artifact operations only; Crew will not execute plugin-supplied code`);
  }
}

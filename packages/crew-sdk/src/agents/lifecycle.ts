...
import { CrewClientWithPool } from '../client/index.js';
import type { CrewSession, CrewSessionConfig, CrewReasoningEffort, CrewContextTier } from '../adapter/types.js';
...
export interface AgentHandle {
  ...
  agentName: string;
  /** Underlying session ID from Crew Client */
  sessionId: string;
...
export interface SpawnAgentOptions {
  ...
  reasoningEffortOverride?: CrewReasoningEffort;

  /** User-specified context tier override */
  contextTierOverride?: CrewContextTier;
...
export class AgentLifecycleManager {
  private client: CrewClientWithPool;
   ...
   * 4. Create session via Crew Client
  ...
  async spawnAgent(options: SpawnAgentOptions): Promise<AgentHandle> {
    const span = tracer.startSpan('crew.lifecycle.spawnAgent');
    ...
    try {
      ...
      const crewCharterPath = path.join(this.teamRoot, '.crew', 'agents', agentName, 'charter.md');
      const legacyCharterPath = path.join(this.teamRoot, '.ai-team', 'agents', agentName, 'charter.md');
      let charterPath = crewCharterPath;
      ...
      const compileOptions: CharterCompileOptions = {
        ...
        routingRules,
        decisions,
        pluginContext: await buildActivePluginContext(this.storage, path.join(this.teamRoot, '.crew')),
      };
      ...
      const validEffort = rawEffort && rawEffort !== 'auto' && (VALID_REASONING_EFFORTS as readonly string[]).includes(rawEffort)
        ? rawEffort as CrewReasoningEffort
        : undefined;
      ...
      const validTier = rawTier && rawTier !== 'auto' && (VALID_CONTEXT_TIERS as readonly string[]).includes(rawTier)
        ? rawTier as CrewContextTier
        : undefined;
      const sessionConfig: CrewSessionConfig = {
  ...
  async destroyAgent(handle: AgentHandle): Promise<void> {
    const span = tracer.startSpan('crew.lifecycle.destroyAgent');
  ...
  private async emitPolicyOutcome(
    ...
    if (outcome.warning) {
      console.warn(`[crew:cost-policy] ${agentName}: ${outcome.warning}`);
    }

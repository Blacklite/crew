...
import { CrewCustomAgentConfig } from '../adapter/types.js';
 ...
 */
export interface CompiledCharter extends CrewCustomAgentConfig {
 ...
 * @returns Crew CustomAgentConfig ready for SDK registration
 * @throws {ConfigurationError} If charter is missing or malformed
 */
export function compileCharter(options: CharterCompileOptions): CrewCustomAgentConfig {
...
export function compileCharterFull(options: CharterCompileOptions): CompiledCharter {
  ...
  try {
    ...
    // Add enabled plugin guidance after built-in crew context. Plugins are
  ...
  if (modelMatch) {
    ...
    if (effortMatch) {
      ...
      if (raw !== 'auto' && VALID_EFFORTS.has(raw)) {
        ...
        console.warn(
          `[crew] charter parse: ignoring invalid reasoning effort "${raw}" `
    ...
    if (tierMatch) {
      ...
      if (raw !== 'auto' && VALID_TIERS.has(raw)) {
        ...
        // Surface invalid charter input to the author instead of dropping it silently.
        console.warn(
          `[crew] charter parse: ignoring invalid context tier "${raw}" `

 ...
 *
 * Each compile reads a charter.md (FS or CrewState-backed) and parses it.
...
export class CharterCompiler {
  ...
  constructor(storage: StorageProvider = new FSStorageProvider(), state?: CrewState) {
  ...
  /**
   * Compile a charter from an agent name using CrewState.
   * Reads the charter via the typed agents collection.
   */
  async compileByName(agentName: string): Promise<AgentCharter> {
    if (!this.state) {
      throw new Error('compileByName requires CrewState — pass state to CharterCompiler constructor');
    }
   ...
   * When CrewState is available, uses the typed agents collection.
   * Otherwise scans .crew/agents/{name}/charter.md, skipping scribe and _alumni.
   */
  async compileAll(teamRoot: string): Promise<AgentCharter[]> {
    ...
    const agentsDir = join(teamRoot, '.crew', 'agents');
...
export class AgentSessionManager {
  ...
  async spawn(charter: AgentCharter, mode: 'lightweight' | 'standard' | 'full' = 'standard'): Promise<AgentSessionInfo> {
    const span = tracer.startSpan('crew.agent.spawn');
  ...
  async resume(agentName: string): Promise<AgentSessionInfo> {
    const span = tracer.startSpan('crew.agent.resume');
  ...
  async destroy(agentName: string): Promise<void> {
    const span = tracer.startSpan('crew.agent.destroy');
    span.setAttribute('agent.name', agentName);
    try {
      const agent = this.agents.get(agentName);
      if (!agent) return;

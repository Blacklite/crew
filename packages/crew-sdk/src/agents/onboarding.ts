...
- Maintain .crew/rai/audit-trail.md with review records
- Reference .crew/rai/policy.md for team-specific standards
...
/**
 * Onboard a new agent to the Crew.
 * 
 * Creates:
 * - Agent directory at .crew/agents/{name}/
...
export async function onboardAgent(
  ...
  const agentDir = join(teamRoot, '.crew', 'agents', normalizedName);
  ...
  // Write agent files — use CrewState when available, raw storage otherwise
  ...
  if (state) {
    // CrewState.agents.create() writes charter + generic history.
...
/**
 * Update an agent's configuration to crew.config.ts (if it exists).
 * 
...
export async function addAgentToConfig(
  ...
  const configPath = join(teamRoot, 'crew.config.ts');

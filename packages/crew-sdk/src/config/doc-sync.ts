 ...
 *
 * Keeps agent documentation (.agent.md) in sync with typed CrewConfig.
 * The .agent.md file is the *reference* document; CrewConfig is the
...
import type { CrewConfig, AgentConfig } from './schema.js';
...
/**
 * Merge metadata extracted from an agent doc into an existing CrewConfig.
 *
...
/**
 * Generate (or update) an agent doc from a typed CrewConfig.
 *
...
function buildIdentityBlock(config: CrewConfig): string {
  const lines: string[] = [];
...
function buildCapabilitiesBlock(config: CrewConfig): string {
...
function buildRoutingBlock(config: CrewConfig): string {
...
function buildConstraintsBlock(_config: CrewConfig): string {
  // Constraints aren't first-class in CrewConfig yet – placeholder
  return '_No constraints defined._';
}

function buildToolsBlock(config: CrewConfig): string {

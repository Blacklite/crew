 ...
 * and availability checking. Implements the model tier system from crew.agent.md.
 * 
...
import type { CrewReasoningEffort, CrewContextTier } from '../adapter/types.js';
...
/**
 * Shape of model preference fields within `.crew/config.json`.
 */
...
/**
 * Reads the economy mode setting from `.crew/config.json`.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @returns True if economyMode is enabled, false otherwise
 */
export function readEconomyMode(crewDir: string, storage: StorageProvider = new FSStorageProvider()): boolean {
...
/**
 * Writes the economy mode setting to `.crew/config.json`.
 * Merges with existing config — does not overwrite other fields.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @param enabled - Whether economy mode should be enabled
 */
export function writeEconomyMode(crewDir: string, enabled: boolean, storage: StorageProvider = new FSStorageProvider()): void {
  const configPath = join(crewDir, 'config.json');
...
/**
 * Reads the persistent model preference from `.crew/config.json`.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @returns The defaultModel string if set, or null
 */
export function readModelPreference(crewDir: string, storage: StorageProvider = new FSStorageProvider()): string | null {
  const configPath = join(crewDir, 'config.json');
  if (!storage.existsSync(configPath)) {
    return null;
  }
...
/**
 * Reads per-agent model overrides from `.crew/config.json`.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @returns Record of agent name → model ID, or empty object
 */
export function readAgentModelOverrides(crewDir: string, storage: StorageProvider = new FSStorageProvider()): Record<string, string> {
  const configPath = join(crewDir, 'config.json');
  if (!storage.existsSync(configPath)) {
    return {};
  }
...
/**
 * Writes a persistent model preference to `.crew/config.json`.
 * Merges with existing config — does not overwrite other fields.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @param model - Model ID to persist, or null to clear
 */
export function writeModelPreference(crewDir: string, model: string | null, storage: StorageProvider = new FSStorageProvider()): void {
  const configPath = join(crewDir, 'config.json');
  let config: Record<string, unknown> = {};
  if (storage.existsSync(configPath)) {
    try {
...
/**
 * Writes per-agent model overrides to `.crew/config.json`.
 * Merges with existing config — does not overwrite other fields.
 *
 * @param crewDir - Path to the `.crew/` directory
 ...
 * clause keeps it in lock-step with the canonical {@link CrewReasoningEffort} type.
 */
export const VALID_REASONING_EFFORTS = ['low', 'medium', 'high', 'xhigh'] as const satisfies readonly CrewReasoningEffort[];
/** Canonical reasoning-effort union (alias of {@link CrewReasoningEffort}). */
export type ValidReasoningEffort = CrewReasoningEffort;
...
/**
 * Reads the persistent reasoning effort preference from `.crew/config.json`.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @returns The defaultReasoningEffort string if set, or null
 */
export function readReasoningEffort(crewDir: string, storage: StorageProvider = new FSStorageProvider()): string | null {
  const configPath = join(crewDir, 'config.json');
  if (!storage.existsSync(configPath)) {
    return null;
  }
...
/**
 * Reads per-agent reasoning effort overrides from `.crew/config.json`.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @returns Record of agent name → reasoning effort, or empty object
 */
export function readAgentReasoningEffortOverrides(crewDir: string, storage: StorageProvider = new FSStorageProvider()): Record<string, string> {
  const configPath = join(crewDir, 'config.json');
  if (!storage.existsSync(configPath)) {
    return {};
  }
...
/**
 * Writes a persistent reasoning effort preference to `.crew/config.json`.
 * Merges with existing config — does not overwrite other fields.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @param effort - Reasoning effort to persist, or null to clear
 */
export function writeReasoningEffort(crewDir: string, effort: string | null, storage: StorageProvider = new FSStorageProvider()): void {
  const configPath = join(crewDir, 'config.json');
  let config: Record<string, unknown> = {};
  if (storage.existsSync(configPath)) {
    try {
  ...
  if (effort === null) {
    ...
    console.warn(
      `[crew] writeReasoningEffort: ignoring invalid reasoning effort "${effort}" `
...
/**
 * Writes per-agent reasoning effort overrides to `.crew/config.json`.
 * Merges with existing config — does not overwrite other fields.
 *
 * @param crewDir - Path to the `.crew/` directory
...
export function writeAgentReasoningEffortOverrides(
  ...
  if (overrides === null || Object.keys(overrides).length === 0) {
    ...
    if (dropped.length > 0) {
      console.warn(
        `[crew] writeAgentReasoningEffortOverrides: ignoring invalid reasoning effort `
 ...
 *   Layer 0a: Per-agent persistent override (.crew/config.json agentReasoningEffortOverrides)
 *   Layer 0b: Global persistent config (.crew/config.json defaultReasoningEffort)
...
export function resolveReasoningEffort(options: {
  ...
  const { agentName, crewDir, spawnOverride, charterPreference } = options;
  ...
  if (!resolved && crewDir && agentName) {
    const agentOverrides = readAgentReasoningEffortOverrides(crewDir, storage);
  ...
  if (!resolved && crewDir) {
    const persistedEffort = readReasoningEffort(crewDir, storage);
 ...
 * "auto" is a permitted stored sentinel that resolvers treat as "not set".
 * Canonical runtime list — import this instead of duplicating. The `satisfies`
 * clause keeps it in lock-step with the canonical {@link CrewContextTier} type.
 *
 ...
 */
export const VALID_CONTEXT_TIERS = ['default', 'long_context'] as const satisfies readonly CrewContextTier[];
/** Canonical context-tier union (alias of {@link CrewContextTier}). */
export type ValidContextTier = CrewContextTier;
...
/**
 * Reads the persistent context tier preference from `.crew/config.json`.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @returns The defaultContextTier string if set, or null
 */
export function readContextTier(crewDir: string, storage: StorageProvider = new FSStorageProvider()): string | null {
  const configPath = join(crewDir, 'config.json');
  if (!storage.existsSync(configPath)) {
    return null;
  }
...
/**
 * Reads per-agent context tier overrides from `.crew/config.json`.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @returns Record of agent name → context tier, or empty object
 */
export function readAgentContextTierOverrides(crewDir: string, storage: StorageProvider = new FSStorageProvider()): Record<string, string> {
  const configPath = join(crewDir, 'config.json');
  if (!storage.existsSync(configPath)) {
    return {};
  }
...
/**
 * Writes a persistent context tier preference to `.crew/config.json`.
 * Merges with existing config — does not overwrite other fields.
 *
 * @param crewDir - Path to the `.crew/` directory
 * @param tier - Context tier to persist, or null to clear
 */
export function writeContextTier(crewDir: string, tier: string | null, storage: StorageProvider = new FSStorageProvider()): void {
  const configPath = join(crewDir, 'config.json');
  let config: Record<string, unknown> = {};
  if (storage.existsSync(configPath)) {
    try {
  ...
  if (tier === null) {
    ...
    // Pass null explicitly to clear.
    console.warn(
      `[crew] writeContextTier: ignoring invalid context tier "${tier}" `
...
/**
 * Writes per-agent context tier overrides to `.crew/config.json`.
 * Merges with existing config — does not overwrite other fields.
 *
 * @param crewDir - Path to the `.crew/` directory
...
export function writeAgentContextTierOverrides(
  ...
  if (overrides === null || Object.keys(overrides).length === 0) {
    ...
    if (dropped.length > 0) {
      console.warn(
        `[crew] writeAgentContextTierOverrides: ignoring invalid context tier `
 ...
 *   Layer 0a: Per-agent persistent override (.crew/config.json agentContextTierOverrides)
 *   Layer 0b: Global persistent config (.crew/config.json defaultContextTier)
 *   Layer 1: Spawn-time override (caller-provided)
...
export function resolveContextTier(options: {
  ...
  // Layer 0a: Per-agent persistent override
  if (!resolved && crewDir && agentName) {
    const agentOverrides = readAgentContextTierOverrides(crewDir, storage);
  ...
  // Layer 0b: Global persistent config
  if (!resolved && crewDir) {
    const persistedTier = readContextTier(crewDir, storage);
 ...
 *   Layer 0: Persistent config (.crew/config.json defaultModel)
...
export function resolveModel(options: {
  ...
  const { agentName, crewDir, sessionDirective, charterPreference, taskModel } = options;
  const storage = options.storage ?? new FSStorageProvider();

  // Layer 0a: Per-agent persistent override (explicit — economy does not apply)
  if (crewDir && agentName) {
    const agentOverrides = readAgentModelOverrides(crewDir, storage);
  ...
  if (crewDir) {
    const persistedModel = readModelPreference(crewDir, storage);
    if (persistedModel) {
  ...
  const isEconomy =
    options.economyMode !== undefined
      ? options.economyMode
      : (crewDir ? readEconomyMode(crewDir, storage) : false);

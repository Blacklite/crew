 ...
 *
 * Reads legacy crew.agent.md files and .ai-team/ directory structures,
 * maps them to typed CrewConfig, and merges with any existing config
...
/**
 * Legacy configuration extracted from crew.agent.md and .ai-team/ files.
 */
...
/**
 * Agent definition extracted from a legacy crew.agent.md.
 */
...
const LEGACY_AGENT_PATHS = [
  join('.github', 'agents', 'crew.agent.md'),
  join('.ai-team', 'crew.agent.md'),
] as const;
...
/**
 * Detect whether a project uses the legacy crew.agent.md format.
 *
 * Checks for:
 * - .github/agents/crew.agent.md
 * - .ai-team/crew.agent.md
...
/**
 * Load and parse a legacy crew.agent.md file from the given directory.
 *
 * Search order:
 *  1. .github/agents/crew.agent.md
 *  2. .ai-team/crew.agent.md
 *
...
/**
 * Merge a legacy configuration with a typed CrewConfig.
 *
 * The typed config **always wins** on conflict. Legacy values fill gaps
 * where the typed config has no data.
 *
 * @param legacy - Legacy configuration from crew.agent.md
 * @param config - Typed Crew configuration (takes precedence)
...
export function emitDeprecationWarning(sourcePath: string): void {
  console.warn(
    `[crew] DEPRECATION: Legacy crew.agent.md detected at "${sourcePath}". ` +
      'Migrate to crew.config.ts for typed configuration. ' +
      'Run `crew init --migrate` to auto-convert.',
  );
}
...
/**
 * Extract agent definitions from crew.agent.md content.
...
function extractAgentDefs(
  ...
  // Pattern 1: ### Agent sections (common in crew.agent.md)

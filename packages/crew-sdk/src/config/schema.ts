...
export interface CrewConfig {
  ...
  /**
   * Agent harness that executes sessions for this team.
   * 'copilot' — GitHub Copilot CLI (default); 'claude' — Claude Code CLI.
   * Overridable per invocation via --harness or CREW_HARNESS/CREW_HARNESS.
   */
...
export interface AgentConfig {
  ...
  /** Per-agent harness override ('copilot' | 'claude'). Defaults to the team harness. */
  harness?: 'copilot' | 'claude';
}
...
export function defineConfig(config: Partial<CrewConfig>): CrewConfig {
  return {
    ...DEFAULT_CONFIG,
    ...config,
    team: {
...
export function validateConfig(config: unknown): config is CrewConfig {
  if (typeof config !== 'object' || config === null) return false;
  
  const c = config as Partial<CrewConfig>;

...
const AGENT_DIRS = ['.crew/agents', '.ai-team/agents'] as const;
...
export class LocalAgentSource implements AgentSource {
  ...
  constructor(
    ...
    /**
     * Explicit agents directory — skips the `<basePath>/.crew/agents`
     * probing. Needed for externalized state, where agents live at
     * `<externalStateDir>/agents` with no `.crew` nesting (#1399).
     */
    private agentsDir?: string,
  ) {}

  /**
   * Resolve the agents directory, preferring .crew/agents over .ai-team/agents.
...
export class GitHubAgentSource implements AgentSource {
  ...
  constructor(repo: string, options?: { ref?: string; pathPrefix?: string; fetcher?: GitHubFetcher }) {
    ...
    this.pathPrefix = options?.pathPrefix ?? '.crew/agents';

 ...
 *
 * Registers real migration functions for known Crew version transitions.
...
// ============================================================================
// 0.4.x → 0.5.x: Directory rename + team.md → crew.config.ts
// ============================================================================
 ...
 *
 * Changes:
 * - Renames `.ai-team/` directory references to `.crew/`
 * - Converts team.md-style flat config to crew.config.ts typed format
 * - Normalises agent names to lowercase with `@` prefix
 */
export const migration_0_4_to_0_5: Migration = {
  fromVersion: '0.4.0',
  toVersion: '0.5.0',
  description: '.ai-team/ → .crew/ directory rename, team.md → crew.config.ts',
  ...
  migrate(config: Record<string, unknown>): Record<string, unknown> {
    ...
    if (typeof result.configDir === 'string') {
      result.configDir = (result.configDir as string).replace(/\.ai-team/g, '.crew');
    }
    ...
    if (typeof result.agentDir === 'string') {
      result.agentDir = (result.agentDir as string).replace(/\.ai-team/g, '.crew');
    }

    // Mark config format
    result.configFormat = 'crew.config.ts';
  ...
  rollback(config: Record<string, unknown>): Record<string, unknown> {
    ...
    if (typeof result.configDir === 'string') {
      result.configDir = (result.configDir as string).replace(/\.crew/g, '.ai-team');
    }
    ...
    if (typeof result.agentDir === 'string') {
      result.agentDir = (result.agentDir as string).replace(/\.crew/g, '.ai-team');
    }
...
export const migration_0_5_to_0_6: Migration = {
  ...
  migrate(config: Record<string, unknown>): Record<string, unknown> {
    ...
    if (!result.agentSources) {
      result.agentSources = [{ type: 'local', name: 'local', path: '.crew/agents' }];
    }

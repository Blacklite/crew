...
export const REAL_COPILOT_UNAVAILABLE_REASON =
  'Real Copilot Memory API unavailable: no concrete callable API was found in installed @github/copilot SDK/tooling. Crew will not fake provider=copilot; use hostInjectedCopilotAdapter only when a host supplies a client.';
...
export async function ensureMemoryGovernanceDefaults(
  ...
  const memoryDir = path.join(projectRoot, '.crew', 'memory');
...
export class LocalMemoryStore {
  ...
  constructor(
    ...
    this.crewDir = options.rootKind === 'crew' ? rootDir : path.join(rootDir, '.crew');
  ...
  async write(request: MemoryWriteRequest): Promise<MemoryWriteResult> {
    ...
    const fullPath = path.join(this.crewDir, relativePath);
    ...
    const entry: MemoryIndexEntry = {
      ...
      loadGuidance: classification.loadGuidance,
      title,
      path: path.join('.crew', relativePath),
      status: 'active',
      createdAt: now,
      updatedAt: now,
    };
  ...
  async delete(id: string, actor?: string): Promise<boolean> {
    ...
    return this.withIndexLock(async () => {
      ...
      const tombstonePath = path.join(this.crewDir, 'memory', 'tombstones', `${id}.json`);
      const tombstoneData =
        JSON.stringify({
          id,
          deletedAt,
  ...
  async configureHostInjectedCopilotAdapter(options: {
    ...
    await this.storage.write(path.join(this.crewDir, 'memory', 'config.json'), JSON.stringify(next, null, 2) + '\n');
    await this.audit({
      action: 'configure',
  ...
  async auditLog(): Promise<MemoryAuditRecord[]> {
    await this.ensureInitialized();
    const content = await this.storage.read(path.join(this.crewDir, 'memory', 'audit.jsonl'));
    if (!content) return [];
    return content
      .split(/\r?\n/)
      .filter(Boolean)
  ...
  private async ensureInitialized(): Promise<void> {
    const memoryDir = path.join(this.crewDir, 'memory');
    for (const dir of ['local', 'policy-inbox', 'semantic-inbox', 'tombstones']) {
  ...
  private async readConfig(): Promise<MemoryGovernanceConfig> {
    const content = await this.storage.read(path.join(this.crewDir, 'memory', 'config.json'));
  ...
  private async readIndex(): Promise<MemoryIndexEntry[]> {
    const indexPath = path.join(this.crewDir, 'memory', 'index.json');
  ...
  private async audit(record: Omit<MemoryAuditRecord, 'timestamp'>): Promise<void> {
    ...
    await this.storage.append(path.join(this.crewDir, 'memory', 'audit.jsonl'), JSON.stringify(auditRecord) + '\n');
  }
  ...
  private async rotateAuditIfNeeded(): Promise<void> {
    ...
    const auditPath = path.join(this.crewDir, 'memory', 'audit.jsonl');
    ...
    for (let index = maxArchives; index >= 1; index--) {
      const current = path.join(this.crewDir, 'memory', `audit.${index}.jsonl`);
      const next = path.join(this.crewDir, 'memory', `audit.${index + 1}.jsonl`);
    ...
    if (maxArchives > 0) {
      await this.storage.rename(auditPath, path.join(this.crewDir, 'memory', 'audit.1.jsonl'));
    } else {
  ...
  private absoluteFromEntryPath(entryPath: string): string {
    ...
    const relative = normalizedInput.startsWith('.crew')
      ? normalizedInput.slice('.crew'.length + 1)
      : normalizedInput;
    const normalized = path.normalize(relative);
    if (path.isAbsolute(normalized) || normalized === '..' || normalized.startsWith(`..${path.sep}`)) {
      throw new Error(`Unsafe memory path blocked: ${entryPath}`);
    }
    return path.join(this.crewDir, normalized);
  }

/**
 * Crew Remote Control — RemoteBridge
 *
 * WebSocket server that bridges Crew's EventBus to remote PWA clients.
...
export class RemoteBridge {
  ...
  private auditLogPath: string = path.join(this.auditLogDir, `crew-audit-${Date.now()}.jsonl`);
  ...
  async start(): Promise<number> {
    ...
    this.server = http.createServer((req, res) => {
      ...
      if (this.staticHandler) {
        ...
        res.end('Crew Remote Control Bridge');
      }
    });
  ...
  private handleSessionsAPI(res: http.ServerResponse): void {
    try {
      const output = execFileSync('devtunnel', ['list', '--labels', 'crew', '--json'], {
        encoding: 'utf-8',
        timeout: 10000,
      ...
      const sessions = (data.tunnels || []).map((t: any) => {
        ...
        // Labels format: [crew, repo-name, branch-name, machine-hostname, port-NNNN]
      ...
      if (this.passthroughWrite) {
        ...
        try {
          ...
          if (msg.method === 'session/new' && msg.params) {
            msg.params.cwd = this.config.crewDir
              ? this.config.crewDir.replace(/[/\\]\.(?:crew|ai-team)$/, '')
              : process.cwd();

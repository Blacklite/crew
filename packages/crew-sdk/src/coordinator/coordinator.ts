/**
 * Crew Coordinator (M3-1, Issue #135)
 *
 * The brain of Crew — receives user messages, decides how to handle them,
...
export interface CrewCoordinatorOptions {
  /** Crew configuration */
  config: CrewConfig;
...
export class CrewCoordinator {
  ...
  constructor(options: CrewCoordinatorOptions) {
  ...
  async handleMessage(
    ...
    const span = tracer.startSpan('crew.coordinator.handleMessage');
   ...
   */
  updateConfig(config: CrewConfig): void {
    this.config = config;
  ...
  private compileRouter(config: CrewConfig): CompiledRouter {

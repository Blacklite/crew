 ...
 *
 * The central orchestration engine. Replaces the 32KB crew.agent.md
...
import type { CrewClient, CrewSessionConfig } from '../client/index.js';
...
export interface CoordinatorConfig {
  /** Path to the team root (.crew/ directory) */
  teamRoot: string;
...
export class Coordinator {
  ...
  async initialize(): Promise<void> {
    const span = tracer.startSpan('crew.coordinator.initialize');
    try {
      if (this.eventBus) {
        this.unsubscribers.push(
          this.eventBus.subscribe('session:created', (event) => {
            const s = tracer.startSpan('crew.coordinator.session.created');
        ...
        this.unsubscribers.push(
          this.eventBus.subscribe('session:error', (event) => {
            const s = tracer.startSpan('crew.coordinator.session.error');
            s.setAttribute('session.id', event.sessionId ?? 'unknown');
            s.end();
          }),
        );
        this.unsubscribers.push(
          this.eventBus.subscribe('session:destroyed', (event) => {
            const s = tracer.startSpan('crew.coordinator.session.destroyed');
            s.setAttribute('session.id', event.sessionId ?? 'unknown');
            s.end();
          }),
        );
  ...
  async route(message: string): Promise<RoutingDecision> {
    const span = tracer.startSpan('crew.coordinator.route');
  ...
  async execute(decision: RoutingDecision, message: string): Promise<void> {
    const span = tracer.startSpan('crew.coordinator.execute');
    span.setAttribute('routing.tier', decision.tier);
    span.setAttribute('target.agents', decision.agents.join(','));
    try {
      if (this.eventBus) {
  ...
  async shutdown(): Promise<void> {
    const span = tracer.startSpan('crew.coordinator.shutdown');
    try {

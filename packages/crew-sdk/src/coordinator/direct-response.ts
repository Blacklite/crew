...
import type { CrewConfig } from '../runtime/config.js';
import type { EventBus, CrewEvent } from '../runtime/event-bus.js';
...
const DEFAULT_PATTERNS: DirectResponsePattern[] = [
  ...
  },
  {
    ...
    handler: () => {
      return [
        '**Crew Coordinator** — I route work to specialist agents.',
        '',
        'You can ask me to:',
  ...
  },
  {
    ...
    handler: (_message, context) => {
      ...
      const agents = context.activeAgents ?? [];
      if (agents.length === 0) {
        return 'No team roster available. Configure agents in your crew.config.ts.';
      }
  ...
  },
  {
    ...
    handler: () => {
      return 'Hello! I\'m the Crew coordinator. Describe what you need and I\'ll route it to the right agent, or type "help" for options.';
    },
  },
];
...
export class DirectResponseHandler {
   ...
   */
  shouldHandleDirectly(message: string, config?: CrewConfig): boolean {
    ...
    // Check configurable patterns from crew config
  ...
  handleDirect(message: string, context: CoordinatorContext): DirectResponseResult {
    ...
    if (context.eventBus) {
      context.eventBus.emit({
        type: 'coordinator:routing' as CrewEvent['type'],

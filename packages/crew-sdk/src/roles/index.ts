 ...
 * referencing built-in roles in crew.config.ts.
 *
 ...
 *
 * This is the primary way to use base roles in `crew.config.ts`:
 *
 * ```typescript
 ...
 * @returns AgentDefinition ready for defineCrew()
...
/**
 * Generate Crew charter.md content from a base role definition.
 */
function buildCharterFromRole(
  ...
  lines.push('');
  lines.push('Before starting work, run `git rev-parse --show-toplevel` to find the repo root, or use the `TEAM ROOT` provided in the spawn prompt. All `.crew/` paths must be resolved relative to this root — do not assume CWD is the repo root (you may be in a worktree or subdirectory).');
  lines.push('');
  lines.push('Before starting work, read `.crew/decisions.md` for team decisions that affect me.');
  lines.push('After making a decision others should know, write it to `.crew/decisions/inbox/{my-name}-{brief-slug}.md` — the Scribe will merge it.');

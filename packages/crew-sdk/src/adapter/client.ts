...
/**
 * Adapts @github/copilot-sdk CopilotSession to our CrewSession interface.
 ...
 */
class CopilotSessionAdapter implements CrewSession {
  /**
   * Maps Crew short event names → @github/copilot-sdk dotted event names.
   * SDK uses dotted-namespace prefixes (e.g., `assistant.message_delta`),
   * while Crew uses short names (e.g., `message_delta`).
  ...
  /** Reverse map: SDK dotted names → Crew short names. */
  ...
  private readonly unsubscribers = new Map<CrewSessionEventHandler, Map<string, () => void>>();
  ...
  /**
   * Normalizes an SDK event into a CrewSessionEvent.
   * Maps the dotted type back to the Crew short name and
   * flattens `event.data` onto the top-level object so callers
   * can access fields directly (e.g., `event.inputTokens`).
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private static normalizeEvent(sdkEvent: any): CrewSessionEvent {
    const crewType = CopilotSessionAdapter.REVERSE_EVENT_MAP[sdkEvent.type] ?? sdkEvent.type;
    return {
      type: crewType,
      ...(sdkEvent.data ?? {}),
    };
...
/**
 * Connection state for CrewClient.
 */
export type CrewConnectionState = "disconnected" | "connecting" | "connected" | "reconnecting" | "error";

/**
 * Options for creating a CrewClient.
 */
...
/**
 * CrewClient wraps CopilotClient with enhanced lifecycle management.
 * 
 * Features:
...
export class CrewClient {
  ...
  private options: Required<Omit<CrewClientOptions, "cliUrl" | "githubToken" | "useLoggedInUser" | "cliPath" | "cliArgs" | "eventBus">> & {
    cliUrl?: string;
    githubToken?: string;
    useLoggedInUser?: boolean;
    cliPath?: string;
  ...
  /**
   * Creates a new CrewClient instance.
   * 
  ...
  constructor(options: CrewClientOptions = {}) {
    ...
    // Build a RuntimeConnection from the legacy Crew options.
  ...
  async connect(): Promise<void> {
    ...
    const span = tracer.startSpan('crew.client.connect');
    ...
    this.connectPromise = (async () => {
      ...
      try {
        ...
        if (elapsed > 2000) {
          console.warn(`CrewClient connection took ${elapsed}ms (> 2s threshold)`);
        }
      } catch (error) {
        this.state = "error";
  ...
  async disconnect(): Promise<Error[]> {
    const span = tracer.startSpan('crew.client.disconnect');
    try {
  ...
  async createSession(config: CrewSessionConfig = {}): Promise<CrewSession> {
    const span = tracer.startSpan('crew.session.create');
    ...
    try {
      ...
      try {
        ...
        const normalizedConfig: CrewSessionConfig = config.onPermissionRequest
          ? {
              ...config,
              onPermissionRequest: async (
                req: Parameters<NonNullable<CrewSessionConfig['onPermissionRequest']>>[0],
                inv: Parameters<NonNullable<CrewSessionConfig['onPermissionRequest']>>[1],
              ) => {
        ...
        if (this.options.eventBus) {
          ...
          result.on('usage', (event: CrewSessionEvent) => {
  ...
  /**
   * Resume an existing Crew session by ID.
   * 
  ...
  async resumeSession(sessionId: string, config: CrewSessionConfig = {}): Promise<CrewSession> {
    const span = tracer.startSpan('crew.session.resume');
  ...
  async listSessions(): Promise<CrewSessionMetadata[]> {
    const span = tracer.startSpan('crew.session.list');
    try {
      if (!this.isConnected()) {
      ...
      try {
        ...
        const result = sessions.map((s): CrewSessionMetadata => ({
  ...
  async deleteSession(sessionId: string): Promise<void> {
    const span = tracer.startSpan('crew.session.delete');
    span.setAttribute('session.id', sessionId);
    try {
      if (!this.isConnected()) {
        throw new Error("Client not connected");
  ...
  async listModels(): Promise<CrewModelInfo[]> {
    ...
    try {
      ...
      return models.map((m): CrewModelInfo => ({
        id: m.id,
        name: m.name,
   ...
   *
   * Creates a `crew.session.message` span for the full call and a
   * child `crew.session.stream` span that tracks streaming duration
  ...
  async sendMessage(session: CrewSession, options: CrewMessageOptions): Promise<void> {
    const messageSpan = tracer.startSpan('crew.session.message');
    ...
    const streamSpan = tracer.startSpan('crew.session.stream');
    ...
    const streamListener = (event: CrewSessionEvent) => {
  ...
  /**
   * Close a session (alias for deleteSession with `crew.session.close` span).
   *
   * @param sessionId - ID of the session to close
   */
  async closeSession(sessionId: string): Promise<void> {
    const span = tracer.startSpan('crew.session.close');
    span.setAttribute('session.id', sessionId);
    try {
  ...
  async sendAndWait(session: CrewSession, options: CrewMessageOptions, timeout?: number): Promise<unknown> {
    const span = tracer.startSpan('crew.session.sendAndWait');
    ...
    const usageListener = (event: CrewSessionEvent) => {
      if (event.type === 'usage') {
        inputTokens = typeof event['inputTokens'] === 'number' ? event['inputTokens'] : 0;
        outputTokens = typeof event['outputTokens'] === 'number' ? event['outputTokens'] : 0;
        model = typeof event['model'] === 'string' ? event['model'] : 'unknown';

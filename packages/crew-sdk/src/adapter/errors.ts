 ...
 *
 * Wraps Copilot SDK errors with Crew-specific context and diagnostic information.
...
/**
 * Base error class for all Crew SDK errors.
 * Wraps SDK exceptions with Crew context and diagnostic information.
 */
 ...
 */
export class SDKConnectionError extends CrewError {
 ...
 */
export class SessionLifecycleError extends CrewError {
 ...
 */
export class ToolExecutionError extends CrewError {
  constructor(message: string, context: ErrorContext, originalError?: Error) {
 ...
 */
export class ModelAPIError extends CrewError {
 ...
 */
export class ConfigurationError extends CrewError {
  constructor(message: string, context: ErrorContext, originalError?: Error) {
 ...
 */
export class AuthenticationError extends CrewError {
  constructor(message: string, context: ErrorContext, originalError?: Error) {
 ...
 */
export class RateLimitError extends CrewError {
...
/**
 * Runtime error — internal Crew runtime failure.
 */
export class RuntimeError extends CrewError {
  constructor(message: string, context: ErrorContext, originalError?: Error) {
 ...
 */
export class ValidationError extends CrewError {
  constructor(message: string, context: ErrorContext, originalError?: Error) {
...
/**
 * Factory for creating Crew errors from SDK errors.
 * Maps SDK error patterns to appropriate Crew error types.
 */
export class ErrorFactory {
  /**
   * Wrap an SDK error with Crew context.
   * Automatically detects error type and maps to appropriate Crew error class.
   * 
   * @param error - Original SDK error
   * @param context - Crew diagnostic context
   * @returns Crew error with full context
   * 
   * @example
   * ```typescript
   * try {
   ...
   * ```
   */
  static wrap(error: unknown, context: Partial<ErrorContext>): CrewError {
  ...
  start(operation: string, context?: { sessionId?: string; agentName?: string; metadata?: Record<string, unknown> }): {
    ...
    return {
      ...
      failure: (error: unknown) => {
        const category = error instanceof CrewError ? error.category : ErrorCategory.UNKNOWN;

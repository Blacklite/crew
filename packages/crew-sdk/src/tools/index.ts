 ...
 *
 * Defines Crew's custom tools registered with the SDK via defineTool().
 * Tools provide agents with typed, validated orchestration primitives:
 *   - crew_route:  Route work to another agent via session pool
 *   - crew_decide: Write a typed decision to the inbox drop-box
 *   - crew_memory: Append to agent history (learnings, updates)
...
import type { CrewTool, CrewToolResult } from '../adapter/types.js';
import { trace, SpanStatusCode } from '../runtime/otel-api.js';
import type { StorageProvider } from '../storage/storage-provider.js';
import { FSStorageProvider } from '../storage/fs-storage-provider.js';
import type { CrewState } from '../state/crew-state.js';
...
/**
 * Define a typed Crew tool with JSON schema parameters.
 * Creates a CrewTool object compatible with the adapter layer.
 */
export function defineTool<TArgs = unknown>(config: {
  name: string;
  description: string;
  parameters: Record<string, unknown>;
  handler: (args: TArgs) => Promise<CrewToolResult> | CrewToolResult;
  ...
  return {
    ...
    handler: async (args: TArgs) => {
      const span = tracer.startSpan('crew.tool.call', {
        attributes: {
      ...
      try {
        ...
        span.addEvent('crew.tool.result', {
...
/** Agent name format: alphanumeric, hyphens, underscores. Same rule as crew_decide/crew_memory. */
 ...
 * Strips absolute filesystem paths by replacing the crewRoot prefix with [team-root],
 * and collapses multi-line errors to prevent stack trace leakage.
 */
function sanitizeErrorForLlm(error: unknown, crewRoot: string): string {
  const raw = error instanceof Error ? error.message : String(error);
  const stripped = raw.replace(new RegExp(crewRoot.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'g'), '[team-root]');
...
export class ToolRegistry {
  private tools: Map<string, CrewTool<any>> = new Map();
  private crewRoot: string;
  ...
  constructor(
    ...
    this.memoryStore = new LocalMemoryStore(storage, crewRoot, {
      rootKind: 'crew',
      hostInjectedCopilotAdapterClient,
    });
    this.registerCrewTools();
  }

  private registerCrewTools(): void {
    // crew_route: Route work to another agent
    const crewRoute = defineTool<RouteRequest>({
      name: 'crew_route',
      description: 'Route a task to another agent in the crew. Creates a new session for the target agent with the specified task and context.',
      parameters: {
        type: 'object',
        properties: {
          targetAgent: {
      ...
      handler: async (args) => {
        ...
        if (this.state) {
          try {
            ...
            if (isNotFound) {
              return {
                textResultForLlm: `Agent '${targetAgent}' not found in the team roster. Check .crew/agents/ for available agents.`,
                resultType: 'failure',
                error: 'agent-not-in-roster',
              };
            }
            return {
              textResultForLlm: `Roster check failed for '${targetAgent}': ${sanitizeErrorForLlm(err, this.crewRoot)}`,
              resultType: 'failure',
        ...
        try {
          ...
          return {
            textResultForLlm: `Cannot route to ${targetAgent}: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
        ...
        if (!fanOutDeps) {
          return {
            textResultForLlm:
              ...
              `Wire a fanOutDepsGetter into ToolRegistry, or intercept crew_route via CrewSessionHooks.onPreToolUse.`,
            resultType: 'failure',
            error: 'fan-out-deps-unavailable',
            toolTelemetry: { routeRequest },
          };
        ...
        try {
          ...
          if (!result || result.status !== 'success') {
            return {
              textResultForLlm: `Failed to route to ${targetAgent}: ${sanitizeErrorForLlm(result?.error ?? 'unknown error', this.crewRoot)}`,
              resultType: 'failure',
              error: 'spawn-failed',
          ...
          return {
            textResultForLlm: `Failed to route to ${targetAgent}: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
    ...
    // crew_decide: Write a decision to the inbox
    const crewDecide = defineTool<DecisionRecord>({
      name: 'crew_decide',
      description: 'Write a decision to the team decision inbox. Decisions are stored in .crew/decisions/inbox/ for team review.',
      parameters: {
        type: 'object',
        properties: {
          author: {
      ...
      handler: async (args) => {
        ...
        try {
          const inboxDir = path.join(this.crewRoot, 'decisions', 'inbox');
          ...
          return {
            textResultForLlm: `Failed to write decision: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
            error: String(error),
          };
        }
    ...
    const crewMemory = defineTool<MemoryEntry>({
      name: 'crew_memory',
      ...
      handler: async (args) => {
        ...
        try {
          // Use CrewState agents collection when available
          if (this.state) {
          ...
          const historyFile = path.join(this.crewRoot, 'agents', args.agent, 'history.md');
          ...
          return {
            textResultForLlm: `Failed to update agent memory: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
            error: String(error),
          };
        }
    ...
    const stateRead = defineTool<StateReadRequest>({
      ...
      description: 'Read mutable Crew state by key through the configured state backend. Keys are relative to .crew/; do not use shell git or direct file reads for mutable state.',
      parameters: {
        type: 'object',
        properties: {
          key: { type: 'string', description: 'State key relative to .crew/, for example decisions.md or agents/data/history.md' },
        },
        required: ['key'],
      },
      handler: async (args) => {
        try {
          ...
          const content = this.storage.readSync(path.join(this.crewRoot, key));
          if (content === undefined) {
            return {
          ...
          return {
            textResultForLlm: `Failed to read state: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
            error: String(error),
          };
        }
    ...
    const stateWrite = defineTool<StateWriteRequest>({
      ...
      description: 'Write mutable Crew state through the configured state backend. Always use this tool for mutable state when available. Keys are relative to .crew/; static config such as config.json, team.md, routing.md, charters, templates, and skills is not mutable state.',
      parameters: {
        type: 'object',
        properties: {
          key: { type: 'string', description: 'State key relative to .crew/' },
      ...
      handler: async (args) => {
        ...
        try {
          ...
          this.storage.writeSync(path.join(this.crewRoot, key), args.content);
          return {
          ...
          return {
            textResultForLlm: `Failed to write state: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
            error: String(error),
          };
        }
    ...
    const stateAppend = defineTool<StateAppendRequest>({
      name: 'crew_state_append',
      description: 'Append to mutable Crew state through the configured state backend. Always use this tool for mutable state when available. Keys are relative to .crew/; static config cannot be mutated through this tool.',
      parameters: {
        type: 'object',
        properties: {
          key: { type: 'string', description: 'State key relative to .crew/' },
      ...
      handler: async (args) => {
        ...
        try {
          const key = normalizeStateToolKey(args.key);
          validateMutableStateToolKey(key);
          this.storage.appendSync(path.join(this.crewRoot, key), args.content);
          return {
          ...
          return {
            textResultForLlm: `Failed to append state: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
            error: String(error),
          };
        }
    ...
    const stateDelete = defineTool<StateDeleteRequest>({
      name: 'crew_state_delete',
      description: 'Delete mutable Crew state through the configured state backend. Always use this tool for mutable state when available. Keys are relative to .crew/; static config cannot be deleted through this tool.',
      parameters: {
        type: 'object',
        properties: {
          key: { type: 'string', description: 'State key relative to .crew/' },
      ...
      handler: async (args) => {
        try {
          const key = normalizeStateToolKey(args.key);
          validateMutableStateToolKey(key);
          this.storage.deleteSync(path.join(this.crewRoot, key));
          return {
          ...
          return {
            textResultForLlm: `Failed to delete state: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
            error: String(error),
          };
        }
    ...
    const stateList = defineTool<StateListRequest>({
      name: 'crew_state_list',
      description: 'List mutable Crew state entries through the configured state backend. Directories are relative to .crew/.',
      parameters: {
        type: 'object',
        properties: {
          dir: { type: 'string', description: 'Directory relative to .crew/; omit for root' },
        },
      },
      handler: async (args) => {
        try {
          const dir = normalizeStateToolDir(args.dir);
          const entries = this.storage.listSync(path.join(this.crewRoot, dir));
          return {
          ...
          return {
            textResultForLlm: `Failed to list state: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
            error: String(error),
          };
        }
    ...
    const stateHealth = defineTool<Record<string, never>>({
      name: 'crew_state_health',
      description: 'Report the active Crew state storage layer so agents can verify they are using runtime-owned state instead of manual git/file choreography.',
    ...
    const memoryClassify = defineTool<GovernedMemoryRequest>({
      ...
      description: 'Classify proposed memory with Crew governance policy without persisting content.',
      parameters: {
        type: 'object',
        properties: {
    ...
    const memoryWrite = defineTool<GovernedMemoryRequest>({
      name: 'memory.write',
      description: 'Classify and write governed local Crew memory. External semantic memory requires an explicit configured bridge.',
      parameters: {
        type: 'object',
        properties: {
    ...
    const memorySearch = defineTool<GovernedMemorySearchRequest>({
      name: 'memory.search',
      description: 'Search governed local Crew memory entries.',
      parameters: {
        type: 'object',
        properties: {
    ...
    // crew_status: Query session pool state
    const crewStatus = defineTool<StatusQuery>({
      name: 'crew_status',
    ...
    // crew_skill: Read/write agent skills
    const crewSkill = defineTool<SkillRequest>({
      name: 'crew_skill',
      ...
      handler: async (args) => {
        ...
        try {
          const projectRoot = path.dirname(this.crewRoot);
          // .github/skills/ is the canonical write location (matches crew init/upgrade
          ...
          const legacySkillDir = path.join(this.crewRoot, 'skills', args.skillName);
          ...
          return {
            textResultForLlm: `Failed to ${args.operation} skill: ${sanitizeErrorForLlm(error, this.crewRoot)}`,
            resultType: 'failure',
            error: String(error),
          };
        }
    ...
    // Register all tools
    this.tools.set('crew_route', crewRoute);
    this.tools.set('crew_decide', crewDecide);
    this.tools.set('crew_memory', crewMemory);
    this.tools.set('crew_state_read', stateRead);
    this.tools.set('crew_state_write', stateWrite);
    this.tools.set('crew_state_append', stateAppend);
    this.tools.set('crew_state_delete', stateDelete);
    this.tools.set('crew_state_list', stateList);
    this.tools.set('crew_state_health', stateHealth);
    ...
    this.tools.set('crew_status', crewStatus);
    this.tools.set('crew_skill', crewSkill);
  }

  /** Get all registered tools for session config */
  getTools(): CrewTool<any>[] {
    return Array.from(this.tools.values());
  }

  /** Get tools filtered by agent's allowed tool list */
  getToolsForAgent(allowedTools?: string[]): CrewTool<any>[] {
  ...
  getTool(name: string): CrewTool<any> | undefined {
   ...
   */
  applySkillHandlers(tools: CrewTool<any>[]): void {

/**
 * State Module — CrewState Facade
 *
 * Top-level typed API for reading and writing `.crew/` state.
 * Composes the collection facades (agents, decisions, routing, team)
 * over a pluggable StorageProvider.
 *
 * @module state/crew-state
 */
...
export class CrewState {
  ...
  /** The project root directory this CrewState is bound to. */
  get root(): string {
  ...
  /**
   * Factory method — validates that `rootDir/.crew/` exists before
   * returning a new CrewState instance.
   */
  static async create(
    storage: StorageProvider,
    rootDir: string,
  ): Promise<CrewState> {
    const crewDir = `${rootDir}/.crew`;
    const exists = await storage.exists(crewDir);
    if (!exists) {
      throw new NotFoundError('crew', rootDir);
    }
    return new CrewState(storage, rootDir);
  }

  /**
   * Synchronous factory — creates CrewState without validating `.crew/`
  ...
  /** Check if a `.crew/` directory exists at the root. */
  async isInitialized(): Promise<boolean> {
    return this.storage.exists(`${this._rootDir}/.crew`);
  }
}

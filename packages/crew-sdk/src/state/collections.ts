 ...
 * `.crew/` collection, backed by StorageProvider + IO layer.
 *
...
export class AgentsCollection {
  ...
  /** List agent names from `.crew/agents/`. */
  async list(): Promise<string[]> {
    const agentsDir = `${this.rootDir}/.crew/agents`;
  ...
  async create(name: string, charter: string): Promise<void> {
    const agentDir = `${this.rootDir}/.crew/agents/${name}`;
...
export class SkillsCollection {
  ...
  /** List all skill IDs (directory names under .crew/skills/). */
  async list(): Promise<string[]> {
    const skillsDir = `${this.rootDir}/.crew/skills`;
...
export class TemplatesCollection {
  ...
  /** List template filenames under .crew/templates/. */
  async list(): Promise<string[]> {
    const templatesDir = `${this.rootDir}/.crew/templates`;
...
/** Serializable subset of config stored in `.crew/config.json`. */
...
export class ConfigCollection {
  ...
  /** Read and parse `.crew/config.json`. Returns defaults when file is missing or invalid. */
...
export class LogCollection {
  ...
  /** List log entry filenames under .crew/log/. */
  async list(): Promise<string[]> {

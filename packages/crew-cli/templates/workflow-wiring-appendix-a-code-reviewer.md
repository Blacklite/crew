...
> End-to-end example of adding a code reviewer to your crew and wiring their gate so it actually gets enforced. This walkthrough addresses a common failure: a reviewer is on the roster but never reviews a single PR because the gate wasn't wired.
...
Because the routing table says WHO handles what — it's for incoming requests ("review PR #42"). It does NOT say "after every agent completes work, route their output to {ReviewerName}." The coordinator routes work TO agents, but nothing tells it to route COMPLETED work to a reviewer. The "After Agent Work" flow in `crew.agent.md` says: collect results → present → spawn Scribe. No review step.
...
Most crews get layers 1 and 2 right. Layer 3 — enforcement — is what's usually missing.

## Step-by-Step Walkthrough

### Step 1: Create the reviewer's identity

Create `.crew/agents/{name}/charter.md`:
...
Create `.crew/agents/{name}/history.md` seeded with project context.

### Step 2: Add to team.md roster

```markdown
| 👑 {Name} | Code Reviewer | `.crew/agents/{name}/charter.md` | ✅ Active |
```
...
In `.crew/templates/issue-lifecycle.md`, the "Coordinator Post-Work Steps" section should reference your reviewer by name:

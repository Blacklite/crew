...
tools:
  ...
  - name: "github-mcp-server-search_issues"
    description: "Search for candidate issues or prior crew responses"
...
- Every action is logged to `.crew/comms/audit/`.
...
- Filter for items with **no crew team response**.
- Limit to items created in the last 7 days.
- Exclude items labeled `crew:internal` or `wontfix`.
...
| "How do I...?", "Can Crew...?", "Is there a way to...?" | Feature Guidance | T3 |
| Wrong repo, out of scope for Crew | Redirect | T4 |
...
|-----------|----------|---------|
| 🟢 High | Answer exists in Crew docs or FAQ, similar question answered before, no technical ambiguity | "How do I install Crew?" |
| 🟡 Medium | Technical answer is sound but involves judgment calls, OR docs exist but don't perfectly match the question, OR tone is tricky | "Can Crew work with Azure DevOps?" (yes, but setup is nuanced) |
| 🔴 Needs Review | Technical uncertainty, policy/roadmap question, potential reputational risk, author is frustrated/angry, question about unreleased features | "When will Crew support Claude?" |
...
- Location: `.crew/comms/audit/{timestamp}.md`
- Required fields vary by action — see `.crew/comms/templates/audit-entry.md` Conditional Fields table
...
| # | Item | Author | Type | Confidence | Read | Preview |
|---|------|--------|------|------------|------|---------|
| 1 | Issue #426 | @newdev | Welcome | 🟢 | 1/1 | "Hey @newdev! Welcome to Crew..." |
...
Hey @newdev! Welcome to Crew 👋 Thanks for opening this.
...
https://github.com/Blacklite/crew/issues/426#issuecomment-123456
```

### T1 — Welcome

```text
Hey {author}! Welcome to Crew 👋 Thanks for opening this.
...
Thanks for reporting this, {author} — it made Crew better.
```
...
- ❌ Responding to issues labeled `crew:internal` or `wontfix`
- ❌ Skipping audit logging (every action must be recorded)
- ❌ Drafting for issues where a crew member already responded (avoid duplicates)

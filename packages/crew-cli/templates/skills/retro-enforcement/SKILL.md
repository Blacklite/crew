...
This skill addresses a specific, measured failure mode: **0% completion rate on markdown retro action items across 6 consecutive retrospectives**. GitHub Issues have an 85%+ completion rate in the same crew. The format was the problem, not the people.
...
The function checks `.crew/log/` for any file matching `*retrospective*` dated within the last `$WindowDays` days (default: 7). If none is found, the retro is overdue.

**File naming convention:** `.crew/log/{ISO8601-timestamp}-retrospective.md`

Example: `.crew/log/2026-03-24T14-45-00Z-retrospective.md`
...
3. **Wait for the log file** to be written to `.crew/log/`
...
- Weekly crews: window = 7 days
- Bi-weekly crews: window = 14 days
...
function Invoke-RoundStart {
    ...
    if (Test-RetroOverdue -LogDir "$RepoRoot/.crew/log" -WindowDays 7) {
...
| **Dependencies** | `.crew/log/` directory, GitHub Issues API |

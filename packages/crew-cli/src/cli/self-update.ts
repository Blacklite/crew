 ...
 * version of @blacklite/crew-cli and displays a passive banner when
 * an update is available. Results are cached for 24 hours.
 *
 * Disable with: CREW_NO_UPDATE_CHECK=1
 *
...
const PACKAGE_NAME = '@blacklite/crew-cli';
...
/** Directory for crew CLI cache files. */
function getCacheDir(): string {
  const base = process.env.APPDATA
    ?? (process.platform === 'darwin'
      ? path.join(os.homedir(), 'Library', 'Application Support')
      : path.join(os.homedir(), '.config'));
  return path.join(base, 'crew-cli');
}

/** Path to the update-check cache file (shared with `crew update-check`). */
...
export async function notifyIfUpdateAvailable(currentVersion: string): Promise<void> {
  try {
    // Respect opt-out
    if (process.env.CREW_NO_UPDATE_CHECK === '1') return;
    ...
    if (compareVersions(latest, currentVersion) > 0) {
      console.log(
        `\n${YELLOW}⚡${RESET} ${BOLD}Crew v${latest}${RESET} available ${DIM}(you have v${currentVersion})${RESET}` +
        `\n   Run: ${BOLD}npm install -g @blacklite/crew-cli@latest${RESET}\n`,
      );
    }
  } catch {

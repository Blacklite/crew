/**
 * crew link <team-repo-path> — link a project to a remote team root.
 *
 * Writes `.crew/config.json` with a relative `teamRoot` path so the
 * dual-root resolver (resolveCrewPaths) can find the team identity dir.
 *
...
import { FSStorageProvider, clearResolveCrewCache } from '@blacklite/crew-sdk';
...
export function runLink(projectDir: string, teamRepoPath: string): void {
  ...
  // Validate the target contains a .crew/ or .ai-team/ directory
  const hasCrew = storage.existsSync(path.join(absoluteTeam, '.crew'));
  const hasAiTeam = storage.existsSync(path.join(absoluteTeam, '.ai-team'));
  if (!hasCrew && !hasAiTeam) {
    fatal(`Target does not contain a .crew/ directory: ${absoluteTeam}`);
  }

  // Ensure .crew/ exists locally
  ...
  // Link just (re)created `.crew/` and wrote config.json. Any subsequent
  // code in this process that calls resolveCrew()/resolveCrewPaths()

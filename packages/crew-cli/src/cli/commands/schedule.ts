/**
 * Schedule command — manage Crew scheduled tasks (#296)
 *
 * Subcommands:
 *   crew schedule list     — show configured schedules
 *   crew schedule run <id> — manually trigger a scheduled task
 *   crew schedule init     — create a default schedule.json
 *   crew schedule status   — show last run times and next due
 */
...
import type { ScheduleManifest, ScheduleState, ScheduleEntry, ScheduleProvider } from '@blacklite/crew-sdk/runtime/scheduler';

// Re-export so tests can mock through this module
async function loadSchedulerModule() {
  return import('@blacklite/crew-sdk/runtime/scheduler');
}

function resolveSchedulePath(cwd: string): string {
  const crewInfo = detectCrewDir(cwd);
  return path.join(crewInfo.path, 'schedule.json');
}

function resolveStatePath(cwd: string): string {
  const crewInfo = detectCrewDir(cwd);
  return path.join(crewInfo.path, '.schedule-state.json');
}

/**
 * Entry point for `crew schedule` subcommands.
 */
export async function runSchedule(cwd: string, subcommand: string, args: string[]): Promise<void> {
  switch (subcommand) {
    ...
    case 'run': {
      const id = args[0];
      if (!id) {
        fatal('Usage: crew schedule run <id>');
      }
...
async function scheduleList(cwd: string): Promise<void> {
  ...
  try {
    ...
    fatal(`No schedule.json found — run 'crew schedule init' to create one`);
  }
...
async function scheduleRun(cwd: string, id: string): Promise<void> {
  ...
  if (!entry) {
    fatal(`Schedule '${id}' not found. Run 'crew schedule list' to see available schedules.`);
  }
...
async function scheduleInit(cwd: string): Promise<void> {
  ...
  const crewInfo = detectCrewDir(cwd);
  if (!storage.existsSync(crewInfo.path)) {
    storage.mkdirSync(crewInfo.path, { recursive: true });
  }
  ...
  info(`Edit it to configure your scheduled tasks, then run 'crew schedule list' to verify.`);
}

/**
 * crew preset — manage crew presets (curated agent collections)
 *
 * Presets are saved to CREW_HOME/presets/ (default: ~/.crew/presets/).
 * Each preset is a directory with a preset.json manifest + agents/ charters.
 *
 * Subcommands:
 *   crew preset list           — list available presets
 *   crew preset show <name>    — show preset details
 *   crew preset apply <name>   — install preset agents into current crew
 *   crew preset save <name>    — save current project agents as a preset
 *   crew preset install <src>  — install a preset from a GitHub URL or local path
 *   crew preset init           — initialize presets directory in crew home
 *
 * Note: Presets capture agents only (charters). For full crew snapshots
 * including casting state, skills, and routing rules — e.g. to share a
 * configured crew or publish to an agent toolbox — use `crew export`.
 *
...
import { listPresets, loadPreset, applyPreset, savePreset, seedBuiltinPresets, installPresetFromSource } from '@blacklite/crew-sdk/presets';
import { resolveCrew } from '@blacklite/crew-sdk/resolution';
import { success, warn, info, BOLD, RESET, DIM } from '../core/output.js';
import { fatal } from '../core/errors.js';

/**
 * Entry point for `crew preset` subcommands.
 */
export async function runPreset(cwd: string, subcommand: string, args: string[]): Promise<void> {
  switch (subcommand) {
    ...
    case 'show': {
      const name = args[0];
      if (!name) {
        fatal('Usage: crew preset show <name>');
      }
    ...
    case 'apply': {
      const name = args[0];
      if (!name) {
        fatal('Usage: crew preset apply <name> [--force]');
      }
    ...
    case 'save': {
      const name = args[0];
      if (!name) {
        fatal('Usage: crew preset save <name> [--force] [--description "..."]');
      }
      const force = args.includes('--force');
    ...
    case 'install': {
      const source = args[0];
      if (!source) {
        fatal('Usage: crew preset install <source> [--name <override>] [--force]\n' +
              '  <source> can be:\n' +
      ...
      if (nameIdx >= 0) {
        // Defend against `crew preset install <src> --name` (no value) and
        // `--name --force` (value is another flag) — both currently produce
        // confusing downstream errors. Fail fast with a clear usage hint
        // per review on Blacklite/crew#1225.
        const candidate = args[nameIdx + 1];
        if (!candidate || candidate.startsWith('-')) {
          fatal('`--name` requires a value. Usage: crew preset install <source> --name <override>');
        }
    ...
    default:
      fatal(
        ...
        `  crew preset save <name>\n` +
        `  crew preset install <source> [--name <override>] [--force]\n` +
...
async function presetInit(remote: boolean): Promise<void> {
  ...
  info(`  Run 'crew preset list' to see available presets.`);
  console.log();
  info(`${DIM}Tip: Run 'crew preset init --remote' to back your crew home`);
...
async function presetInitRemote(): Promise<void> {
  ...
  const homeDir = envHome ? path.resolve(envHome) : path.join(os.homedir(), '.crew');
  const repoName = 'crew-home';
  ...
  // Check if CREW_HOME is already a git repo
  if (fs.existsSync(path.join(homeDir, '.git'))) {
    info(`Crew home is already a git repo: ${homeDir}`);
    const seeded = seedBuiltinPresets();
    if (seeded.length > 0) {
      info(`  Built-in presets installed: ${seeded.join(', ')}`);
    }
    success('Crew home ready — presets roam via git push/pull.');
    return;
  }

  // If ~/.crew/ doesn't exist yet, try to clone existing remote repo
  if (!fs.existsSync(homeDir)) {
    ...
    if (repoExists) {
      // Second machine — clone existing crew home
      ...
      try {
        ...
        success(`Crew home cloned from ${repoFullName}`);
        info(`  Path: ${homeDir}`);
    ...
    try {
      ...
      execSync(`gh repo create ${repoName} --private --source "${homeDir}" --push --description "Crew home — presets and config"`, {
        stdio: 'inherit',
      });
    } catch {
    ...
    // ~/.crew/ exists but isn't a git repo — init and connect
    info(`Initializing git in existing crew home: ${homeDir}`);
    execSync(`git init`, { cwd: homeDir, stdio: 'pipe' });
  ...
  try {
    ...
    execSync(`git -C "${homeDir}" commit -m "Initialize crew home with presets"`, { stdio: 'pipe' });
    execSync(`git -C "${homeDir}" push -u origin main`, { stdio: 'pipe' });
  } catch {
    // May fail if nothing to commit or push
  }

  success(`Crew home initialized with private repo: ${repoFullName}`);
  info(`  Path: ${homeDir}`);
  if (seeded.length > 0) {
    info(`  Built-in presets installed: ${seeded.join(', ')}`);
  }
  ...
  info(`  ${BOLD}crew preset init --remote${RESET}`);
}
...
async function presetList(): Promise<void> {
  ...
  if (!presetsDir) {
    ...
    info('  Run `crew preset init --remote` to set up with a GitHub repo (recommended).');
    info('  Or `crew preset init` for local-only setup.');
    return;
  }
...
async function presetShow(name: string): Promise<void> {
  ...
  if (!preset) {
    fatal(`Preset '${name}' not found. Run 'crew preset list' to see available presets.`);
  }
...
async function presetApply(cwd: string, name: string, force: boolean): Promise<void> {
  // Find target crew directory
  const crewDir = resolveCrew(cwd);
  if (!crewDir) {
    fatal('No .crew/ directory found. Run `crew init` first, or use from a repo with a crew.');
  }

  const targetAgentsDir = path.join(crewDir, 'agents');
...
async function presetSave(cwd: string, name: string, force: boolean, description?: string): Promise<void> {
  const crewDir = resolveCrew(cwd);
  if (!crewDir) {
    fatal('No .crew/ directory found. Initialize a crew first with `crew init`.');
  }

  try {
    const destDir = savePreset(name, crewDir, { force, description });
    success(`Preset '${name}' saved`);
    info(`  Location: ${destDir}`);
    info(`  Use it in any project: crew preset apply ${name}`);
    console.log();
    info(`${DIM}Tip: Presets save agents only (charters). For a full crew snapshot`);
    info(`including casting state, skills, and routing rules — e.g. to share`);
    info(`a configured crew or publish to an agent toolbox — use 'crew export'.${RESET}`);
  } catch (err) {
    fatal(String(err));
  }
}
...
/**
 * Install a preset from a remote URL or local path into $CREW_HOME/presets/<name>/.
 * After install, the preset is available to `crew preset apply <name>`.
 */
async function presetInstall(source: string, nameOverride: string | undefined, force: boolean): Promise<void> {
  ...
  try {
    ...
    console.log();
    info(`  Apply in a project:  ${BOLD}crew preset apply ${result.installedName}${RESET}`);
    info(`  Show details:        ${BOLD}crew preset show ${result.installedName}${RESET}`);
    info(`  Use during init:     ${BOLD}crew init --preset ${result.installedName}${RESET}`);
  } catch (err) {

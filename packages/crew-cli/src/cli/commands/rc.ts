/**
 * Crew Remote Control — CLI Command
 *
 * `crew rc` or `crew remote-control`
...
import { FSStorageProvider, RemoteBridge } from '@blacklite/crew-sdk';

const storage = new FSStorageProvider();
import type { RemoteBridgeConfig } from '@blacklite/crew-sdk';
 ...
 * marker when present (#1398). The passed `crewDir` stays the local
 * working-tree dir — only the team.md read is redirected.
 */
export function loadRosterAgents(crewDir: string): Array<{ name: string; role: string }> {
...
export async function runRC(cwd: string, options: RCOptions): Promise<void> {
  ...
  // Resolve crew directory
  const crewDir = storage.existsSync(path.join(cwd, '.crew'))
    ? path.join(cwd, '.crew')
    : storage.existsSync(path.join(cwd, '.ai-team'))
      ? path.join(cwd, '.ai-team')
      : '';

  console.log(`\n${BOLD}🎮 Crew Remote Control${RESET}\n`);
  console.log(`  ${DIM}Repo:${RESET}    ${repo}`);
  console.log(`  ${DIM}Branch:${RESET}  ${branch}`);
  console.log(`  ${DIM}Machine:${RESET} ${machine}`);
  console.log(`  ${DIM}Crew:${RESET}   ${crewDir || 'not found'}\n`);

  // Load team roster if crew dir exists
  const agents: Array<{name: string; role: string}> = [];
  if (crewDir) {
    try {
      agents.push(...loadRosterAgents(crewDir));
  ...
  const config: RemoteBridgeConfig = {
    ...
    onCommand: (name) => {
      ...
      if (name === 'status') {
        bridge.addMessage('system', `Crew RC | Repo: ${repo} | Branch: ${branch} | Agents: ${agents.length} | Copilot: ${copilotReady ? 'passthrough' : 'off'} | Connections: ${bridge.getConnectionCount()}`);
      } else if (name === 'agents') {

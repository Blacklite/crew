...
import { withClaudeInvocationArgs } from '../core/claude-invocation.js';
import { resolveHarness } from '@blacklite/crew-sdk/client';
import {
  CapabilityRegistry,
...
export interface LoopConfig {
  ...
  agentCmd?: string;
  /** Agent harness: 'copilot' (default) or 'claude'. */
...
function buildLoopAgentCommand(
  prompt: string,
  options: { agentCmd?: string; copilotFlags?: string; teamRoot?: string; harness?: 'copilot' | 'claude' },
  ...
  if (options.harness === 'claude') {
    return { cmd: 'claude', args: withClaudeInvocationArgs('claude', ['-p', prompt], options.teamRoot) };
  }
  const args = ['-p', prompt];
...
async function checkClaudeCli(): Promise<void> {
  return new Promise<void>((resolve, reject) => {
    execFile('claude', ['--version'], { shell: process.platform === 'win32', timeout: 5000 }, (err) => {
      if (err) reject(err);
      else resolve();
    });
  });
...
export async function runLoop(dest: string, options: LoopConfig): Promise<void> {
  ...
  // Detect crew directory (must exist) — follows external state if configured
  const { local: crewDirInfo, stateDir } = effectiveCrewDir(workTreeRoot);
  ...
  if (!existsSync(loopFilePath)) {
    console.log(`\n💤 No loop.md found. Create one with: ${BOLD}crew loop --init${RESET}`);
    return;
  }
  ...
  const description = frontmatter.description ?? (isOnboarding ? 'Crew Onboarding' : 'Crew Loop');
  ...
  // Resolve harness (flag > CREW_HARNESS env > .crew/config.json > copilot)
  const harness = resolveHarness({ explicit: options.harness, teamRoot: dest });

  // Preflight: verify the harness CLI is available (skip if user overrides the agent command)
  if (!options.agentCmd) {
    if (harness === 'claude') {
      try {
        await checkClaudeCli();
      } catch {
        fatal('Claude Code CLI required for --harness claude. Install with `npm install -g @anthropic-ai/claude-code`');
      }
    } else {
      try {
  ...
  // Startup banner
  console.log(`\n${BOLD}🔄 Crew Loop${RESET} — ${description}`);

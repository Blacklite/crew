...
import { resolveCrewState } from '@blacklite/crew-sdk';
...
const MCP_TOOL_ALIASES: Record<string, string> = {
  crew_decide: 'crew_decide',
  crew_state_read: 'crew_state_read',
  crew_state_write: 'crew_state_write',
  crew_state_append: 'crew_state_append',
  crew_state_delete: 'crew_state_delete',
  crew_state_list: 'crew_state_list',
  crew_state_health: 'crew_state_health',
...
export function createStateMcpToolRegistry(startDir: string): ToolRegistry {
  const context = resolveCrewState(startDir);
  if (!context) {
    throw new Error(`No .crew directory found from ${startDir}`);
  }
...
export function createStateMcpSession(
  ...
  const tools = Object.entries(MCP_TOOL_ALIASES).map(([mcpName, runtimeName]) => {
    ...
    if (!tool) throw new Error(`Missing Crew runtime state tool: ${runtimeName}`);
    ...
    try {
      switch (request.method) {
        ...
        case 'tools/call': {
          ...
          if (!entry) {
            write({ jsonrpc: '2.0', id: request.id ?? null, error: { code: -32602, message: `Unknown Crew state tool: ${name}` } });
            break;
          }
...
export async function runStateMcp(startDir = process.cwd()): Promise<void> {
  ...
  server.setRequestHandler(CallToolRequestSchema, async request => {
    ...
    if (!entry) {
      throw new Error(`Unknown Crew state tool: ${request.params.name}`);
    }
...
export function printStateMcpHelp(): void {
  stderr.write([
    'crew state-mcp — MCP bridge for Crew runtime state tools',
    '',
    'Usage: crew state-mcp',
    '',
    'This command is intended to be launched by GitHub Copilot CLI through',
    '.copilot/mcp-config.json. It exposes crew_decide and state.* tools',
    'backed by the configured Crew state backend.',
    '',
  ].join('\n'));
}

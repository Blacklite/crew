/**
 * `crew update-check` — expose cached CLI update status for tooling.
 *
...
function printHuman(result: UpdateCheckResult): void {
  ...
  if (!result.latest) {
    console.log('No cached update status yet. Run `crew update-check --refresh` to check now.');
    return;
  }
  console.log(`Latest:  ${result.latest}`);
  if (result.updateAvailable) {
    console.log('Update available. Run `crew upgrade --self` to install.');
  } else {
...
/**
 * CLI entry point for `crew update-check`.
...
export async function runUpdateCheckCommand(args: string[]): Promise<number> {
  ...
  if (process.env.CREW_NO_UPDATE_CHECK === '1') {
    if (json) console.log('{}');
    return 0;
  }

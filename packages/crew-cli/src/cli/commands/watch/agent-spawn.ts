 ...
 *
 * @see https://github.com/Blacklite/crew/issues/920
 * @see https://github.com/Blacklite/crew/issues/923
 */
...
export function buildAgentCommand(
  ...
  // Claude harness: spawn the Claude Code CLI with crew's MCP + permission args
  if (context.harness === 'claude') {
    return { cmd: 'claude', args: withClaudeInvocationArgs('claude', ['-p', prompt], context.teamRoot) };
  }
 ...
 * `--additional-mcp-config`/`--yolo` workaround injected so `crew_state_*`

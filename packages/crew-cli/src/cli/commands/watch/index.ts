...
import {
  ...
  triageIssue,
  type TriageIssue,
} from '@blacklite/crew-sdk/ralph/triage';
import { RalphMonitor } from '@blacklite/crew-sdk/ralph';
...
import type { MachineCapabilities } from '@blacklite/crew-sdk/ralph/capabilities';
import {
  PredictiveCircuitBreaker,
  getTrafficLight,
} from '@blacklite/crew-sdk/ralph/rate-limiting';
import { createPlatformAdapter } from '@blacklite/crew-sdk/platform';
import type { PlatformAdapter, WorkItem, PullRequest as SdkPullRequest } from '@blacklite/crew-sdk/platform';
...
async function checkPRs(roster: ReturnType<typeof parseRoster>, adapter: PlatformAdapter, vlog?: VerboseLogger): Promise<PRBoardState> {
  ...
  const crewPRs = prs.filter(pr =>
    pr.labels.some(l => l.name.startsWith('crew')) || pr.headRefName.startsWith('crew/'),
  );

  vlog?.log(`PRs found: ${prs.length} total`);
  for (const pr of crewPRs) {
  ...
  const drafts = crewPRs.filter(pr => pr.isDraft);
  const changesRequested = crewPRs.filter(pr => pr.reviewDecision === 'CHANGES_REQUESTED');
  const approved = crewPRs.filter(pr => pr.reviewDecision === 'APPROVED' && !pr.isDraft);
  const ciFailures = crewPRs.filter(pr =>
  ...
  const needsReview = crewPRs.filter(pr =>
...
async function runCheck(
  ...
  const timestamp = new Date().toLocaleTimeString();
  try {
    const issues = await listWatchWorkItems(adapter, { label: 'crew', state: 'open', limit: 20 });
    ...
    const { filterByCapabilities } = await import('@blacklite/crew-sdk/ralph/capabilities');
    ...
    if (hasCopilot && autoAssign) {
      try {
        const copilotIssues = await listWatchWorkItems(adapter, { label: 'crew:copilot', state: 'open', limit: 10 });
...
interface SubCrew { name: string; dir: string; labels: string[] }

function discoverSubCrews(teamRoot: string): SubCrew[] {
  const subcrewDir = path.join(teamRoot, '.crew', 'subcrews');
  if (!storage.existsSync(subcrewDir)) return [];
  try {
    const entries = storage.listSync?.(subcrewDir) ?? [];
    const dirs = Array.isArray(entries) ? entries : [];
    const crews: SubCrew[] = [];
    for (const entry of dirs) {
      const entryPath = path.join(subcrewDir, entry);
      ...
      crews.push({ name: entry, dir: entryPath, labels });
    }
    return crews;
  } catch { return []; }
}
...
function loadCBState(crewDir: string): CircuitBreakerState {
  const filePath = path.join(crewDir, 'ralph-circuit-breaker.json');
  try {
...
function saveCBState(crewDir: string, state: CircuitBreakerState): void {
  storage.writeSync(
    path.join(crewDir, 'ralph-circuit-breaker.json'),
...
export async function runWatch(dest: string, options: WatchOptions | WatchConfig): Promise<void> {
  ...
  // Detect crew directory — follows external state if configured
  const { local: crewDirInfo, stateDir } = effectiveCrewDir(dest);
  const teamMd = path.join(stateDir, 'team.md');
  const routingMdPath = path.join(stateDir, 'routing.md');
  const teamRoot = path.dirname(crewDirInfo.path);
  ...
  const { loadCapabilities } = await import('@blacklite/crew-sdk/ralph/capabilities');
  ...
  // Pre-create crew member labels so addTag never fails on missing labels
  if (adapter.ensureTag) {
    for (const member of roster) {
      try {
        await adapter.ensureTag(member.label, { color: 'd4c5f9', description: `Crew triage: ${member.name}` });
      } catch { /* best-effort — continue if label creation fails */ }
    }
    try {
      await adapter.ensureTag('crew:copilot', { color: 'd4c5f9', description: 'Crew triage: Copilot coding agent' });
    } catch { /* best-effort */ }
    console.log(`${DIM}Labels: ensured ${roster.length + 1} crew labels exist${RESET}`);
  }
  ...
  const monitor = new RalphMonitor({
    ...
    statePath: path.join(crewDirInfo.path, '.ralph-state.json'),
  });
  ...
  // Load external capabilities from .crew/capabilities/
  ...
  console.log(`${DIM}Polling every ${interval} minute(s) for crew work. Ctrl+C to stop.${RESET}`);
  ...
  let cbState = loadCBState(crewDirInfo.path);
  let round = 0;
  ...
  async function executeRound(): Promise<void> {
    ...
    if (cbState.status === 'open') {
      ...
      saveCBState(crewDirInfo.path, cbState);
    }
    ...
    // Phase 1: pre-scan (self-pull, subcrew discovery)
    await runPhase('pre-scan', enabledCapabilities, roundContext, config);

    // SubCrew discovery (informational, not a capability)
    const subCrews = discoverSubCrews(teamRoot);
    if (subCrews.length > 0 && round === 1) {
      console.log(`${DIM}📂 Discovered ${subCrews.length} subcrew(s): ${subCrews.map(s => s.name).join(', ')}${RESET}`);
    }

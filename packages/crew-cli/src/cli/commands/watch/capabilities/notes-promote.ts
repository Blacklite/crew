 ...
 * `two-layer`, enumerates `refs/notes/crew/*` and promotes flagged notes
...
import { FSStorageProvider, resolveStateBackend, TwoLayerBackend } from '@blacklite/crew-sdk';
...
/** List `refs/notes/crew/*` short names present in the repo. */
...
export class NotesPromoteCapability implements WatchCapability {
  ...
  async preflight(context: WatchContext): Promise<PreflightResult> {
    const crewDir = path.join(context.teamRoot, '.crew');
    if (!storage.existsSync(crewDir)) {
      return { ok: false, reason: '.crew/ directory not found' };
    }
  ...
  async execute(context: WatchContext): Promise<CapabilityResult> {
    ...
    try {
      const resolved = resolveStateBackend(crewDir, repoRoot);
    ...
    const refs = listCrewNotesRefs(repoRoot);
    if (refs.length === 0) {
      return { success: true, summary: 'notes-promote: no crew notes refs' };
    }

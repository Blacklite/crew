...
/** Check whether an issue carries a crew or crew:* label. */
function hasCrewLabel(issue: ExecutableWorkItem): boolean {
  return issue.labels.some(l => l.name === 'crew' || l.name.startsWith('crew:'));
}
...
export function findExecutableIssues(
  ...
  return issues.filter(
    issue => hasCrewLabel(issue) && !isAssigned(issue) && !hasBlockingLabel(issue),
  );
}
...
export function buildAgentPrompt(
  ...
  const hasInstructions = existsSync(path.join(teamRoot, '.crew', 'ralph-instructions.md'));

  if (hasInstructions) {
    return [
      'Ralph, Go! Read .crew/ralph-instructions.md for your full instructions. Follow ALL sections there. MAXIMIZE PARALLELISM — spawn agents for ALL actionable issues simultaneously.',
      '',
      'Here are the current open crew issues:',
      issueList,
      '',
      'Task: Read the issues, follow your instructions in .crew/ralph-instructions.md, and work on what\'s actionable.',
      'WHY: Keep the crew pipeline moving — no idle work.',
  ...
  return [
    'You are Ralph, the autonomous work monitor. Review the open crew issues below and work on every actionable one. Skip issues that are blocked, waiting on external input, or already assigned.',
    '',
    'Here are the current open crew issues:',
    issueList,
    '',
...
export class ExecuteCapability implements WatchCapability {
  ...
  async execute(context: WatchContext): Promise<CapabilityResult> {
    ...
    try {
      ...
      const sdkItems = await context.adapter.listWorkItems({ tags: ['crew'], state: 'open', limit: 50 });
      ...
      // Minimal filter: must have crew or crew:* label (agent decides the rest)
      ...
      if (eligible.length === 0) {
        return { success: true, summary: 'no crew-labeled issues found' };
      }

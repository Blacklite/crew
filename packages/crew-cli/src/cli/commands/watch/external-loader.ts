/**
 * External capability loader — scans .crew/capabilities/ for user-defined
...
/**
 * Load external WatchCapability modules from `{teamRoot}/.crew/capabilities/`.
 *
...
export async function loadExternalCapabilities(
  ...
  const capDir = join(teamRoot, '.crew', 'capabilities');
  ...
  console.log(
    `\n${YELLOW}${BOLD}⚠️  SECURITY: Loading ${jsFiles.length} external capability file(s) from .crew/capabilities/${RESET}`,
  );

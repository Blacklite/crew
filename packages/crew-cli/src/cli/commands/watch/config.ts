 ...
 * Watch config loader.
 *
 * Priority: CLI flag > .crew/config.json "watch" section > defaults.
 */
...
export interface WatchConfig {
  ...
  agentCmd?: string;
  /** Agent harness for spawned sessions: 'copilot' (default) or 'claude'. */
  ...
  stateContext?: CrewStateContext | null;
}
...
/**
 * Load watch config from `.crew/config.json` then merge CLI overrides.
 *
 * @param teamRoot   - Root directory containing `.crew/`.
...
export function loadWatchConfig(
  ...
  try {
    const configPath = path.join(teamRoot, '.crew', 'config.json');
  ...
  const merged: WatchConfig = {
    ...
    harness: cliOverrides.harness ?? fileConfig.harness ?? DEFAULTS.harness,
...
function normalizeFileConfig(raw: Record<string, unknown>): Partial<WatchConfig> {
  ...
  if (raw['harness'] === 'copilot' || raw['harness'] === 'claude') result.harness = raw['harness'];

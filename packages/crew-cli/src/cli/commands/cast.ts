/**
 * crew cast — show current session cast (project + personal agents merged)
 *
 * Displays the merged session cast with project agents and personal agents,
 * highlighting which agents come from the personal crew.
 *
 * @module cli/commands/cast
 */

import * as path from 'node:path';
import { LocalAgentSource } from '@blacklite/crew-sdk/config/agent-source';
import { resolvePersonalAgents, mergeSessionCast } from '@blacklite/crew-sdk/agents/personal';
import { resolveCrewPaths, resolveExternalStateDir } from '@blacklite/crew-sdk/resolution';
...
export async function runCast(cwd: string): Promise<void> {
  ...
  // LocalAgentSource appends .crew/agents to its base path, so we must supply:
  //   - local mode: parent of paths.projectDir (the repo root)
  //   - remote mode: paths.teamDir (the team repo root, which itself contains .crew/agents)
  const agentBase =
    paths.mode === 'remote'
      ? paths.teamDir
      : path.resolve(paths.projectDir, '..');
  // #1399: when state is externalized, agents live at <externalStateDir>/agents
  // (no .crew nesting), so the base-path probing above can't reach them —

 ...
 * - worktree → orphan
 * - worktree → two-layer
 * - orphan ↔ two-layer (both write to the crew-state orphan branch via different layers)
 *
 * In addition to flipping the `stateBackend` key in `.crew/config.json`, this
 * function MIGRATES pre-existing working-tree state (`decisions.md`,
 * `agents/<name>/history.md`) onto the crew-state orphan branch when moving
...
import { addCrewStateGitignoreBlock, removeCrewStateGitignoreBlock } from '@blacklite/crew-sdk';
...
/** Paths inside .crew/ that should be carried over to the orphan branch on migration. */
const MIGRATABLE_PATHS = [
  'decisions.md',
];

/** Best-effort: ensure the crew-state orphan branch exists. Returns true on success. */
export function ensureOrphanBranch(dest: string): boolean {
  try {
    ...
    try {
      ...
      console.log(`${YELLOW}⚠ Could not create crew-state branch: ${err instanceof Error ? err.message : err}${RESET}`);
      return false;
    }
  }
}
 ...
 * Returns array of {path, content} pairs (paths are relative to .crew/).
 */
function collectWorktreeState(dest: string): Array<{ relPath: string; content: string }> {
  ...
  for (const p of MIGRATABLE_PATHS) {
    const full = path.join(crewDir, p);
...
/**
 * Write a set of files onto the crew-state orphan branch via git plumbing.
...
function writeFilesToOrphanBranch(dest: string, files: Array<{ relPath: string; content: string }>): number {
  ...
  try {
    // Load the current crew-state tree into the index using a temporary index file
    const gitDir = execFileSync('git', ['rev-parse', '--git-dir'], {
      cwd: dest, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'],
    }).trim();
    const indexFile = path.resolve(dest, gitDir, 'crew-migrate-index');
    if (fs.existsSync(indexFile)) fs.unlinkSync(indexFile);

    const env = { ...process.env, GIT_INDEX_FILE: indexFile };

    // Seed index with existing crew-state tree
    execFileSync('git', ['read-tree', 'refs/heads/crew-state'], {
    ...
    const parentSha = execFileSync('git', ['rev-parse', 'refs/heads/crew-state'], {
      cwd: dest, encoding: 'utf-8', stdio: ['pipe', 'pipe', 'pipe'],
    }).trim();
    ...
    execFileSync('git', ['update-ref', 'refs/heads/crew-state', commitHash, parentSha], {
      cwd: dest, stdio: ['pipe', 'pipe', 'pipe'],
    });
    ...
    console.log(`${YELLOW}⚠ Could not migrate state onto crew-state branch: ${err instanceof Error ? err.message : err}${RESET}`);
    return -1;
  }
}

/**
 * Migrate the state backend for an existing crew project.
 *
 * Fixes:
 ...
 *     onto the crew-state orphan branch so post-upgrade agents see prior state.
...
export async function migrateStateBackend(dest: string, target: string): Promise<void> {
  ...
  const configPath = path.join(dest, '.crew', 'config.json');
  ...
  if (ORPHAN_BACKENDS.has(target)) {
    ...
    console.log(`  ${GREEN}✓${RESET} crew-state branch ready`);
  }
  ...
  if (WORKTREE_BACKENDS.has(current) && ORPHAN_BACKENDS.has(target)) {
    ...
    if (files.length > 0) {
      ...
      if (wrote > 0) {
        console.log(`  ${GREEN}✓${RESET} migrated ${wrote} state file(s) onto crew-state branch:`);
        for (const f of files) {
          console.log(`      ${DIM}.crew/${f.relPath}${RESET}`);
        }
        ...
        for (const f of files) {
          const full = path.join(crewDir, f.relPath);
          try {
        ...
        // Clean up now-empty agent directories (e.g. .crew/agents/data/) so
        ...
        if (removed.length > 0) {
          console.log(`  ${GREEN}✓${RESET} removed ${removed.length} stale working-tree file(s) (now sourced from crew-state):`);
          for (const r of removed) {
            console.log(`      ${DIM}.crew/${r}${RESET}`);
          }
        }
  ...
  // Step 5: Manage .gitignore marker block for crew-state files.
  ...
  if (ORPHAN_BACKENDS.has(target) && !ORPHAN_BACKENDS.has(current)) {
    ...
    const added = addCrewStateGitignoreBlock(gitignorePath, storage);
    if (added) {
      console.log(`  ${GREEN}✓${RESET} added 2 entries to .gitignore (.crew/decisions.md, .crew/agents/*/history.md) — these now live on crew-state branch`);
    }
    ...
    const removed = removeCrewStateGitignoreBlock(gitignorePath, storage);
    if (removed) {
      console.log(`  ${GREEN}✓${RESET} removed 2 entries from .gitignore — .crew/decisions.md and agent histories are now committable again`);
    }
  }
...
/**
 * INSIDER3-INIT-LEAK fix: when `crew init --state-backend orphan|two-layer`
 ...
 * orphan branch is created, lifts those mutable files onto the crew-state

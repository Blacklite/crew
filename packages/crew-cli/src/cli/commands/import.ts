 ...
 * Imports crew from crew-export.json (local file or GitHub repo)
 */

import path from 'node:path';
import { FSStorageProvider, importFromRepo, parseRepoString } from '@blacklite/crew-sdk';
import type { RepoSpec } from '@blacklite/crew-sdk';
...
function applyManifest(
  ...
  if (storage.existsSync(crewDir)) {
    ...
    const archiveDir = path.join(dest, `${crewInfo.name}-archive-${ts}`);
    storage.renameSync(crewDir, archiveDir);
    info(`Archived existing crew to ${path.basename(archiveDir)}`);
  }

  // Create directory structure
  storage.mkdirSync(path.join(crewDir, 'casting'), { recursive: true });
  storage.mkdirSync(path.join(crewDir, 'decisions', 'inbox'), { recursive: true });
  storage.mkdirSync(path.join(crewDir, 'orchestration-log'), { recursive: true });
  storage.mkdirSync(path.join(crewDir, 'log'), { recursive: true });
  ...
  // Write top-level crew files
  storage.writeSync(path.join(crewDir, 'decisions.md'), decisionsContent);
  storage.writeSync(path.join(crewDir, 'team.md'), teamContent);
  if (manifest.routing_md !== undefined) {
    storage.writeSync(path.join(crewDir, 'routing.md'), manifest.routing_md);
  }

  // Write casting state
  for (const [key, value] of Object.entries(manifest.casting)) {
    storage.writeSync(
      path.join(crewDir, 'casting', `${key}.json`),
  ...
  for (const name of agentNames) {
    ...
    const agentDir = path.join(crewDir, 'agents', name);
    assertPathUnder(agentDir, path.join(crewDir, 'agents'));
  ...
  // Output
  success(`Imported crew from ${sourceLabel}`);
  ...
  console.log();
  info('Next steps:');
  info('  1. Open Copilot and select Crew');
  info('  2. Tell the team about this project — they\'ll adapt');
  console.log();
}

/**
 * Import crew from JSON (local file or GitHub repo)
 */

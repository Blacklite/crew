/**
 * Crew Remote Control — Devtunnel lifecycle management
 *
 * Creates, hosts, and cleans up devtunnels with crew labels
...
/** Create a devtunnel with crew labels and host it */
export async function createTunnel(port: number, labels: TunnelLabels): Promise<TunnelInfo> {
  ...
  const labelValues = ['crew', sanitize(labels.repo), sanitize(labels.branch), sanitize(labels.machine), `port-${port}`];

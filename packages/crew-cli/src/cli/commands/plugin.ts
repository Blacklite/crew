...
export async function runPlugin(dest: string, args: string[]): Promise<void> {
  ...
  if (subCmd !== 'marketplace') {
    await runPluginLifecycle(dest, crewDirInfo.path, args);
    return;
  }
...
async function runPluginLifecycle(dest: string, crewDir: string, args: string[]): Promise<void> {
  const action = args[0];
  await mkdir(crewDir, { recursive: true });
  await mkdir(join(crewDir, 'plugins'), { recursive: true });
  const stateStorage = new FSStorageProvider();

  if (action === 'validate' || action === 'dry-run') {
    const source = args[1];
    if (!source) {
      fatal(`Usage: crew plugin ${action} <local-plugin-dir>`);
    }
  ...
  if (action === 'install') {
    const source = args[1];
    if (!source) {
      fatal('Usage: crew plugin install <local-plugin-dir> [--dry-run]');
    }
    ...
    try {
      ...
      await writePluginStates(stateStorage, states, crewDir);
    } catch (err: unknown) {
      await Promise.all(copiedTargets.map((target) => rm(target, { force: true })));
      await writePluginStates(stateStorage, previousStates, crewDir);
      const message = err instanceof Error ? err.message : String(err);
  ...
  if (action === 'list-json' || (action === 'list' && args.includes('--json'))) {
    const states = await readPluginStates(stateStorage, crewDir);
  ...
  if (action === 'enable' || action === 'disable') {
    ...
    if (!pluginId) {
      fatal(`Usage: crew plugin ${action} <plugin-id>`);
    }
    const states = await readPluginStates(stateStorage, crewDir);
  ...
  if (action === 'switch') {
    ...
    if (!role || !pluginId) {
      fatal('Usage: crew plugin switch <role> <plugin-id>');
    }
    const states = await readPluginStates(stateStorage, crewDir);
  ...
  if (action === 'uninstall') {
    const pluginId = args[1];
    if (!pluginId) {
      fatal('Usage: crew plugin uninstall <plugin-id>');
    }
    const states = await readPluginStates(stateStorage, crewDir);
    const plugin = removeInstalledPlugin(states, pluginId);
    for (const file of plugin.files) {
      await rm(safeJoin(crewDir, file.target), { force: true });
    }
    await writePluginStates(stateStorage, states, crewDir);
  ...
  if (action === 'refresh' || action === 'run-lifecycle') {
    ...
    if (!pluginId) {
      fatal(`Usage: crew plugin ${action} <plugin-id>${action === 'run-lifecycle' ? ' <lifecycle-event>' : ''}`);
    }

    if (action === 'run-lifecycle' && !lifecycleEvent) {
      fatal('Usage: crew plugin run-lifecycle <plugin-id> <lifecycle-event>');
    }
    ...
    if (!plugin.enabled) {
      fatal(`Plugin "${pluginId}" is disabled. Enable it first with: crew plugin enable ${pluginId}`);
    }
...
function pluginUsage(): string {
  return 'Usage: crew plugin marketplace add|remove|list|browse OR crew plugin validate|dry-run|install|uninstall|enable|disable|switch|list|verify|refresh|run-lifecycle';
}
...
function printCopilotDependencies(copilot: ReturnType<typeof parsePluginManifestContent>['copilot']): void {
  ...
  console.log(`${DIM}Crew records these dependencies but does not install or run Copilot plugin commands.${RESET}`);
}
...
function printExternalMetadata(manifest: ReturnType<typeof parsePluginManifestContent>): void {
  ...
  console.log(`${DIM}Crew records this metadata but does not install packages, start MCP servers, or run external commands.${RESET}`);
}
...
function printProviderContracts(manifest: ReturnType<typeof parsePluginManifestContent>): void {
  ...
  for (const provider of providers) {
    ...
    const artifact = provider.artifact ? ` artifact=.crew/${provider.artifact}` : '';
  ...
  console.log(`${DIM}Crew records provider contracts but does not start MCP servers, call provider tools, or query live provider backends.${RESET}`);
}

/**
 * crew build — compile TypeScript crew definitions into .crew/ markdown.
 *
 * Loads crew.config.ts (or crew/index.ts) and generates:
 *   - .crew/team.md
 *   - .crew/routing.md
 *   - .crew/agents/{name}/charter.md
 *   - .crew/ceremonies.md
 *
...
import type { SkillDefinition as BuilderSkillDefinition } from '@blacklite/crew-sdk/builders';

// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------

const GENERATED_HEADER = '<!-- generated by crew build — do not edit -->\n\n';
...
interface LoadedConfig {
  config: CrewSDKConfig;
  source: string;
}

/**
 * Discover and dynamically import the crew config file.
 * Search order:
 *   1. crew/index.ts
 *   2. crew.config.ts
 *   3. crew.config.js
 */
async function loadCrewConfig(cwd: string): Promise<LoadedConfig> {
  const storage = new FSStorageProvider();
  const candidates = [
    path.join(cwd, 'crew', 'index.ts'),
  ...
  for (const candidate of candidates) {
    if (storage.existsSync(candidate)) {
      try {
        ...
        const config: CrewSDKConfig = mod.default ?? mod.config ?? mod.crewConfig;
        if (!config) {
          fatal(`${candidate} must export default, "config", or "crewConfig"`);
        }
  ...
  fatal(
    'No crew config found. Create crew.config.ts or crew/index.ts\n' +
    '       Run "crew init" to bootstrap a new project.',
  );
}
...
function generateTeamMd(config: CrewSDKConfig): string {
  ...
  lines.push('|------|------|-------|');
  lines.push('| Crew | Coordinator | Routes work, enforces handoffs and reviewer gates. Does not generate domain artifacts. |');
  lines.push('');
 ...
 * preference or falling back to crew-level defaults.
 */
...
interface GeneratedFile {
  /** Relative path from project root (e.g. ".crew/team.md") */
  relPath: string;
  /** Generated content */
  content: string;
}

function buildFilePlan(config: CrewSDKConfig): GeneratedFile[] {
  const files: GeneratedFile[] = [];

  // team.md
  files.push({
    relPath: '.crew/team.md',
  ...
  for (const agent of config.agents) {
    files.push({
      relPath: `.crew/agents/${agent.name}/charter.md`,
...
function writeFiles(cwd: string, files: GeneratedFile[], stateDir?: string): BuildResult {
  ...
  for (const file of files) {
    ...
    // When state is externalized, .crew/ paths are written to the external dir
    const baseDir = (stateDir && file.relPath.startsWith('.crew/'))
      ? path.join(stateDir, file.relPath.slice('.crew/'.length))
      : path.join(cwd, file.relPath);

    const absPath = (stateDir && file.relPath.startsWith('.crew/'))
      ? baseDir
      : path.join(cwd, file.relPath);
...
export async function runBuild(cwd: string, options: BuildOptions = {}): Promise<void> {
  ...
  if (options.watch) {
    ...
    info(`   For now, run ${BOLD}crew build${RESET} manually after changes.`);
    return;
  }
  ...
  try {
    const dirs = effectiveCrewDir(cwd);
    if (dirs.stateDir !== dirs.local.path) {
      stateDir = dirs.stateDir;
      dim(`  State dir: ${stateDir} (externalized)`);
    }
  } catch {
    // No .crew/ found — build will proceed with default cwd-relative writes
  }

  // Load config
  const { config, source } = await loadCrewConfig(cwd);
  ...
  if (files.length === 0) {
    warn('Config produced no files to generate. Check your crew.config.ts.');
    return;
  }
  ...
  if (options.check) {
    ...
    console.log(`\n  Run ${BOLD}crew build${RESET} to regenerate.\n`);
    process.exit(1);
  }
  ...
  success(`crew build complete — generated ${result.written} file(s)`);
  if (result.skipped > 0) {

/**
 * Aspire command — Launch Aspire dashboard for Crew observability
 * (Issue #265)
 *
 * Starts the Aspire dashboard and configures OTLP export so Crew
...
const ASPIRE_CONTAINER_NAME = 'crew-aspire-dashboard';
...
export async function runAspire(options: AspireOptions = {}): Promise<void> {
  ...
  console.log(`\n${BOLD}🔭 Crew Aspire — OpenTelemetry Dashboard${RESET}\n`);
  ...
  console.log(`\n${DIM}Crew OTel will automatically export to this endpoint.${RESET}`);

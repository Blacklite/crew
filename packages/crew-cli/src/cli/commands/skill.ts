 ...
 *
 * crew skill publish [<name>]  — export a skill to APM format
 * crew skill install <name>    — install a skill from APM registry
 * crew skill list              — list installed skills
 *
...
/**
 * Resolve the skills directory, preferring .copilot/skills/ over .crew/skills/.
...
function resolveSkillsDir(dest: string): { dir: string; relPrefix: string } {
  ...
  const crewSkills = join(crewDirInfo.path, 'skills');
  if (existsSync(crewSkills)) {
    return { dir: crewSkills, relPrefix: `${crewDirInfo.name}/skills` };
  }
...
async function publish(dest: string, skillName?: string): Promise<void> {
  ...
  const lines = [
    ...
    `instructions:`,
    `  - path: .crew/copilot-instructions.md`,
...
async function install(dest: string, source: string): Promise<void> {
  if (!source) {
    fatal('Usage: crew skill install <owner/repo>[/<skill-name>] | <url>');
  }
...
async function installFromGitHub(
  ...
  if (!apmContent) {
    ...
    await installSkillsFromCrewDir(owner, repo, skillFilter, skillsDir);
    return;
  }
  ...
  if (installed > 0) {
    info(`\n${DIM}Run 'crew skill publish' to refresh apm.yml with newly installed skills${RESET}`);
  }
}

async function installSkillsFromCrewDir(
  owner: string,
  repo: string,
  skillFilter: string | undefined,
  skillsDir: string,
): Promise<void> {
  // Try .copilot/skills/ (standard) then .crew/skills/ (legacy) then bare skills/
  const candidates = ['.copilot/skills', '.crew/skills', 'skills'];
  ...
  if (entries.length === 0) {
    fatal(`No skills directory found in ${owner}/${repo}. The repo may not use APM or Crew conventions.`);
  }
  ...
  if (installed > 0) {
    info(`\n${DIM}Run 'crew skill publish' to refresh apm.yml${RESET}`);
  }
}
 ...
 * crew skill list
 *
 * Lists installed skills from .copilot/skills/ (or legacy .crew/skills/).
 */
async function listSkills(dest: string): Promise<void> {
  const { dir: skillsDir, relPrefix } = resolveSkillsDir(dest);

  if (!existsSync(skillsDir)) {
    info('No skills directory found. Run "crew init" first or create .copilot/skills/');
    return;
  }
  ...
  if (skills.length === 0) {
    ...
    info(`Install a skill: crew skill install <owner/repo>`);
    return;
  }
...
/**
 * Entry point for `crew skill` command.
 *
 * @param dest - Working directory (usually process.cwd())
 * @param args - Remaining args after `crew skill`
 */
export async function runSkill(dest: string, args: string[]): Promise<void> {
  const subCmd = args[0];

  if (!subCmd || subCmd === 'help' || subCmd === '--help') {
    console.log(`\n${BOLD}crew skill${RESET} — APM (Agent Package Manager) integration\n`);
    console.log(`Usage:`);
    console.log(`  crew skill publish [<skill-name>]   Export skill(s) to APM format (apm.yml)`);
    console.log(`  crew skill install <source>         Install from APM registry`);
    console.log(`  crew skill list                     List installed skills`);
    console.log(`\nInstall sources:`);

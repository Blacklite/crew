/**
 * crew notes — manage crew git-notes state (Round 5, P0.3 A3).
 *
 * Subcommand:
 *   promote [--ref <ref>] [--all] [--dry-run]
 *
 * Production caller for {@link TwoLayerBackend.promoteNotes}. Walks notes on
 * each `refs/notes/crew/*` ref, moves notes flagged `promote_to_permanent`
...
import { resolveStateBackend, TwoLayerBackend, type PromoteNotesResult } from '@blacklite/crew-sdk';
...
/** Parsed args for `crew notes promote`. */
...
function printPromoteHelp(): void {
  console.log(`\n${BOLD}crew notes promote${RESET} — promote/archive flagged git-notes to permanent storage\n`);
  console.log(`Usage: crew notes promote [options]\n`);
  console.log(`Options:`);
  console.log(`  --ref <ref>    Restrict to a single notes ref (e.g. crew/picard)`);
  console.log(`  --all          Promote across all refs/notes/crew/* (default)`);
  console.log(`  --dry-run      Report what would be promoted without writing`);
  console.log(`  --help, -h     Show this help\n`);
  console.log(`Requires stateBackend: 'two-layer' in .crew/config.json.`);
  console.log(`Idempotent — promoted notes are removed from source, archived notes stay in place.\n`);
}

/**
 * Enumerate crew notes refs (`refs/notes/crew/*`) present in the repo.
 * Returns short names (e.g. `crew/picard`, not `refs/notes/crew/picard`).
 */
function listCrewNotesRefs(repoRoot: string): string[] {
  try {
    const out = execFileSync(
      'git',
      ['for-each-ref', '--format=%(refname)', 'refs/notes/crew/'],
    ...
    return out
      ...
      .filter((r) => /^crew\/[A-Za-z0-9_\-./]+$/.test(r));
  } catch {
    return [];
  }
}
...
/**
 * Run `crew notes promote`. Returns process exit code.
 */
export async function runNotesPromote(cwd: string, args: string[]): Promise<number> {
  const opts = parsePromoteArgs(args);

  const paths = resolveCrewPaths(cwd);
  if (!paths) {
    fatal('No crew found. Run "crew init" first.');
  }

  const crewDir = paths.projectDir;
  const repoRoot = path.resolve(crewDir, '..');

  const backend = resolveStateBackend(crewDir, repoRoot);
  if (!(backend instanceof TwoLayerBackend)) {
    ...
    console.log(`${DIM}  Run 'crew upgrade --state-backend two-layer' to enable note promotion.${RESET}`);
    return 0;
  }
  ...
  if (opts.ref) {
    ...
    refs = listCrewNotesRefs(repoRoot);
    if (refs.length === 0) {
      console.log(`${DIM}No crew notes refs found (refs/notes/crew/*). Nothing to do.${RESET}`);
      return 0;
    }
  }
...
export async function runNotes(cwd: string, args: string[]): Promise<void> {
  ...
  fatal(`Unknown 'crew notes' subcommand: ${sub}`);
}

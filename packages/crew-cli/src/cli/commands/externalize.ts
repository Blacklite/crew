/**
 * crew externalize — move .crew/ state out of the working tree.
 *
 * Moves all crew state to `{resolveGlobalCrewPath()}/projects/{projectKey}/`
 * (platform-specific: `%APPDATA%/crew/` on Windows, `~/Library/Application Support/crew/`
 * on macOS, `$XDG_CONFIG_HOME/crew/` on Linux) and writes a thin `.crew/config.json`
 ...
 * - A `.crew/config.json` marker lets the walk-up resolver find state
 *
 * To restore local state, run `crew internalize`.
 *
 * @module cli/commands/externalize
 */

import path from 'node:path';
import { FSStorageProvider, resolveExternalStateDir, deriveProjectKey } from '@blacklite/crew-sdk';
import { fatal } from '../core/errors.js';

const storage = new FSStorageProvider();

/** Entries under .crew/ that must NOT be externalized (they stay in the repo).
 *  All of these are read from the working tree by runtime code that does not
 *  go through external-state resolution (detectCrewDir → local .crew/).
 *  - config.json: thin marker read by the walk-up resolver
 *  - manifest.json: public contract read by cross-crew discovery
 *  - workstreams.json: workstream config read by streams resolver
 *  - upstream.json: upstream registry read by cross-crew discover/delegate
 *  - crew-registry.json: crew registry read by cross-crew discovery
 *  - _upstream_repos: git clone cache read locally by upstream/cross-crew resolvers
 */
...
/**
 * Move .crew/ state to the external directory.
 *
 * @param projectDir - Absolute path to the project root (where .crew/ lives).
...
export function runExternalize(projectDir: string, projectKey?: string): void {
  ...
  if (!storage.existsSync(crewDir)) {
    fatal('.crew/ directory not found. Run `crew init` first.');
  }
  ...
  console.log(`📦 Externalizing .crew/ state to: ${externalDir}`);

  let movedCount = 0;

  // Dynamically discover everything in .crew/ — move it all except KEEP_LOCAL.
  // This eliminates silent orphaning when new state artifacts are added.
  const entries = storage.listSync?.(crewDir) ?? [];
  for (const entry of entries) {
    if (KEEP_LOCAL.has(entry)) continue;
    const src = path.join(crewDir, entry);
  ...
  ensureGitignored(projectDir, '.crew/config.json');

  console.log(`✅ Externalized ${movedCount} items. State now at: ${externalDir}`);
  console.log(`   .crew/config.json marker left in repo (gitignored).`);
  console.log(`   Run \`crew internalize\` to move state back.`);
}
...
export function runInternalize(projectDir: string): void {
  ...
  if (!storage.existsSync(configPath)) {
    fatal('.crew/config.json not found. State is already local or not initialized.');
  }
  ...
  try {
    ...
    fatal('.crew/config.json is malformed.');
    return;
  }
  ...
  for (const entry of entries) {
    ...
    const dest = path.join(crewDir, entry);
    if (storage.isDirectorySync(src)) {
      copyDirRecursive(src, dest);
    } else {
  ...
  // so loadDirConfig() returns null and resolveCrewPaths() falls through to
  // local mode.
  ...
  console.log(`✅ Internalized ${movedCount} items. State is back in .crew/.`);
}
...
function ensureGitignored(projectDir: string, entry: string): void {
  ...
  if (!existing.includes(entry)) {
    const block = (existing && !existing.endsWith('\n') ? '\n' : '')
      + '# Crew: local config (machine-specific, never commit)\n'
      + entry + '\n';

...
function readConfiguredLogLevel(projectRoot: string): MemoryDiagnosticLevel | undefined {
  const configPath = path.join(projectRoot, '.crew', 'config.json');
  if (!fs.existsSync(configPath)) return undefined;

  let parsed: CrewConfig;
  try {
    parsed = JSON.parse(fs.readFileSync(configPath, 'utf8')) as CrewConfig;
  } catch {
    return undefined;
  }
...
function parseDiagnostics(projectRoot: string, args: string[]): { args: string[]; logLevel: MemoryDiagnosticLevel } {
  ...
  return {
    args: filtered,
    logLevel: logLevel ?? (
      process.env['CREW_MEMORY_LOG_LEVEL'] !== undefined
        ? parseLogLevel(process.env['CREW_MEMORY_LOG_LEVEL'])
  ...
  try {
    ...
    if (operation === 'promote') {
      ...
      if (!id || !targetClass || targetClass === 'FORBIDDEN' || targetClass === 'TRANSIENT') {
        throw new Error('Usage: crew memory promote <id> --class LOCAL|DECISION|POLICY|COPILOT_MEMORY');
      }
    ...
    if (operation === 'delete') {
      const id = commandArgs[1];
      if (!id) throw new Error('Usage: crew memory delete <id>');
    ...
    console.log([
      'Usage: crew memory <classify|write|search|promote|delete|audit|provider>',
      ...
      '  diagnostics: --log-level none|error|info|debug, --verbose, CREW_MEMORY_LOG_LEVEL, or .crew/config.json memory.logLevel (stderr; never logs raw memory content or search text).',

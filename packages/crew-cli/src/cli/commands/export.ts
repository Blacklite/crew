 ...
 * Exports crew to crew-export.json (local or GitHub repo)
 */

import path from 'node:path';
import { FSStorageProvider, exportToRepo, parseRepoString } from '@blacklite/crew-sdk';
...
/**
 * Build the export manifest from the effective crew state directory
 * (the external state dir when state is externalized, otherwise local .crew/).
 */
function buildManifest(dest: string, storage: FSStorageProvider, crewInfo: { path: string }): ExportManifest {
  const manifest: ExportManifest = {
    version: '1.0',
    exported_at: new Date().toISOString(),
    crew_version: getPackageVersion(),
    casting: {},
    agents: {},
    skills: [],
  };

  // Read top-level crew files (team.md, decisions.md, routing.md)
  const teamMdPath = path.join(crewInfo.path, 'team.md');
  ...
  const decisionsMdPath = path.join(crewInfo.path, 'decisions.md');
  ...
  const routingMdPath = path.join(crewInfo.path, 'routing.md');
  ...
  // Read agents
  const agentsDir = path.join(crewInfo.path, 'agents');
  try {
  ...
  const skillSources = [
    ...
    { dir: path.join(crewInfo.path, 'skills'), layout: 'nested' as const },
...
/**
 * Export crew to JSON (local file or GitHub repo)
 */
export async function runExport(dest: string, outPath?: string, repoOptions?: ExportRepoOptions): Promise<void> {
  ...
  const finalOutPath = outPath || path.join(dest, 'crew-export.json');
  ...
  success(`Exported crew to ${displayPath}`);
  warn('Review agent histories, decisions, and team content before sharing — they may contain project-specific information');
}

  ...
  const handleSubmit = useCallback((input: string) => {
    ...
    if (awaitingInitPrompt && parsed.type !== 'slash_command') {
      ...
      if (!onDispatch) {
        appendMessages(prev => [...prev, {
          role: 'system' as const,
          content: 'SDK not connected. Try: (1) crew doctor to check setup, (2) check your internet connection, (3) restart the shell to reconnect.',
          timestamp: new Date(),
        }]);
        return;
      }
  ...
  const headerElement = useMemo(() => {
    ...
    if (tier === 'narrow') {
      return (
        <Box flexDirection="column" paddingX={1}>
          <Text bold color={noColor ? undefined : 'cyan'}>CREW</Text>
    ...
    if (tier === 'normal') {
      return (
        <Box flexDirection="column" borderStyle="round" borderColor={noColor ? undefined : 'cyan'} paddingX={1}>
          <Text bold color={noColor ? undefined : 'cyan'}>CREW v{version}</Text>
    ...
    return (
      <Box flexDirection="column" borderStyle="round" borderColor={noColor ? undefined : 'cyan'} paddingX={1}>
        ...
        <Text color={noColor ? undefined : 'yellow'} dimColor>⚠️  Experimental preview — file issues at github.com/Blacklite/crew</Text>
      </Box>
    );
    ...
    return (
      <Box flexDirection="column" paddingX={1} paddingY={1}>
        {rosterAgents.length > 0 ? (
          <>
            <Text color={noColor ? undefined : 'green'} bold>Your crew is assembled.</Text>
            <Text> </Text>
            <Text bold>Try: <Text bold color={noColor ? undefined : 'cyan'}>What should we build first?</Text></Text>
            <Text dimColor>Crew automatically routes your message to the best agent.</Text>
            ...
            <Box key={item.key} flexDirection="column" width={contentWidth}>
              ...
              <Box gap={1} paddingLeft={msg.role === 'user' ? 0 : 2}>
                {msg.role === 'user' ? (
                  ...
                  <>
                    <Text color={noColor ? undefined : 'green'} bold>{emoji ? `${emoji} ` : ''}{(msg.agentName === 'coordinator' ? 'Crew' : msg.agentName) ?? 'agent'}:</Text>

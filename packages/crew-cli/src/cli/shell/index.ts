/**
 * Crew Interactive Shell — entry point
 *
...
import { createAgentClient, resolveHarness } from '@blacklite/crew-sdk/client';
import type { CrewSession } from '@blacklite/crew-sdk/client';
import type { CrewPermissionHandler } from '@blacklite/crew-sdk/client';
import { RateLimitError } from '@blacklite/crew-sdk/adapter/errors';
import type { ShellMessage } from './types.js';
import { FSStorageProvider, initCrewTelemetry, TIMEOUTS, StreamingPipeline, recordAgentSpawn, recordAgentDuration, recordAgentError, recordAgentDestroy, RuntimeEventBus, resolveCrew, resolveGlobalCrewPath, loadDirConfig, resolveExternalStateDir } from '@blacklite/crew-sdk';
import type { UsageEvent } from '@blacklite/crew-sdk';
 ...
 */
const approveAllPermissions: CrewPermissionHandler = () => ({ kind: 'approve-once' });

/** Debug logger — writes to stderr only when CREW_DEBUG=1. */
function debugLog(...args: unknown[]): void {
  if (process.env['CREW_DEBUG'] === '1') {
    console.error('[CREW_DEBUG]', ...args);
  }
}
...
export async function runShell(): Promise<void> {
  // First-run check: before requiring a TTY, detect if no .crew/ exists locally.
  ...
  const localCrew = resolveCrew(cwd);
  const globalCrewDir = join(resolveGlobalCrewPath(), '.crew');
  const hasAnyCrew = !!localCrew || storage.existsSync(globalCrewDir);

  if (!hasAnyCrew && !process.stdin.isTTY) {
    console.log('Welcome to Crew\n');
    console.log('Get started by initializing your crew:');
    console.log('  crew init "describe what you want to build"\n');
    console.log('Or run: crew help');
    process.exit(0);
  }

  // Ink requires a TTY for raw mode input — bail out early when piped (#576)
  if (!process.stdin.isTTY) {
    console.error('✗ Crew shell requires an interactive terminal (TTY).');
    console.error('  Piped or redirected stdin is not supported.');
    console.error("  Tip: Run 'crew --preview' for non-interactive usage.");
    process.exit(1);
  }

  // Show immediate feedback — users need to see something within 100ms
  console.error('◆ Loading Crew shell...');

  // Configurable REPL timeout: CREW_REPL_TIMEOUT (seconds) > TIMEOUTS.SESSION_RESPONSE_MS (ms)
  const replTimeoutMs = (() => {
    const envSeconds = process.env['CREW_REPL_TIMEOUT'];
    if (envSeconds) {
  ...
  // Resolve teamRoot: local .crew/ → global crew → cwd (init mode)
  const teamRoot = (() => {
    const cwd = process.cwd();
    // 1. Walk up from cwd looking for a local .crew/
    const localCrew = resolveCrew(cwd);
    if (localCrew) {
      return pathResolve(localCrew, '..');
    }
    // 2. Fall back to global (personal) crew path
    const globalPath = resolveGlobalCrewPath();
    const globalCrewDir = join(globalPath, '.crew');
    if (storage.existsSync(globalCrewDir)) {
      return globalPath;
    }
    // 3. No crew found — use cwd (triggers init mode)
    return cwd;
  })();
  ...
  const localCrewDir = join(teamRoot, '.crew');
  const dirConfig = loadDirConfig(localCrewDir);
  ...
  const telemetry = initCrewTelemetry({ serviceName: 'crew-cli', mode: 'cli', eventBus });
  ...
  // Create SDK client for the resolved harness (copilot or claude);
  // auto-connects on first session creation
  const shellHarness = resolveHarness({ teamRoot });
  const client = createAgentClient({
    harness: shellHarness,
    teamRoot,
    copilot: { cwd: teamRoot },
    claude: { cwd: teamRoot },
  });

  let shellApi: ShellApi | undefined;
  let origAddMessage: ((msg: ShellMessage) => void) | undefined;
  const agentSessions = new Map<string, CrewSession>();
  let coordinatorSession: CrewSession | null = null;
  let activeInitSession: CrewSession | null = null;
   ...
   */
  async function awaitStreamedResponse(session: CrewSession, prompt: string): Promise<string> {
  ...
  function ghostRetry(
    ...
    return withGhostRetry(sendFn, {
      ...
      onExhausted: (max) => {
        ...
        shellApi?.addMessage({
          role: 'system',
          content: `❌ Agent did not respond after ${totalAttempts} attempts. Try again or run \`crew doctor\`.`,
          timestamp: new Date(),
        });
      },
    });
  ...
  async function handleInitCast(parsed: ParsedInput, skipConfirmation?: boolean): Promise<void> {
    ...
    // Check for a stored init prompt (from `crew init "prompt"`)
    const initPromptFile = join(teamRoot, '.crew', '.init-prompt');
    ...
    let initSession: CrewSession | null = null;
    try {
      // Check for .init-roles marker (set by `crew init --roles` or `/init --roles`)
      const initRolesMarker = join(teamRoot, '.crew', '.init-roles');
      ...
      if (!proposal) {
        ...
        shellApi?.addMessage({
          ...
          content: [
            ...
            '',
            'Try again, or run: crew init "describe your project"',
          ].join('\n'),
          timestamp: new Date(),
        });
        return;
  ...
  async function finalizeCast(proposal: CastProposal, parsed: ParsedInput): Promise<void> {
    ...
    const verifyTeamPath = join(teamRoot, '.crew', 'team.md');
    ...
    if (!hasRosterEntries(verifyContent)) {
      ...
      const initPromptCleanup = join(teamRoot, '.crew', '.init-prompt');
      if (storage.existsSync(initPromptCleanup)) {
        try { await storage.delete(initPromptCleanup); } catch { /* ignore */ }
      }
      shellApi?.addMessage({
        role: 'system',
        content: '⚠ Team creation completed but roster is empty — skipping dispatch. Check .crew/team.md.',
        timestamp: new Date(),
      });
      shellApi?.setActivityHint(undefined);
      shellApi?.setProcessing(false);
  ...
  async function handleDispatch(parsed: ParsedInput): Promise<void> {
    ...
    // Guard: require a Crew team before processing work requests
    const teamFile = join(teamRoot, '.crew', 'team.md');
    if (!storage.existsSync(teamFile)) {
      ...
      shellApi?.addMessage({
        role: 'system',
        content: '\u26A0 No Crew team found. Run /init to create your team first.',
        timestamp: new Date(),
      });
      return;
    }
    ...
    try {
      ...
      if (shellApi) {
        ...
        if (isRateLimit) {
          ...
          // Persist rate limit status so `crew doctor` can surface it.
          try {
            const crewDir = join(teamRoot, '.crew');
            storage.writeSync(
              join(crewDir, 'rate-limit-status.json'),
              JSON.stringify({
                timestamp: new Date().toISOString(),
                retryAfter,
                model,
        ...
        } else if (process.env['CREW_DEBUG'] === '1') {
    ...
    React.createElement(ErrorBoundary, null,
      React.createElement(App, {
        ...
        onReady: (api: ShellApi) => {
          ...
          const initPromptPath = join(teamRoot, '.crew', '.init-prompt');
          const teamFilePath = join(teamRoot, '.crew', 'team.md');
  ...
  try {
    ...
    if (storage.existsSync(configPath)) {
      ...
      if (parsed && typeof parsed === 'object' && (parsed as { consult?: boolean }).consult === true) {
        ...
        console.log(`   Run ${highlight}crew extract${reset} to bring learnings home.`);
        console.log(`   Run ${highlight}crew extract --clean${reset} to extract and remove project .crew/`);
        console.log('');
      }
    }
  } catch {
  ...
  const coordSession = coordinatorSession as CrewSession | null;
  if (coordSession) {
  ...
  if (messageCount > 0) {
    ...
    console.log(`${prefix}Crew out. ${durationStr}${agentStr} ${messageCount} message${messageCount === 1 ? '' : 's'}.`);
  } else {
    console.log(`${prefix}Crew out.`);
  }

...
export function teamConfigGuidance(issue: string): ErrorGuidance {
  return {
    ...
    recovery: [
      "Run 'crew doctor' to diagnose",
      "Run 'crew init' to regenerate team.md",
      'Check .crew/team.md exists and has valid YAML',
    ],
  };
}
...
export function agentSessionGuidance(agentName: string, detail?: string): ErrorGuidance {
  return {
    ...
    recovery: [
      ...
      "Run 'crew doctor' to check setup",
...
export function rateLimitGuidance(opts?: { retryAfter?: number; model?: string }): ErrorGuidance {
  ...
  return {
    ...
    recovery: [
      retryStr,
      'Enable economy mode to switch to cheaper models: `crew economy on`',
      'Or set a different model: add `"defaultModel": "gpt-5-mini"` to .crew/config.json',
    ],
  };
}
...
export function genericGuidance(detail: string): ErrorGuidance {
  return {
    ...
    recovery: [
      ...
      "Run 'crew doctor' for diagnostics",
      'Check your internet connection',
    ],
  };
}
...
export function timeoutGuidance(agentName?: string): ErrorGuidance {
  ...
  return {
    ...
    recovery: [
      ...
      'Set CREW_REPL_TIMEOUT=120 for a longer timeout (seconds)',
      "Run 'crew doctor' to verify connectivity",
    ],
  };
}

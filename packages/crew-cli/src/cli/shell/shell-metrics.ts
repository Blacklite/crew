 ...
 * No-op when CREW_TELEMETRY !== '1' or OTel is not configured.
 *
 * Privacy-first: opt-in via CREW_TELEMETRY=1 env var. No PII collected.
 *
 * @module shell/shell-metrics
 */

import { getMeter } from '@blacklite/crew-sdk';
...
/** Check if shell telemetry is opt-in enabled via CREW_TELEMETRY=1. */
export function isShellTelemetryEnabled(): boolean {
  return process.env['CREW_TELEMETRY'] === '1';
}

function ensureMetrics(): ShellMetrics | undefined {
  if (!_enabled) return undefined;
  if (!_metrics) {
    const meter = getMeter('crew-shell');
    _metrics = {
      sessionDuration: meter.createHistogram('crew.shell.session_duration_ms', {
        description: 'Shell session duration in milliseconds',
        unit: 'ms',
      }),
      agentLatency: meter.createHistogram('crew.shell.agent_response_latency_ms', {
        description: 'Time from message send to first response token in milliseconds',
        unit: 'ms',
      }),
      errorCount: meter.createCounter('crew.shell.error_count', {
        description: 'Total errors during shell session',
      }),
      sessionCount: meter.createCounter('crew.shell.session_count', {
 ...
 * Always enabled when OTel is configured; CREW_TELEMETRY=1 also enables.

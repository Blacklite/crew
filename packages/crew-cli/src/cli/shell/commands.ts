...
function handleHelp(args: string[]): CommandResult {
  ...
  if (width < 80) {
    ...
    return {
      ...
      output: [
        'How it works:',
        '  Just type what you need — Crew routes your message to the right agent.',
...
function handleInit(args: string[], context: CommandContext): CommandResult {
  ...
  if (useBaseRoles) {
    ...
    const rolesMarker = path.join(context.teamRoot, '.crew', '.init-roles');
  ...
  return {
    ...
    output: [
      'To cast your Crew team, just type what you want to build.',
      '',
      ...
      '',
      `Team file: ${context.teamRoot}/.crew/team.md`,
    ].join('\n'),
  };
}

...
import { listRoles, searchRoles, FSStorageProvider, loadDirConfig, resolveExternalStateDir } from '@blacklite/crew-sdk';
...
This project doesn't have a Crew team yet.

**You MUST NOT do any project work.** Instead, tell the user:
1. "This project doesn't have a Crew team yet."
2. Suggest running \`crew init\` or the \`/init\` command to set one up.
...
This project has a Crew scaffold (.crew/ directory) but no team has been cast yet.
 ...
 * Activated by `crew init --roles` or `/init --roles`.
 */
...
export async function buildCoordinatorPrompt(config: CoordinatorConfig): Promise<string> {
  const crewRoot = config.teamRoot;
  const storage = new FSStorageProvider();

  // Resolve effective state dir (external when externalized)
  const localCrewDir = join(crewRoot, '.crew');
  ...
  try {
    ...
    routingContent = raw ?? '(No routing.md found — run `crew init` to create one)';
  } catch (err) {
    debugLog('buildCoordinatorPrompt: failed to read routing.md at', routingPath, err);
    routingContent = '(No routing.md found — run `crew init` to create one)';
  }

  return `You are the Crew Coordinator — you route work to the right agent.

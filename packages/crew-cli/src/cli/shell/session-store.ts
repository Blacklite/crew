 ...
 *
 * Sessions are stored as JSON files in `.crew/sessions/`.
 * Each file is named `{safeTimestamp}_{sessionId}.json`.
 */

import { randomUUID } from 'node:crypto';
import { join } from 'node:path';
import { FSStorageProvider, safeTimestamp } from '@blacklite/crew-sdk';
...
function sessionsDir(teamRoot: string, stateDir?: string): string {
  return stateDir ? join(stateDir, 'sessions') : join(teamRoot, '.crew', 'sessions');
}

 ...
 * - Initialization (load team, resolve crew path, populate registry)
...
export class ShellLifecycle {
  ...
  /**
   * Initialize the shell — verify .crew/, load team.md, discover agents.
   *
  ...
  async initialize(): Promise<void> {
    ...
    const localCrewDir = path.resolve(this.options.teamRoot, '.crew');
    if (!await storage.exists(localCrewDir) || !await storage.isDirectory(localCrewDir)) {
      this.state.status = 'error';
      const err = new Error(
        `No team found. Run \`crew init\` to create one.`
      );
      debugLog('initialize: .crew/ directory not found at', localCrewDir);
      throw err;
    }

    // Resolve effective state dir (external when externalized)
    const crewDir = resolveStateDir(localCrewDir);
    ...
    if (teamContent === undefined) {
      this.state.status = 'error';
      const err = new Error(
        `No team manifest found. The .crew/ directory exists but has no team.md. Run \`crew init\` to fix.`
      );
    ...
    if (this.discoveredAgents.length === 0) {
      const initPromptPath = path.join(localCrewDir, '.init-prompt');
      if (!await storage.exists(initPromptPath)) {
        console.warn('⚠ No agents found in team.md. Run `crew init "describe your project"` to cast a team.');
      }
...
export interface WelcomeData {
  ...
  focus: string | null;
  /** True on the very first launch after `crew init`. */
  isFirstRun: boolean;
}

/**
 * Load welcome screen data from .crew/ directory.
 *
...
export function loadWelcomeData(teamRoot: string): WelcomeData | null {
  try {
    const storage = new FSStorageProvider();
    const localCrewDir = path.join(teamRoot, '.crew');
    const stateDir = resolveStateDir(localCrewDir);
    const teamPath = path.join(stateDir, 'team.md');
    const content = storage.readSync(teamPath);
    if (content === undefined) return null;

    const titleMatch = content.match(/^#\s+Crew Team\s+—\s+(.+)$/m);
    const projectName = titleMatch?.[1] ?? 'Crew';
    ...
    // Detect and consume first-run marker from `crew init`

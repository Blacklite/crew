 ...
 *
 * Creates SDK sessions via CrewClient, sends the task, and streams the response.
 */
...
import { CrewState, FSStorageProvider } from '@blacklite/crew-sdk';
...
export interface SpawnOptions {
  ...
  /** CrewClient instance for SDK session creation */
  client?: CrewClient;
...
/**
 * Load agent charter from .crew/agents/{name}/charter.md
 *
 * Reads via CrewState → AgentHandle.charter() so all file access is
...
export async function loadAgentCharter(agentName: string, teamRoot?: string): Promise<string> {
  ...
  if (teamRoot) {
    ...
    if (!crewDir) {
      debugLog('loadAgentCharter: no .crew/ directory found');
      throw new Error('No team found. Run `crew init` to set up your project.');
    }
    rootDir = dirname(crewDir);
  }

  const storage = new FSStorageProvider();
  let state: CrewState;
  try {
    state = await CrewState.create(storage, rootDir);
  } catch {
    debugLog('loadAgentCharter: no .crew/ directory at', rootDir);
    throw new Error('No team found. Run `crew init` to set up your project.');
  }
  ...
  try {
    ...
    throw new Error(`No charter found for "${agentName}". Check that .crew/agents/${agentName.toLowerCase()}/charter.md exists.`);
  }
}
 ...
 *
 * When a CrewClient is provided via options.client, creates a real SDK session,
...
export async function spawnAgent(
  ...
  try {
    ...
    const session: CrewSession = await options.client.createSession({
      streaming: true,
    ...
    return {
      agentName: name,
      status: 'error',
      error: `Failed to spawn ${name}: ${msg.replace(/^Error:\s*/i, '')}. Try again or run \`crew doctor\`.`,
    };
  }
}

/**
 * Crew upgrade command — overwrites crew-owned files, runs migrations
...
import { detectCrewDir } from './detect-crew-dir.js';
...
import { resolveCrewStateMcpSpec, type CrewStateMcpSpec } from './mcp-spec.js';
export { resolveCrewStateMcpSpec } from './mcp-spec.js';
...
function readCrewConfig(crewDir: string): Record<string, unknown> {
...
function writeAgentTemplate(agentSrc: string, agentDest: string, cliVersion: string, mcpConfigMode: McpConfigMode, isGitHub: boolean, options?: { dryRun?: boolean }): void {
  ...
  // Back up locally-customized crew.agent.md before overwriting (#1052)
  if (storage.existsSync(agentDest)) {
    ...
    const stripVersion = (s: string) => s.replace(/<!-- crew-cli v[\d.]+[-\w.]* -->\n?/g, '');
    ...
    if (normalizedExisting !== normalizedTemplate && normalizedExisting.trim().length > 0) {
      ...
      if (options?.dryRun) {
        warn(`crew.agent.md has local customizations — would back up to ${path.basename(backupPath)}`);
        return;
      }
      storage.writeSync(backupPath, existing);
      warn(`crew.agent.md has local customizations — backed up to ${path.basename(backupPath)}`);
      info(`  To restore: copy ${path.basename(backupPath)} → ${path.basename(agentDest)}`);
    } else if (options?.dryRun) {
      info('crew.agent.md is unmodified — would refresh with latest template');
      return;
    }
  } else if (options?.dryRun) {
    info('crew.agent.md does not exist — would create from template');
    return;
  }
 ...
 * via parent-directory matching (e.g. `.crew/` covers `.crew/log/`).
 */
 ...
 * Skips entries already covered by a parent path (e.g. `.crew/` covers `.crew/log/`).
 */
...
export function ensureCastingDefaults(dest: string, templatesDir?: string): string[] {
  const castingDir = path.join(dest, '.crew', 'casting');
  ...
  for (const file of templateMap) {
    ...
    if (!storage.existsSync(filePath)) {
      ...
      try {
        ...
        created.push(`.crew/casting/${file.name}`);
      } catch (err: unknown) {
        if (err instanceof Error && 'code' in err && ['EPERM', 'EACCES'].includes((err as NodeJS.ErrnoException).code ?? '')) {
          warn(`Could not write ${file.name} to .crew/casting/ (read-only). Create it manually.`);
          return created;
        }
        throw err;
      }
 ...
 * `TEMPLATE_MANIFEST` (manifest-curated Crew skills) — does NOT touch
 ...
 *
 * See Blacklite/crew#1126 (canonical issue) — this is the migration
...
function syncAllSkills(dest: string, templatesDir: string): number {
  ...
  for (const entry of skillEntries) {
    ...
    const destPath = path.join(crewDir, entry.destination);
...
/**
 * Copy full templates/ directory to .crew/templates/ for user access
 */
function refreshCrewTemplatesDir(dest: string, templatesDir: string): void {
  const crewTemplatesDest = path.join(dest, '.crew', 'templates');
  storage.mkdirSync(crewTemplatesDest, { recursive: true });
  ...
  for (const entry of entries) {
    ...
    const destPath = path.join(crewTemplatesDest, entry);
 ...
 * node_modules stays unpatched and `crew doctor` keeps failing its
...
async function runEnsureChecks(dest: string, templatesDir: string, filesUpdated: string[]): Promise<void> {
  ...
  refreshCrewTemplatesDir(dest, templatesDir);
  success('refreshed .crew/templates/');
  filesUpdated.push('.crew/templates/');

  // iter-8: write crew_state MCP entry to repo-root `.mcp.json`
  ...
  try {
    ...
    if (rootResult.written) {
      success(`installed crew_state MCP server to .mcp.json (${describeMcpSpec(pinnedSpec)}) — Copilot CLI will auto-load on next invocation`);
  ...
  // `--additional-mcp-config @.mcp.json`. See Blacklite/crew#1296.
...
export function ensureMemoryGovernanceUpgradeDefaults(dest: string): string[] {
  ...
  for (const [file, content] of Object.entries(defaults)) {
    ...
    if (!storage.existsSync(fullPath)) {
      ...
      created.push(path.join('.crew', 'memory', file));
    }
  }
  return created;
}
 ...
 * as templates but may be missing from older crews. Idempotent — only writes
 ...
 * exist in any crew that ran a prior init, and their charters are inlined in
 * cast.ts (no shipped template file). Adding them here would risk overwriting
 * customized versions on legacy crews.
 *
 * @param dest Root directory containing .crew/
...
export function ensureBuiltinAgents(dest: string, templatesDir: string): string[] {
  const agentsDir = path.join(dest, '.crew', 'agents');
  ...
  //   - dirName: case-preserving directory name under .crew/agents/
  ...
  for (const agent of builtins) {
    ...
    if (storage.existsSync(tplPath)) {
      ...
      charterContent = `# ${agent.displayName}\n\n> Charter template not found in shipped templates. Run \`crew upgrade\` after reinstalling the CLI to repair.\n`;
    }

    try {
      storage.mkdirSync(agentDir, { recursive: true });
      storage.writeSync(charterPath, charterContent);
      created.push(path.join('.crew', 'agents', agent.dirName, 'charter.md'));

      storage.writeSync(historyPath, `# ${agent.displayName} — History\n\n## Learnings\n\nInitial scaffold via \`crew upgrade\`. Ready for work.\n`);
      created.push(path.join('.crew', 'agents', agent.dirName, 'history.md'));
    } catch (err: unknown) {
      if (err instanceof Error && 'code' in err && ['EPERM', 'EACCES'].includes((err as NodeJS.ErrnoException).code ?? '')) {
        warn(`Could not scaffold built-in agent ${agent.displayName} (read-only). Create .crew/agents/${agent.dirName}/ manually.`);
        continue;
      }
      throw err;
    }
...
export async function runUpgrade(dest: string, options: UpgradeOptions = {}): Promise<UpdateInfo> {
  ...
  if (crewDirInfo.isLegacy) {
    warn('DEPRECATION: .ai-team/ is deprecated and will be removed in v1.0.0');
    warn("Run 'crew upgrade --migrate-directory' to migrate to .crew/");
    console.log();
  }
  ...
  const crewConfig = readCrewConfig(crewDirInfo.path);
  const mcpConfigMode = detectMcpConfigMode(crewConfig, agentDest);
  const isGitHubForMcp = detectIsGitHubForMcp(dest, crewConfig);
  ...
  if (options.dryRun) {
    ...
    const agentSrc = path.join(templatesDir, 'crew.agent.md.template');
    if (storage.existsSync(agentSrc)) {
      writeAgentTemplate(agentSrc, agentDest, cliVersion, mcpConfigMode, isGitHubForMcp, { dryRun: true });
    }
    const filesToUpgrade = TEMPLATE_MANIFEST.filter(f => f.overwriteOnUpgrade && f.source !== 'crew.agent.md.template');
    if (filesToUpgrade.length > 0) {
      info(`Would overwrite ${filesToUpgrade.length} crew-owned file(s):`);
      for (const file of filesToUpgrade) {
        const destPath = path.join(crewDirInfo.path, file.destination);
  ...
  if (isAlreadyCurrent) {
    ...
    const migrationsApplied = await runMigrations(crewDirInfo.path, oldVersion, cliVersion);
    ...
    if (storage.existsSync(workflowsSrc)) {
      ...
      success(`upgraded crew workflows (${wfFiles.length} files)`);
    ...
    if (storage.existsSync(agentSrc)) {
      ...
      success('upgraded crew.agent.md');
      filesUpdated.push('crew.agent.md');
    } else {
      warn('crew.agent.md.template not found — crew.agent.md was not refreshed. Reinstall or repair the CLI to restore the missing template.');
    }
  ...
  // Upgrade crew.agent.md
  const templatesDir = getTemplatesDir();
  const agentSrc = path.join(templatesDir, 'crew.agent.md.template');

  if (!storage.existsSync(agentSrc)) {
    fatal('crew.agent.md.template not found in templates — installation may be corrupted');
  }
  ...
  // Upgrade crew-owned files from template manifest
  // Exclude crew.agent.md — already copied and version-stamped above
  const filesToUpgrade = TEMPLATE_MANIFEST.filter(f => f.overwriteOnUpgrade && f.source !== 'crew.agent.md.template');
  ...
  if (filesToUpgrade.length > 0) {
    success(`upgraded ${filesToUpgrade.length} crew-owned files`);
  }
  ...
  const teamMdPath = path.join(crewDirInfo.path, 'team.md');
  ...
  if (migrationsApplied.some(m => m.toLowerCase().includes('scrub email'))) {
    dim('Privacy scrub applied to .crew/ files (email addresses removed)');
  } else {
...
/**
 * Self-upgrade the Crew CLI package via the detected package manager.
 *
...
export async function selfUpgradeCli(options: SelfUpgradeOptions = {}): Promise<void> {
  ...
  const pkg = `@blacklite/crew-cli@${tag}`;
  ...
  try {
    ...
    if (isPermission) {
      ...
      hint = `A file is in use (likely another crew shell is running). Close other crew CLI processes and retry: ${cmd}`;
    } else {

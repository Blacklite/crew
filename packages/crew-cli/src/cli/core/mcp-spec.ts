 ...
 *
 * Used by BOTH `crew init` and `crew upgrade` so the runtime-MCP fallback
...
export interface ResolveCrewStateMcpSpecOptions {
  /**
...
/**
 * Resolve the crew_state MCP launch spec given the running CLI version.
 *
 ...
 * as pre-iter-5.
 */
export async function resolveCrewStateMcpSpec(
  cliVersion: string,
  options: ResolveCrewStateMcpSpecOptions = {},
): Promise<CrewStateMcpSpec> {
...
async function defaultPublishedCheck(version: string): Promise<boolean> {
  const { isCrewCliVersionPublished } = await import('./npm-registry.js');
  return await isCrewCliVersionPublished(version);
}

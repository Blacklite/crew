/**
 * Crew directory detection — zero dependencies
 */

import fs from 'node:fs';
import path from 'node:path';

export interface CrewDirInfo {
  path: string;
  name: '.crew' | '.ai-team';
  isLegacy: boolean;
}
 ...
 */
export function detectCrewDir(dest: string): CrewDirInfo {
  ...
  if (mainCheckout) {
    ...
    if (fs.existsSync(mainCrewDir)) {
      return { path: mainCrewDir, name: '.crew', isLegacy: false };
    }

 ...
 * public npm registry. Used by `ensureCrewStateMcpPinned` to avoid pinning a
 * `crew_state` MCP launch spec to a version that `npx` cannot resolve
 ...
 *
 * See `.crew/files/validation/MCP-LOADER-ROOT-CAUSE.md` (data-15 Option A).
 */
...
/**
 * Returns true if `@blacklite/crew-cli@<version>` is reachable on the npm
 ...
 */
export async function isCrewCliVersionPublished(
  version: string,
  timeoutMs = 2000,
): Promise<boolean> {
  ...
  const url = `https://registry.npmjs.org/@blacklite%2Fcrew-cli/${encodeURIComponent(version)}`;

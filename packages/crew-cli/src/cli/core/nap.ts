/**
 * Nap — context hygiene engine for .crew/ state
...
function collectMetrics(crewDir: string): NapMetrics {
  const agentsDir = path.join(crewDir, 'agents');
  const orchLogDir = path.join(crewDir, 'orchestration-log');
  const logDir = path.join(crewDir, 'log');
  const decisionsFile = path.join(crewDir, 'decisions.md');
  const inboxDir = path.join(crewDir, 'decisions', 'inbox');
...
function cleanInbox(crewDir: string, dryRun: boolean): NapAction[] {
  const inboxDir = path.join(crewDir, 'decisions', 'inbox');
  const decisionsFile = path.join(crewDir, 'decisions.md');
 ...
 *
 * @param crewDir - Absolute path to the `.crew` directory.
 ...
 */
function archiveDecisions(crewDir: string, dryRun: boolean): NapAction | null {
  const decisionsFile = path.join(crewDir, 'decisions.md');
  ...
  if (!dryRun) {
    const archivePath = path.join(crewDir, 'decisions-archive.md');
    if (archiveContent.trim()) {
      storage.appendSync(archivePath, archiveContent);
    }
...
function checkJournal(crewDir: string): boolean {
  return storage.existsSync(path.join(crewDir, JOURNAL_FILE));
}

function writeJournal(crewDir: string): void {
  storage.writeSync(
    path.join(crewDir, JOURNAL_FILE),
    `nap started at ${new Date().toISOString()}\n`,
  );
}

function removeJournal(crewDir: string): void {
  const jp = path.join(crewDir, JOURNAL_FILE);
...
export async function runNap(options: NapOptions): Promise<NapResult> {
  const { crewDir, deep = false, dryRun = false } = options;
  const actions: NapAction[] = [];
  ...
  const before = collectMetrics(crewDir);

  if (!dryRun) writeJournal(crewDir);
  ...
  try {
    ...
    // Log pruning
    actions.push(...pruneLogs(path.join(crewDir, 'orchestration-log'), dryRun));
    actions.push(...pruneLogs(path.join(crewDir, 'log'), dryRun));

    // Inbox cleanup
    actions.push(...cleanInbox(crewDir, dryRun));

    // Decision archival
    const archiveAction = archiveDecisions(crewDir, dryRun);
    if (archiveAction) actions.push(archiveAction);

  } finally {
    if (!dryRun) removeJournal(crewDir);
  }

  // Collect after metrics
  const after = dryRun ? estimateAfterMetrics(before, actions) : collectMetrics(crewDir);

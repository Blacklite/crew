/**
 * Helpers for spawning the Copilot CLI from crew-managed code paths.
 *
 ...
 * a UI prompt, so the `crew_state` MCP entry written by `crew init` /
 * `crew upgrade` to the repo-root `.mcp.json` is silently ignored every time
 * crew spawns `copilot -p` — leaving `crew_state_*` tools unwired.
 *
 * Mitigation: every crew-internal `copilot` spawn prepends two flags:
 ...
 *      `.mcp.json` so `crew_state_*` tools register for that session.
 *
 * We only inject when:
 ...
 *
 * See `.crew/files/validation/COMBINED-FIX-BRANCH-MANIFEST.md` (iter-9) for
 ...
 *  - `.mcp.json` does not exist under `teamRoot` (crew init not run, repo

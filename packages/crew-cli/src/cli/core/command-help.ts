/**
 * Per-command help text for `crew <cmd> --help` / `-h`.
 *
 ...
 * destructive side effects (`crew init --help` would scaffold files into
 * the cwd, `crew triage --help` / `watch --help` would start a polling
 * loop, etc.).
 *
 ...
 * fallback that points users at the top-level `crew help`.
 */
...
function header(cmd: string, version: string, tagline: string): void {
  console.log(`\n${BOLD}crew ${cmd}${RESET} v${version} — ${tagline}\n`);
}
...
const COMMAND_HELP: Record<string, HelpPrinter> = {
  init: (version) => {
    header('init', version, 'Initialize Crew in the current project');
    console.log(`Usage: crew init [options]`);
    console.log(`       crew init --mode remote <team-repo-path>\n`);
    console.log(`Creates a markdown-based crew layout under .crew/ plus default agent`);
    console.log(`workflows under .github/. Safe to re-run — existing files are preserved.\n`);
    console.log(`Options:`);
    console.log(`  ${BOLD}--sdk${RESET}                       Use SDK builder syntax (crew.config.ts)`);
    console.log(`  ${BOLD}--roles${RESET}                     Seed the team with built-in base roles`);
    console.log(`  ${BOLD}--global${RESET}                    Initialize in the personal (global) crew directory`);
    ...
    console.log(`  ${BOLD}--mode remote <path>${RESET}        Point at an external team root (creates .crew/config.json)\n`);
  },

  upgrade: (version) => {
    header('upgrade', version, 'Update Crew-owned files to the latest version');
    console.log(`Usage: crew upgrade [options]\n`);
    console.log(`Overwrites Crew-owned files (crew.agent.md, .crew/templates/) while`);
    console.log(`leaving your team state under .crew/ and .ai-team/ untouched.\n`);
    console.log(`Local customizations to crew.agent.md are backed up automatically.`);
    console.log(`Use --dry-run to preview changes before applying.\n`);
    console.log(`Options:`);
    console.log(`  ${BOLD}--dry-run${RESET}                   Preview changes without writing`);
    console.log(`  ${BOLD}--global${RESET}                    Upgrade the personal (global) crew`);
    console.log(`  ${BOLD}--migrate-directory${RESET}         Rename legacy .ai-team/ to .crew/`);
    console.log(`  ${BOLD}--state-backend <type>${RESET}      Migrate to a new backend (orphan|two-layer)`);
    console.log(`  ${BOLD}--self${RESET}                      Upgrade the crew CLI package itself`);
  ...
  'update-check': (version) => {
    ...
    console.log(`Usage: crew update-check [options]\n`);
  ...
  migrate: (version) => {
    header('migrate', version, 'Convert between markdown and SDK-First crew formats');
    console.log(`Usage: crew migrate --to sdk|markdown [options]\n`);
    console.log(`Options:`);
  ...
  status: (version) => {
    header('status', version, 'Show which crew is active and why');
    console.log(`Usage: crew status\n`);
    console.log(`Reports the resolved crew directory, the resolution reason (repo vs.`);
    console.log(`global), and the state of the personal crew path.\n`);
  },

  roles: (version) => {
    header('roles', version, 'List built-in Crew roles');
    console.log(`Usage: crew roles [--category <name>] [--search <query>]\n`);
    console.log(`Options:`);
  ...
  cost: (version) => {
    ...
    console.log(`Usage: crew cost [options]\n`);
    console.log(`Options:`);
  ...
  loop: (version) => {
    ...
    console.log(`Usage: crew loop [options]\n`);
    ...
    console.log(`  ${BOLD}--harness <name>${RESET}      Agent harness: copilot (default) or claude`);
    ...
    console.log(`\nExamples:`);
    console.log(`  crew loop                          ${DIM}# run loop.md${RESET}`);
    console.log(`  crew loop --init                   ${DIM}# generate boilerplate${RESET}`);
    console.log(`  crew loop --file ops/loop.md       ${DIM}# custom loop file${RESET}`);
    console.log(`  crew loop --monitor-email          ${DIM}# with email monitoring${RESET}`);
  },

  cast: (version) => {
    header('cast', version, 'Show roster or add a new agent to the team');
    console.log(`Usage: crew cast                        ${DIM}# show the current cast (roster)${RESET}`);
    console.log(`       crew cast --name <name> --role <role>  ${DIM}# add a new agent${RESET}\n`);
    ...
    console.log(`  ${BOLD}--role <role>${RESET}               Pre-select a built-in role (see 'crew roles')`);
    console.log(`\nAlias: ${BOLD}crew hire${RESET} (always runs the creation wizard)\n`);
  },

  copilot: (version) => {
    header('copilot', version, 'Add or remove the Copilot coding agent (@copilot)');
    console.log(`Usage: crew copilot [--off] [--auto-assign]\n`);
    console.log(`Options:`);
  ...
  plugin: (version) => {
    ...
    console.log(`Usage: crew plugin marketplace add|remove|list|browse\n`);
  ...
  export: (version) => {
    header('export', version, 'Export crew to a portable JSON snapshot');
    console.log(`Usage: crew export [--out <path>]\n`);
    console.log(`Options:`);
    console.log(`  ${BOLD}--out <path>${RESET}                Output file (default: crew-export.json)\n`);
  },

  import: (version) => {
    header('import', version, 'Import crew from an export file');
    console.log(`Usage: crew import <file> [--force]\n`);
    console.log(`Options:`);
    console.log(`  ${BOLD}--force${RESET}                     Overwrite existing files in target\n`);
  },

  'scrub-emails': (version) => {
    header('scrub-emails', version, 'Remove email addresses from Crew state files');
    console.log(`Usage: crew scrub-emails [directory]\n`);
    console.log(`Defaults to scrubbing .ai-team/ (or .crew/) in the current directory.\n`);
  },

  start: (version) => {
    header('start', version, 'Start Copilot with remote access from phone / browser');
    console.log(`Usage: crew start [--tunnel] [--port <n>] [--command <cmd>] [copilot flags...]\n`);
    console.log(`Options:`);
    ...
    console.log(`  crew start --tunnel --yolo`);
    console.log(`  crew start --tunnel --model claude-sonnet-4.6`);
    console.log(`  crew start --tunnel --command "gh copilot"\n`);
  },

  nap: (version) => {
    header('nap', version, 'Context hygiene — compress, prune, archive .crew/ state');
    console.log(`Usage: crew nap [--deep] [--dry-run]\n`);
    console.log(`Options:`);
  ...
  memory: (version) => {
    ...
    console.log(`Usage: crew memory <subcommand> [options]\n`);
  ...
  'state-mcp': (version) => {
    header('state-mcp', version, 'MCP bridge exposing Crew runtime state tools');
    console.log(`Usage: crew state-mcp\n`);
    console.log(`Starts a Model Context Protocol server over stdio that exposes Crew`);
    console.log(`state read/write tools to MCP-compatible hosts.\n`);
  },

  doctor: (version) => {
    header('doctor', version, 'Validate crew setup');
    console.log(`Usage: crew doctor\n`);
  ...
  consult: (version) => {
    header('consult', version, 'Enter consult mode with your personal crew');
    console.log(`Usage: crew consult [--status] [--check]\n`);
    console.log(`Options:`);
  ...
  extract: (version) => {
    ...
    console.log(`Usage: crew extract [options]\n`);
    console.log(`Options:`);
  ...
  subcrews: (version) => {
    header('subcrews', version, 'Manage Crew SubCrews (multi-Codespace scaling)');
    console.log(`Usage: crew subcrews <list|status|activate <name>>\n`);
  ...
  link: (version) => {
    ...
    console.log(`Usage: crew link <team-repo-path>\n`);
  },

  build: (version) => {
    header('build', version, 'Compile crew.config.ts into .crew/ markdown');
    console.log(`Usage: crew build [options]\n`);
    console.log(`Options:`);
    ...
    console.log(`  ${BOLD}--watch${RESET}                     Rebuild on changes to crew.config.ts\n`);
  },

  aspire: (version) => {
    header('aspire', version, 'Launch the Aspire dashboard for observability');
    console.log(`Usage: crew aspire [options]\n`);
    console.log(`Options:`);
  ...
  schedule: (version) => {
    header('schedule', version, 'Manage scheduled Crew tasks');
    console.log(`Usage: crew schedule <list|run <id>|init|status>\n`);
  },

  personal: (version) => {
    header('personal', version, 'Manage your personal crew (ambient agents)');
    console.log(`Usage: crew personal <init|list|add <name>|remove <name>> [options]\n`);
    console.log(`Options:`);
    console.log(`  ${BOLD}--role <role>${RESET}               Built-in role to attach (see 'crew roles')\n`);
  },

  preset: (version) => {
    header('preset', version, 'Manage crew presets (curated agent collections)');
    console.log(`Usage: crew preset <list|show <name>|apply <name>|save <name>|install <source>|init> [options]\n`);
    console.log(`Subcommands:`);
  ...
  'copilot-bridge': (version) => {
    ...
    console.log(`Usage: crew copilot-bridge\n`);
  },

  'init-remote': (version) => {
    header('init-remote', version, 'Link project to a remote team root (shorthand)');
    console.log(`Usage: crew init-remote <team-repo-path>\n`);
  },

  'rc-tunnel': (version) => {
    header('rc-tunnel', version, 'Check devtunnel CLI availability');
    console.log(`Usage: crew rc-tunnel\n`);
  },

  discover: (version) => {
    header('discover', version, 'List known crews and their capabilities');
    console.log(`Usage: crew discover\n`);
  },

  delegate: (version) => {
    header('delegate', version, 'Create work in another crew');
    console.log(`Usage: crew delegate <crew-name> <description>\n`);
  },

  upstream: (version) => {
    header('upstream', version, 'Manage upstream Crew sources');
    console.log(`Usage: crew upstream <add|remove|list|sync> [options]\n`);
    console.log(`Examples:`);
    console.log(`  crew upstream add <source> [--name <n>] [--ref <branch>]`);
    console.log(`  crew upstream remove <name>`);
    console.log(`  crew upstream list`);
    console.log(`  crew upstream sync [name]\n`);
  },
  ...
  externalize: (version) => {
    header('externalize', version, 'Move local crew state to an external team root');
    console.log(`Usage: crew externalize\n`);
  },

  internalize: (version) => {
    header('internalize', version, 'Pull an external team root back into the project');
    console.log(`Usage: crew internalize\n`);
  },

  config: (version) => {
    header('config', version, 'Manage Crew configuration');
    console.log(`Usage: crew config <subcommand> [options]\n`);
  },
};

function printWatchHelp(name: 'triage' | 'watch', version: string): void {
  header(name, version, 'Scan for work and categorize issues');
  console.log(`Usage: crew ${name} [--interval <minutes>] [--execute] [options]\n`);
  ...
  console.log(`Capabilities (opt-in via --<name> or .crew/config.json):`);
  console.log(`  ${BOLD}--self-pull${RESET}                 git fetch/pull at round start`);
...
function printRcHelp(name: 'rc' | 'remote-control', version: string): void {
  ...
  console.log(`Usage: crew ${name} [--tunnel] [--port <n>] [--path <dir>]\n`);
  console.log(`Options:`);
  console.log(`  ${BOLD}--tunnel${RESET}                    Expose the bridge over a public devtunnel`);
  console.log(`  ${BOLD}--port <n>${RESET}                  Bind to a specific local port`);
  console.log(`  ${BOLD}--path <dir>${RESET}                Crew path to expose (default: cwd)\n`);
 ...
 * command (e.g. `streams` / `workstreams` both route to `subcrews`). The
 * help registry is keyed by the canonical name only, so alias lookups must
 * be normalized before the registry lookup — otherwise `crew streams --help`
 ...
 * fall back to a generic "see `crew help`" message.
 *
...
export function printGenericCommandHelp(cmd: string): void {
  ...
  console.log(`Run '${BOLD}crew help${RESET}' for the full command list and usage.\n`);
}

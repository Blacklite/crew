...
} from '@blacklite/crew-sdk/casting';
...
After making a decision others should know, write it to \`.crew/decisions/inbox/${nameLower}-{brief-slug}.md\`.
...
After making a decision others should know, write it to \`.crew/decisions/inbox/fact-checker-{brief-slug}.md\`.
...
- \`.crew/rai/policy.md\` — Canonical RAI policy (terms, anti-patterns, taxonomy)
- \`.crew/rai/audit-trail.md\` — Evidence log (append-only, redacted)
- \`.crew/agents/Rai/history.md\` — Learnings across sessions
...
Read \`.crew/rai/policy.md\` for the canonical check definitions.
Append findings to \`.crew/rai/audit-trail.md\` (redacted — never raw secrets or harmful text).
After making a decision others should know, write it to \`.crew/decisions/inbox/Rai-{brief-slug}.md\`.
`;
}
...
function buildMembersTable(allMembers: CastMember[]): string {
  ...
  for (const m of allMembers) {
    ...
    table += `| ${m.name} | ${m.role} | \`.crew/agents/${nameLower}/charter.md\` | ${status} |\n`;
  }
  return table;
}
...
/**
 * Create all crew agent files for a cast proposal.
 * teamRoot is the project root (parent of .crew/).
 */
export async function createTeam(teamRoot: string, proposal: CastProposal): Promise<CastResult> {
  ...
  if (storage.existsSync(teamPath)) {
    ...
    // Create from scratch — fresh project with no prior .crew/ directory
  ...
  // Create .crew/rai/ directory with default policy and audit trail
  ...
  // Sync new agents into crew.config.ts (if present)
  for (const member of allMembers) {

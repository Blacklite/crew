...
interface Migration {
  version: string;
  description: string;
  run: (crewDir: string) => Promise<void> | void;
}
...
const migrations: Migration[] = [
  {
    ...
    run(crewDir: string) {
      const skillsDir = path.join(crewDir, 'skills');
  ...
  {
    version: '0.9.0',
    description: 'Copy legacy .crew skills into .copilot/skills',
    run(crewDir: string) {
      const projectRoot = path.dirname(crewDir);
      const legacySkillsDir = path.join(crewDir, 'skills');

/**
 * Crew Remote Control — PTY Terminal PWA
  ...
  function renderDashboard(sessions) {
    ...
    if (filtered.length === 0) {
      html += '<div style="padding:20px 12px;color:var(--text-dim);text-align:center">' +
        (sessions.length === 0 ? 'No Crew RC sessions found.' : 'No online sessions. Tap "Show offline" to see stale ones.') +
        '</div>';
    } else {
      html += filtered.map(s => `
